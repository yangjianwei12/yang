...
PRESERVE_TYPE_FOR_DEBUGGING(ps_key_map_t)
...
static void handleMessageAppInitStart(void)
{
    ...
    DEBUG_LOG_FN_ENTRY("handleMessageAppInitStart");
    UsbDongleInit_StartInitialisation();
}
...
{
    UNUSED(evt);
    DEBUG_LOG_FN_ENTRY("appHandleSubsystemEventReport, ss_id %d, level %d, id %d, cpu %d, occurrences %d, time %d",
...
static void appHandleCommonInitCfm(void)
{
    ...
    UsbDongleInit_CompleteInitialisation();
}
...
/* Configure PIO required to drive the external PA for form factor */
static void appSmPioEnableControl(uint8 pio, bool enable)
{
    uint16 bank;
    uint32 mask;
    ...
    PanicNotZero(PioSet32Bank(bank, mask, enable?mask:0x00));
}
#endif
...
#ifdef HAVE_EXTERNAL_PA
    appSmPioEnableControl(EXTERNAL_PA_CONFIG_PIO, TRUE);
#endif
