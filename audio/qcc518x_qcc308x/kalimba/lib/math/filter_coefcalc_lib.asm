...
calc_peaking_2nd:
//------------------------------------------------------------------------------
// calculate coefficients for second order peaking EQ
//
//                s^2 + g * Bs + 1
//   H(s) =  ------------------------
//                  S^2 + Bs + 1
//
//------------------------------------------------------------------------------
// peak(boost V0 = 10^(G/20))
//
//          1 + V0/Q*K + K^2    2 * (K^2 - 1)         1 - V0/Q*K + K^2
//          ---------------- + --------------- z^-1 + ---------------- z^-2
//          1 + 1/Q*K  + K^2   1 + 1/Q*K + K^2        1 + 1/Q*K + K^2
//   H(z) = ----------------------------------------------------------------
//                              2 * (K^2 - 1)         1 - V0/Q*K + K^2
//                1          + --------------- z^-1 + ---------------- z^-2
//                             1 + 1/Q*K + K^2        1 + 1/Q*K + K^2
//
// peak(cut V0 = 10^(-G/20))
//
//          1 + 1/Q*K + K^2      2 * (K^2 - 1)        1 - V0/Q*K + K^2
//          ---------------- + --------------- z^-1 + ---------------- z^-2
//          1 + V0/Q*K  + K^2   1 + V0/Q*K + K^2      1 + V0/Q*K + K^2
//   H(z) = ----------------------------------------------------------------
//                              2 * (K^2 - 1)         1 - V0/Q*K + K^2
//                1          + --------------- z^-1 + ---------------- z^-2
//                             1 + V0/Q*K + K^2       1 + V0/Q*K + K^2
//
//
//------------------------------------------------------------------------------
// on entry - all parameters on stack
.const calc_peaking_2nd.entry_Fs.e         8*ADDR_PER_WORD;
.const calc_peaking_2nd.entry_Fs.m         7*ADDR_PER_WORD;
.const calc_peaking_2nd.entry_fc.e         6*ADDR_PER_WORD;
.const calc_peaking_2nd.entry_fc.m         5*ADDR_PER_WORD;
.const calc_peaking_2nd.entry_q.e          4*ADDR_PER_WORD;
.const calc_peaking_2nd.entry_q.m          3*ADDR_PER_WORD;
.const calc_peaking_2nd.entry_gain.e       2*ADDR_PER_WORD;
.const calc_peaking_2nd.entry_gain.m       1*ADDR_PER_WORD;
.const calc_peaking_2nd.entry_stack_size   8*ADDR_PER_WORD;
//------------------------------------------------------------------------------
// local stack space (accessed via frame pointer)
.const calc_peaking_2nd.stack_temp        10*ADDR_PER_WORD;
.const calc_peaking_2nd.local_1           calc_peaking_2nd.stack_temp + 0*ADDR_PER_WORD;
.const calc_peaking_2nd.local_2           calc_peaking_2nd.stack_temp + 1*ADDR_PER_WORD;
.const calc_peaking_2nd.local_3           calc_peaking_2nd.stack_temp + 2*ADDR_PER_WORD;
.const calc_peaking_2nd.local_4           calc_peaking_2nd.stack_temp + 3*ADDR_PER_WORD;
.const calc_peaking_2nd.local_5           calc_peaking_2nd.stack_temp + 4*ADDR_PER_WORD;
.const calc_peaking_2nd.local_6           calc_peaking_2nd.stack_temp + 5*ADDR_PER_WORD;
.const calc_peaking_2nd.local_stack_size  6*ADDR_PER_WORD;
//------------------------------------------------------------------------------
// on exit - all return values on stack
//------------------------------------------------------------------------------
// registers destroyed
...
//   return values     = 6
//------------------------------------------------------------------------------
.const ZEROING_SIGN_BIT_MASK 0x000000FF;

    pushm <fp(=sp),r0,r1,r2,r3,r4,r5,r6,r7,rlink>;
    sp = sp + calc_peaking_2nd.local_stack_size;

    // make space for return coefficient of "__scale_coefficients"
    sp = sp + __scale_coefficients.exit_stack_size;

    r0 = M[fp - calc_peaking_2nd.entry_fc.e];
    r1 = M[fp - calc_peaking_2nd.entry_fc.m];
    r2 = M[fp - calc_peaking_2nd.entry_Fs.e];
    r3 = M[fp - calc_peaking_2nd.entry_Fs.m];

    call $kal_filter_coef_lib.__warp.call_entry;   // warp the analogue frequency to digital frequency
    r2 = r0;                                       // r0:r1 = w
    r3 = r1;                                       // r2:r3 = w
    call $kal_float_lib.mul;
    r6 = r0;
    ...
    r0 = r2;
    r1 = r3;                                       // r0:r1 = w
    r2 = M[fp - calc_peaking_2nd.entry_q.e];
    r3 = M[fp - calc_peaking_2nd.entry_q.m];
    call $kal_float_lib.div;                       // w/q
    r4 = r0;                                       // r0:r1 = w/q
    r5 = r1;                                       // r4:r5 = w/q

    r0 = M[fp - calc_peaking_2nd.entry_gain.e];
    r1 = M[fp - calc_peaking_2nd.entry_gain.m];

    r0 = r0 AND ZEROING_SIGN_BIT_MASK;             // gaurantee the gain is positive by zeroing the sign bit

    call $kal_filter_coef_lib.__db2lin.call_entry; // v0 = 10^(G/20);
    r2 = r4;
    r3 = r5;
    call $kal_float_lib.mul;                       // r0:r1 = v0*w/q
    M[fp + calc_peaking_2nd.local_5] = r0;
    M[fp + calc_peaking_2nd.local_6] = r1;         // r0:r1 = v0*w/q

    r0 = $kal_float_lib.const.unity.exp;
    r1 = $kal_float_lib.const.unity.mant;          // r0:r1 = 1
    r2 = r6;
    r3 = r7;                                       // r2:r3 = w^2
    call $kal_float_lib.add;                       // 1 + w^2
    m[fp + calc_peaking_2nd.local_3] = r0;
    m[fp + calc_peaking_2nd.local_4] = r1;         // save 1 + w^2

    // Check if the gain is negative
    Null = M[fp - calc_peaking_2nd.entry_gain.e];
    if NEG jump calculate_peaking_cut;

    // Gain>0, calculate the boost EQ

    // r0:r1 = 1 + w^2
    r2 = r4;
    r3 = r5;                                       // r2:r3 = w/q
    call $kal_float_lib.add;                       // r0:r1 = denominator   = 1 + w^2 + w/q
    call $kal_float_lib.recip;                     // r0:r1 = 1/denominator = 1/(1 + w^2 + v0*w/q)
    m[fp + calc_peaking_2nd.local_1] = r0;
    m[fp + calc_peaking_2nd.local_2] = r1;         // save 1/denominator

    // b0 = (1 + w/q + w^2) / denominator;
    r0 = m[fp + calc_peaking_2nd.local_3];
    r1 = m[fp + calc_peaking_2nd.local_4];         // r0:r1 = 1 + w^2
    r2 = m[fp + calc_peaking_2nd.local_5];
    r3 = m[fp + calc_peaking_2nd.local_6];         // r2:r3 = v0*w/q
    call $kal_float_lib.add;                       // r0:r1 = 1 + w^2 + v0*w/q
    r2 = m[fp + calc_peaking_2nd.local_1];
    r3 = m[fp + calc_peaking_2nd.local_2];
    call $kal_float_lib.mul;                       // (1 + w/q + w^2) / denominator
    pushm <r0,r1>;

    // b1 = (2 * (w^2 - 1)) / denominator;
    r0 = r6;
    r1 = r7;
    r2 = $kal_float_lib.const.unity.exp;
    r3 = $kal_float_lib.const.unity.mant;
    call $kal_float_lib.sub;                       // r0:r1 = w^2 - 1
    r0 = r0 + 1;                                   // r0:r1 = 2 * (w^2 - 1)
    r2 = m[fp + calc_peaking_2nd.local_1];
    r3 = m[fp + calc_peaking_2nd.local_2];
    call $kal_float_lib.mul;                       // (2 * (w^2 - 1)) / denominator
    pushm <r0,r1>;

    r6 = r0;
    r7 = r1;                                       // r6:r7 = b1;

    // b2 = (1 - v0*w/q + w^2) / denominator;
    r0 = m[fp + calc_peaking_2nd.local_3];
    r1 = m[fp + calc_peaking_2nd.local_4];         // r0:r1 = 1 + w^2
    r2 = M[fp + calc_peaking_2nd.local_5];
    r3 = M[fp + calc_peaking_2nd.local_6];         // r2:r3 = v0*w/q
    call $kal_float_lib.sub;                       // r0:r1 = 1 + w^2 - v0*w/q
    r2 = m[fp + calc_peaking_2nd.local_1];
    r3 = m[fp + calc_peaking_2nd.local_2];
    call $kal_float_lib.mul;                       // (1 + w^2 - v0*w/q) / denominator
    pushm <r0,r1>;

    // a1 = b1
    pushm <r6,r7>;

    // a2 = (1 - v0*w/q + w^2) / denominator
    r0 = m[fp + calc_peaking_2nd.local_3];
    r1 = m[fp + calc_peaking_2nd.local_4];        // r0:r1 = 1 + w^2
    r2 = r4;
    r3 = r5;
    call $kal_float_lib.sub;                      // r0:r1 = 1 + w^2 - w/q
    r2 = m[fp + calc_peaking_2nd.local_1];
    r3 = m[fp + calc_peaking_2nd.local_2];        // r2:r3 = denominator
    call $kal_float_lib.mul;                      // r0:r1 = (1 - v0*w/q + w^2) / denominator
    pushm <r0,r1>;
    jump exit_peaking_2nd_calculation;

calculate_peaking_cut:
    // r0:r1 = 1 + w^2
    r2 = m[fp + calc_peaking_2nd.local_5];
    r3 = m[fp + calc_peaking_2nd.local_6];        // r2:r3 = v0*w/q
    call $kal_float_lib.add;                      // r0:r1 = denominator = 1 + w^2 + v0*w/q
    call $kal_float_lib.recip;                    // r0:r1 = 1/denominator = 1/(1 + w^2 + v0*w/q)
    m[fp + calc_peaking_2nd.local_1] = r0;
    m[fp + calc_peaking_2nd.local_2] = r1;        // save 1/denominator
    ...
    r2 = r4;
    r3 = r5;                                      // r2:r3 = w/q
    call $kal_float_lib.add;                      // r0:r1 = 1 + w^2 + w/q
    r2 = m[fp + calc_peaking_2nd.local_1];
    r3 = m[fp + calc_peaking_2nd.local_2];
    call $kal_float_lib.mul;                      // r0:r1 = (1 + w/q + w^2) / denominator
    pushm <r0,r1>;
    ...
    r1 = r7;                                     // r0:r1 = w^2
    r2 = $kal_float_lib.const.unity.exp;
    r3 = $kal_float_lib.const.unity.mant;        // r0:r1 = 1
    call $kal_float_lib.sub;                     // r0:r1 = w^2 - 1
    r0 = r0 + 1;                                 // r0:r1 = 2 * (w^2 - 1)
    r2 = m[fp + calc_peaking_2nd.local_1];
    r3 = m[fp + calc_peaking_2nd.local_2];       // r2:r3 = 1/denominator
    call $kal_float_lib.mul;                     // r0:r1 = (2 * (w^2 - 1)) / denominator
    pushm <r0,r1>;

    r6 = r0;
    r7 = r1;                                     // r6:r7 = b1;

    // b2 = (1 - w/q + w^2) / denominator;
    r0 = m[fp + calc_peaking_2nd.local_3];
    r1 = m[fp + calc_peaking_2nd.local_4];      // r0:r1 = 1 + w^2
    r2 = r4;
    r3 = r5;                                    // r2:r3 = w/q
    ...
    r2 = m[fp + calc_peaking_2nd.local_1];
    r3 = m[fp + calc_peaking_2nd.local_2];      // r2:r3 = 1/denominator
    call $kal_float_lib.mul;                    // r0:r1 = (1 + w^2 - w/q) / denominator
    pushm <r0,r1>;
...
exit_peaking_2nd_calculation:
    call $kal_filter_coef_lib.__scale_coefficients;

    // copy return value from "__scale_coefficients" onto return stack
    pop r0;
    m[fp - (calc_peaking_2nd.entry_stack_size+biquad_coeficients.exit_scale)] = r0;
    pop r0;
    m[fp - (calc_peaking_2nd.entry_stack_size+biquad_coeficients.exit_a2)] = r0;
    pop r0;
    m[fp - (calc_peaking_2nd.entry_stack_size+biquad_coeficients.exit_a1)] = r0;
    pop r0;
    m[fp - (calc_peaking_2nd.entry_stack_size+biquad_coeficients.exit_b2)] = r0;
    pop r0;
    m[fp - (calc_peaking_2nd.entry_stack_size+biquad_coeficients.exit_b1)] = r0;
    pop r0;
    m[fp - (calc_peaking_2nd.entry_stack_size+biquad_coeficients.exit_b0)] = r0;

    sp = sp - calc_peaking_2nd.local_stack_size;
    popm <fp,r0,r1,r2,r3,r4,r5,r6,r7,rlink>;
    sp = sp - calc_peaking_2nd.entry_stack_size;
