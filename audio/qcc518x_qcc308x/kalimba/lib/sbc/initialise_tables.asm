...
//
// DESCRIPTION:
//    C callable version of the $sbcenc.init_tables function.
//    This function will populate all the tables in RAM with data in tables in
//    Flash/ROM needed by the SBC encoder.
//
// INPUTS:
//    - R0 pointer to encoder structure, which has valid data object with buffers
//      allocated
//
// OUTPUTS:
//    - none
...
//
// *****************************************************************************
.set $_sbcenc_init_tables,  $sbcenc._init_tables

.MODULE $M.sbcenc._init_tables;
   .CODESEGMENT SBC_INIT_TABLES_PM;
   .DATASEGMENT DM;
...
.MODULE $M.sbcdec.private.init_tables;
   ...
   //Get proprietary config flag
   r1 = M[r9 + $sbc.mem.WBS_SBC_FRAME_OK_FIELD];
   Null = r1;
   if Z jump copy_8sb_dec_table;
        //Get which proprietary filter should be used
        r1 = M[ r9 + $sbc.mem.CONVERT_TO_MONO_FIELD ];
        Null = 64 - r1 ;
        if Z jump use_64_tap_synthesis_filter;
             //Use aptx lite 80 tap filter
             r0 = &$sbcdec.synthesis_coefs_aptxlite_m8;
             jump copy_8sb_dec_table;

        use_64_tap_synthesis_filter:
        //Use aptx lite 64 tap filter
        r0 = &$sbcdec.synthesis_coefs_aptxlite_64_m8;

   copy_8sb_dec_table:
   r1 = 128;

   call $mem.ext_copy_to_ram;
   ...
   r1 = M[r9 + $sbc.mem.WBS_SBC_FRAME_OK_FIELD];
   Null = r1;
   if Z jump copy_4sb_dec_table;
        //If set, force the aptxlite coefs
        r0 = &$sbcdec.synthesis_coefs_aptxlite_m4;

   copy_4sb_dec_table:
   r1 = 32;
   Null = r2;
   if NZ call $mem.ext_copy_to_ram;
#endif
...
.MODULE $M.sbcenc.private.init_tables;
   ...
   r1 = M[r9 + $sbc.mem.WBS_SBC_FRAME_OK_FIELD];
   Null = r1;
   if Z jump copy_8sb_enc_table;
        //Get which proprietary filter to use
        r1 = M[ r9 + $sbc.mem.CONVERT_TO_MONO_FIELD ];
        Null = 64 - r1 ;
        if Z jump use_64_tap_analysis_filter;
            //Use the aptx lite 80 tap filter
            r0 = &$sbcenc.analysis_coefs_aptxlite_m8;
            jump copy_8sb_enc_table;

        use_64_tap_analysis_filter:
        //Use the aptx lite 64 tap filter
        r0 = &$sbcenc.analysis_coefs_aptxlite_64_m8;
   ...
   r1 = M[r9 + $sbc.mem.WBS_SBC_FRAME_OK_FIELD];
   Null = r1;
   if Z jump copy_4sb_enc_table; 
        //If set, force the aptxlite coefs
        r0 = &$sbcenc.analysis_coefs_aptxlite_m4;
...
.MODULE $M.sbc.private.init_tables_common;
   ...
   r1 = M[r9 + $sbc.mem.WBS_SBC_FRAME_OK_FIELD];
   Null = r1;
   if Z jump set_8sb_filter_taps;
        //Get which proprietary filter to use
        r1 = M[ r9 + $sbc.mem.CONVERT_TO_MONO_FIELD ];
        Null = 64 - r1 ;
        if Z jump set_64_coef_filter;
             //Use aptxlite 80 coefs filter
             r0 = &$sbc.win_coefs_aptxlite_m8;
             r1 = M[ r9 + $sbc.mem.CONVERT_TO_MONO_FIELD ];
             jump copy_8sb_common_table;
   
        set_64_coef_filter:
        //Use aptxlite 64 coefs filter
        r0 = &$sbc.win_coefs_aptxlite_64_m8;
        r1 = M[ r9 + $sbc.mem.CONVERT_TO_MONO_FIELD ];
        jump copy_8sb_common_table;
   
   set_8sb_filter_taps:
   //Internally initialise the coef filter taps table to the default
   r1 = 80 ;

   copy_8sb_common_table:
   ...
   r1 = M[r9 + $sbc.mem.WBS_SBC_FRAME_OK_FIELD];
   Null = r1;
   if Z jump copy_4sb_common_table;
        //If set, force the aptxlite coefs
        r0 = &$sbc.win_coefs_aptxlite_m4;

   copy_4sb_common_table:
   //Internally initialise the coef filter taps table to the default
   r1 = 40;
   Null = r2;
   if NZ call $mem.ext_copy_to_ram;
