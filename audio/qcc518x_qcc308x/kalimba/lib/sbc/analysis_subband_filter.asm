...
.MODULE $M.sbcenc.analysis_subband_filter;
   ...
   // M2 = 1 - (M * 2)
   M2 = M1 - M0;
   r0 = r3 * 8 (int); //Used for standard-sized filter lengths
   r1 = 48 ;          //Alternative filter length
   r2 = M[r9 + $sbc.mem.CONVERT_TO_MONO_FIELD];
   Null = 64 - r2;
   if Z r0 = r1; //This should only occur with M=8
   Words2Addr(r0);
   //M3 is used with the win_coefs to jump back to the front
   //of the table during the loop
   M3 = M1 - r0;
   ...
   if Z jump M_equals_4;
      ...
      //Alternative length reg for X ring buffer
      ...
      block_loop_M8:
         ...
         //Get size of the window coefs field
         r0 = M[r9 + $sbc.mem.CONVERT_TO_MONO_FIELD];
         Null = 64 - r0 ;
         if Z jump use_64_tap_filter;
            call share_code;
            jump perform_matrix_loop;
...
share_code_64:
         ...
         // and read 1st value from win_coef
         r0 = M[I4, M0],         r1 = M[I0, M0];
         do window_calc_loop_M4_reduced;
            rMAC = r0 * r1,         r0 = M[I4, M0],      r1 = M[I0, M0];
            rMAC = rMAC + r0 * r1,  r0 = M[I4, M0],      r1 = M[I0, M0];
            rMAC = rMAC + r0 * r1,  r0 = M[I4, M3],      r1 = M[I0, M3];
            rMAC = rMAC + r0 * r1,  r0 = M[I4, M0],      r1 = M[I0, M0];
