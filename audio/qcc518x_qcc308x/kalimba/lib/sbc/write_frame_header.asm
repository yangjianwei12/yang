...
.MODULE $M.sbcenc.write_frame_header;
   ...
   // check if we are using a custom version of SBC
   r0 = M[r9 + $sbc.mem.WBS_SBC_FRAME_OK_FIELD];
   if Z jump standard_header;
...
// *****************************************************************************
   // write syncword
   r0 = 8;
   r1 = $sbc.SYNC_WORD_CUSTOM_MODE;
   call $sbcenc.putbits;

   // Bits indicating the custom operating mode
   r0 = 8;
   r1 = M[r9 + $sbc.mem.WBS_SBC_FRAME_OK_FIELD];

   call $sbcenc.putbits;

   // check for joint stereo mode. config_id = 16
   r1 = r1 - 16;
   if Z jump joint;
