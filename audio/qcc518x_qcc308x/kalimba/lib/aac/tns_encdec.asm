...
.MODULE $M.aacdec.tns_encdec;
   ...
   window_loop:
      ...
      n_filt_loop:
         ...
         //detect poles outside the Z transform unit circle:
         // for(i = order; i > 0; i--)
         //    rci = a(i+1);
         //    if abs(rci) >= 1
         //       not stable
         //    for(j = 0; j < i; j++)
         //       b(j) = ( a(j) - rci * a(i+1 - j) ) / 1-rci^2;

         //setup tmp mem
         I4 = r2;
         I0 = r9 + $aac.mem.TMP + (3*ADDR_PER_WORD);
         r0 = $aacdec.TNS_STABILITY_1; //1.0 Q28
         M[I0, MK1] = r0;
         r10 = r6;
         do stability_check_setup;
            r0 = M[I4, MK1];
            M[I0, MK1] = r0;
         ...
         r5 = r6;                                     //filter order: N
         I1 = I0 - MK1;                               //a[N - 1]
         I0 = r9 + $aac.mem.TMP + (3*ADDR_PER_WORD);  //a[0]
         I4 = r9 + $aac.mem.TMP + (23*ADDR_PER_WORD); //b[0]
         stability_check_outer_loop:
            I5 = I1;
            r1 = M[I1, -MK1];                      //r1 = rci
            r0 = ABS r1;
            Null = $aacdec.TNS_STABILITY_1 - r0;   //if abs(rci) >= 1
            if NEG jump lpc_unstable;
            rMAC = r1 * r1;                        //Q56
            rMAC = rMAC ASHIFT 3;                  //rMAC1 = Q28
            r3 = $aacdec.TNS_STABILITY_1 - rMAC;   //r3 = 1 - rci^2
            r10 = r5;
            do stability_check_inner_loop;
               rMAC = M[I5, -MK1];        //rMAC = a[i + 1 - j]
               rMAC = rMAC * r1;
               r0 = rMAC ASHIFT 3;        //r0 = rci * a[i + 1 - j]
               rMAC = M[I0, MK1];         //rMAC = a[j]
               rMAC = rMAC - r0;          //r0 = a[j] - rci * a[i + 1 - j]
               rMAC = rMAC ASHIFT 0 (LO); //prepare dividend for integer divide
               Div = rMAC / r3;
               r0 = DivResult;
               rMAC = DivRemainder;       //Q31 remainder
               rMAC = rMAC ASHIFT -1;
               Div = rMAC / r3;           //remainder / divisor
               rMAC = r0;
               rMAC = rMAC ASHIFT 28;     //Shift Q0 integer to Q28
               r0 = DivResult;
               r0 = r0 ASHIFT -3;         //Shift Q31 fractional result down to Q28
               rMAC = rMAC + r0;
               M[I4, MK1] = rMAC;         //b[j] = ans
            stability_check_inner_loop:

            I0 = r9 + $aac.mem.TMP + (3*ADDR_PER_WORD);  //a[0]
            I4 = r9 + $aac.mem.TMP + (23*ADDR_PER_WORD); //b[0]
            r10 = r5;
            do stability_check_copy_loop;
               r0 = M[I4, MK1];
               M[I0, MK1] = r0;
            stability_check_copy_loop:

            I0 = r9 + $aac.mem.TMP + (3*ADDR_PER_WORD);  //a[0]
            I4 = r9 + $aac.mem.TMP + (23*ADDR_PER_WORD); //b[0]
            r5 = r5 - 1;
         if NZ jump stability_check_outer_loop;
         jump lpc_stable;

         lpc_unstable:
         //adjust lpc coefs to avoid instability
         I4 = r2;
         r1 = $aacdec.TNS_STABILITY_COEFF_SCALAR;  //0.025 Q3.28
         r10 = r6;
         do lpc_adjust_loop;
            rMAC = M[I4, 0];
            rMAC = rMAC * r1;
