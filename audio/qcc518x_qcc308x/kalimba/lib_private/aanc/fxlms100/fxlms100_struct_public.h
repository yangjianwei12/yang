...
typedef struct _FXLMS100_FILTER
{
    ...
    tCbuffer *p_input;             /* Pointer to input data cbuffer */
    tCbuffer *p_output;            /* Pointer to output data cbuffer */
    unsigned frame_size;           /* Number of samples to filter each call */
    uint8 shift;                   /* Shift to apply to output samples */
    bool saturated:8;              /* Saturation detection flag */
} FXLMS100_FILTER;
...
typedef struct _FXLMS100_DMX
{
    ...
    unsigned sample_rate_config;    /* Sample rate of model coefficients */
    ...
    unsigned adaptive_gain1;   /* Calculated adaptive gain value for the ANC inst 1*/
