...
#include "aov_interface/aov_interface.h"
    ...
    /*
     * Changing active mode clocks via graph manager is not supported on
     * kalsim builds
     */
#include "dls_vagm_clk_lib_if.h"
#endif
/**
...
#define MAX_OPERATORS_DISABLED 4
...
typedef struct VAGM_OP_DATA
{
    ...
    unsigned active_mode_clk_val;
    unsigned trigger_mode_clk_val;
    /* flag to check if any operator is delegated to GM */
    bool delegated;
}VAGM_OP_DATA;
...
*/
static bool vagm_create(OPERATOR_DATA *op_data, void *message_data,unsigned *response_id, void **response_data);
...
/* We need at least SVA to be delegated. */
#define VAGM_MIN_DELEGATED_OPERATORS 1
...
const opmsg_handler_lookup_table_entry vagm_opmsg_handler_table[] =
{
    ...
    {OPMSG_OP_CLIENT_REPLY_ID_DELEGATED_OPERATORS, vagm_delegated_ops_main},
    ...
    {OPMSG_VA_GM_ID_SET_GRAPH_MODE, vagm_set_graph_mode},

    {OPMSG_VA_GM_ID_SET_TRIGGER_MODE_CLOCK, vagm_set_trigger_mode_clock},
    {0, NULL}
};
...
/**
 * \brief initialize VA graph manager operator struct with defaults.
 *
...
/**
 * \brief Handles notification from the Framework that the VA operators have
 *        been delegated to VA Graph Manager without VAD. The notification message 
 *        contains the operator [external] ids of the VA operators, as seen by the
 *        application client.
 *
 * \param op_data Pointer to the operator instance data.
 * \param message_data Pointer to the request message
...
/**
 * \brief Reads from bitmask which operators have been trimmed from delegation list.
 *
 * \param op_data Pointer to the operator instance data.
 * \param message_data Pointer to the request message
 * \param response_id Location to write the response message id
...
/**
 * \brief Sets clock values for positive and negative VAD triggers while DSP is in active mode 
 *
 * \param op_data Pointer to the operator instance data.
 * \param message_data Pointer to the request message
 * \param response_id Location to write the response message id
...
/**
 * \brief Send requests to clk_mgr to change clock value
 * \param op_data Pointer to the operator instance data.
