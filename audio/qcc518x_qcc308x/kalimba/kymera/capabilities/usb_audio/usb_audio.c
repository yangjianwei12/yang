...
#ifdef ENABLE_GIP_AUDIO
#define NUM_OF_GIP_PACKETS 2
#define HEADER_LENGTH_IN_OCTECTS 12
#endif
...
#ifdef ENABLE_GIP_AUDIO
    {OPMSG_USB_AUDIO_ID_ENABLE_GIP_HANDLING, usb_audio_opmsg_enable_gip_handling},
#endif
...
/**
 * \brief Updates a cbuffer structure that wraps up scratch memory after a scratch commit
 *
 ...
 * used for the buffer.
 */
static void usb_audio_scratch_commit_update(tCbuffer *cbuffer, void *scratch_mem)
{
...
#ifdef ENABLE_GIP_AUDIO
/**
 * usb_audio_opmsg_enable_gip_handling
 * \brief message handler for OPMSG_USB_AUDIO_ENABLE_GIP_HANDLING message
 *
 * \param op_data Pointer to the operator instance data.
 * \param message_data Pointer to the start request message
 * \param resp_length pointer to location to write the response message length
 ...
 * response
 */
bool usb_audio_opmsg_enable_gip_handling(OPERATOR_DATA *op_data, void *message_data,
                                          unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    USB_AUDIO_OP_DATA *usb_audio_data = get_instance_data(op_data);
    unsigned enable_gip;

    /* read the gip handling value */
    enable_gip = OPMSG_FIELD_GET(message_data,
                                   OPMSG_USB_AUDIO_ENABLE_GIP_HANDLING,
                                   ENABLE_GIP);
    /* enable_gip */
    usb_audio_data->gip_enabled = enable_gip;
    L2_DBG_MSG1("USB Audio Operator, enable gip: %d", usb_audio_data->gip_enabled);
    return TRUE;
}
#endif
                      ...
                      unsigned *response_id, void **response_data)
{
    ...
    /* Disable internal rate matching when GIP based audio is enabled */
#ifndef ENABLE_GIP_AUDIO
...
#ifdef ENABLE_GIP_AUDIO
        usb_audio_data->gip_tx_sequence_id = 0;
#endif
    }
...
bool usb_audio_configure_connected_usb_audio_endpoint(OPERATOR_DATA * op_data)
{
    ...
    if(usb_audio_data->gip_enabled)
    {
        /* We send a header (6 bytes) for every 1ms of audio data. 
         * The way the GIP data needs to be packed in the audio subsystem means that
         * the 6byte header is actually packed into 3 audio “samples”,
         * because only 16 bits of each 32bit audio sample gets sent over USB.*/
        L2_DBG_MSG3("USB configuring ep sample_rate : %d channels : %d subframe_size %d", usb_audio_data->sample_rate, usb_audio_data->nrof_channels, usb_audio_data->subframe_size);
        return stream_usb_audio_configure_ep(usb_audio_data->usb_audio_ep,
                                             usb_audio_data->sample_rate + (3000 / usb_audio_data->nrof_channels),
                                             usb_audio_data->nrof_channels,
                               ...
                               unsigned *response_id, void **response_data)
{
    ...
    L2_DBG_MSG1("USB Buffer Details buf_size : %d", buf_size);
                                     ...
                                     unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    L2_DBG_MSG1("USB pcm_buffer_size %d", usb_audio_data->pcm_buffer_size);
    return TRUE;
                                     ...
                                     unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    L2_DBG_MSG1("USB target latency %d", usb_audio_data->target_latency);
    ttp_configure_latency(usb_audio_data->time_to_play, usb_audio_data->target_latency);
                                        ...
                                        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    L2_DBG_MSG2("USB min_latency %d max_latency %d", min_latency, max_latency);
                                                  ...
                                                  unsigned written_frames)
{
    ...
    if(!sra_mtrans->first_ts_tag_observed &&
       input_mtag != NULL &&
       IS_TIME_TO_PLAY_TAG(input_mtag))
    {
    ...
    if(sra_mtrans->first_ts_tag_observed)
    {
        ...
        TIME_INTERVAL time_passed = (TIME_INTERVAL) (((uint48)written_frames*SECOND)/sra_mtrans->sample_rate);
        time_passed -= frac_mult(time_passed, sra_mtrans->current_sra_sp_adjust);

        /* calculate time stamp for fist output sample */
        if(input_mtag != NULL && IS_TIME_TO_PLAY_TAG(input_mtag))
        {
            ...
            TIME_INTERVAL time_back = (TIME_INTERVAL) (((uint48)samples_b4*SECOND)/sra_mtrans->sample_rate);
...
void usb_audio_tx_transfer_metadata(USB_AUDIO_OP_DATA *usb_audio_data, unsigned consumed_samples, unsigned written_frames)
{
    ...
    L3_DBG_MSG3("USB Tx : consumed_samples %d written_frames %d time %d ", consumed_samples, written_frames, time_get_time());
...
#ifdef ENABLE_GIP_AUDIO
            /* Add additional meta-data length in case of GIP based USB audio.
             * There are 12 bytes (3 32bit samples) associated with each header.
             * So, every time we add a header, we modify the metadata tags to 
             * account for 12 additional bytes. */
            if(usb_audio_data->gip_enabled)
            {
                mtag->length += (HEADER_LENGTH_IN_OCTECTS * NUM_OF_GIP_PACKETS);
                afteridx += (HEADER_LENGTH_IN_OCTECTS * NUM_OF_GIP_PACKETS);
                L3_DBG_MSG2("USB Tx : length %d afteridx %d", mtag->length, afteridx);
            }
#endif
        }
        else
        {
            L3_DBG_MSG1(" mtag NULL !!! time %d ", time_get_time());
        }
...
void usb_audio_process_data(OPERATOR_DATA * op_data, TOUCHED_TERMINALS *touched)
{
    ...
    if(usb_audio_data->data_format == UADF_T1_PCM)
    {
...
#ifdef ENABLE_GIP_AUDIO
        unsigned total_amount_written = 0;
#endif
        ...
        if(USB_AUDIO_IS_TX_OPERATOR(usb_audio_data))
        {
             ...
             * usb_audio_buf is the output buffer to which we write the data*/
            ...
            for(chan = 0; chan < usb_audio_data->nrof_channels; ++chan)
            {
                ...
                L3_DBG_MSG1("USB Tx : channel_amount %d", channel_amount);
                amount_data_available = MIN(amount_data_available, channel_amount);
            }
            L3_DBG_MSG4("USB Tx : Packet Length %d amount_to_read %d amount_data_available %d time %d", packet_len, amount_to_read, amount_data_available, time_get_time());
            ...
            if ((usb_audio_data->pcm_metadata_buffer != NULL) && buff_has_metadata(usb_audio_data->pcm_metadata_buffer))
            {
                ...
                L3_DBG_MSG2("USB Tx : amount_data_available %d input_meta_available %d", amount_data_available, input_meta_available);
            }
            ...
            if(usb_audio_data->gip_enabled)
            {
                if(amount_to_read >= ((packet_len + 4) * NUM_OF_GIP_PACKETS ))
                {
                    //read only NUM_OF_GIP_PACKETS 
                    amount_to_read = packet_len * NUM_OF_GIP_PACKETS;
                }
                else
                {
                    L2_DBG_MSG2("USB Tx : amount to read %d is less than %d", amount_to_read,(packet_len + 4) * NUM_OF_GIP_PACKETS);
                    /* adding this to kick back for more data.*/
                    amount_to_read = 0;
                }
            }
#endif
            if(amount_to_read != 0)
            {
                if(usb_audio_data->usb_tx_rate_adjust_enable)
                {
                    ...
                    for(chan = 0; chan < usb_audio_data->nrof_channels; ++chan)
                    {
                        ...
                        usb_audio_scratch_commit_update(usb_audio_data->pcm_scratch_buf[chan], scratch_buf);
                    }
                ...
                else /* usb_audio_data->usb_tx_rate_adjust_enable */
                {
                    ...
                    if(usb_audio_data->gip_enabled)
                    {
                        for(unsigned i=0; i<NUM_OF_GIP_PACKETS; i++)
                        {
                            usb_audio_data->gip_tx_sequence_id++;

                            /* GIP Sequence ID 0x00 is reserved.
                             * Sending a packet with this ID will cause a glitch of missing samples at the host */
                            if (usb_audio_data->gip_tx_sequence_id == 0) usb_audio_data->gip_tx_sequence_id++;

                            /* initial flow-rate to be set to 192 as per spec this will change based on opmsg received
                             * from application to modify the flow rate*/
                            unsigned flow_rate = 0x00C00000;
                            unsigned seq_and_length_bytes = ((unsigned) usb_audio_data->gip_tx_sequence_id) << 16;

                            /* since the expected mic capture is 24khz and 16bit audio we are sending 50 bytes of payload
                             * to accomodate 48bytes of audio data and 2 bytes of flow-rate as part of single GIP packet */ 
                            seq_and_length_bytes = seq_and_length_bytes | 0x32000000;

                            /* write header in the 
                             * offset | Value  |   Name          | Description
                             * ------------------------------------------------------------------------------------------- 
                             * 0x00   | 0x60   | GIP MessageType | Audio Data Class, Command 0
                             * 0x01   | 0x2X   | GIP Flags       | System Packet
                             * 0x02   | 0xZZ   | GIP Sequence ID | Wrapping counter for Audio Data Class that is incremented
                             *        |        |                 | each time Audio packet is sent. 0x00 is reserved.
                             * 0x03   | 0xYY   | GIP Length      | Length of Payload. (1-4 bytes)
                             * Varies | 0xXXXX | Flow Rate       | Little endian value to indicate how many bytes of render data
                             *        |        |                 | the host should send in each message. (2 bytes)
                             * Data   |        | Data            | Audio stream data
                             */
                            int header[3] = {0x20600000, (int) seq_and_length_bytes, (int) flow_rate};

                            /* writing header to the output buffer */
                            PL_ASSERT(cbuffer_write(usb_audio_data->usb_audio_buf, header, 3) == 3);

                            /* interleave the pcm channel into usb buffer */
                            amount_written = interleave_buffers(usb_audio_data->usb_audio_buf,
                                                                usb_audio_data->pcm_channel_buf,
                                                                usb_audio_data->nrof_channels,
                                                                (amount_to_read/NUM_OF_GIP_PACKETS));

                            total_amount_written += amount_written;
                            L3_DBG_MSG4("USB Tx : amount_data_available %d amount_to_read %d  amount_written %d total_amount_written %d", amount_data_available, (amount_to_read/NUM_OF_GIP_PACKETS), amount_written, total_amount_written);

                            /* something is wrong if not all expected samples copied */
                            PL_ASSERT(amount_written == (amount_to_read/NUM_OF_GIP_PACKETS));
                        }
                        /* update amount data available */
                        amount_data_available -= amount_to_read;
                        amount_written = total_amount_written;
                    }
#else
                    /* interleave the pcm channel into usb buffer */
                    amount_written = interleave_buffers(usb_audio_data->usb_audio_buf,
                    ...
                    /* update amount data available */
                    amount_data_available -= amount_to_read;
                    L3_DBG_MSG3("USB Tx : amount_data_available %d amount_to_read %d  amount_written %d", amount_data_available, amount_to_read, amount_written);
            ...
            if(amount_data_available < (2*packet_len))
            {
                ...
                L3_DBG_MSG2("USB Tx : kick the input side if less than 2ms audio left in ip_buf %d < %d", amount_data_available, 2*packet_len);
        ...
        else
        {
            ...
            if(usb_audio_data->gip_enabled)
            {
                if (!usb_audio_data->gip_rx_header_processed)
                {
                    unsigned min_amount_to_read = cbuffer_calc_amount_data_in_words(usb_audio_data->usb_audio_buf);
                    L3_DBG_MSG1("USB Rx : Processing GIP HEADER with samples %d", min_amount_to_read);
                    int sample = 0;
                    while (cbuffer_read(usb_audio_data->usb_audio_buf, &sample, 1) && sample != 0x20600000){}
                    if (sample == 0x20600000)
                    {
                        bool can_read_remaining_header = cbuffer_calc_amount_data_in_words(usb_audio_data->usb_audio_buf) >= 2;
                        if (can_read_remaining_header)
                        {
                            int remaining_header[] = {0, 0};
                            cbuffer_read(usb_audio_data->usb_audio_buf, remaining_header, 2);

                            unsigned char len_bytes[MAX_NUM_GIP_HEADER_LEN_BYTES] = {0, 0, 0};

                            len_bytes[0] = (unsigned char) (remaining_header[0] >> 24);
                            len_bytes[1] = (unsigned char) (remaining_header[1] >> 16);
                            len_bytes[2] = (unsigned char) (remaining_header[1] >> 24);

                            volatile unsigned packet_data_len_bytes = len_bytes[0] & 0x7F;  //TODO: remove volatile (was for debugging)
                            volatile unsigned multiplier = 1;  //TODO: remove volatile
                            for (int i = 1; i < MAX_NUM_GIP_HEADER_LEN_BYTES && len_bytes[i - 1] & 0x80; i++){
                                multiplier *= 0x80;
                                packet_data_len_bytes += (len_bytes[i] & 0x7F) * multiplier;
                            }

                            usb_audio_data->gip_rx_packet_num_audio_samples = packet_data_len_bytes / 2; //16 bits per sample
                            L3_DBG_MSG2("USB Rx : packet_data_len_bytes %d gip_rx_packet_num_audio_samples %d", packet_data_len_bytes, usb_audio_data->gip_rx_packet_num_audio_samples);
                            PL_ASSERT(usb_audio_data->gip_rx_packet_num_audio_samples == 96);  //TODO: Temporary!!
                            usb_audio_data->gip_rx_header_processed = TRUE;
                        }
                        else
                        {
                            cbuffer_advance_read_ptr(usb_audio_data->usb_audio_buf, -1);
                        }
                    }
                }

                if (!usb_audio_data->gip_rx_header_processed) return;
            }
#endif
            ...
            L3_DBG_MSG1("USB Rx : Interleaving amount_to_read %d", amount_to_read);
            for(chan = 0; chan < usb_audio_data->nrof_channels; ++chan)
            {
            ...
            if(usb_audio_data->gip_enabled)
            {
                if((amount_to_read * usb_audio_data->nrof_channels) >= usb_audio_data->gip_rx_packet_num_audio_samples) // ensure you have more than 1ms of audio
                {
                    amount_to_read = usb_audio_data->gip_rx_packet_num_audio_samples / usb_audio_data->nrof_channels;
                }
                else
                {
                    L3_DBG_MSG2("USB Rx : !!! amount_to_read * chls : %d < %d gip_rx_packet_samples !!!", amount_to_read * 2, usb_audio_data->gip_rx_packet_num_audio_samples);
                    return;
                }
            }
#endif
            if(amount_to_read)
            {
                ...
                L3_DBG_MSG2("USB Rx : amount_to_read %d amount_written %d", amount_to_read, amount_written);
                /* something is wrong if not all expected samples copied */
                PL_ASSERT(amount_written == amount_to_read);
                ...
                if(amount_written)
                {
                    ...
                    if(usb_audio_data->gip_enabled)
                    {
                        usb_audio_data->gip_rx_header_processed = FALSE;
                    }
#endif
                }
            }
...
{
    bool retval;
    LP_USB_AUDIO_PARAMETERS usb_rx_params ;
    TIME_INTERVAL min_latency, max_latency;

    USB_AUDIO_OP_DATA *usb_audio_data = get_instance_data(op_data);


    retval = cpsSetParameterMsgHandler(&usb_audio_data->parms_def ,message_data, resp_length,resp_data);
    /*Latency change is allowed only for USB Rx path*/
    if(retval == TRUE && !USB_AUDIO_IS_TX_OPERATOR(usb_audio_data))
    {
        usb_rx_params = (LP_USB_AUDIO_PARAMETERS)usb_audio_data->parms_def.param_ptr;
        if(usb_rx_params != NULL)
        {
            if(usb_rx_params->OFFSET_ENABLE_LATENCY_CHANGE == 1)
            {
                 /* get the latency from payload */
                usb_audio_data->target_latency = usb_rx_params->OFFSET_TTP_LATENCY;
                ...
                if(usb_audio_data->time_to_play != NULL)
                {
                    ...
                    if(usb_audio_data->target_latency > 15*1000)
                    {
                        min_latency = usb_audio_data->target_latency - 15*1000 ;
                    }
                    else
                    {
                        min_latency = 0;
                    }
                    max_latency = usb_audio_data->target_latency + 15*1000; 
                    ttp_configure_latency_limits(usb_audio_data->time_to_play, min_latency, max_latency);
                    L2_DBG_MSG1("USB target latency changed to %d",usb_audio_data->target_latency);  
                }
                else
                {
                    L2_DBG_MSG("USB target latency NOT SET because ttp_context NULL");      
                }
            }
        }
    }
...
bool usb_audio_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(resp)
    {
        ...
        resp=  cpsPack1Word(op_extra_data->is_usb_audio_tx, resp);
        resp = cpsPack1Word(op_extra_data->target_latency, resp);
