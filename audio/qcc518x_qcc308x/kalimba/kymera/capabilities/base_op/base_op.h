...
/* Lightweight version of base_op handlers, used to save some programme memory by
 * reducing the number of parameters passed. */
extern bool base_op_destroy_lite(OPERATOR_DATA *op_data, void **response_data);
extern bool base_op_buffer_details_lite(OPERATOR_DATA *op_data, void **response_data);
extern bool base_op_create_lite(OPERATOR_DATA *op_data, void **response_data);
...
/* The typedefs are just here to cause a firmware build failure if CAPID
 * or CAPTYPE aren't valid identifiers, so that typos in MAP_INSTANCE_DATA()
 * invocations don't slip through undetected. The resulting typedefs aren't
 * used for anything. */
#define MAP_INSTANCE_DATA(CAPID,CAPTYPE) \
    /* This is the clue that ACAT uses: */ \
    GLOBAL_DEBUG_STRING_ATTR const char ACAT_INSTANCE_TYPE_##CAPID[] = "" #CAPTYPE ""; \
    /* These are just compile-time existence checks: */ \
    typedef enum { MAP_INSTANCE_DATA_dummy1_val_##CAPID = CAPID } MAP_INSTANCE_DATA_dummy1_##CAPID; \
    typedef CAPTYPE MAP_INSTANCE_DATA_dummy2_##CAPID;
#else /* __KCC__ */
#define MAP_INSTANCE_DATA(CAPID,CAPTYPE)
#endif /* __KCC__ */
