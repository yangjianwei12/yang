...
$cbops.aec_ref_sidetone_op.amount_to_use:
   ...
   // The index of the first microphone is always 0
   r0 = 0;
   ...
   // check if we have another buffer in the input. If the MIC_IDX_FIELD is non-zero,
   // that means that we have a second mic buffer at index 1
   r0 = M[r7 + $cbops_aec_ref_sidetone_op.aec_ref_sidetone_op_struct.MIC_IDX_FIELD];
   if Z jump mono_sidetone_path;
   // we have a second mic for the sidetone path at index 1
   r0 = 1;
   call $cbops.get_cbuffer;
   r1 = r6;
...
no_spkr_backend:
   ...
   // get number of speakers (1 or 2)
   // register M3 has the number of speaker into which sidetone
   // will be mixed. For no speaker backend use-case, we limit this
   // to 1.
   r0 = M[r7 + $cbops_aec_ref_sidetone_op.aec_ref_sidetone_op_struct.NR_SPKRS_FIELD];
   M3 = r0;
   ...
   r0 = M[r8 + $cbops.param_hdr.CHANNEL_INDEX_START_FIELD];
   call $cbops.get_cbuffer;
   // register R0 has the sidetone_buf cbuffer pointer.
   // save it into I5.
   I5 = r0;

   // get the read_addr, size and start_addr of sidetone_buf
   call $cbuffer.get_read_address_and_size_and_start_address;
   I4 = r0;
   L4 = r1;
   push r2;
   ...
   r0 = M[r0];
   r2 = r0 + r3;
   // I7 has the amount of data in the sidetone buffer
   I7 = r2;
   // no data
   if Z jump exit;

   // get speaker buffer - destination buffer
   r0 = M[r7 + $cbops_aec_ref_sidetone_op.aec_ref_sidetone_op_struct.SPKR_BUFS_FIELD];
   r0 = M[r0];
   I3 = r0;

   // r0 has the spkr MMU buffer
   call $cbuffer.get_write_address_and_size_and_start_address;
   // r0: write address - save in I0
   // r1: buffer size in address
   // r2: buffer start address
   // get base address for this speaker channel - save in B0
   push r2;
   pop B0;
   I0 = r0;
   L0 = r1;
   ...
   // update sidetone buffer
   // call cbuffer.set_read_address with 
   ...
   // call $cbuffer.set_write_address with
   // r0: pointer to the cbuffer structure
   // r1: write address
