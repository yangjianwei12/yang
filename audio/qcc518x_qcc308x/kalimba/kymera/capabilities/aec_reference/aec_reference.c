...
{{OPMSG_COMMON_ID_GET_CAPABILITY_VERSION,           base_op_opmsg_get_capability_version},
 ...
 {OPMSG_AEC_REFERENCE_ID_ENABLE_LOW_LATENCY_MODE,    aec_reference_opmsg_enable_low_latency_path},
...
#ifdef AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH
static void aec_ref_llm_custom_op_map_ui_params(AEC_REFERENCE_PARAMETERS *params_ptr,
                                                cbops_aec_ref_llm_custom_op_ui *custom_params);
#endif
...
{
    AEC_REFERENCE_OP_DATA *op_extra_data = NULL;

    patch_fn_shared(aec_reference);

    op_extra_data = get_instance_data(op_data);
...
#ifdef AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH
    /* Disable the low latency framework by default */
    op_extra_data->low_latency_st_path_enabled = FALSE;
#endif /* AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH */
...
{
    AEC_REFERENCE_OP_DATA *op_extra_data = NULL;
    unsigned i, *lpadcgains = NULL;

    patch_fn_shared(aec_reference);

    op_extra_data = get_instance_data(op_data);
    lpadcgains = (unsigned*)&op_extra_data->params.OFFSET_ADC_GAIN1;
...
#ifndef AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH
...
#else
    /* free cbuffer structure for clone mic buffs */
    if(op_extra_data->low_latency_st_path_enabled)
    {
        for(i=0; i<AEC_REFERENCE_MAX_SIDETONE_MICS; i++)
        {
            if(op_extra_data->sidetone_mic_buf_arr[i] != NULL)
            {
                cbuffer_destroy_struct(op_extra_data->sidetone_mic_buf_arr[i]);
                op_extra_data->sidetone_mic_buf_arr[i] = NULL;
            }
        }
    }
    else
...
/**
 * aec_reference_get_connected_mic_no_from_index
 * \brief gets connected mic number from index
 *
 * \param op_extra_data Pointer to the AEC reference operator specific data.
 * \param mic_index mic index
 *
 * \return the index of connected mic, MAX_NUMBER_MICS if not connected
 *
 * Note: This function is only relevant to and will only be useful
 *       after all mics for the graph are connected.
 */
static unsigned aec_reference_get_connected_mic_no_from_index(AEC_REFERENCE_OP_DATA *op_extra_data, unsigned mic_index)
{
    unsigned mic_no = 0;
    unsigned idx;
    patch_fn_shared(aec_reference);
    ...
    {
        if(NULL != op_extra_data->input_stream[MicrophoneTerminalByIndex(idx)])
        {
            if(idx == mic_index)
            {
                /* found connected mic, return mic number */
                return mic_no;
            }
            mic_no++;
        }
    ...
    /* not found, return MAX_NUMBER_MICS */
    return MAX_NUMBER_MICS;
}

#ifdef AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH
/**
 * build_sidetone_graph_with_low_latency_support
 * \brief updates speaker graph to include/exclude side tone mixing
 * with low latency framework support
 *
 * \param op_extra_data Pointer to the AEC reference operator specific data.
 */
bool build_sidetone_graph_with_low_latency_support(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    tCbuffer *mic_buf[AEC_REFERENCE_MAX_SIDETONE_MICS];
    unsigned *idxs;
    cbops_graph *sidetone_graph;
    cbops_op *op_ptr;
    cbops_op *override_op_ptr;

    /* Buffer indices for cbops chain */
    unsigned st_mic_idx = 0;          /* starting buffer idx for mic input */
    /* starting input buffer for the custom operator in LL FW */
    unsigned st_custom_opr_ip_idx = st_mic_idx + op_extra_data->num_sidetone_mics;
    /* output buffer for the custom operator in LL FW */
    unsigned st_custom_opr_op_idx = st_custom_opr_ip_idx + op_extra_data->num_sidetone_mics;
    /* buffer index for sidetone filter output */
    unsigned st_filter_out_idx = st_custom_opr_op_idx + 1;
    ...
    unsigned num_io = sidetone_idx + 1; /* +1 since indexes start from 0 */

    unsigned sidetone_buf_size;
    unsigned spkr_threshold;
    unsigned safety_threshold;
    unsigned spkr_channel_status;
    unsigned num_sidetone_spkrs = 1;
    /* assumption is that speaker path is active by default */
    unsigned spkr_backend = 1;
    unsigned i,j;

    /* scratch buffer indices used by the low latency FW */
    unsigned scratch_idx_resampler = 0;
    unsigned scratch_idx_custom_opr_op = 1;
    unsigned scratch_idx_custom_opr_ip = 2;

    /* --------------------------------------------------------------------------------------------------
     *
     *  MIC_BUFFER_1 ->
     *                  Custom LLM -> Sidetone filter -> DC_RM -> resampler -> latency-control -> mix to SPKR_BUFFER
     *  MIC_BUFFER_2 ->
     *
    ...
    if ((op_extra_data->num_spkr_channels == 0) || (op_extra_data->num_sidetone_mics == 0))
    {
        return TRUE;
    }
    ...
    /* get the mic buffer for sidetone path input */
    for(i=0; i<op_extra_data->num_sidetone_mics; i++)
    {
        mic_buf[i] = NULL;
        mic_buf[i] = op_extra_data->input_stream[MicrophoneTerminalByIndex(op_extra_data->sidetone_mics[i])];
        PL_ASSERT(NULL != mic_buf[i]);
    }
    spkr_threshold = frac_mult(op_extra_data->spkr_rate, op_extra_data->kick_period_frac)+1;
    spkr_channel_status = GetSpkrChannelStatus(op_extra_data);
    ...
    /* If the sidetone graph is running without a speaker backend connection - limit
     * the number of sidetone speakers to 1
     */
    if(spkr_channel_status == AEC_REFERENCE_CONSTANT_CONN_SPKR_1_OUTPUT_ONLY)
    {
        /* No speaker backend connected */
        spkr_backend = 0;
        /* Limit number of speakers to 1 */
        num_sidetone_spkrs = MIN(1,num_sidetone_spkrs);
    }
    ...
    /* create clone cbuffers for mic buffer - num_sidetone_mics is always
     * less than or equal to AEC_REFERENCE_MAX_SIDETONE_MICS.
     */
    for(i=0; i<op_extra_data->num_sidetone_mics; i++)
    {
        /* we do not expect the sidetone buffer to be already existing at this point */
        PL_ASSERT(op_extra_data->sidetone_mic_buf_arr[i] == NULL);
        /* create clone cbuffers for the mic buffer */
        op_extra_data->sidetone_mic_buf_arr[i] = cbuffer_create(mic_buf[i]->base_addr,
                                                                cbuffer_get_size_in_words(mic_buf[i]),
                                                                BUF_DESC_SW_BUFFER);
        if(op_extra_data->sidetone_mic_buf_arr[i] == NULL)
        {
            return FALSE;
    ...
    for(i=0,j=scratch_idx_custom_opr_ip; i<op_extra_data->num_sidetone_mics; i++,j++)
    {
        cbops_set_input_io_buffer(sidetone_graph,
                                  st_mic_idx + i,
                                  st_mic_idx,
                                  op_extra_data->sidetone_mic_buf_arr[i]);
        /* Input buffer for the custom cbop operator */
        cbops_set_internal_io_buffer(sidetone_graph,
                                     st_custom_opr_ip_idx + i,
                                     st_custom_opr_ip_idx,
                                     op_extra_data->scratch_bufs[j]);
    }
    ...
    /* Internal buffer for the low latency framework. Used as an actual
     * input to the existing sidetone chain
     */
    cbops_set_internal_io_buffer(sidetone_graph,
                                 st_custom_opr_op_idx,
                                 st_custom_opr_op_idx,
                                 op_extra_data->scratch_bufs[scratch_idx_custom_opr_op]);

    /* Create a cbop copy operator to copy data from the mic buffers into intermediate
     * buffers to be used by the custom cbop operator
     */
    op_ptr = create_copy_op(op_extra_data->num_sidetone_mics,    // Number of channels
                            &idxs[st_mic_idx],                   // input index
                            &idxs[st_custom_opr_ip_idx]);        // output index
    if(!op_ptr)
    {
        goto aFailed;
    }
    ...
    /* Create the custom cbop operator. This can be either a 1:1 operator
     * or a 2:1 operator. No other configuration is currently supported.
     */
    op_ptr = create_aec_ref_llm_custom_op(op_extra_data->num_sidetone_mics, // Number of inputs
                                          &idxs[st_custom_opr_ip_idx],      // Input index
                                          &idxs[st_custom_opr_op_idx]);     // Output index
    if(!op_ptr)
    {
        goto aFailed;
    }
    cbops_append_operator_to_graph(sidetone_graph, op_ptr);
    op_extra_data->custom_op = op_ptr;

    /* Map the UI parameters */
    cbops_aec_ref_llm_custom_op_ui custom_op_ui;
    aec_ref_llm_custom_op_map_ui_params(&op_extra_data->params, &custom_op_ui);
    /* Configure the operator */
    configure_aec_ref_llm_custom_op(op_ptr, &custom_op_ui);
...
Note:  Sidetone is before resampler.
    ...
    */
    ...
    op_ptr = create_sidetone_filter_op(st_custom_opr_op_idx,
                                       st_filter_out_idx,
                                       3,
    ...
    if(resampler_out_idx != 0)
    {
        ...
        cbops_set_internal_io_buffer(sidetone_graph,
                                     st_filter_out_idx,
                                     st_filter_out_idx,
                                     op_extra_data->scratch_bufs[scratch_idx_resampler]);
    ...
    if(!spkr_backend)
    {
        /* If there is no speaker backend, then safety threshold does not make sense */
        safety_threshold = 0;
    }

    override_op_ptr = create_aec_ref_sidetone_op(
        mic_buf[0],                                            /* mic mmu buffer */
        st_mic_idx + op_extra_data->num_sidetone_mics - 1,     /* last index of clone mic buffer */
...
bool build_sidetone_graph(AEC_REFERENCE_OP_DATA* op_extra_data)
{
    ...
    unsigned spkr_channel_status;
    unsigned num_sidetone_spkrs = 1;
    unsigned sidetone_mic_idx = op_extra_data->sidetone_mics[0]; /* index of mic that will be used for sidetone */
    unsigned spkr_backend = 1;
    ...
    /* get the mic buffer for sidetone path input */
    mic_buf = op_extra_data->input_stream[MicrophoneTerminalByIndex(sidetone_mic_idx)];

    /* Sanity check, sidetone mic must be connected */
    PL_ASSERT(NULL != mic_buf);
                                                ...
                                                bool sidetone_enabled)
{
    ...
    mic_graph = op_extra_data->mic_graph;
    num_mics = op_extra_data->num_mic_channels;
    ...
    if(op_extra_data->mic_rate!=op_extra_data->output_rate)
    {
        ...
        if(op_ptr == NULL)
        {
            op_extra_data->validata_channels_error_code |= CREATE_IIR_RESAMPLE_OP_FAIL;
            return FALSE;
        }
    ...
    else if (sidetone_enabled)
    {
        ...
        if(op_ptr == NULL)
        {
            op_extra_data->validata_channels_error_code |= CREATE_SHIFT_OP_FAIL;
            return FALSE;
        }
        cbops_append_operator_to_graph(mic_graph,op_ptr);
    }
    ...
    if(!op_ptr)
    {
        op_extra_data->validata_channels_error_code |= CREATE_MUTE_OP_FAIL;
        return FALSE;
    }
    cbops_append_operator_to_graph(mic_graph,op_ptr);
    ...
    if(op_ptr == NULL)
    {
        op_extra_data->validata_channels_error_code |= CREATE_MIC_LATENCY_OP_FAIL;
        return FALSE;
    }
    cbops_append_operator_to_graph(mic_graph,op_ptr);
...
bool build_mic_graph(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    ...
    num_mics = op_extra_data->num_mic_channels;
    sidetone_enabled = (op_extra_data->using_sidetone&SIDETONE_ENABLE_FLAG) != 0;
    /*********************************************************************
    ...
    */
    ...
    /* initialise sidetone output index, 0 means that mic graph doesn't
     * support sidetone, this will change to a non-zero value if graph
     * built with sidetone support.
     */
    op_extra_data->mic_st_idx = 0;
    ...
    if(sidetone_enabled)
    {
        ...
        /* sidetone path input */
        op_extra_data->mic_st_input_idx0 = intern_idx;
    ...
    if (op_extra_data->mic_rate_ability == RATEMATCHING_SUPPORT_HW)
    {
        ...
        if(!op_extra_data->mic_sync_enable)
        {
            ...
            if(!op_ptr)
            {
                op_extra_data->validata_channels_error_code |= CREATE_RATE_MONITOR_OP_FAIL;
                goto aFailed;
            }
        ...
        )
    {
        ...
        if(!op_ptr)
        {
            op_extra_data->validata_channels_error_code |= CREATE_RATE_ADJUST_OP_FAIL;
            goto aFailed;
        }
        ...
        if(!op_ptr)
        {
            op_extra_data->validata_channels_error_code |= CREATE_DC_REMOVE_OP_FAIL;
            goto aFailed;
        }
        cbops_append_operator_to_graph(mic_graph,op_ptr);
...
bool build_spkr_graph(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    ...
    **********************************************************************/
    ...
    /* If we are only doing sidetone with a separete graph, then there
     * is no need for a speaker path */
    if(spkr_channel_status==AEC_REFERENCE_CONSTANT_CONN_SPKR_1_OUTPUT_ONLY)
    {
        return TRUE;
    }
    ...
    if(num_inputs<num_spkrs)
    {
        ...
        op_extra_data->spkr_num_idxs = num_io+num_spkrs;
#endif
    }
    else
    {
        idxs = create_default_indexes(num_io);
#ifdef ENABLE_STEREO_REFERENCE
        op_extra_data->spkr_num_idxs = num_io;
#endif
    }
    if(idxs == NULL)
    {
    ...
    if(!spkr_graph)
    {
        op_extra_data->validata_channels_error_code |= ALLOC_GRAPH_FAIL;
        goto aFailed;
    }
    ...
    */
    ...
    if(!overrid_op_ptr)
    {
        op_extra_data->validata_channels_error_code |= CREATE_AEC_SPKR_OP_FAIL;
        goto aFailed;
    }
    ...
    if(num_inputs > num_spkrs)
    {
        for(i=1;i<num_inputs;i++)
        {
            ...
            /*  - cascade a mix Operator - stereo to mono (left = a*left+(1-a)*right a=i/(i+1.0))*/
            op_ptr = create_mixer_op(intern_ins_idx, intern_ins_idx+i, intern_ins_idx, 0, frac_div(i, i+1));
            if(!op_ptr)
            {
                op_extra_data->validata_channels_error_code |= CREATE_MIXER_OP_FAIL;
                goto aFailed;
            }
            cbops_append_operator_to_graph(spkr_graph,op_ptr);
        }
...
bool validate_channels_and_build(OPERATOR_DATA *op_data)
{
    ...
    op_extra_data = get_instance_data(op_data);
    spkr_changed = aec_reference_update_spkr_channel_status(op_extra_data);
    mic_changed = aec_reference_update_mic_channel_status(op_extra_data);
    ref_changed = aec_reference_update_ref_channel_status(op_extra_data);
    ...
    {
        op_extra_data->validata_channels_error_code |= BUILD_GRAPHS_FAIL;
        goto aFailed;
    }
    ...
    if(ref_changed || spkr_changed)
    {
        ...
        {
            op_extra_data->validata_channels_error_code |= INCLUDE_REF_PATH_FAIL;
            goto aFailed;
        }
    }
#if defined(AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH) && defined(AEC_REFERENCE_ENABLE_STANDALONE_ST_PATH)
    if(op_extra_data->low_latency_st_path_enabled && op_extra_data->sidetone_graph != NULL)
    {
        /* If low latency sidetone path is enabled, then we can have only
         * the sidetone graph running without any mic or spkr backend
         * connected. In that case, we allow the graph to run
         */
         st_only = TRUE;
    }
#endif /* AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH && AEC_REFERENCE_ENABLE_STANDALONE_ST_PATH*/
       ...
       st_only)
    {
        ...
        {
            timer_schedule_event_in_atomic(op_extra_data->kick_period, aec_reference_timer_task,(void*)op_data, &op_extra_data->kick_id);
        }
    }
    else
    {
...
#ifdef ENABLE_STEREO_REFERENCE
    op_extra_data->num_ref_channels = 0;
    if(op_extra_data->output_stream[AEC_REF_REFERENCE_TERMINAL])
    {
        op_extra_data->num_ref_channels = 1;
        if(op_extra_data->output_stream[AEC_REF_REFERENCE_TERMINAL1])
        {
            /* Stereo Reference output */
            op_extra_data->num_ref_channels = 2;
        }
    }
#endif

#if defined(AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH) && defined(AEC_REFERENCE_ENABLE_STANDALONE_ST_PATH)
    /* This allows forming a standlone sidetone path independent of the
     * backend speaker path connection.
     */
    if(spkr_channel_status == 0)
    {
        /* We cannot form a speaker path, check if we can form
         * as 0->1 speaker path, this would enable supporting
         * sidetone without connected speaker input
         */
         inputBuf = op_extra_data->input_stream[SpeakerInputTerminalByIndex(0)];
         outputBuf = op_extra_data->output_stream[SpeakerTerminalByIndex(0)];
         if((inputBuf == NULL) && (outputBuf != NULL))
         {
             /* There is no input stream connected to the input terminal 0
              * But there is a stream connected to the speaker terminal 0.
              * In this case, we can have a 0->1 speaker path
              */
              spkr_channel_status = AEC_REFERENCE_CONSTANT_CONN_SPKR_1_OUTPUT_ONLY;
              number_spkrs = 1;

            /* However, also check that all other in/spkr are disconnected */
            for(i=1; i<MAX_NUMBER_SPEAKERS;i++)
            {
                inputBuf = op_extra_data->input_stream[SpeakerInputTerminalByIndex(i)];
                outputBuf = op_extra_data->output_stream[SpeakerTerminalByIndex(i)];
                if(inputBuf != NULL || outputBuf != NULL)
                {
                    /* Another speaker input and/or output is connected,
                     * we cannot have 0->1 speaker path either
                     */
                     spkr_channel_status = 0;
                     number_spkrs = 0;
                     break;
                }
            }
...
bool aec_reference_update_mic_channel_status(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    ...
    if(mic_channel_status == 0)
    {
        ...
        if(inputBuf != NULL && outputBuf == NULL)
        {
...
#ifndef AEC_REFERENCE_ENABLE_STEREO_ST_MIC
            ...
            for(i=1;i<MAX_NUMBER_MICS;i++)
            {
                ...
                if(outputBuf != NULL)
                {
                    /* An output is connected. We cannot have N->0 mic path */
                    mic_channel_status = 0;
                    number_mics = 0;
                    break;
                }
                if(inputBuf != NULL)
                {
                    /* Multiple microphones as input to the sidetone path is possible
                     * only if we are in the low latency mode
                     */
                    if(op_extra_data->low_latency_st_path_enabled)
                    {
                        /* Another mic is connected. However ensure that we do not have
                         * more than AEC_REFERENCE_MAX_SIDETONE_MICS mics
                         */
                         number_mics++;
                         if(number_mics > AEC_REFERENCE_MAX_SIDETONE_MICS)
                         {
                             mic_channel_status = 0;
                             number_mics = 0;
                             break;
                    ...
                    {
                         /* Not allowed - Another mic input is connected */
                         mic_channel_status = 0;
                         number_mics = 0;
                         break;
                    }
    ...
    if((GetMicChannelStatus(op_extra_data)) == mic_channel_status && (op_extra_data->num_mic_channels == number_mics))
    {
...
/**
 * aec_reference_update_sidetone_mics
 * \brief sets the number of mics that are needed for sidetone mixing
 *
 * \param op_extra_data Pointer to the AEC reference operator specific data.
 *
 * Note: currently this function limits sidetone_mic to maximum 1, in other words
 *      it only checks if a mic has been configured for first speaker, if it
 *      has then the mic will be used for all speaker, if it hasn't then no sidetone
 *      mixing will happen.
 */
void aec_reference_update_sidetone_mics(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    patch_fn_shared(aec_reference);
    /* 32-bit sidetone_map0 config word has 8 nibbles(1 nibble = 4bits), each corresponding
     * to spkr0 up to spkr7. It tells which mic shall be mixed into that speaker:
     *
     * sidetone_map0 = |spkr7_mic|.....|spkr1_mic|spkr0_mic|
     *
     * Currently we only support one mic for sidetone, so this code currently looks at
     * only spkr0_mic and the rest of configs will be ignored:
     **
     **    if spkr0_mic is a valid and available mic:
     *          it will will be be used as source of sidetone
     *          when sidetone is enabled
     *     else:
     *          as if sidetone has been disabled by the user
     */
    uint32 sidetone_mics_map0 = (uint32)op_extra_data->params.OFFSET_SIDETONE_MICS_MAP0;
    unsigned sidetone_spkr0_mic_idx = GetSidetoneMicForSpeaker(sidetone_mics_map0, 0);
    op_extra_data->num_sidetone_mics = 0;
    if(sidetone_spkr0_mic_idx != AEC_REF_SIDETONE_MIC_NONE)
    {
        /* sidetone_spkr0_mic is valid */
        if(NULL != op_extra_data->input_stream[MicrophoneTerminalByIndex(sidetone_spkr0_mic_idx)])
        {
            /* sidetone_spkr0_mic is valid and available, it will
             * be used for sidetone source when sidetone is needed.
             */
            op_extra_data->num_sidetone_mics = 1;
            op_extra_data->sidetone_mics[0] = sidetone_spkr0_mic_idx;
        }
#ifdef AEC_REFERENCE_ENABLE_STEREO_ST_MIC
        /* Select the secondary microphone to be used for the low latency framework.
         * Read this value only if the low latency fw is enabled
         */
        if(op_extra_data->num_sidetone_mics == 1 && op_extra_data->low_latency_st_path_enabled)
        {
            uint32 secondary_mic_idx = (uint32)op_extra_data->params.OFFSET_LOW_LATENCY_FW_SECONDARY_MIC;
            if(secondary_mic_idx >= MAX_NUMBER_MICS)
            {
                secondary_mic_idx = 0;
            }
            /* Ensure that both the microphones input to the low latency framework are
             * different. If both of them are same, it means that we are working in
             * one-mic mode */
            if(secondary_mic_idx != sidetone_spkr0_mic_idx)
            {
                if (NULL != op_extra_data->input_stream[MicrophoneTerminalByIndex(secondary_mic_idx)]) 
                {
                    op_extra_data->num_sidetone_mics = 2;
                    op_extra_data->sidetone_mics[1] = secondary_mic_idx;
                }
            }
        }
#endif /* AEC_REFERENCE_ENABLE_STEREO_ST_MIC */
    }
}
...
#ifdef AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH
    /* We also need sidetone path to be enabled if the
     * low latency mode is enabled irrespective of how the
...
#endif
    /* update mics that shall be used for sidetone */
    aec_reference_update_sidetone_mics(op_extra_data);

    /* see if we need sidetone */
    if((op_extra_data->num_mic_channels > 0)
       && (op_extra_data->num_spkr_channels > 0)
       && (op_extra_data->num_sidetone_mics > 0))
    {
...
void aec_reference_update_sidetone_method(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    ...
    if(op_extra_data->low_latency_st_path_enabled)
    {
        /* For the low latency path, we use the software sidetone path
         * and use the separate sidetone graph for sidetone mixing. If
         * this mode is enabled, we ignore the settings in the params
         * and set the sidetone_method to AEC_SIDETONE_IN_SW_USING_SEPARATE_GRAPH
         */
         sidetone_method = AEC_SIDETONE_IN_SW_USING_SEPARATE_GRAPH;
    }
#endif
...
{
    cbops_graph *spkr_graph = NULL;
    cbops_graph *mic_graph = NULL;

    patch_fn_shared(aec_reference);
    spkr_graph = op_extra_data->spkr_graph;
    mic_graph = op_extra_data->mic_graph;
    ...
    if(include_sidetone && op_extra_data->mic_st_idx == 0)
    {
        /* We are forming sidetone path, but already-formed mic path doesn't support it.
         * Normally this happens when mic path is formed with SIDETONE_DISABLE = TRUE,
         * but later and before forming speaker path it is set to FALSE. That is not a valid
         * change, we gracefully return failure to user.
         */
        L1_DBG_MSG("AEC_REFERENCE: failed to setup sidetone path,"
                   "possibly change of SIDETONE_DISABLE flag after mic connections");
        return FALSE;
    }
    ...
    if(include_sidetone)
    {
        ...
        cbops_op    *op_ptr;
        unsigned sidetone_mic_idx = op_extra_data->sidetone_mics[0]; /* index of mic that will be used for sidetone */
        unsigned sidetone_mic_no = aec_reference_get_connected_mic_no_from_index(op_extra_data, sidetone_mic_idx);

        /* Sanity check, sidetone mic must be connected */
        PL_ASSERT(sidetone_mic_no < op_extra_data->num_mic_channels);
        /* get the cbops buffer index for sidetone path input */
        unsigned mic_st_input_idx = op_extra_data->mic_st_input_idx0 + sidetone_mic_no;
        ...
        if(op_extra_data->mic_rate != op_extra_data->spkr_rate)
        {
            ...
            mic_st_rs_idx = mic_st_input_idx;
        }
        else
        {
        ...
        op_ptr = create_sidetone_filter_op(mic_st_input_idx, mic_st_rs_idx, 3,
                                           (cbops_sidetone_params*)&op_extra_data->params.OFFSET_ST_CLIP_POINT,
                                           (void*)&op_extra_data->params.OFFSET_ST_PEQ_CONFIG);
...
bool aec_reference_spkr_include_ref_path(AEC_REFERENCE_OP_DATA* op_extra_data, bool include_ref_path)
{
    ...
    if(include_ref_path)
    {
        ...
        unsigned ref_input_idx = op_extra_data->spkr_ref_input_idx; // The start of the scratch buffers
        unsigned ref_rm_in_idx = ref_input_idx;
        cbops_op *after = op_extra_data->spkr_ref_point_op;
        unsigned spkr_num_ref_ops = 0;

#ifdef ENABLE_STEREO_REFERENCE
        unsigned num_ref_channels = op_extra_data->num_ref_channels;
        unsigned spkr_channel_status = GetSpkrChannelStatus(op_extra_data);
        unsigned *idxs = create_default_indexes(op_extra_data->spkr_num_idxs);

        /* Add a check for the requirement for a second ref output (i.e. 2 inputs, 2 speakers and second ref
         * channel connected. Note that it is assumed that the speaker path channels are perfectly aligned */
        unsigned num_inputs = op_extra_data->num_spkr_channels;
...
#ifdef ENABLE_STEREO_REFERENCE
        /* Add the extra output buffer for the second ref channel in cbops graph (if needed) */
        if(num_ref_channels==2)
        {
            cbops_set_output_io_buffer(spkr_graph,
                                   ref_idx+1,
                                   ref_idx,
                                   op_extra_data->output_stream[AEC_REF_REFERENCE_TERMINAL1]);
        }

        /* handle the case where we have 2 reference channels but only one input channel */
        if(num_ref_channels==2 && num_inputs==1)
        {
            // Set-up the next operator to copy from the same index twice
            idxs[ref_input_idx+1] = idxs[ref_input_idx];
        }
#endif
        ...
        if(op_extra_data->input_rate != op_extra_data->output_rate)
        {
...
#ifdef ENABLE_STEREO_REFERENCE
            /* This should be + num_ref_channels */
            ref_rm_in_idx = ref_idx + num_ref_channels;
#else
            ...
            if(num_ref_channels==2)
            {
                cbops_set_internal_io_buffer(spkr_graph,
                                         ref_rm_in_idx + 1,
                                         ref_rm_in_idx,
                                         op_extra_data->scratch_bufs[op_extra_data->spkr_ref_scratch_idx + 1]);
            }

            /* Add reference sample rate conversion (input_rate --> output_rate) - for up to two channels only */
            op_ptr = create_iir_resamplerv2_op(num_ref_channels, &idxs[ref_input_idx], &idxs[ref_rm_in_idx],
...
#ifdef ENABLE_STEREO_REFERENCE
        op_ptr = create_sw_rate_adj_op(num_ref_channels, &idxs[ref_rm_in_idx], &idxs[ref_idx],
                                       CBOPS_RATEADJUST_COEFFS,
        ...
        /* For stereo Ref it is not needed to look at both channels, as it is assumed that they are 
         * perfectly aligned */
...
#ifdef ENABLE_STEREO_REFERENCE
        // remove the stereo reference channel, if it is present
        cbops_unset_buffer(spkr_graph, ref_idx+1);
#endif
...
#ifdef ENABLE_STEREO_REFERENCE
            // remove the stereo reference channel, if it is present
            cbops_unset_buffer(spkr_graph, op_extra_data->spkr_ref_rs_idx+1);
#endif
...
bool aec_reference_allocate_scratch_buffers_for_cbops_graphs(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    ...
    if(op_extra_data->low_latency_st_path_enabled)
    {
        /* Number of scratch buffers required for the low latency FW */
        if(num_scratch_buffs < AEC_REFERENCE_LL_FW_NUM_SCRATCH_BUFF)
        {
            num_scratch_buffs = AEC_REFERENCE_LL_FW_NUM_SCRATCH_BUFF;
        }
    }
#endif
...
bool aec_reference_build_graphs(AEC_REFERENCE_OP_DATA *op_extra_data, bool spkr_changed, bool mic_changed)
{
    ...
    {
        op_extra_data->validata_channels_error_code |= MALLOC_FAIL;
    ...
    /** Setup Latency Control */
    {
        ...
        op_extra_data->sync_block.jitter     =  one_task_samples/2;               /* 1.5 times task period */
        op_extra_data->sync_block.ref_delay  = 2*one_ms_samples;                                       /* 1.0 msec */
    ...
    if(spkr_changed)
    {
        ...
        {
            op_extra_data->validata_channels_error_code |= BUILD_SPKR_FAIL;
            return FALSE;
        }
    }
    ...
    if(mic_changed)
    {
        ...
        {
            op_extra_data->validata_channels_error_code |= BUILD_MIC_FAIL;
            return FALSE;
        }
    }
    ...
    {
        op_extra_data->validata_channels_error_code |= INCLUDE_SIDETONE_FAIL;
        return FALSE;
    }
    ...
    {
        op_extra_data->validata_channels_error_code |= BUILD_SIDETONE_FAIL;
        return FALSE;
    }
#else
    /* build separate sidetone graph with low latency framework support */
    if(op_extra_data->low_latency_st_path_enabled)
    {
        if(!build_sidetone_graph_with_low_latency_support(op_extra_data))
        {
            return FALSE;
        }
    }
...
bool aec_reference_connect(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    tCbuffer* pterminal_buf = NULL;
    tCbuffer **bufp = NULL;

    patch_fn_shared(aec_reference);

    op_extra_data = get_instance_data(op_data);
    terminal_id = OPMGR_GET_OP_CONNECT_TERMINAL_ID(message_data);    /* extract the terminal_id */
...
#ifdef ENABLE_STEREO_REFERENCE
                || terminal_id==AEC_REF_REFERENCE_TERMINAL1
#endif
               )
        {
...
bool aec_reference_buffer_details(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    op_extra_data = get_instance_data(op_data);
    term_id = OPMGR_GET_OP_BUF_DETAILS_TERMINAL_ID(message_data);
    ...
    if ( term_id & TERMINAL_SINK_MASK)
    {
        if(IsMicrophoneTerminal(term_id))
        {
            ...
            resp->b.buffer_size  = op_extra_data->mic_buffer_size;
        }
        else
        {
    ...
    else
    {
        if(IsSpeakerTerminal(term_id))
        {
            ...
            resp->needs_override = TRUE;
            resp->b.buffer_size  = op_extra_data->spkr_buffer_size;
        }
        else
        {
...
#ifdef ENABLE_STEREO_REFERENCE
                        || AEC_REF_REFERENCE_TERMINAL1 == term_id
#endif
                        )
                {
...
bool aec_reference_get_sched_info(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    op_extra_data = get_instance_data(op_data);
    terminal_id = OPMGR_GET_OP_SCHED_INFO_TERMINAL_ID(message_data);
...
bool aec_reference_opmsg_obpm_set_params(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(retval == TRUE)
    {
        ...
        AEC_REFERENCE_PARAMETERS *aec_params = (AEC_REFERENCE_PARAMETERS*) (op_extra_data->parms_def.param_ptr);
        if(aec_params != NULL)
        {
            if(aec_params->OFFSET_ENABLE_LATENCY_CHANGE == 1)
            {

                /* get the latency from payload */
                op_extra_data->mic_target_latency = aec_params->OFFSET_TTP_LATENCY;
                ...
                if(op_extra_data->mic_time_to_play != NULL)
                {
                    ...
                    /* configure the time to play limits again with 15msec delta */
                    if(op_extra_data->mic_target_latency > 15*1000)
                    {
                        min_latency = op_extra_data->mic_target_latency - 15*1000 ;
                    }
                    else
                    {
                        min_latency = 0;
                    }
                    max_latency = op_extra_data->mic_target_latency + 15*1000; 
                    ttp_configure_latency_limits(op_extra_data->mic_time_to_play, min_latency, max_latency);
                    

                    /* receiving this message (with latency != 0) will enable metadata generation */

                    op_extra_data->mic_metadata_enable = op_extra_data->mic_target_latency != 0;
                    L2_DBG_MSG1("AEC_REF LATENCY CHANGED to %d",op_extra_data->mic_target_latency);
                
                }
                else
                {
                    L2_DBG_MSG("AEC_REF LATENCY NOT SET because ttp_context NULL");     
                }
...
bool aec_reference_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(resp)
    {
        ...
        {
            resp = cpsPack1Word(op_extra_data->mic_target_latency, resp);
        }
    }
                    ...
                    uint16 length, unsigned* data, STATUS_KYMERA status,uint16 extra_status_info)
{
    ...
    op_extra_data = get_instance_data((OPERATOR_DATA*)instance_data);
...
void aec_reference_mic_generate_metadata_with_ttp(AEC_REFERENCE_OP_DATA *op_extra_data, unsigned samples)
{
    ...
    tCbuffer *met_buf = NULL;

    patch_fn_shared(aec_reference);

    met_buf = op_extra_data->mic_metadata_buffer;
    ...
    if (mtag != NULL)
    {
        ...
        TIME_INTERVAL time_passed = (TIME_INTERVAL) (((uint48)(amount_in_buffer)*SECOND)/op_extra_data->mic_rate) +
            (TIME_INTERVAL) (((uint48)(samples)*SECOND)/op_extra_data->output_rate);
...
bool aec_reference_opmsg_enable_mic_sync(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
...
#ifdef ENABLE_STEREO_REFERENCE
       (NULL != op_extra_data->output_stream[AEC_REF_REFERENCE_TERMINAL1]) ||
#endif
    ...
    L2_DBG_MSG1("AEC REFERENCE input-output use same clock: %d ", op_extra_data->mic_sync_enable);
...
bool aec_reference_opmsg_set_task_period(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(op_extra_data->low_latency_st_path_enabled)
    {
        /* TASK_PERIOD cannot be changed once the low latency mode is enabled
         * print log and exit
         */
         L2_DBG_MSG("AEC REFERENCE cannot set task period in low latency mode");
         return FALSE;
    }
#endif /* AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH */
 ...
 *        can be used to configure required buffer size for input, output, spkr
 *        and mic terminals.
 * \param op_data Pointer to the operator instance data.
 * \param message_data Pointer to the start request message
 * \param resp_length pointer to location to write the response message length
                                         ...
                                         unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    /* ----------------------------------------------------------------
     * 1.   see which terminal types need configuring buffer size,
     *    all terminals of the same type will report same required
     *    buffer size, so only first terminal of that type is checked
     * ---------------------------------------------------------------*/

    /* see if input buffer size config requested */
    bool config_input_buffers = (sinks & (1<<AEC_REF_INPUT_TERMINAL1)) != 0;

    /* see if output buffer size config requested */
    bool config_output_buffers = (sources & (1<<AEC_REF_OUTPUT_TERMINAL1)) != 0;

     /* see if spkr buffer size config requested */
    bool config_spkr_buffers = (sources & (1<<AEC_REF_SPKR_TERMINAL1)) != 0;

     /* see if mic buffer size config requested */
    bool config_mic_buffers = (sinks & (1<<AEC_REF_MIC_TERMINAL1)) != 0;

    /* -------------------------------------------------------------------------------
     * 2. for each terminal type check if the buffer size can change, all the terminals
     * of that type must be disconnected at this time but the operator can be running.
     *-------------------------------------------------------------------------------*/
    unsigned idx;
    if(config_output_buffers)
    {
    ...
    if(config_spkr_buffers)
    {
        /* No spkrs must be connected */
        for(idx = 0; idx < MAX_NUMBER_SPEAKERS; idx++)
        {
            if(NULL != op_extra_data->output_stream[SpeakerTerminalByIndex(idx)])
            {
                return FALSE;
            }
        }
    ...
    if(config_mic_buffers)
    {
        /* No mic must be connected */
        for(idx = 0; idx<MAX_NUMBER_MICS; idx++)
        {
            if(NULL != op_extra_data->input_stream[MicrophoneTerminalByIndex(idx)])
            {
    ...
    /* --------------------------------------------------
     * 3. All checks done, set the required buffer size
     * -------------------------------------------------*/
    if(config_output_buffers)
    {
    ...
    if(config_spkr_buffers)
    {
        /* set the speaker buffer size */
        op_extra_data->spkr_buffer_size = buffer_size;
        L2_DBG_MSG1("AEC_REFERENCE: minimum spkr buffer size set to %d words ", buffer_size);
    }

    if(config_mic_buffers)
    {
        /* set the mic buffer size */
        op_extra_data->mic_buffer_size = buffer_size;
        L2_DBG_MSG1("AEC_REFERENCE: minimum mic buffer size set to %d words ", buffer_size);
    }
    return TRUE;
}
...
void aec_reference_mic_ref_latency_limit_control(AEC_REFERENCE_OP_DATA *op_extra_data)
{
...
#ifdef ENABLE_STEREO_REFERENCE
    tCbuffer *ref_buf1 = op_extra_data->output_stream[AEC_REF_REFERENCE_TERMINAL1];
#endif
    ...
    if(sync_block->mic_data < block_size)
    {
        if(NULL != ref_buf)
        {
            unsigned spkr_delay = sync_block->speaker_delay;
            /* just update the current speaker delay */
            sync_block->speaker_delay = sync_block->speaker_data - sync_block->mic_data;
            if(spkr_delay!= sync_block->speaker_delay)
            {
                L2_DBG_MSG2("Speaker delay changed at line %d to %d", __LINE__, sync_block->speaker_delay);
            }
        }
        return;
    }
    ...
    if(NULL != ref_buf)
    {
         ...
         unsigned target_latency = sync_block->mic_data + sync_block->ref_delay;
         unsigned speaker_data_low = target_latency - sync_block->jitter;
         unsigned speaker_data_high = target_latency + sync_block->jitter;
         if(speaker_data < (int)speaker_data_low)
         {
             /* reached lower limit, insert silence into reference path */
             unsigned amount_to_insert = target_latency - speaker_data;
             ...
             if(ref_buf1!=NULL)
             {
                cbuffer_block_fill(ref_buf1, amount_to_insert, 0);
             }
#endif
             sync_block->speaker_inserts += amount_to_insert;
             speaker_data += amount_to_insert;
             L2_DBG_MSG3("@%d: Amount to insert %d speaker_data = %d", time_get_time(),
                         amount_to_insert, speaker_data);
             if(op_extra_data->spkr_ref_active)
             {
         ...
         else if (speaker_data > (int) speaker_data_high)
         {
             ...
             unsigned amount_to_drop = speaker_data - target_latency;
             ...
             if(ref_buf1!=NULL)
             {
                cbuffer_advance_write_ptr(ref_buf1, cbuffer_get_size_in_words(ref_buf1)-amount_to_drop);
             }
#endif
             sync_block->speaker_drops += amount_to_drop;
             speaker_data -= amount_to_drop;
             L2_DBG_MSG3("@%d: Amount to drop %d speaker_data = %d", time_get_time(),
                         amount_to_drop, speaker_data);
             if(op_extra_data->spkr_ref_active)
             {
                 /* Any update outside cbops needs refreshing cbops graph */
                 cbops_refresh_buffers(op_extra_data->spkr_graph);
...
void aec_reference_timer_task(void *kick_object)
{
    ...
    patch_fn(aec_reference_timer1);

    base_op_profiler_start((OPERATOR_DATA*) kick_object);

    op_data = (OPERATOR_DATA*) kick_object;
    op_extra_data = get_instance_data(op_data);
    ...
    if(op_extra_data->ReInitFlag==TRUE)
    {
        ...
        {
            /* set ramping steps for sidetone gain */
            cbops_sidetone_filter_set_ramping_step(op_extra_data->mic_sidetone_op,
                                                   op_extra_data->params.OFFSET_STF_GAIN_RAMP_EXP,
                                                   op_extra_data->params.OFFSET_STF_GAIN_RAMP_LIN,
                                                   op_extra_data->kick_period);
...
#endif /* INSTALL_AEC_REFERENCE_HOWL_LIMITER */
#ifdef AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH
        if(op_extra_data->custom_op)
        {
            /* If the custom operator exists, this means that low latency framework
             * is enabled
             */
             cbops_aec_ref_llm_custom_op_ui custom_op_ui;
             /* Read the UI params */
             aec_ref_llm_custom_op_map_ui_params(&op_extra_data->params, &custom_op_ui);
             /* Configure the operator */
             configure_aec_ref_llm_custom_op(op_extra_data->custom_op, &custom_op_ui);
        }
#endif /* AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH */
    }
...
#endif /* AEC_REFERENCE_CAN_PERFORM_INPLACE_SIDETONE_MIXING */
    {
        ...
        if(mic_graph_active)
        {
            ...
            /* run cbops process for mic graph */
            ...
            if(!op_extra_data->mic_graph_no_output)
            {
                ...
                {
                    /* get new amount written to mic outputs by cbops process
                     * Note: mic_latency_index is cbops buffer index for first mic output buffer */
                    unsigned mic_new_transfer = cbops_get_amount(op_extra_data->mic_graph, op_extra_data->mic_latency_index);

                    /* generate metadata for newly written output chunk */
                    aec_reference_mic_generate_metadata_with_ttp(op_extra_data, mic_new_transfer);
                }
            }
#endif /* AEC_REFERENCE_GENERATE_MIC_TIMESTAMP */
        }

        patch_fn(aec_reference_timer3);
        ...
        if(spkr_graph_active)
        {
            ...
            if(met_buf!= NULL && buff_has_metadata(met_buf))
            {
                ...
                aec_reference_spkr_ttp_run(op_data, &max_to_process);
#endif
            }
...
{
    AEC_REFERENCE_OP_DATA *op_extra_data = NULL;
    OPMSG_GET_CONFIG *msg = NULL;
    OPMSG_GET_CONFIG_RESULT *result = NULL;
    unsigned term_idx;
    ...
    msg = message_data;
    result = msg->result;
    term_idx = msg->header.cmd_header.client_id;
...
bool aec_reference_opmsg_ep_configure(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    op_extra_data = get_instance_data(op_data);
    msg = message_data;
    terminal_id = msg->header.cmd_header.client_id;
    value = msg->value;
    ...
    switch(msg->key)
    {
        ...
        case OPMSG_OP_TERMINAL_PROC_TIME:
            /* uint32 - N/A an operator will never receive this */
            break;
        ...
        case OPMSG_OP_TERMINAL_RATEMATCH_ENACTING:
            ...
            if(terminal_id == (AEC_REF_INPUT_TERMINAL1|TERMINAL_SINK_MASK) )
            {
                ...
                {
                    /* We have received a new RATEMATCH_ENACTING config while speaker path is running,
                     * this normally can happen if there is a disconnection/connection in upstream
                     * while the speaker path is running. If speaker path is in timed-playback mode it
                     * will stay in that mode for as long as speaker path is running, applying new
                     * config will not change that but might cause glitches, so we ignore it.
                     */
                    break;
                }
...
bool aec_reference_opmsg_ep_clock_id(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    msg = message_data;
    terminal_id = msg->header.cmd_header.client_id;
    int_id = base_op_get_int_op_id(op_data);
...
#ifdef AEC_REFERENCE_ENABLE_LOW_LATENCY_PATH
/* Map the user parameters for the custom cbop operator */

static void aec_ref_llm_custom_op_map_ui_params(AEC_REFERENCE_PARAMETERS *params_ptr,
                                                cbops_aec_ref_llm_custom_op_ui *custom_params)
{
    custom_params->rsv_param_0 = params_ptr->OFFSET_RSV_PARAM_0;
    custom_params->rsv_param_1 = params_ptr->OFFSET_RSV_PARAM_1;
    custom_params->rsv_param_2 = params_ptr->OFFSET_RSV_PARAM_2;
    custom_params->rsv_param_3 = params_ptr->OFFSET_RSV_PARAM_3;
    custom_params->rsv_param_4 = params_ptr->OFFSET_RSV_PARAM_4;
}


bool aec_reference_opmsg_enable_low_latency_path(OPERATOR_DATA *op_data, void *message_data,
                                                 unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    AEC_REFERENCE_OP_DATA *op_extra_data = get_instance_data(op_data);
    ...
    bool enable = OPMSG_FIELD_GET(message_data, OPMSG_AEC_ENABLE_LOW_LATENCY_MODE, ENABLE);
    bool status = TRUE;
    if(enable)
    {
        if(!op_extra_data->low_latency_st_path_enabled)
        {
            /* The low latency mode in AEC_REF uses the software sidetone path. The path is
             * typically enabled by the following parameters:
             * 1. Disabling the SIDETONE_DISABLE config bit.
             * 2. Enable the SIDETONEENA config bit.
             * Sending the ENABLE_LOW_LATENCY_PATH operator message enables the use of a
             * separate sidetone chain irrespective of the parameter settings. If this mode
             * is enabled, then the side-tone path usage cannot be disabled by using QACT also.
             * The application should send this same operator message to exit this mode.
             * This message also sets up the proper value of the task period required for the
             * sidetone graph. It uses the existing value of the task period and adjust only the
             * decimation factor such that the effective period of the sidetone path is set to
             * AEC_REFERENCE_SIDETONE_PATH_TASK_PERIOD.
             *
             * To note is that since the decimation factor computation depends on the current
             * task period the AEC_REF, the capability will not accept the 'SET_TASK_PERIOD' once
             * this mode is set.
             */
            unsigned task_period = op_extra_data->task_period;

            /* To enable low latency sidetone mode in AEC_REFERENCE, the task period of the
             * AEC_REF should be an integer multiple of AEC_REFERENCE_SIDETONE_PATH_TASK_PERIOD us
             */
            if(task_period % AEC_REFERENCE_SIDETONE_PATH_TASK_PERIOD != 0)
            {
                L2_DBG_MSG("AEC_REFERENCE: To Enable Low latency mode, task period should be multiple of 500us");
                status = FALSE;
                return status;
            }
            unsigned decim_factor = task_period / AEC_REFERENCE_SIDETONE_PATH_TASK_PERIOD;

            status = aec_reference_set_task_period(op_extra_data, task_period, decim_factor);
            if(status)
            {
                op_extra_data->low_latency_st_path_enabled = TRUE;
            }
        }
    }
    else
