...
static void vol_ctrl_kick_waiting(VOL_CTRL_DATA_T *op_extra_data, TOUCHED_TERMINALS *touched, VOL_CTRL_KICK_REASON reason)
{
    ...
    switch(reason)
    {
        case VOL_CTRL_KICK_WAIT_ON_DATA:
        case VOL_CTRL_KICK_WAIT_ON_DATA_LATE:
        {
             L2_DBG_MSG1("volume_control: Data is late kicking back ... with reason: %d", reason);
             touched->sinks |= op_extra_data->touched_sink;
             break;
        }
        default:
        {
             L3_DBG_MSG3("volume_control: Need to kick back / forward because of kick reason %d touched source 0x%x touched sink 0x%x", reason, touched->sources, touched->sinks);
             break;
        }
    }
...
bool vol_ctlr_opmsg_set_ucid(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    key = MAP_CAPID_UCID_SBID_TO_PSKEYID(_VOL_CTRL_VOL_CAP_ID,
                                         op_extra_data->parms_def.ucid,
...
bool vol_ctlr_opmsg_set_sample_rate(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    op_extra_data->sample_rate = SAMPLE_RATE_FROM_COMMON_OPMSG(message_data);
