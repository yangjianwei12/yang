...
typedef enum
{
    ...
    AANC_ANC_INSTANCE_BOTH_ID = 0x0003
} AANC_ANC_INSTANCE;
...
/* Fine gain update function */
#ifdef AANC_MAOR_V20
#define AANC_UPDATE_FINE_GAIN(instance, path, gain) \
    stream_anc_set_anc_foreground_fine_gain(instance, path, gain)
#define AANC_UPDATE_BACKGROUND_GAINS(mask) \
    stream_anc_update_background_gains(mask)
#else
#define AANC_UPDATE_FINE_GAIN(instance, path, gain) \
    stream_anc_set_anc_fine_gain(instance, path, gain)
#define AANC_UPDATE_BACKGROUND_GAINS(mask) (void)0
#endif
...
typedef struct aanc_exop
{
    ...
    AANC_GAIN ec_gain;
#ifdef AANC_MAOR_V20
    AANC_GAIN rx_ffa_mix_gain;
    AANC_GAIN rx_ffb_mix_gain;
#endif
...
#ifdef AANC_MAOR_V20
    AANC_GAIN rx_ffa_mix_gain_prev;
    AANC_GAIN rx_ffb_mix_gain_prev;
#endif
...
#ifdef AANC_MAOR_V20
    AANC_GAIN rx_ffa_mix_static_gain;
    AANC_GAIN rx_ffb_mix_static_gain;
#endif

    /* ANC channel controlled by the capability */
    AANC_ANC_INSTANCE anc_channel;
    /* ANC mask for synchronous gain updates */
    AANC_ANC_INSTANCE anc_mask;
    /* ANC path for FF and FB gains */
    AANC_ANC_PATH anc_ff_path;
    AANC_ANC_PATH anc_fb_path;
    /* ANC clock check value for ensuring valid clocks are being used */
