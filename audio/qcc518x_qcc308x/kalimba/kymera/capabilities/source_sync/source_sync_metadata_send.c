...
#else /* SOSY_VERBOSE */
static inline void src_sync_trace_send_tags( SRC_SYNC_OP_DATA* op_extra_data,
                                      SRC_SYNC_TERMINAL_GROUP* dest_grp,
                                      const metadata_tag* tag)
{
}
        ...
        SRC_SYNC_METADATA_DEST* dest_state )
{
    ...
    else
    {
        if (dest_state->remaining_octets < octets_before)
        {
            ...
            if (IS_TIME_TO_PLAY_TAG(tags) || IS_TIME_OF_ARRIVAL_TAG(tags))
            {
        ...
        if (dest_state->eof_tag != NULL)
        {
            ...
            SOSY_MSG1( SRC_SYNC_TRACE_EOF,
                       "moved EOF info success %d", SOSY_IF_VERBOSE(move_success));
