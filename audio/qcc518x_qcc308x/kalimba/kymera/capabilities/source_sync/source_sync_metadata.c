...
{
    /* The cast is only there to avoid warnings on targets where TIME is a long type */
    unsigned advance = (unsigned) rate_samples_to_usec(nr_of_samples, sample_period);
...
{
    /* The cast is only there to avoid warnings on targets where TIME is a long type */
    unsigned reverse = rate_samples_to_usec(nr_of_samples, sample_period);
...
void src_sync_check_md_transport_pre(SRC_SYNC_OP_DATA* op_extra_data)
{
          ...
          src_grp = next_source_group(src_grp))
    {
             ...
             &&(src_grp->common.metadata_buffer != NULL))
        {
            ...
            {
                L2_DBG_MSG4("src_sync 0x%04x ERROR src_g%d md_wp changed since "
                            "last pt: %d -> %d",
                            op_extra_data->id, src_grp->common.idx,
                            src_grp->last_md_write_idx, md_wp);
#ifdef SOSY_ASSERT_METADATA_TRANSPORT_POINTERS
                PL_ASSERT(FALSE);
#endif
            }
            src_grp->last_md_write_idx = md_wp;
                  ...
                  src_ptr = next_source_entry(src_ptr) )
            {
                if (src_ptr->common.buffer != NULL)
                {
                    ...
                    {
                        L2_DBG_MSG4("src_sync 0x%04x ERROR src_%d cb_wp changed "
                                    "since last pt: %d -> %d",
...
void src_sync_check_md_transport_post(SRC_SYNC_OP_DATA* op_extra_data)
{
          ...
          src_grp = next_source_group(src_grp))
    {
             ...
             &&(src_grp->common.metadata_buffer != NULL))
        {
            ...
            {
                L2_DBG_MSG3("src_sync 0x%04x ERROR src_g%d md "
                  ...
                  src_ptr = next_source_entry(src_ptr) )
            {
                if (src_ptr->common.buffer != NULL)
                {
                    ...
                    {
                        L2_DBG_MSG5("src_sync 0x%04x ERROR src_g%d md wp_o %d "
                                    "!= src_%d cb wp_w %d",
                                    op_extra_data->id, src_grp->common.idx,
                    ...
                    if (cb_wr_w != md_wr_w)
                    {
                        L2_DBG_MSG5("src_sync 0x%04x ERROR src_g%d md_wr_w %d "
                                    "!= src_%d cb_wr_w %d",
                                    op_extra_data->id, src_grp->common.idx,
