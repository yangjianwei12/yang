...
const opmsg_handler_lookup_table_entry rtp_decode_opmsg_handler_table[] =
    ...
    {OPMSG_COMMON_GET_RAW_LATENCY,               rtp_decode_opmsg_get_raw_ttp_latency},
    {OPMSG_COMMON_RESYNC_TTP,                    rtp_decode_opmsg_resync_ttp},
    {OPMSG_COMMON_ID_GET_STATUS,                 rtp_ttp_get_stats},
    {0, NULL}};
...
static void rtp_notify_ttp(OPERATOR_DATA *op_data, RTP_FRAME_DECODE_DATA *frame_data)
{
    ...
    common_send_unsolicited_message( op_data, (unsigned)(OPMSG_REPLY_ID_TTP_RTP_SEQ | OPMSG_REPLY_ID_DISCARDABLE_MESSAGE),
...
static bool tag_valid(metadata_tag* tag, RTP_DECODE_OP_DATA *opx_data )
{
    ...
    if (cbuffer_get_usable_data_read_offset(opx_data->ip_buffer) != tag->index )
    {
...
/**
 * Maintain a histogram of the deltas between time of arrival and ttp
 */
static void update_ttp_stats(RTP_DECODE_OP_DATA *opx_data, TIME ttp, TIME time_of_arrival)
{
    TIME_INTERVAL ttp_bin_value[TTP_BIN_COUNT] = {  0 * US_PER_MS,
                                                   25 * US_PER_MS,
                                                   50 * US_PER_MS,
                                                  100 * US_PER_MS,
                                                  150 * US_PER_MS,
                                                  INT_MAX};
    unsigned cnt =0;
    TIME_INTERVAL remaining = time_sub(ttp, time_of_arrival);

    for(cnt = 0; cnt < TTP_BIN_COUNT; cnt++)
    {
        if(time_lt(remaining, ttp_bin_value[cnt]))
        {
            opx_data->ttp_bin_count[cnt]++;
            break;
        }
    }
}
        ...
        unsigned packet_size)
{
    ...
    if ((frame_data->valid) && (frame_data->nr_of_frames > 0))
    {
        ...
        update_ttp_stats(opx_data, status.ttp, time_of_arrival);
...
{
    L2_DBG_MSG("rtp_decode_opmsg_set_packing: data format is 32, there is no need to pack");
    return TRUE;
}
...
bool rtp_decode_opmsg_get_raw_ttp_latency(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{ 
    RTP_DECODE_OP_DATA *opx_data = get_instance_data(op_data);
    ...
    *resp_data = (OP_OPMSG_RSP_PAYLOAD *)xzpmalloc(OPMSG_RSP_PAYLOAD_SIZE_RAW_DATA(1)*sizeof(unsigned));
    if (*resp_data == NULL)
    {
        return FALSE;
    }
    ...
    unsigned ttp_raw_latency = ttp_get_raw_latency(opx_data->ttp_instance);
    (*resp_data)->u.raw_data[0] = ( ttp_raw_latency  >> 16 );
    (*resp_data)->u.raw_data[1] = ( ttp_raw_latency & 0x0000FFFF );
...
bool rtp_ttp_get_stats(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    RTP_DECODE_OP_DATA *opx_data = get_instance_data(op_data);
    unsigned* resp;
    unsigned cnt;
    if(!common_obpm_status_helper(message_data, resp_length, resp_data,
                                  sizeof(opx_data->ttp_bin_count),
                                  &resp))
    {
         return FALSE;
    }
    ...
    if(resp)
    {
        for(cnt = 0; cnt < TTP_BIN_COUNT; cnt++)
        {
            resp = cpsPack1Word(opx_data->ttp_bin_count[cnt], resp);
        }
    }
