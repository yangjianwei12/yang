...
#define PEQ_PEQ_VERSION_MINOR 1
...
const opmsg_handler_lookup_table_entry peq_wrapper_opmsg_handler_table[] =
    ...
    {OPMSG_PEQ_ID_SET_STAGES_PER_FRAME,                  peq_wrapper_opmsg_set_stages_per_frame},
    {0, NULL}};
...
const CAPABILITY_DATA peq_cap_data =
{
    ...
    0,                                            /* Reserved */
    sizeof(PEQ_OP_DATA_EXT)                       /* Size of capability-specific per-instance data */
};
...
#ifdef PEQ_OFFLOAD
    if (opmgr_op_thread_offload(op_data) && thread_offload_is_active())
    {
...
void peq_wrapper_process_data(OPERATOR_DATA *op_data, TOUCHED_TERMINALS *touched)
{
    ...
    if (multi_chan_offload_active(op_data))
    {
        ...
        if (!thread_offload_rpc_is_queued(opid))
        {
