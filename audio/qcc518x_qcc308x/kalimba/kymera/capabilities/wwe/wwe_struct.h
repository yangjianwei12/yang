...
/** Function prototype of function that sends trigger notifications */
typedef bool (*WWE_NOTIFY_FN)(OPERATOR_DATA *op_data);
...
typedef struct WWE_CAP_VIRTUAL_TABLE
{
    ...
    /* The function that notifies the host of trigger detections */
    WWE_NOTIFY_FN wwe_notify_fn;
...
typedef struct WWE_TIMESTAMPS
{
    ...
    bool end_timestamp_computed_flag;
...
typedef struct WWE_COMMON_DATA
{
    ...
    TIME last_timestamp;
    int saved_sp_adjust;
    unsigned num_samples_fully_processed;  /** Store the munber of samples actually processed by the engine since the last reset/re-init */
} WWE_COMMON_DATA;
