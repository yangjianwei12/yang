...
static void wwe_initialize_timestamps(WWE_COMMON_DATA *p_wwe_data)
{
    ...
    p_wwe_data->ts.end_timestamp_computed_flag = FALSE;
    p_wwe_data->last_timestamp = 0;
    p_wwe_data->saved_sp_adjust = 0;
}
                     ...
                     unsigned frame_size)
{
    ...
    p_wwe_data->num_samples_fully_processed = 0;
...
void wwe_process_data(OPERATOR_DATA *op_data, TOUCHED_TERMINALS *touched)
{
    ...
    if (p_wwe_data->init_flag)
    {
        ...
        wwe_initialize_timestamps(p_wwe_data);
        p_wwe_data->num_samples_fully_processed = 0; /* We reset this value upon re-initializing */
        p_wwe_data->init_flag = FALSE;
    }

    if (vt->process_fn != NULL)
    {
        L4_DBG_MSG1("WWE: last_timestamp = %d \0",p_wwe_data->last_timestamp);
        if (!vt->process_fn(op_data, touched))
        {
            wwe_handle_data_flow(p_wwe_data, touched);
            return;
        }
        p_wwe_data->num_samples_fully_processed = p_wwe_data->num_samples_fully_processed + p_wwe_data->frame_size;
    }
                          ...
                          TOUCHED_TERMINALS *touched)
{
    ...
    while (mtag_ip != NULL)
    {
        if (IS_TIME_TO_PLAY_TAG(mtag_ip) || IS_TIME_OF_ARRIVAL_TAG(mtag_ip))
        {
    ...
    if (last_ttp_mtag_ip)
    {
        ...
        last_timestamp = ttp_get_next_timestamp(
                ...
                p_wwe_data->sample_rate, last_ttp_mtag_ip->sp_adjust);
        p_wwe_data->saved_sp_adjust = last_ttp_mtag_ip->sp_adjust;
    }
    else
    {
        /* No timestamped tag.
        ...
        last_timestamp = ttp_get_next_timestamp(
                ...
                p_wwe_data->sample_rate, p_wwe_data->saved_sp_adjust);
    }

    /* Save the last sample's timestamp in case next run will have no tag.*/
    p_wwe_data->last_timestamp = last_timestamp;
    if ((p_wwe_data->trigger==TRUE) && (p_wwe_data->ts.end_timestamp_computed_flag==FALSE))
    {
        /* If the voice assistant capability hasn't already computed this then we assume
            that last_timestamp at the the point of trigger is the end time stamp */
...
bool wwe_send_notification(OPERATOR_DATA *op_data)
{
    WWE_COMMON_DATA *p_wwe_data = get_wwe_data(op_data);
    unsigned msg_size = \
                        ...
                        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    key = MAP_CAPID_UCID_SBID_TO_PSKEYID(base_op_get_cap_id(op_data),
                         ...
                         OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    return cpsGetUcidMsgHandler(&p_wwe_data->parms_def, base_op_get_cap_id(op_data),
                     ...
                     unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if ((new_mode > 0) && (new_mode < WWE_SYSMODE_MAX_MODES))
    {
        ...
        if(new_mode == WWE_SYSMODE_FULL_PROC)
        {
            p_wwe_data->init_flag = TRUE;
        }
        return TRUE;
