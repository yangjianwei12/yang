                              ...
                              unsigned ip_proc_samples, unsigned out_data_octets, unsigned sample_rate)
{
    ...
    /* With addition of source sync in the To-air chain to transmit silence encoded packet.
     * We have a corner case where 1st input metadata tag is void followed by metadata tags
     * with ttp and group of tags(1ms each) form a SCO packet of 7.5ms.
     * In this case we can avoid updating last_tag_samples and let this complete frame treated as void packet.*/
    if ((mtag_ip != NULL) && (base_ttp))
    {
