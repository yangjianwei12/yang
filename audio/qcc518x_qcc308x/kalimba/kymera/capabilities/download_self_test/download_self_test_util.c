...
#pragma maxim
static void func_increase_one_maxim(void)
{
    a++;
}
...
static unsigned int download_self_test_opmsg_start_self_test2_dm_as_pm(void)
{
    /* This will create an anonymous variable */
    unsigned int* my_local_array[] = {&my_static_array[0], &my_static_array[1], &my_static_array[2]};
    int i = 0;
...
#pragma codesection DM_AS_PM
static void func_decrease_dm_as_pm(unsigned int num)
{
    if (num > 0)
    {
        /* Exercise recursion */
...
#pragma codesection DM_AS_PM
static void func_increase_one_dm_as_pm(void)
{
    a++;
}
...
#pragma maxim
#pragma codesection dm_as_pm
static void func_increase_one_dm_as_pm_maxim(void)
{
    a++;
}
...
#if defined(CHIP_HAS_NVRAM_ACCESS_TO_DM)
#pragma codesection DM_AS_PM
unsigned int download_self_test_array_offset_access_dm_as_pm(void)
{
    /* Force a relocation record in PM to z_array with an offset
     * KCSMaker will find the address of z_array by examining
     * the opcode's immediate and subtracting the offset (-2).
...
#pragma codesection DM_AS_PM
unsigned int download_self_test_array_offset_access_check_dm_as_pm(void)
{
...
DOWNLOAD_HELPER_STRUCT* func_return_struct_ptr(void)
{
    /* Allocate zeroed memory */
    DOWNLOAD_HELPER_STRUCT *my_var = xzpmalloc(sizeof(DOWNLOAD_HELPER_STRUCT));
...
{
    DOWNLOAD_HELPER_STRUCT *helper_struct = func_return_struct_ptr();
    DOWNLOAD_SELF_TEST_OP_DATA op_data;

    op_data.helper_struct = helper_struct;
...
void func_decrease_one(void)
{
    ...
    func_decrease(2);
#if defined(CHIP_HAS_NVRAM_ACCESS_TO_DM)
    func_increase_one_dm_as_pm();
    func_decrease_dm_as_pm(1);
    func_increase_one_dm_as_pm();
    func_decrease_one_dm_as_pm();
#endif /* CHIP_HAS_NVRAM_ACCESS_TO_DM */
}

#if defined(CHIP_HAS_NVRAM_ACCESS_TO_DM)
#pragma codesection DM_AS_PM
void func_decrease_one_dm_as_pm(void)
{
    void (*func_ptr)(void);
    func_increase_one_dm_as_pm();
    func_decrease_dm_as_pm(2);
    func_increase_one();
    func_decrease(1);
...
#pragma codesection DM_AS_PM
unsigned int download_self_test_sqrt_test_dm_as_pm(void)
{
    float f = 0.40;
    unsigned increments = 0;
    while (sqrtf(f)<0.7)
...
unsigned int download_self_test_util_helper(void)
{
    ...
    func_ptr();
#if defined(CHIP_HAS_NVRAM_ACCESS_TO_DM)
    func_ptr = &func_increase_one_dm_as_pm;
    func_ptr();
    func_ptr = &func_decrease_one_dm_as_pm;
    func_ptr();
    func_ptr = &func_increase_one_dm_as_pm_maxim;
    func_ptr();
    func_ptr = &func_decrease_one_dm_as_pm;
    func_ptr();
#endif /* CHIP_HAS_NVRAM_ACCESS_TO_DM */
    func_increase_one_maxim();
    if (a == 0)
    {
...
#if defined(CHIP_HAS_NVRAM_ACCESS_TO_DM)
        if (download_self_test_opmsg_start_self_test2_dm_as_pm() != 0x600D)
        {
            return 0xBAD;
        }
#endif /* CHIP_HAS_NVRAM_ACCESS_TO_DM */
...
#if defined(CHIP_HAS_NVRAM_ACCESS_TO_DM)
        if (download_self_test_sqrt_test_dm_as_pm() != 0x600D)
        {
            return 0xBAD;
        }
#endif /* CHIP_HAS_NVRAM_ACCESS_TO_DM */
...
unsigned int download_self_test_extra_memory(void)
{
    unsigned *extra_memory;
    unsigned result;

    extra_memory = xppnew(unsigned, MALLOC_PREFERENCE_EXTRA);
    if (!pmalloc_preference_is_available(MALLOC_PREFERENCE_EXTRA))
    {
        /* There is no fallback for extra memory, if the per-chip configuration
           does not reserve some memory for it then the allocation must fail. */
        result = (extra_memory == NULL) ? 0x600D : 0xBAD;
    }
    else if (extra_memory == NULL)
    {
        result = 0xBAD;
    }
    else
    {
        ...
        ptr = extra_memory;
        *ptr = 0x12345678;
        result = (*ptr == 0x12345678) ? 0x600D : 0xBAD;
    }
