...
const opmsg_handler_lookup_table_entry ttp_passthrough_opmsg_handler_table[] =
    ...
    {OPMSG_COMMON_ID_SET_UCID,                     basic_passthrough_opmsg_set_ucid},
    {OPMSG_COMMON_ID_GET_LOGICAL_PS_ID,            basic_passthrough_opmsg_get_ps_id},
    {OPMSG_COMMON_GET_RAW_LATENCY,                 ttp_passthrough_opmsg_get_raw_ttp_latency},
...
const CAPABILITY_DATA ttp_passthrough_cap_data =
{
    ...
    BASIC_PASSTHROUGH_TTP_VERSION_MAJOR, 2,  /* Version information - hi and lo parts */
    MAX_CHANS, MAX_CHANS,                    /* Max number of sinks/inputs and sources/outputs */
    &basic_passthrough_handler_table,       /* Pointer to message handler function table */
...
static void metadata_transport_with_ttp(BASIC_PASSTHROUGH_OP_DATA *opx_data, unsigned samples)
{
    ...
    if (mtag != NULL)
    {
        ...
        if (IS_TIME_OF_ARRIVAL_TAG(mtag) || IS_TIME_TO_PLAY_TAG(mtag))
        {
...
#error The process data function kick backwards logic is only valid when block size is 1
    ...
    unsigned (*input_samples_fn)(tCbuffer *cbuffer) = cbuffer_calc_avail_data_words;
    unsigned (*output_space_fn)(tCbuffer *cbuffer) = cbuffer_calc_avail_space_words;
    ...
    else
    {
        ...
        case AUDIO_DATA_FORMAT_ENCODED_DATA:
            buffers_ex = TRUE;
            input_samples_fn = cbuffer_calc_avail_data_ex;
            output_space_fn = cbuffer_calc_avail_space_ex;
            break;
...
bool ttp_passthrough_opmsg_get_raw_ttp_latency(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    BASIC_PASSTHROUGH_OP_DATA *opx_data = get_instance_data(op_data);
    ...
    *resp_data = (OP_OPMSG_RSP_PAYLOAD *)xzpmalloc(OPMSG_RSP_PAYLOAD_SIZE_RAW_DATA(1)*sizeof(unsigned));
    ...
    unsigned ttp_raw_latency = ttp_get_raw_latency(opx_data->time_to_play);
