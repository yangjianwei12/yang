...
header_files_names_list=['pryon_lite_error.h', 'pryon_lite_metadata.h', 'pryon_lite_vad_PRL1000.h', 'pryon_lite_ww.h']
...
def parse_args():
    ...
    parser.add_argument('-b', '--build_config',
                        required=True,
                        help='Target build config. Valid options are streplus_rom_release, streplus_rom_kalsim_kalcmd2_release, maor_rom_release, maor_rom_kalsim_kalcmd2_release (REQUIRED)')

    parser.add_argument('-c', '--chip',
                        required=True,
                        help='Target chip variant. Valid options are qcc512x-rom-v21, qcc512x-rom-v02, qcc514x_qcc304x, qcc515x_qcc305x, qcc516x_qcc306x, qcc517x_qcc307x, qcc518x_qcc308x, qcc519x_qcc309x (REQUIRED)')
...
def export_file(model, locale, src_root_dir, models_dest_dir):
    ...
    if(len(model) > 1):
        ...
        if model_50k:
            ...
            print_warning("No 50k model found for locale {}".format(locale))
            print_warning("Ignoring locale {}".format(locale))
            return
    if(len(model) > 1):
    ...
    locales_in_model = model[start_index_of_locale:end_index_of_locale].split('+')
    primary_locale = locales_in_model[-1];
    if(locale == primary_locale):
        ...
        print('locale %s model not required, as locale is supported in %s model (%s) ' % (locale, primary_locale, os.path.join(models_dest_dir, primary_locale)))
    add_locale_to_model_entry(locale, primary_locale)
...
def extract(zip_dir, zip_file_partial_name, build_config, chip_type, operator):
    ...
    if(operator == "apva"):
        if(chip_type == 'qcc515x_qcc305x'): #We treat StrettoPlusV1.2 the same as StrettoPlusV1.1 for APVA
            chip_type_alias = 'qcc514x_qcc304x'
        elif(chip_type == 'qcc518x_qcc308x'): #We treat qcc518x the same as qcc517x for APVA
            chip_type_alias = 'qcc517x_qcc307x'
        elif(chip_type == 'qcc519x_qcc309x'): #We treat qcc519x the same as qcc517x for APVA
            chip_type_alias = 'qcc517x_qcc307x'
        else:
