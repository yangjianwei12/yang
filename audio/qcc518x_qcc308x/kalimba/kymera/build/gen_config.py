...
class Configuration:
    ...
    def read_config_file(self, config_path, config_file):
        ...
        while config_line:
            ...
            if debug:
                print_line("current_file  " + current_file)
                print_line("config_line " + config_line)
                print_line("match.groups() starting with group 1 {0}\n".format(match.groups()))
            ...
            if match.group(2) != None: # State changer instruction read!
                ...
                if state not in supported_states:
                    print_line("State %s not supported!" % state)
                    sys.exit(1)
            ...
            if (match.group(3) != None) or (match.group(4) != ""):
                ...
                if state == "include":
                    ...
                    if data is None:
                        print_line("Unable to open %s included in %s\n" % (next_file, current_file))
                        sys.exit(1)
                    ...
                    if match.group(2) != None and state == match.group(2)[1:]:
                        ...
                        print_line("Bad config line %s in %s." % (state, current_file))
                        sys.exit(1)

                else:
                    print_line("Bad state %s. %s is the current config file processed." %
                          (state, current_file))
                    sys.exit(1)
...
def update_file(file_to_write, content):
    ...
    if read_file(file_to_write, stop_if_failure=False, split_to_lines=False) != content:
        if debug:
            print_line(file_to_write + " was updated!")
        with open(file_to_write, "w") as targetfile:
            targetfile.write(content)
        return True
    else:
        if debug:
            print_line(file_to_write + " remains the same.")
        return False
...
def touch_dependencies_on_changes(kymera_path, dirs, suffixes, changes):
    ...
    for directory in dirs:
        ...
        if not os.path.exists(directory):
            print_line("Directory {0} ".format(directory) + "included in ALL_SRCDIRS, ALL_INCDIRS " +
            ...
            for file_name in os.listdir(directory):
                ...
                if matching_file(suffixes, full_file_path):
                    ...
                    if grep_words(changes, full_file_path):
                        print_line("Mark file for rebuild: " + full_file_path)
...
def print_line(*objects):
    print(*objects, end = "_END_\n")
...
class ConfigList:
    ...
    def remove_from_current_list(self, value):
        ...
        if self.current_list != None:
            ...
            print_line("Remove when current_list for ConfigList is None ")
            sys.exit(1)
    ...
    def pop(self, value):
        ...
        if value != None:
            if not value in self.list_dict:
                print_line("Error list not previously defined")
                sys.exit(1)
...
class ConfigDefs(set):
    ...
    def add(self, element):
        ...
        if element[0] == "-":
            # this is an undefine
            print_line(" Remove {0}".format(element[1:]))
        ...
        for key in self:
            if (self.get_def_name(element) == self.get_def_name(key)) and (element != key):
                print_line(" Redefining {0} to {1}".format(key, element))
                self.remove(key)
                return
...
if __name__ == "__main__":
    ...
    try:
        ...
        print_line("config file %s not recognised " % options.config_file)
        sys.exit(1)
    ...
    if options.download_cfg_files is not None and options.build_name is not None:
        if options.download_cfg_files == []:
            print_line("Error no download capability config file provided")
            sys.exit(1)
        else:
            ...
            config.config_list.pop("TASKS")
            # Remove other lists of libs from config.mkf file
            # (These are probably harmless, but not useful.)
            if config.config_list.get_list("AUTOGEN_CFG_LIBS") is not None:
                config.config_list.pop("AUTOGEN_CFG_LIBS")
    ...
    if builddefs_mkf_changed:
        ...
        if defs is None:
            print_line("Unable to locate last_defines.txt from the previous build.")
                    ...
                    if define in changed_defs_name: # def without value
                        ...
                        for new_def in config.config_cpp_defs & changed_defs:
                            if ConfigDefs.get_def_name(new_def) == ConfigDefs.get_def_name(define):
                                print_line("\tUpdate " + define.replace("\n", ", ") +\
                                      " to " + new_def.replace("\n", ", "))
                else: # should be in new build only
                    if define in changed_defs_name: # def without value
                        # New define
                        print_line("\tNew def " + define.replace("\n", ", "))
                    else:
                        # An updated def which was/will be displayed
                        pass

            if debug:
                print_line("changed_defs_name\n" + changed_defs_name)
