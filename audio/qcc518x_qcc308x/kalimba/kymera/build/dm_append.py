...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("dm_append filename" +
              "[-dmsize <size>] [-offset <offset>] [-merge extra.xuv]\n")
        sys.exit(2)

    base_file_name = sys.argv[1]
    dm_size, dm_offset, mergefile = parse_options()
    ...
    # Report ROM size
    print("\n ======= DM Append: XUV size (octets) = {0:#x} ======== \n".format(2 * (last_xuv_address+1)))
