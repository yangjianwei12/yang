...
def main():
    ...
    bundlesWithFlags = determine_additional_combinations(bundlesForConfig, bundleDefinitions, args.config)
    show_bundles_for_config(args.config, bundlesWithFlags)
    
    output_result(bundlesWithFlags, args.ofile)
...
# ----------------------------------------------------------------------------
def determine_additional_combinations(bundlesForConfig, bundleDefinitions, buildConfig):
    """
    Insert extra combinations with flags for selected bundles, for supported platforms. 
    The bundles are taken from the ordered list of supported bundles, so bundle
    dependencies are ensured.
    """

    logging.info("\n#{0}\nDetermining additional combinations for {1}".format('-'*60, buildConfig))
    
    bundlesWithFlags = []
    for bundle in bundlesForConfig:
        try:
            bundle += ".bdl"
            addl_comb = bundleDefinitions[bundle]['additional_combinations']
            for platform in addl_comb:
                if re.match(platform, buildConfig):
                    flags = bundleDefinitions[bundle]['additional_combinations'][platform]
                    # There can be more than one flag, ALL_DM_AS_PM or/and DATA_IN_EXTRA_HEAP
                    for flag in flags:
                        recurseDepth = 1
                        insert_bundle_in_list(bundle, bundlesWithFlags, bundleDefinitions, recurseDepth, flag)
        except KeyError:
            pass

    bundlesForConfig.extend(bundlesWithFlags)

    return bundlesForConfig


# ----------------------------------------------------------------------------
def insert_bundle_in_list (bundle, bundlesForConfig, bundleDefinitions, recurseDepth, flag = None):
    """
    ...
    if not bundleNoSuffix in bundlesForConfig and flag is None:
        ...
        # the dependency is already in the list
            bundleNoSuffix += " " + flag
            bundlesForConfig.append(bundleNoSuffix)
    else:
