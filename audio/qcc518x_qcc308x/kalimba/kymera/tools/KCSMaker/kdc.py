...
class KDCFile(downloadFile):
    """Container format that extends the built-in list to include comment strings
    ...
    def __init__(self, varLut):
        ...
        self.DM1_size = 0
        self.DM2_size = 0
        self.DM1_extra_size = 0
        self.DM2_extra_size = 0
        self.CONST_size = 0
        self.CONST16_size = 0
        self.supports_dm_as_pm = False
        self.supports_dm_banks_powered_off = False
        super(KDCFile,self).__init__()
    
    def print_memory_usage(self):
        print("-> PM Size:          {s1} bytes ({s2} KiB)".format(s1=self.PM_size, s2=round(self.PM_size/1024.0,3)))
        if self.supports_dm_as_pm:
            print("-> DM as PM Size:    {s1} bytes ({s2} KiB)".format(s1=self.DM_as_PM_size, s2=round(self.DM_as_PM_size/1024.0,3)))
        print("-> DM1 Size:         {s1} bytes ({s2} KiB)".format(s1=self.DM1_size, s2=round(self.DM1_size/1024.0,3)))
        if self.supports_dm_banks_powered_off:
            print("-> DM1 Extra Size:   {s1} bytes ({s2} KiB)".format(s1=self.DM1_extra_size, s2=round(self.DM1_extra_size/1024.0,3)))
        print("-> DM2 Size:         {s1} bytes ({s2} KiB)".format(s1=self.DM2_size, s2=round(self.DM2_size/1024.0,3)))
        if self.supports_dm_banks_powered_off:
            print("-> DM2 Extra Size:   {s1} bytes ({s2} KiB)".format(s1=self.DM2_extra_size, s2=round(self.DM2_extra_size/1024.0,3)))
