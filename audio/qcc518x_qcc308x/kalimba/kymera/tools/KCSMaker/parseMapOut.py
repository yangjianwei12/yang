...
def is_valid_gap(cur_low_pc, prev_cur_high_pc):
    # check if prev is DM_AS_PM and current is PM
    if (cur_low_pc & 0xFF00000 == 0x4200000) and\
        (prev_cur_high_pc & 0xFF00000 == 0x200000):
        return True
    return False
    ...
    for mod in mod_list[1:]:
        if mod.get_low_pc() != prev_mod.get_high_pc() + 1:
            if (mod.get_low_pc() > prev_mod.get_high_pc() + 1 + MAX_GAP_MINIM_BYTES):
                if is_valid_gap(mod.get_low_pc(), prev_mod.get_high_pc()) is False:
