...
def convert_to_boolean(bool_string):
...
def proc_cmd_line(args):
    ...
    for arg in args:
        if "RELEASE" in arg:
            ...
            user = arg.split("=")[1]
        elif "-alldmaspm" in arg:
            ALL_DM_AS_PM = convert_to_boolean(arg.split("=")[1])
        elif "-datainextraheap" in arg:
            DATA_IN_EXTRA_HEAP = convert_to_boolean(arg.split("=")[1])
        elif "-testenvironment" == arg:
    ...
    cmd_line_args = {
        ...
        "ALL_DM_AS_PM": ALL_DM_AS_PM,
        "DATA_IN_EXTRA_HEAP": DATA_IN_EXTRA_HEAP,
...
class KCSMaker(object):
    def __init__(self, cmd_line_args):
        ...
        self.all_dm_as_pm = cmd_line_args["ALL_DM_AS_PM"]
        self.data_in_extra_heap = cmd_line_args["DATA_IN_EXTRA_HEAP"]
        self.test_environment = cmd_line_args["test_environment"]
        self.bundle_name_flags = self.bundle_name
        self.build_flags = ""
    ...
    def main(self, get_kcc_version):
        ...
        chip = chip_info.chip_info(rel_binfo.get_firmware_build_config(), rel_binfo.get_elf_name())
        if chip.is_supported() == False:
            raise Exception("Chip NOT supported: " + str(chip.get_string()))

        if self.all_dm_as_pm and chip.supports_dm_as_pm() and self.test_environment:
            self.build_flags += "_adap"
        
        if self.data_in_extra_heap and chip.supports_dm_banks_powered_off() and self.test_environment:
            self.build_flags += "_dieh"

        self.bundle_name_flags += self.build_flags
        ...
        if self.linkscript_path == "":
            self.linkscript_path = os.path.join(
                self.kcsmaker_path,
                "templates",
                chip.get_linkscript_template_name(self.all_dm_as_pm)
            )
        ...
        if (os.path.exists(os.path.join(self.kcs_output_dir, "last_bundle.txt"))):
            ...
            if prev_bundle != self.bundle_name_flags:
                ...
                logging.info(
                    ...
                    Forcing clean build".format(
                        ...
                        cur_build = self.bundle_name_flags
                    )
                )
        ...
        for kdc_name in mod.KDCs.keys():
            ...
            last_bundle_file.write(self.bundle_name_flags)
            last_bundle_file.close()
        ...
        if self.release_flag:
            ...
            for file_ext in file_ext_list:
                source_with_flags = os.path.join(
                                    output_folder_path,
                                    self.bundle_name_flags + file_ext
                                )

                if not os.path.exists(source_with_flags):
                    # The build output files do not have the suffix, they need
                    # to be renamed here.
                    source_without_flags = os.path.join(
                                         output_folder_path,
                                         self.bundle_name + file_ext
                                    )
                    os.rename(source_without_flags, source_with_flags)
            ...
            # check that all files have the suffixes, if necessary
            for filename in os.listdir(output_folder_path):
                if self.bundle_name_flags not in filename:
                    # This file doesn't have the suffixes in the name
                    if self.bundle_name in filename:
                        # This is a file that should be renamed
                        new_filename = filename.replace(self.bundle_name, self.bundle_name_flags)
                        old_file_path = os.path.join(output_folder_path, filename)
