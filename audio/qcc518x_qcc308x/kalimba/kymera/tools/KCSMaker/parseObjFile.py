...
def parseRelocObjFile(lines, prog, var, const, filename, mappings, dmprog, partial_dm_as_pm):
    class Reference:
            ...
            if m is not None:
                ...
                if name in dmprog:
                    ...
                    if region.endswith('dmconst') or region.endswith('const') or region.endswith('const16') or region.endswith('dmconst_windowed16'):
                        ...
                        const[name] = currSym
                    elif region.endswith('dm_as_pm') and partial_dm_as_pm == True:
                        logging.debug("Storing in DM prog...")
                        dmprog[name]  = currSym
    ...
    logging.debug("parse_dmprog:" + str(dmprog))
...
def getVariables(files, objDumpPath, mappings, func_import, kdc_maker, partial_dm_as_pm):
    ...
    for f in files:
        if os.path.exists(f):
            ...
            parseRelocObjFile(lines, prog, var, const, f, mappings, dmprog, partial_dm_as_pm)
    ...
    # Build from ADK has not been tested with more than 1 private libs.
    if len(func_export) > 1:
        logging.debug("WARNING: Attempting to generate {f1} private libs".format(f1=func_export))
    ...
    return (prog, var, const, func, func_export, var_sizes, dmprog)
