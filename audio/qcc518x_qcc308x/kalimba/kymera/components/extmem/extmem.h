 ...
 *
 * Low level functions to control the interface to an external memory chip
 * connected via SPI.
 */
/** 
 * \file extmem.h
 * \ingroup extmem
 *
 * Public definitions for the extmem module. Some of the chip variants supports
 * an external data memory( currently only SPI RAM). The APIs present here
 ...
 * \param cback    callback to return the enable status if present
 *
 ...
 * \param mode      power mode. EXTMEM_ON to enabled nd EXTMEM_OFF to disable
 * \param cback     callback to return the enable status if present
 * \param wait_time maximum attempt time in micro seconds
 *
 * \return TRUE on initiating the operation.
 *
 ...
 */
extern bool extmem_enable_with_retry(EXTMEM_TYPE type,
                                     EXTMEM_MODE mode,
                                     EXTMEM_CBACK cback,
									 TIME wait_time);
