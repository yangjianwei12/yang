...
DM_SHARED_ZI static unsigned ttp_raw_latency;
#endif /* TTP_PLOTTER */
...
void ttp_configure_latency(ttp_context *context, TIME_INTERVAL target_latency)
{
     ...
     */
    TTP_DBG_MSG1("TTP target latency = %d", target_latency);
...
bool ttp_configure_sp_adjustment(ttp_context *context, int sp_adjustment)
{
    ...
    if (context->state != TTP_STATE_RUNNING)
    {
        TTP_DBG_MSG1("TTP sample period adjustment = %x", sp_adjustment);
...
void ttp_adjust_ttp_timestamp(ttp_context *context, TIME_INTERVAL ttp_adjust_ts)
{
    ...
    context->ttp_frac = 0;
    context->delta_remainder = 0;
    TTP_DBG_MSG3("TTP timestamp adjustment = %x, adjusted ttp =%4d , changed previous source time =%4d", ttp_adjust_ts, context->ttp, context->prev_source_time);
}
...
void ttp_configure_latency_limits(ttp_context *context, TIME_INTERVAL min_latency, TIME_INTERVAL max_latency)
{
    ...
    TTP_DBG_MSG2("TTP latency min = %d max = %d", min_latency, max_latency);
...
void ttp_update_ttp_from_source_time(ttp_context *context, TIME toa, TIME source_time, ttp_status *status)
{
    ...
    context->ttp_raw_latency = time_sub(context->ttp, toa);
    TTP_PLOTTER_MACRO(ttp_raw_latency = context->ttp_raw_latency);
}
...
void ttp_update_ttp(ttp_context *context, TIME time, unsigned samples, ttp_status *status)
{
    ...
    context->ttp_raw_latency = time_sub(context->ttp, time);
    TTP_PLOTTER_MACRO(ttp_raw_latency = context->ttp_raw_latency);
}
 ...
 * ttp_get_raw_latency
 *
 * \brief Function returns the raw ttp latency value which is updated in ttp_update_ttp_from_source_time/ttp_update_ttp.
 */
unsigned ttp_get_raw_latency(ttp_context *context)
{
    patch_fn_shared(ttp_gen);
    return context->ttp_raw_latency;
}