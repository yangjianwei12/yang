...
struct TIMED_PLAYBACK_STRUCT
{
    ...
    /**
     * Pointer to endpoint struct, passed to rate adjust function
     */
    ENDPOINT *rm_adjust_ep;

    /**
     * Function to call to adjust output rate
     */
    void (*rate_adjust)(ENDPOINT *ep, int adjust_val);
    ...
    /**
     * Callback for sending an unachievable latency unsolicited message
     */
...
static unsigned timed_playback_get_reframe_period_in_samples(unsigned sample_rate);
...
* \param timed_pb Pointer to the timed_pb playback instance.
*
* \return Returns the minimum available space calculated in
...
#if defined(DBG_USE_CBUFFER)
/**
* \brief Function copy samples in all channels.
*
* \param timed_pb        Pointer to the timed_pb playback instance.
* \param samples_to_copy Number of samples to copy.
*/
static void ttp_buffer_copy(TIMED_PLAYBACK *timed_pb,
...
*
* \param timed_pb           Pointer to the timed_pb playback instance
* \param samples_to_discard Number of samples to discard.
*/
static void ttp_buffer_discard(TIMED_PLAYBACK *timed_pb,
                               unsigned samples_to_discard)
{
    unsigned channel;
    for (channel = 0;channel < timed_pb->used_channels; channel++)
    {
 ...
 */
static TIME_INTERVAL get_timestamp_error(TIMED_PLAYBACK *timed_pb,
                                         unsigned tag_timestamp)
{
...
#else
#define check_input_buff_read_index_validity(timed_pb) ((void)0)
 ...
 *
 * \param timed_pb        Pointer to the timed_pb playback instance.
 * \param samples_to_play Number of samples to play.
 */
static void timed_playback_do_rate_adjust(TIMED_PLAYBACK* timed_pb,
                                          unsigned samples_to_play)
{
#ifdef TTP_BUFFER_DEBUG
    ...
    unsigned buffer_size, before_rd_offset, after_rd_offset, num_cbops_read;
    tCbuffer *source_buffer = timed_pb->tag_reframe.metadata_buff;
    ...
    /* Call the endpoint (or delegate) to adjust the rate */
    timed_pb->rate_adjust(timed_pb->rm_adjust_ep, pid_warp);

    /* ... and finally, run the cbops chain. */

    before_rd_offset = cbuffer_get_read_offset(source_buffer);
    cbops_mgr_process_data(timed_pb->cbops_manager, samples_to_play);
    after_rd_offset = cbuffer_get_read_offset(source_buffer);

    buffer_size = cbuffer_get_size_in_words(source_buffer);
    ...
    {
        num_cbops_read = buffer_size + after_rd_offset - before_rd_offset;
    }

    if (num_cbops_read != samples_to_play)
    {
        TTP_WARN_MSG2("timed_playback_do_rate_adjust amount %u different from "
                      "expected %u",
                      num_cbops_read, samples_to_play);
        samples_to_play = num_cbops_read;
    }
...
/**
 * \brief Calculates the samples to the next tag. Should only be called when there is an
...
/**
 * \brief Continue to consume the previous on time tag with the already calculated sra.
 *
 * \param timed_pb - Pointer to the timed_pb playback instance
 *
...
/**
 * \brief Continue to consume the previous late tag with the current given error.
 *
 * \param timed_pb - Pointer to the timed_pb playback instance
...
/**
 * \brief Continue to consume the previous tag.
 *
 * \param timed_pb - Pointer to the timed_pb playback instance
 *
 * \return True, if there is more data to process. False otherwise.
...
static void  timed_playback_discard_samples(TIMED_PLAYBACK* timed_pb, unsigned samples_to_discard)
{
    ...
    timing_trace_record_op_event_int(
            stream_external_id_from_endpoint(timed_pb->rm_adjust_ep)
            | (TIMING_TRACE_TTP_DISCARD_SAMPLES << 16),
            samples_to_discard, timed_pb->current_tag.error);
...
/**
 * \brief Function handles when the metadata tag timestamp is later
 *        than expected.
 *
 * \param timed_pb Pointer to the timed_pb playback instance.
 ...
 * \return True, if there is more data to process. False otherwise.
 */
static bool timed_playback_tag_late(TIMED_PLAYBACK *timed_pb,
                                    TIME_INTERVAL error)
{
...
static void  timed_playback_insert_silence(TIMED_PLAYBACK* timed_pb, unsigned silence_samples)
{
    ...
    timing_trace_record_op_event_int(
            stream_external_id_from_endpoint(timed_pb->rm_adjust_ep)
            | (TIMING_TRACE_TTP_INSERT_SILENCE_SAMPLES << 16),
            silence_samples, timed_pb->current_tag.error);
#endif /* INSTALL_TIMING_TRACE */
...
/**
 * \brief Function handles when the metadata tag timestamp is earlier
 *        than expected.
 *
 * \param timed_pb Pointer to the timed_pb playback instance.
 * \param error    The calculated error of the metadata timestamp.
 ...
 * \return True, if there is more data to process. False otherwise.
 */
static bool timed_playback_tag_early(TIMED_PLAYBACK *timed_pb,
                                     TIME_INTERVAL error)
{
    int silence_samples;
    unsigned space_out_sample;
...
static bool timed_playback_tag_on_time(TIMED_PLAYBACK* timed_pb, TIME_INTERVAL error)
{
    ...
    timing_trace_record_op_event_int(
            stream_external_id_from_endpoint(timed_pb->rm_adjust_ep)
            | (TIMING_TRACE_TTP_PLAYBACK_CONTROL << 16),
            error,
            ttp_pid_controller_get_warp(timed_pb->pid));
#endif /* INSTALL_TIMING_TRACE */

    /* Share the information with the python plotter tool. */
    TTP_PLOTTER_MACRO(timed_playback_sra = ttp_pid_controller_get_warp(timed_pb->pid));
...
/**
 * \brief Saves the last metadata tag from the input buffer.
 *
 * This is important because the PID controller only runs when a metadata
 * is at the front of the buffer.
 *
 * \param timed_pb Pointer to the timed_pb playback instance.
 * \param error    The calculated error of the metadata timestamp.
 * \param status   Early/late/on-time status for the tag.
 */
static void timed_playback_save_tag(TIMED_PLAYBACK *timed_pb,
                                    TIME_INTERVAL error,
                                    tag_timestamp_status status)
{
    ...
    TTP_PLOTTER_MACRO(timed_playback_pb_time = timed_pb->current_tag.playback_time);
    /* playback time = timestamp - error offset. =>
     * error offset  = timestamp - playback time*/
    TTP_PLOTTER_MACRO(timed_playback_err_offset = timed_pb->current_tag.timestamp - timed_pb->current_tag.playback_time);
    /* Share the timed_palyback_sp_adjust  with the plotter. */
    TTP_PLOTTER_MACRO(timed_playback_sp_adjust = reframe_sp_adjust(&timed_pb->tag_reframe));
...
/**
 * \brief works out a suitable reframe period based on sample rate
 *        so the period will be not longer than 20ms
 *        (20ms is practically good max value for running PID controller)
 * \param sample_rate sample rate of output interface
 * \return reframe period in samples
 */
static unsigned timed_playback_get_reframe_period_in_samples(unsigned sample_rate)
{
    patch_fn_shared(timed_playback);

    /* All music and high quality use cases */
                         ...
                         bool need_sra)
{
    ...
    unsigned cbops_remove_flags =  CBOPS_DISCARD | CBOPS_UNDERRUN;
    unsigned reframe_period;
    ...
    /* get suitable reframe period for this sample rate */
    reframe_period = timed_playback_get_reframe_period_in_samples(sample_rate);

    /* Before modifying the cbops set the reframe module to reframe the timstaps in
     * every fixed number of samples (reframe_period) */
    reframe_init(&timed_pb->tag_reframe, timed_pb->in_buffers[0], reframe_period, sample_rate);

    /* Remove the underrun and the discard cbops from the chain.
     * also remove the SRA if not in use 
     */
    if (!need_sra)
    {
        cbops_remove_flags |= CBOPS_RATEADJUST;
    }

    if (!cbops_mgr_remove(timed_pb->cbops_manager, cbops_remove_flags,
    ...
    /* Unachievable latency callback for unsolicited message */
    timed_pb->callback = callback;
    ...
    do{
        ...
        else
        {
            ...
            else if (!reframe_tag_is_void(tag_reframe))
            {
                ...
                TTP_PLOTTER_MACRO(timed_playback_error = error);
                ...
                if ((status == TAG_LATE) && (timed_pb->current_tag.error > error))
                {
                    ...
                    if (timed_pb->later_tag_count == UNACHIEVABLE_LATENCY_LIMIT)
                    {
                        ...
                        if (timed_pb->callback != NULL)
                        {
                            /* also callback for sending an unsolicited message */
                            timed_pb->callback(timed_pb->rm_adjust_ep, -error);
                        }
                    }
                }
