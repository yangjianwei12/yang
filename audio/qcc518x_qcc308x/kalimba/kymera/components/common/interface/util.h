...
/* A set of macros to help create dummy versions of functions used when
   a feature is disabled. The compiler will ignore unused variables while
   still generating optimized code. */
#define IGNORE_FALSE(x) ((((uintptr_t) x) & 0) ? FALSE : FALSE)
#define IGNORE_TRUE(x)  ((((uintptr_t) x) & 0) ? TRUE : TRUE)
#define IGNORE_NULL(x)  ((((uintptr_t) x) & 0) ? NULL : NULL)
 ...
 */
#define ROUND_DOWN_TO_POW2_MULTIPLE(num, bound) ((num)&(~((bound)-1)))

#define ROUND_UP_TO_POW2_MULTIPLE(num, bound) (((num)+((bound)-1))&(~((bound)-1)))

#define ROUND_UP_TO_WHOLE_WORDS(num) ROUND_UP_TO_POW2_MULTIPLE((num), ADDR_PER_WORD)
