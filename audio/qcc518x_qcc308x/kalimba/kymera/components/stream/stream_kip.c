...
/**
 * \brief Get external id from a P0 kip transform.
 *
 * \param transform [IN] - The P0 KIP transform to get the ID from
 *
 * \return               - The (external) transform id
 */
static TRANSFORM_ID stream_kip_external_id_from_transform(STREAM_KIP_TRANSFORM_INFO *transform)
{
...
{
    STREAM_KIP_TRANSFORM_INFO* kipt = kip_transform_list;
    unsigned count;
    ...
    while (kipt != NULL)
    {
        if( ((kipt->sink_id == sink_id) && (kipt->source_id == source_id)) ||
            (kipt->sink_id == sink_id && source_id == 0) || (kipt->source_id == source_id && sink_id == 0) ||
...
void stream_get_p1_connection_list(unsigned source_id, unsigned sink_id, unsigned p0_count, unsigned length, unsigned** conn_list)
{
    ...
    i = p0_count; /* Adding to the P0 list of transforms */
    while (kipt != NULL)
    {
        if( ((kipt->sink_id == sink_id) && (kipt->source_id == source_id)) ||
            (kipt->sink_id == sink_id && source_id == 0) || (kipt->source_id == source_id && sink_id == 0) ||
            (source_id == 0 && sink_id ==0) )
        {
            ...
            (*conn_list)[i++] = stream_kip_external_id_from_transform(kipt);

            /* use external source ID */
            (*conn_list)[i++] = kipt->source_id;

            /* use external sink ID */
            (*conn_list)[i++] = kipt->sink_id;
        }

        kipt = kipt->next;

        if ((kipt != NULL) && (i >= length))
        {
            L2_DBG_MSG2("stream_get_p1_connection_list: more P1 transforms than there is space in conn buffer (%d, P0: %d)", length, p0_count);
            kipt = NULL; /* to break out of the loop */
        }
    }
}
