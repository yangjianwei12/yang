...
typedef struct endpoint_shadow_state
{
     ...
     */
    struct ENDPOINT *head_of_sync;
    /* When an endpoint is not enabled, but it receives a kick, a timer is configured
     * to execute the kick when it fires, in case the endpoint has been enabled
     * in the meantime. */
    tTimerId kick_timer_id;
...
#ifdef INSTALL_ISO_CHANNELS
extern bool iso_sync_endpoints(ENDPOINT *ep1, ENDPOINT *ep2);
#endif /* INSTALL_ISO_CHANNELS */
