...
#define IS_OP_REMOTE(x) ((opid & OP_IS_REMOTE) == OP_IS_REMOTE)
#else
#define IS_OP_REMOTE(x) FALSE
#endif
 ...
 * appropriate list.
 *
 * \param opid               The base ep id of the operators source/sink terminal.
 *                           Comprised of opid | STREAM_EP_OP_SOURCE/SINK
 * \param ops                The array of operator ids that have already been
 *                           visited or will be visited, and index of next opid
 *                           to add.
 * \param num_src_terminals  The number of source terminals that this operator has.
 * \param num_sink_terminals The number of sinks terminals that this operator has.
 * \param ep_list            TODO: Document me.
 */
static void visit_remote_operator_terminals(unsigned opid,
                                            OPERATOR_LIST *ops,
                                            ENDPOINT_LIST *ep_list)
{
    STREAM_KIP_TRANSFORM_INFO *tfm;
    unsigned fnd_sources = 0, fnd_sinks = 0;
    unsigned num_src_terminals;
    unsigned num_sink_terminals;

    num_src_terminals = opmgr_get_num_sink_terminals(opid);
    num_sink_terminals = opmgr_get_num_source_terminals(opid);
    ...
    tfm = stream_kip_get_next_transform(NULL);
    if (tfm == NULL)
    {
    ...
    {
        ENDPOINT *ep;
        unsigned shadow_id, source_id, sink_id;        
        unsigned new_opid;

        source_id = stream_kip_get_transform_source_id(tfm);
        sink_id = stream_kip_get_transform_sink_id(tfm);
        tfm = stream_kip_get_next_transform(tfm);
        ...
        if ((source_id & (STREAM_EP_OP_BIT | STREAM_EP_OPID_MASK)) == opid)
        {
            GRAPH_MSG2("            kip_tfm 0x%04x (op) -> 0x%04x (..)", source_id, sink_id);
            fnd_sources++;
            new_opid = sink_id;
             ...
             */
            shadow_id = source_id & (~STREAM_EP_SHADOW_MASK);
            ep = source_endpoint_list;
        }
        else if ((sink_id & (STREAM_EP_OP_BIT | STREAM_EP_OPID_MASK)) == opid)
        {
            GRAPH_MSG2("            kip_tfm 0x%04x (..) -> 0x%04x (op)", source_id, sink_id);
            fnd_sinks++;
            new_opid = source_id;
            /* We may not need all this information but it saves us duplicating
             * code between branches of this if statemt to populate variables here.
             */
            shadow_id = sink_id & (~STREAM_EP_SHADOW_MASK);
        ...
        else
        {
             ...
             * searched for. */
            GRAPH_MSG2("            kip_tfm 0x%04x (..) -> 0x%04x (..)", source_id, sink_id);
            continue;
        }
        ...
        if ((fnd_sources == num_src_terminals) &&
            (fnd_sinks == num_sink_terminals))
        {
            /* All sources and sinks have been found,
...
#else
#define visit_remote_operator_terminals(p1, p2, p3)
...
/**
 * \brief Find the real endpoints present in a graph.
 *
 * This is done by finding all the operators that are connected to each other
 * and on the way noting all the real endpoints connected to this set of
 * operators.
 *
 * \param start_id The endpoint to use as a starting point for the topological
 *                 search.
 * \param real_eps The location to store the list of sinks and sources that
 *                 are found.
 */
static void find_graph_real_eps(unsigned start_id, ENDPOINT_LIST **real_eps)
{
    ...
    *real_eps = new_ep_list(MAX_REAL_EPS);
    ...
    for (curr_op = 0; curr_op < graph_ops.next_new_op; curr_op++)
    {
        ...
        if (IS_OP_REMOTE(opid))
        {
            visit_remote_operator_terminals(opid & ~OP_IS_REMOTE,
                                            &graph_ops,
                                            *real_eps);
        }
        else
        {
            /* Arbitrarily go through all the connected sinks first, anything
               real is a real source that we haven't encountered yet. Any
               operator needs adding to the list if it isn't already on the
               list. */
            visit_operator_terminals(opid | STREAM_EP_OP_SINK,
                                     &graph_ops,
                                     *real_eps);

            /* Now the connected sources */
            visit_operator_terminals(opid | STREAM_EP_OP_SOURCE,
                                     &graph_ops,
                                     *real_eps);

            find_op_rate_monitor_terminals(opid | STREAM_EP_OP_SINK,
                                           *real_eps);
            find_op_rate_monitor_terminals(opid | STREAM_EP_OP_SOURCE,
                                           *real_eps);
        }
    }
 ...
 *
 * \param enacting     The endpoint which should apply the enactment.
 * \param non_enacting The endpoint which will just report its rate.
 *
