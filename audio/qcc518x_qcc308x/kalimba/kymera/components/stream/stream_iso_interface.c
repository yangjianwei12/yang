...
static bool iso_set_up_endpoint(ENDPOINT * ep, ENDPOINT_DIRECTION dir, unsigned int buf_size)
{
    ...
    if (dir == SOURCE)
    {
         ...
         * MMU buffer.
         */
        cbuffer_set_write_byte_swap(ep->state.iso.cbuffer, TRUE);
    }
    else
    {
         ...
         * AUDIO_DATA_FORMAT_16_BIT_BYTE_SWAP. This is useful when the ISO
         ...
         * 
         * When connecting ENC (P1) -> ISO sink (P0):
         * - on P1: find out the data format of ENC source ep, and that is
         *   advertised in the operator's _get_data_format()
         * - on P0: get the data format of the ISO sink ep, and that is given
         *   by sco_get_data_format(). If byte swap is not set, this will return
         *   AUDIO_DATA_FORMAT_16 = 0, leading to an unresolvable data format
         *   for operators which do not advertise this data format. To allow
         *   operators using AUDIO_DATA_FORMAT_16_BIT_BYTE_SWAP on P1 to
         *   connect to the ISO sink, set this byte swap to TRUE.
         */
        cbuffer_set_read_byte_swap(ep->state.iso.cbuffer, TRUE);
    }
