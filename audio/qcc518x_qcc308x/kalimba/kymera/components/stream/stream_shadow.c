...
/* Data needed when the self kick timer fires. */
typedef struct kick_data
{
    ENDPOINT *ep;
    ENDPOINT_KICK_DIRECTION direction;
}SHADOW_KICK_DATA;
...
/*
 * \brief This function is executed when a timer fires. The timer is started
 * when the endpoint is disabled, but it receives a kick.
 *
 * \param data          Stores the ep that started the timer and the kick direction.
 *
 */
static void shadow_self_kick(void *data)
{
    SHADOW_KICK_DATA *kick_data = (SHADOW_KICK_DATA*)data;
    ENDPOINT *shadow_ep;

    shadow_ep = kick_data->ep;

    /* Check that the endpoint still exists. The kick could have happened during
     * teardown. */
    if (!stream_does_ep_exist(shadow_ep))
    {
        shadow_ep->state.shadow.kick_timer_id = 0;
        pfree(data);
        return;
    }

    /* If the endpoint is still not enabled, start another 1 ms timer.
     * The assumption is that the endpoint won't get closed before it's enabled
     * or enabled and closed within a ms.
     */
    if(!shadow_ep->is_enabled)
    {
        if (shadow_ep->state.shadow.kick_timer_id == 0)
        {
            L1_DBG_MSG1("WARNING: Shadow Endpoint 0x%x is still not enabled", (unsigned)(uintptr_t)shadow_ep);
            pfree(data);
        }
        else
        {
            L2_DBG_MSG1("Rescheduling a self kick ep 0x%04x", shadow_ep->id);
            /* The kick is rescheduled one more time, in one ms. */
            timer_schedule_bg_event_in(1000, shadow_self_kick, (void*)data);
        }
        shadow_ep->state.shadow.kick_timer_id = 0;
    }
    else
    {
        shadow_ep->state.shadow.kick_timer_id = 0;
        pfree(data);
        shadow_kick(shadow_ep, kick_data->direction);
    }
}
...
#if defined(COMMON_SHARED_HEAP)
{
    ...
    /* There are two potential races here: one is when the operator on P1
     * is started and the other is during teardown.
     * If the race happens during the former, the timer makes sure that
     * the kick is not lost, by trying to execute the kick when the timer
     * fires. The timer fires after 1 ms and if the endpoint has not been
     * enabled in the meantime, another 1 ms timer is started. If the
     * endpoint has been enabled when the second timer fires, the kick is
     * executed. Otherwise, the kick is lost.
     * If the race happens during the latter, the timer will still be
     * started, but the kick will never be executed. */
    if(!ep->is_enabled && (shadow->kick_timer_id == 0))
    {
        SHADOW_KICK_DATA *data = xzpnew(SHADOW_KICK_DATA);
        if(data != NULL)
        {
            L2_DBG_MSG1("Scheduling a self kick at shadow ep 0x%x", (unsigned)(uintptr_t)ep);
            data->ep = ep;
            data->direction = ep->direction;
            /* self kick after 1ms */
            shadow->kick_timer_id = timer_schedule_bg_event_in(1000, shadow_self_kick, (void*)data);
            return;
        }
    }
