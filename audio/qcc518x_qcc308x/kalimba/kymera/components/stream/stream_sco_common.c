...
bool sco_start_timers(ENDPOINT *endpoint, KICK_OBJECT *ko)
{
    ...
    ENDPOINT *synced_ep = NULL;
    TIME synced_fire_time = 0;
    unsigned n_intervals = 0;
    unsigned synced_hci_handle = 0;

#ifdef ACTIVATE_SDU_NUMBERING
    unsigned sdus_per_iso = 0, init_sdu_number = SCO_DRV_EXPECTED_TS_UNKNOWN;
#endif /* ACTIVATE_SDU_NUMBERING */
...
#ifdef INSTALL_ISO_CHANNELS
         /* Get the fire_time for the sync-ed endpoint, if any. In this case we
          * have postponed scheduling the first kick of the sync-ed endpoint,
          * so that both could start in the same SDU Interval. Note that the
          * sync-ed endpoint must have been previously set before trying to
          * start any of the two endpoints.
          */
         synced_ep = endpoint->state.iso.synced_ep;
         if (synced_ep != NULL)
         {
             synced_fire_time = synced_ep->state.iso.rm_start_time;
             synced_hci_handle = stream_sco_get_hci_handle(synced_ep);

             if (synced_fire_time != 0)
             {
                 TIME cis_toa[2];
                 TIME_INTERVAL cis_toa_offset;

                 /* We do have a sync-ed endpoint set, and this one has a
                  * fire_time set. After computing a first kick time
                  * (fire_time) for the current endpoint, adjust the sync-ed
                  * endpoint's fire_time if needed such that both endpoints'
                  * timers can have the first kick in the same SDU Interval.
                  */
                 synced_ep->state.iso.start_pending = FALSE;

                 cis_toa[0] = time_add(sco_wallclock_get_time_from_ticks(hci_handle,
                                           endpoint->state.sco.data_avail_bt_ticks),
                                       sco_iso_transport_offset_from_air_get(hci_handle));

                 cis_toa[1] = time_add(sco_wallclock_get_time_from_ticks(synced_hci_handle,
                                            synced_ep->state.sco.data_avail_bt_ticks),
                                       sco_iso_transport_offset_from_air_get(synced_hci_handle));

                 cis_toa_offset = time_sub(cis_toa[0], cis_toa[1]);

                 /* Compare the two CIS-es endpoints first ToA, and see if one
                  * was to be started one interval ahead of the other. Note
                  * that if these endpoints were previously sync-ed, they must
                  * belong to the same CIG.
                  */
                 if (cis_toa_offset < 0)
                 {
                     cis_toa_offset = -cis_toa_offset;
                     /* Add jitter value just in case the offset is, for
                      * example, 7499us for a kick period of 7500us.
                      */
                     cis_toa_offset += 100;
                     endpoint->state.iso.packet_offset = cis_toa_offset / tesco;
                 }
                 else if (cis_toa_offset > 0)
                 {
                     /* Add jitter value just in case the offset is, for
                      * example, 7499us for a kick period of 7500us.
                      */
                     cis_toa_offset += 100;
                     synced_ep->state.iso.packet_offset = cis_toa_offset / tesco;
                 }
             }
             else
             {
                 /* We do have a sync-ed endpoint set, but this one doesn't
                  * have a fire_time set just yet. After computing a first kick
                  * time (fire_time) for the current endpoint, instead of
                  * scheduling the timer, postpone this until the sync-ed
                  * endpoint will try to schedule its first kick, so both
                  * endpoints can their first go in the same SDU Interval.
                  */
...
#ifdef INSTALL_ISO_CHANNELS
    if (!endpoint->state.iso.start_pending &&
        (time_le(fire_time, now) ||
         ((synced_fire_time != 0) && time_le(synced_fire_time, now))))
#else
    if (time_le(fire_time, now))
#endif /* INSTALL_ISO_CHANNELS */
    {
        /* If the time is in the past then bring it into the future, calculate
         * the required delta here. In case we need to sync two endpoints, make
         * sure both fire_time are in the future.
         */
        TIME_INTERVAL project_fwd, delta;
        ...
        if (delta < 0)
        {
            /* This can only be the case when we need to sync two endpoints
             * and only one's fire_time is in the past. In this case the
             * current endpoint's fire_time will remain unchanged, and the
             * sync-ed endpoint's fire_time will have to be projected into the
             * future.
             */
            delta = 0;
        }
...
#ifdef INSTALL_ISO_CHANNELS
        n_intervals = n;
        n += endpoint->state.iso.packet_offset;
#endif /* INSTALL_ISO_CHANNELS */
        ...
        if (synced_fire_time != 0)
        {
            /* The sync-ed endpoints should have the same kick period, so
             * we could use this endpoint's HCI handle to get the ISO
             * Interval in microseconds.
             */
            n_intervals += synced_ep->state.iso.packet_offset;
            project_fwd = n_intervals * (TIME_INTERVAL)tesco;
            synced_ep->state.iso.data_avail_bt_ticks += n_intervals * US_TO_BT_TICKS(tesco);
            synced_ep->state.iso.data_avail_bt_ticks &= BT_TICK_MASK;
            synced_fire_time = time_add(synced_fire_time, project_fwd);

            sco_reinit_ratematching(synced_ep, synced_fire_time);
        }
#endif /* INSTALL_ISO_CHANNELS */
    }
    ...
    if ((endpoint->stream_endpoint_type == endpoint_iso) )
    {
        ...
        if (SOURCE == endpoint->direction)
        {
            ...
            {
                /* On the source EP we can only expect a predictable SDU number
                 * for unframed ISO.
                 */

                /* Sync-ed endpoints should have the same ISO and SDU Interval,
                 * and since we can only sync endpoints for unframed ISOAL, we
                 * can infer sdus_per_iso for both endpoints, using one
                 * endpoint's HCI handle.
                 */
                ...
                if (!endpoint->state.iso.start_pending)
                {
                    /* Get the expected from-air sequence number for this
                     * endpoint.
                     */
                    init_sdu_number = (sco_iso_sdu_number_from_air_get(hci_handle) +
                                       n * sdus_per_iso) & 0xffff;
                    sco_src_drv_update_expected_ts(sco_get_sco_src_drv(endpoint),
                                                   init_sdu_number);

                    if (synced_fire_time != 0)
                    {
                        /* If we need to sync the start of two ISO source EP,
                         * make sure they start with the same SDU sequence
                         * number. Since we cannot guarantee the advertised
                         * from-air sequence number from BTSS (ISO_PARAMS
                         * message) would be the same for both endpoints (same
                         * goes for NextSlotTime), obtain it by using the
                         * sync-ed endpoint's HCI handle.
                         */
                        unsigned synced_sdu_number =
                            (sco_iso_sdu_number_from_air_get(synced_hci_handle) +
                            n_intervals * sdus_per_iso) & 0xffff;

                        PL_ASSERT(synced_sdu_number == init_sdu_number);

                        sco_src_drv_update_expected_ts(sco_get_sco_src_drv(synced_ep),
                                                       synced_sdu_number);
                    }
                }
            }
    ...
    if (endpoint->state.iso.start_pending)
    {
        /* Do not schedule a timer for the current endpoint. This will be done
         * when we'll have a first kick time for the sync-ed endpoint.
         */
        return FALSE;
    }
#endif /* INSTALL_ISO_CHANNELS */
    ...
    if (synced_fire_time != 0)
    {
        /* Schedule a timer for the sync-ed endpoint as well. */
        timer_schedule_event_at_atomic(synced_fire_time,
                                       kick_obj_kick,
                                       (void *)kick_obj_from_sched_endpoint(synced_ep),
                                       &synced_ep->state.iso.kick_id);
    }
#endif /* INSTALL_ISO_CHANNELS */
...
bool sco_configure(ENDPOINT *endpoint, unsigned int key, uint32 value)
{
    ...
    if ((key & ENDPOINT_INT_CONFIGURE_KEYS_MASK) != 0)
    {
        ...
        case EP_DATA_FORMAT:
            if ((endpoint->stream_endpoint_type != endpoint_iso) &&
                (value == AUDIO_DATA_FORMAT_ENCODED_DATA))
            {
                return FALSE;
            }
...
bool sco_get_config(ENDPOINT *endpoint, unsigned int key, ENDPOINT_GET_CONFIG_RESULT* result)
{
    ...
    case EP_RATEMATCH_ABILITY:
        if(SOURCE == endpoint->direction ||
           0 == endpoint->state.sco.external_rate_adjust_opid)
        {
            result->u.value = (uint32)RATEMATCHING_SUPPORT_NONE;
        }
        else
        {
            /* SCO sink endpoint can do TTP rate matching by delegation, report
             * RATEMATCHING_SUPPORT_AUTO so no non-ttp rate matching will be
