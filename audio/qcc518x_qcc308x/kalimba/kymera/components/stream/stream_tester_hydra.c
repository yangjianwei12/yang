...
static void tester_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir);
...
static void tester_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir)
{
    ...
    if (SOURCE == ep->direction)
    {
        ...
        {
            audio_data_service_queue_meta(tester->service_priv, NULL);
        }
    }
    ...
    if (copy_samples != 0)
    {
        ...
        if (SINK == ep->direction)
        {
            ...
            {
                audio_data_service_dequeue_meta(tester->service_priv, copy_samples * OCTETS_PER_SAMPLE, TRUE, NULL);
            }
        }
