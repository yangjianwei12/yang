...
*/
static void enable_op_endpoints_core(ENDPOINT *eplist, EXT_OP_ID opid, ep_en_disable_fnc en_disable_fnc);
static unsigned operator_create_stream_key(EXT_OP_ID opid,
                                           unsigned int idx,
...
static OPMSG_CONFIGURATION_KEYS transform_endpoint_key_to_operator_key(unsigned int key);
...
unsigned stream_operator_get_endpoint_id(EXT_OP_ID opid,
                                         unsigned int idx,
                                         ENDPOINT_DIRECTION dir)
{
    if (!opmgr_is_opidep_valid(opid))
    {
        return 0;
    }

    return operator_create_stream_key(opid, idx, dir);
}
 ...
 *
 */
void stream_enable_operator_endpoints(EXT_OP_ID opid)
{
 ...
 *
 */
void stream_disable_operator_endpoints(EXT_OP_ID opid)
{
...
bool stream_destroy_all_operators_endpoints(EXT_OP_ID opid,
                                            unsigned num_sinks,
...
/* TODO: As this function is exposed to capabilities it should really use
   EXT_OP_ID instead of INT_OP_ID. */
ENDPOINT *stream_get_connected_endpoint_from_terminal_id(INT_OP_ID opid,
                                                         unsigned terminal_id)
{
    ENDPOINT_DIRECTION dir;
    unsigned terminal_number;
    EXT_OP_ID ext_op_id;
    unsigned key;
    ENDPOINT *ep;
    
    dir = ((terminal_id & TERMINAL_SINK_MASK) != 0) ? SINK : SOURCE;
    terminal_number = terminal_id & TERMINAL_NUM_MASK;

    /* create key for the operator endpoint */
    ext_op_id = INT_TO_EXT_OPID(opid);
    key = operator_create_stream_key(ext_op_id, terminal_number, dir);
...
static bool operator_configure(ENDPOINT *endpoint, unsigned int key, uint32 value)
{
    ...
    case EP_DATA_FORMAT:
        ...
        else
        {
            if (opmgr_config_msg_to_operator(stream_external_id_from_endpoint(endpoint), \
                    transform_endpoint_key_to_operator_key(key), value))
            {
                return TRUE;
            }
        }
    ...
    case EP_CBOPS_PARAMETERS:
    {
        ...
        if (opmgr_config_msg_to_operator(stream_external_id_from_endpoint(endpoint), \
                transform_endpoint_key_to_operator_key(key), shift))
        {
            retval = TRUE;
        }
    ...
    switch (key)
    {
        ...
        default:
        {
            ...
            success = opmgr_get_generic_value_from_operator(
                          stream_external_id_from_endpoint(endpoint),
                          transform_endpoint_key_to_operator_key(key),
                          &value);
...
static void operator_get_timing (ENDPOINT *endpoint, ENDPOINT_TIMING_INFORMATION *time_info)
{
    ...
    opmgr_get_sched_info(endpoint->id, &block_size, &is_locally_clocked, &wants_kicks);
...
static void operator_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir)
{
    ...
    timing_trace_record_kicked( get_opid_from_opidep(ep->id), time_get_time());
#endif
...
/**
 * \brief Get operator data from terminal endpoint
 */
OPERATOR_DATA *stream_get_op_data_from_endpoint(ENDPOINT *ep)
{
    if (ep != NULL)
    {
        /* Make sure it really is an operator terminal 
         * so the result of get_opid_from_opidep will be valid 
         */
        if (ep->stream_endpoint_type == endpoint_operator)
        {
            return get_op_data_from_id(get_opid_from_opidep(ep->key));
        }
    }
    return NULL;
}
 ...
 */
OPERATOR_DATA *stream_get_opdata_from_external_transform_id(TRANSFORM_ID tid, bool get_sink)
{
    TRANSFORM *tfm;
    patch_fn_shared(stream_operator);
    /* get the transform */
    ...
    if (tfm != NULL)
    {
        ENDPOINT *ep = get_sink? tfm->sink : tfm->source;
        return stream_get_op_data_from_endpoint(ep);
    }
    return NULL;
}
