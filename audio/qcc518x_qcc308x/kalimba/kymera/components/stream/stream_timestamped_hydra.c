...
****************************************************************************/
/**
 * \file  stream_timestamped_hydra.c
 * \ingroup stream
 */
...
/**
 * \brief translate endpoint metadata to Kymera tag
 * \param tag metadata tag to populate
 * \param data pointer to endpoint data (represented by frame metadata structure)
 * \param length source data length in octets
 */
static void translate_source_tag(metadata_tag *tag, void *data, unsigned length)
{
    /* Source endpoint, converting to Kymera metadata */
    frame_metadata_struct *frame_metadata = (frame_metadata_struct *)data;
    if (length >= sizeof(frame_metadata_struct))
    {
        METADATA_TIMESTAMP_SET(tag, frame_metadata->ttp, METADATA_TIMESTAMP_LOCAL);
        tag->sp_adjust = frame_metadata->spa;
...
/**
 * \brief translate Kymera tag to endpoint metadata
 * \param tag metadata tag to translate
 * \param data pointer to endpoint data (represented by frame metadata structure)
 * \param length pointer to fill in with data length
 */
static void translate_sink_tag(metadata_tag *tag, void *data, unsigned *length)
{
    /* Sink endpoint, converting from Kymera metadata */
    frame_metadata_struct *frame_metadata = (frame_metadata_struct *)data;
    frame_metadata->ttp = tag->timestamp;
    frame_metadata->spa = tag->sp_adjust;
    if (METADATA_STREAM_START(tag) != 0)
    {
...
/**
 * \brief kick internals for timestamped source ep
 * \param ep pointer to the endpoint that is being kicked
 * \param data_available source buffer octets available
 * \return STREAM_KICK_FORWARDS if data was transferred,
 *         else STREAM_KICK_INVALID
 */
static ENDPOINT_KICK_DIRECTION timestamped_source_kick(ENDPOINT *ep, unsigned data_available)
{
    endpoint_timestamped_state *timestamped = &ep->state.timestamped;
    unsigned copy_octets;

    if (buff_has_metadata(timestamped->source_buf))
    {
        audio_data_service_queue_meta(timestamped->service_priv, translate_source_tag);
    ...
    if (copy_octets != 0)
    {
        metadata_strict_transport(timestamped->source_buf,
                  timestamped->sink_buf, copy_octets);

        return STREAM_KICK_FORWARDS;
    }
    return STREAM_KICK_INVALID;
}

/**
 * \brief kick internals for timestamped sink ep
 * \param ep pointer to the endpoint that is being kicked
 * \param data_available source buffer octets available
 * \return STREAM_KICK_BACKWARDS if data was transferred,
 *         else STREAM_KICK_INVALID
 */
static ENDPOINT_KICK_DIRECTION timestamped_sink_kick(ENDPOINT *ep, unsigned data_available)
{
    endpoint_timestamped_state *timestamped = &ep->state.timestamped;
    ...
    if (buff_has_metadata(timestamped->source_buf))
    {
        ...
        copy_octets = audio_data_service_max_sink_data(timestamped->service_priv, complete_tag_octets, timestamped->source_buf);
    }
    else
    {
        copy_octets = data_available;
    ...
    if (copy_octets != 0)
    {
        ...
        {
            audio_data_service_dequeue_meta(timestamped->service_priv, copy_octets, FALSE, translate_sink_tag);
        }

        return STREAM_KICK_BACKWARDS;
    }
    return STREAM_KICK_INVALID;
}
...
static void timestamped_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir)
{
    ...
    ENDPOINT_ID ext_ep_id = stream_toggle_ep_id_between_int_and_ext(ep->id);
...
#ifdef INSTALL_TIMING_TRACE
                timing_trace_record_kicked(ext_ep_id, time_get_time());
#endif /* INSTALL_TIMING_TRACE */
            }
        }
        else
    ...
    ENDPOINT_KICK_DIRECTION propagate_kick_dir;

    if (SOURCE == ep->direction)
    {
        propagate_kick_dir = timestamped_source_kick(ep, data_available);
    }
    else
    {
        propagate_kick_dir = timestamped_sink_kick(ep, data_available);
    }
    if (propagate_kick_dir != STREAM_KICK_INVALID)
    {
        audio_data_service_update_buffers(timestamped->service_priv);
        audio_data_service_kick_consumer(timestamped->service_priv);

#ifdef INSTALL_TIMING_TRACE
        timing_trace_instr_stream_record_end_ep_run(ep, FALSE);
#endif /* INSTALL_TIMING_TRACE */

        propagate_kick(ep, propagate_kick_dir);
    }

#ifdef INSTALL_TIMING_TRACE
    timing_trace_record_end_run(ext_ep_id);
#endif /* INSTALL_TIMING_TRACE */
}
