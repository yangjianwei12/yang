...
void disconnect_in_place_update(TRANSFORM *transform,
                                IN_PLACE_DISCONNECT_PARAMETERS* parameters)
{
    tCbuffer *buff_to_destroy;
    IN_PLACE_BUFFER_LIST* inplace_buffer_list_to_source = parameters->inplace_buffer_list_to_source;
    IN_PLACE_BUFFER_LIST* inplace_buffer_list_to_sink = parameters->inplace_buffer_list_to_sink;
									  ...
									  tCbuffer *sink_buff)
{
    ...
    if (source_buff && sink_buff) /* Case 1 */
    {
        ...
        inplace_buffer_list_to_source = &(parameters->parameters_inplace_buffer_list);
        ...
        if(inplace_buffer_list_to_source->count == 0)
        {
            ...
            if(inplace_buffer_list_to_sink->count != 0)
            {
                ...
                parameters->inplace_buffer_list_to_sink = inplace_buffer_list_to_sink;
                /* cancel kicking */
                in_place_list_cancel_kicks(in_place_get_tail_sink_endpoint(inplace_buffer_list_to_sink));
            }
            else
        ...
        else if(inplace_buffer_list_to_sink->count == 0)
        {
            ...
            parameters->inplace_buffer_list_to_source = inplace_buffer_list_to_source;
            destroy_in_place_buffer_list(inplace_buffer_list_to_sink);
        ...
        else
        {
            ...
            parameters->new_base = (int *)xppmalloc(new_base_size, MALLOC_PREFERENCE_FAST);
            if (parameters->new_base == NULL)
            {
                L0_DBG_MSG("disconnect_in_place_chain: Not enough memory to split the in place chain");
                destroy_in_place_buffer_list(parameters->inplace_buffer_list_to_sink);
                return FALSE;
            }
            else
            {
    ...
    else if (sink_buff)/* Case 3 */
    {
        ...
        inplace_buffer_list_to_sink = &(parameters->parameters_inplace_buffer_list);
        init_in_place_buffer_list(inplace_buffer_list_to_sink);
                               ...
                               IN_PLACE_DISCONNECT_PARAMETERS *parameters)
{
    ...
    parameters->inplace_buffer_list_to_sink = NULL;
    parameters->inplace_buffer_list_to_source = NULL;
    /* Set the new base as null. */
    parameters->new_base = NULL;
    ...
    return disconnect_in_place_chain(transform, parameters,
