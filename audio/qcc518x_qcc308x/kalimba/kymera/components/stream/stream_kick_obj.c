...
void kick_obj_destroy(KICK_OBJECT *ko)
{
    ...
    /* There isn't much complexity associated with a kick object
       so all we need to do is free the memory that was allocated
       and update the kick_object_list. */
    element = &kick_object_list;
