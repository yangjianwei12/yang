 ...
 * \param nrof_channels Mumber of channels in the usb audio connection
 * \param subframe_size Size of subframe in number of bits
 *
 * \return TRUE if configured successfully else FALSE
 */
bool stream_usb_audio_configure_ep(ENDPOINT *ep,
 ...
 * \param rate_adjust_val If endpoint enacting the address of rate adjust
 *                        value will be returned on this parameter, else
 *                        NULL will be returned.
 * \return TRUE if query was successful else FALSE;
 */
bool stream_usb_audio_can_enact_rate_adjust(ENDPOINT *ep,
...
void stream_create_usb_audio(const AUDIO_DATA_SERVICE_EP_CFG *cfg,
                             AUDIO_DATA_SERVICE_EP_CREATE_RESULT* create_res)
{
    ENDPOINT *ep;
    endpoint_usb_audio_state *usb_audio;
    patch_fn_shared(stream_usb_audio_hydra);
 ...
 */
static void usb_audio_rate_measure_reset(endpoint_usb_audio_state *usb_audio,
                                         TIME current_time)
{
 ...
 */
static void usb_audio_rate_measure_update(endpoint_usb_audio_state *usb_audio,
                                          TIME current_time,
										  unsigned n_packets)
{
    usb_audio_rate_measure *rate_measure = usb_audio->rate_measure;
 ...
 * \param cb               Pointer to the Cbuffer struct for the buffer that
 *                         is being connected.
 * \param ep_to_kick       Pointer to the endpoint which will be kicked after a
 *                         successful run. This can be different from the
 *                         "connected to" endpoint when in-place running is
 *                         enabled.
 ...
 *
 * \note The endpoint will only be started if the connected to endpoint wants
 *       to be started too.
 */
static bool usb_audio_connect(ENDPOINT *ep,
                              tCbuffer *cb,
 							  ENDPOINT *ep_to_kick,
 ...
 *
 * \param usb_audio       Pointer to the usb_audio state structure
 * \param frames_consumed TODO:
 * \param frames_written  TODO: 
 */
static void usb_audio_tx_transfer_metadata(endpoint_usb_audio_state *usb_audio,
                                           unsigned frames_consumed,
										   unsigned frames_written)
{
        ...
        buff_has_metadata(usb_audio->sink_buf))
    {
        ...
        audio_data_service_dequeue_meta(usb_audio->service_priv, afteridx, FALSE, NULL);
    }
                                           ...
                                           unsigned packets_copied, unsigned *input_data_available)
{
    ...
    if(packets_copied == 0)
    {
        if(!usb_audio->tx_host_in_pause)
        {
            ...
            if(USB_AUDIO_TX_HOST_PAUSE_THRESHOLD_KICKS <= usb_audio->no_copy_counter)
            {
                ...
                L2_DBG_MSG1("USB AUDIO TX: Host pause detected!, time = %d", time_get_time());
            }
        }
    ...
    else
    {
        ...
        if(usb_audio->tx_host_in_pause)
        {
            ...
            L2_DBG_MSG1("USB AUDIO TX: Host resume detected!, time = %d", time_get_time());
        }
    }
}
