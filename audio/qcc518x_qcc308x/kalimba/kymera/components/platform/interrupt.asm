...
// Code to support Machine Learning multiplier configuration options
#if defined(CHIP_HAS_ENHANCED_MAC_MODES)
   r0 = M[$MAC_OPERAND_MODE];    // Get the new MAC_OPERAND_MODE register value
   push r0;                      // Store it on the stack (last item before calling the SSR)
   M[$MAC_OPERAND_MODE] = Null;  // Set to 0 for backward compatibility mode
#endif
...
#if defined(CHIP_HAS_ENHANCED_MAC_MODES)
   // Note: The MAC_OPERAND_MODE mode is placed on the stack last to
   // maintain stack compatibility with previous code and simplify
   // interpretation by debug tools (e.g. ACAT).
   r1 = M[SP - 8];            // Dig out the saved INT_SAVE_INFO
#else
   pop r1;
#endif
...
#if defined(CHIP_HAS_ENHANCED_MAC_MODES)
   pop r0;                    // Get the saved MAC_OPERAND_MODE register value
   M[$MAC_OPERAND_MODE] = r0; // Restore the MAC_OPERAND_MODE register value
   pop r0;                    // Need to pop the INT_SAVE_INFO left on the stack
#endif
