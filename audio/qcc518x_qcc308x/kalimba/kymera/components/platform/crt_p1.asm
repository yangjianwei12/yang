...
.MODULE $M.crt_p1;
   ...
   call $_thread_offload_is_enabled;
   Null = r0;
...
$_pm_ram_initialise_p1:
   ...
   // The P1 cache mode will be set later, and the exception region
   // enabled at the same time, so return without setting it here.
   rts;
#endif

   // Cache mode is fixed, so region start and size are defined at build time.
   r0 = PM_RAM_P1_CACHE_START_ADDRESS;
   r1 = PM_RAM_CACHE_BANK_SIZE_WORDS;
   // Continue into $_pm_p1_exception_region_init to set the 
   // exception region to cover the cache. 

// void pm_p1_exception_region_init(void *cache_start_addr, unsigned cache_size_words);

$_pm_p1_exception_region_init:

   // Enable programmable exception regions for PM
   M[$PM_PROG_EXCEPTION_REGION_START_ADDR] = r0;
