 ...
 */
static inline bool invalid_heap_size(heap_names heap_num, unsigned size, PROC_ID_NUM core)
{
    NOT_USED(heap_num);
    ...
    if (size < DM_RAM_BANK_SIZE)
    {
        ...
        /* If P1 is disabled, HeapAllocationEx MIB key should be able to specify
         * 0 banks for P1 main heap. */
        if (core > PROC_PROCESSOR_0 && proc_single_core_present())
        {
            return FALSE;
        }
...
static void get_heap_allocation(heap_dyn_size_config *value)
{
    ...
    unsigned heap_internal_aggregated_size, heap_internal_max_size;
    HEAP_MIB_VALUE mib_value;
    ...
    heap_internal_aggregated_size = 0;
    ...
    for (core = PROC_PROCESSOR_0; core < PROC_PROCESSOR_MAX; core++)
    {
        ...
        for (heap_num = HEAP_MAIN; heap_num < HEAP_ARRAY_SIZE; heap_num++)
        {
            ...
            else if (heap_num < HEAP_INTERNAL_SIZE)
            {
                ...
                size *= KIBYTE;
                if (invalid_heap_size(heap_num, size, core))
                {
                    panic(PANIC_AUDIO_HEAP_CONFIGURATION_INVALID);
                }
                total += size;

                heap_internal_aggregated_size += size;
    ...
    heap_internal_max_size = get_region_max_size(HEAP_MAIN);
#if defined(CHIP_HAS_SLOW_DM_RAM)
    heap_internal_max_size += get_region_max_size(HEAP_SLOW);
#endif /* CHIP_HAS_SLOW_DM_RAM */

    if (heap_internal_aggregated_size > heap_internal_max_size)
    {
        panic(PANIC_AUDIO_HEAP_CONFIGURATION_INVALID);
    }
}
