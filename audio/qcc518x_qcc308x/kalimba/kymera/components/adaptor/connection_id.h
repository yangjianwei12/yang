 ...
 * \ingroup adaptor
 *
 * Definition of the types and macros encapsulating the concept of a
 * connection identifier.
 *
 * Connections are half-duplex links. Messages going into the audio subsystem
 * are sent over "ingress connections" while messages going outside of the
 * audio subsystem are sent over "egress connections". Each ingress
 * connection has one and only one associated egress connection. A link is
 * made of 2 peers: the sender and the receiver.
 */
...
/**
 * Value representing and invalid link.
 * This is a theoretically valid value although impossible in practice.
 */
#define CONN_LINK_INVALID  ((CONNECTION_LINK)(SENDER_ID_MASK | RECEIVER_ID_MASK))

/**
 * Value representing and invalid peer.
 * This is a theoretically valid value although impossible in practice.
 */
#define CONN_PEER_INVALID ((CONNECTION_PEER) SENDER_ID_MASK)

/* Legacy definitions. */
#define INVALID_CON_ID    CONN_LINK_INVALID
#define INVALID_CLIENT_ID CONN_PEER_INVALID
...
extern CONNECTION_LINK adaptor_reverse_connection(CONNECTION_LINK con_id);
 ...
 *
 * \param con_id Connection id with processor information.
 *
...
extern CONNECTION_LINK adaptor_pack_proc_id(CONNECTION_LINK conid,
                                            PROC_ID_NUM procid);
