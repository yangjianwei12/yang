 ...
 */
     ...
     * The argument is the program counter where the exception fired; on
     * all current platforms (Crescendo d01+), it's the EXCEPTION_PC
     * register and points exactly at the instruction that caused the
     * exception. (On older platforms, on which Kymera never went to
     * production, a different method was used which could be inexact, due
     * to pipelining.)
     * (Since it's a program counter value, it will have bit 1 set if the
     * processor is in Minim mode; you may need to search for the previous
     * even address to find it in .lst files, etc.)
     * Note that this value is 32 bits internally, and is reported as such
     * in the audio subsystem's log and data structures, but only the least
     * significant 16 bits make it out in messages to Curator / Host.
     */
    ...
    /**
     * The operator returned FALSE but also provided a response message, or
     * returned a response message without response length for payload.
    ...
    /**
     * Failed to get semaphore after a number of retries.
     */
    PANIC_HAL_SEMAPHORE_RETRIES = 0x00A2,

    /**
     * The value of either MIB key HeapAllocation or of
     * MIB key HeapAllocationEx is invalid.
     */
    PANIC_AUDIO_HEAP_CONFIGURATION_INVALID = 0x00A3,

    /**
     * Failed to get semaphore after a certain amount of time.
     */
    PANIC_HAL_SEMAPHORE_TIMEOUT = 0x00A4,

    /**
     * Failed to get a hardware lock after a number of retries.
     */
    PANIC_AUDIO_PL_HWLOCK_RETRIES = 0x00A5,

    /**
     * Failed to get a hardware lock after a certain amount of time.
     */
    PANIC_AUDIO_PL_HWLOCK_TIMEOUT = 0x00A6,

    /**
     * Attempt to release a hardware lock that has already
     * been released.
     */
    PANIC_AUDIO_PL_HWLOCK_ALREADY_RELEASED = 0x00A7,

    /**
     * A spot check found the Kalimba ARITHMETIC_MODE register nonzero in
     * general-purpose code. Probably some capability code forgot to reset it
     * to zero after a calculation. Argument is the value of ARITHMETIC_MODE.
     */
    PANIC_ARITHMETIC_MODE_NONZERO = 0x00A8,


    /*
     * An error has occured in the ML ENGINE Framework. The diatribe will
     * indicate the type of error.
     */ 
    PANIC_ML_ENGINE_ERROR = 0x00B1,

    /* *******************  Hydra common panic IDs start at 0x1000 ********************** */


    /*
     * This is the start of debug panic IDs that MUST never occur in
     * production code. These are ONLY for events in the following categories:
     *
     *   1. Temporary panics in place of recovery code (which must be removed
     *      before the relevant work is complete)
     *
     *   2. Special panics for debugging modes
     *      These should only be emitted with the right debug options enabled.
     *
     * Anything not in the above categories should use one of the following
     * instead:
     *
     * a. Proper error handling
     * b. Asserts, if they represent programming errors.
     * c. A panic ID in the normal range, if it's something that can possibly
     *    go wrong in some (rare) circumstance, and there's no prospect of
     *   usefully recovering or reporting an error.
     *
     * Because these IDs are not intended to be customer-visible,
     * any unused values can be immediately repurposed.
     */
    ...
    /**
     * clk_mgr voltage change request failed.
     */

    PANIC_AUDIO_CLK_MGR_VOLTAGE_CHANGE_REQ_FAILED = 0x4016,

    /**
     * clk_mgr voltage change fsm in error state.
     */

    PANIC_AUDIO_CLK_MGR_VOLTAGE_STATE_MACHINE_ERROR = 0x4017,

    /**
     * LZ4 decompression error
     */

    PANIC_AUDIO_LZ4_UNZIP_ERROR = 0x4018,