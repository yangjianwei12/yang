...
/****************************************************************************
 * cbuffer_get_usable_data_read_offset
 ...
 */
unsigned int cbuffer_get_usable_data_read_offset(tCbuffer *cbuffer)
{
    int *read_ptr;
    unsigned int read_offset;
    unsigned usable_octets = cbuffer_get_usable_octets(cbuffer);
    ...
    read_offset =  ((uintptr_t)read_ptr - (uintptr_t)cbuffer->base_addr);
    if(usable_octets == NR_OF_OCTETS_IN_WORD(USE_16BIT_PER_WORD))
    {
        /* 16-bit unpacked */
        return (read_offset / 2) + (read_offset & 1);
    }
    else
    {
        /* 32-bit packed */
 ...
 */
bool cbuffer_enough_data_to_run_ex(tCbuffer *cbuffer,
                                   unsigned data_size_octets)
{
    unsigned input_data = cbuffer_calc_amount_data_ex(cbuffer);

    if (buff_has_metadata(cbuffer))
    {
        /* Make sure we don't consume more than what metadata indicates. */
        unsigned input_meta = buff_metadata_available_octets(cbuffer);

        if (input_meta < input_data)
        {
            PL_PRINT_P2(TR_CBUFFER,
                        "Less metadata than data: data=%d, meta=%d, capped",
                        input_data, input_meta);
            input_data = input_meta;
        }
    }

    if (input_data < data_size_octets)
    {
        return FALSE;
    }
    else
...
/*
 * cbuffer_calc_avail_data_ex
 */
extern unsigned cbuffer_calc_avail_data_ex(tCbuffer *buffer)
{
    unsigned num_metadata_octets;
    unsigned num_input_octets = cbuffer_calc_amount_data_ex(buffer);
    if (buff_has_metadata(buffer))
    {
        num_metadata_octets = buff_metadata_available_octets(buffer);
        if (num_metadata_octets < num_input_octets)
        {
            /* Limit to available metadata */
            num_input_octets = num_metadata_octets;
        }
    }
    return num_input_octets;
}

/*
 * cbuffer_calc_avail_space_ex
 */
extern unsigned cbuffer_calc_avail_space_ex(tCbuffer *buffer)
{
    unsigned metadata_space_octets;
    unsigned output_space_octets = cbuffer_calc_amount_space_ex(buffer);
    if (buff_has_metadata(buffer))
    {
        metadata_space_octets = buff_metadata_available_space(buffer);
        if (metadata_space_octets < output_space_octets)
        {
            /* Limit to available metadata */
            output_space_octets = metadata_space_octets;
        }
    }
    return output_space_octets;
}
