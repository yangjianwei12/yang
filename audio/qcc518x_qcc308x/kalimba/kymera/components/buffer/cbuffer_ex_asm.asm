...
$cbuffer.get_read_address_ex:
    ...
    // and its size is >= MAX_CIRCULAR_BUFFER_SIZE_WORDS
    // return a buffer size of 0
    BUFFER_ADDRS_TO_WORDS_ASM(r2);
    Null = r2 - MAX_CIRCULAR_BUFFER_SIZE_WORDS;
    if LE jump restore_buffer_size;
    pushm <rMAC, r0, rLink>;
    r0 = r3;
    ...
    popm <rMAC, r0, rLink>;
restore_buffer_size:
    // r2 contains the buffer size in words or 0
...
$_cbuffer_advance_write_ptr_ex:
    ...
    r5 = r0;// r5 cbuffer address
    r6 = r1; // r6 octets to advance by

    call $cbuffer.get_write_address_ex;
    // returns: r0 write, r1 offset, r2 size, r3 base

    r6 = r6 + r1;       // add the existing word offset to the octets to advance by
    r3 = r3 + r2;       // Calculate base + size, which is the first address after the buffer.
    r7 = r2;            // Save the buffer size to r7
    Null = r6 - r2;     // if octets to advance by >= buffer size
    if POS call $error; //    fail.

    r2 = r4 - 1;        // Calculated the offset mask. mask = usable_octets -1. 2 => 1 and 4 => 3.
    // We only support usable octets of 2 and 4. 2 => 1 and 4 => 3
    r2 = r6 AND r2;     // r2 = new offset

    r6 = r6 - r2;       // amount_to_advance = amount_to_advance - new offset
    Null = ADDR_PER_WORD - r4;
    if Z jump calculate_true_write_address;
    r6 = r6 LSHIFT 1;  // double the amount if usable octets is 2. Only half of the space is used.

    calculate_true_write_address:
    r1 = r0 + r6;       // write + amount_to_advance
    Null = r1 - r3;     // if write >= base + size
    if GE r1 = r1 - r7; //     write = write - size

    r0 = r5; //set r0 as cbuffer address
    // r1 is already the true write address
    // r2 is already holding the offset
...
$_cbuffer_advance_read_ptr_ex:
    ...
    call $cbuffer.get_read_address_ex;
    // returns: r0 read, r1 offset, r2 size, r3 base
    ...
    r6 = r6 - r2;       // amount_to_advance = amount_to_advance - new offset
    Null = ADDR_PER_WORD - r4;
    if Z jump calculate_true_read_address;
    r6 = r6 LSHIFT 1;  // double the amount if usable octets is 2. Only half of the space is used.

    calculate_true_read_address:
    r1 = r0 + r6;       // read + amount_to_advance
    Null = r1 - r3;     // if read >= base + size
    if GE r1 = r1 - r7; //     read = read - size

    r0 = r5; //set r0 as cbuffer address
    // r1 is already the true read address
    // r2 is already holding the offset
