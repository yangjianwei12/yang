 ...
 */
    ...
    /**
     *  0x0050 is reserved. It was FAULT_AUDIO_METADATA_DATA_ALLOCATION_FAILED
     *  but is now unused. However, FAULT_AUDIO_METADATA_DATA_ALLOCATION_FAILED
     ...
     * Alternatively, the security task failed to initialise. In this
     * case, the diatribe is 0x5ecada ('sec'urity 'ada'pter).
     */
    FAULT_AUDIO_LICENSE_CHECK_NOT_AVAILABLE = 0x0054,

    /**
     * Was FAULT_SCO_METADATA_NOT_PRESENT but it is no longer used
     * in new firmware; please don't re-use this ID because it will
     * cause confusion if it has a different meaning on a different chip.
     */
    ...
    /**
     * An attempted dynamic PM allocation failed.
     * The argument is the number of words requested.
     */
    FAULT_AUDIO_INSUFFICIENT_PROGRAM_MEMORY = 0x006C,

    /**
     * All AOV ping-pong buffers are in use
     */
    FAULT_AUDIO_NO_AOV_BUFFER_AVAILABLE = 0x006D,

    /**
     * The handler for AOV EOD interrupt is NULL or invalid.
     * It is likely that the endpoint was stopped and the handler was removed.
     */
    FAULT_AUDIO_AOV_EOD_HANDLER_INVALID = 0x006E,

    /**
     * This was possibly an attempt to disable Empty on Demand mode
     * for an invalid AoV instance.
     */
    FAULT_AUDIO_AOV_EOD_DISABLE_FAILURE = 0x006F,

    /**
     * Always on Voice configuration key
     * ACCMD_CONFIG_KEY_STREAM_CODEC_AOV_MAX_ENTRIES or
     * ACCMD_CONFIG_KEY_STREAM_DIGITAL_MIC_AOV_MAX_ENTRIES not set.
     */
    FAULT_AUDIO_AOV_BUFFER_SIZE_NOT_CONFIGURED = 0x0070,

   /**
     * The source operator endpoint supplied a cbuffer structure not in shared
     * memory and the sink operator is on P1. The diatribe is the cbuff ptr.
     */
    FAULT_AUDIO_SUPPLIED_CBUFFER_NOT_IN_SHARED_MEMORY = 0x0071,

   /**
     * Lock address passed to the HW lock API is NULL.
     */
    FAULT_AUDIO_PL_HWLOCK_INVALID = 0x0072,

    /**
     * Incorrect superwideband-SCO (SWBS) validate return value.
     */
    FAULT_AUDIO_INVALID_SWBS_VALIDATE_RESULT = 0x0073,

    /**
     * The current decoder does not support error correction using
     * weak bit mask.
     */
    FAULT_AUDIO_WBM_NOT_SUPPORTED = 0x0074,

    /**
     * Clock frequency requested is invalid either value is out of range
     * supported for a given mode or more than the maximum clock
     * supported by the chip. The ditaribe is the requested clock value.
     */
    FAULT_AUDIO_INVALID_CLOCK_REQUEST = 0x0078,
    ...
    /**
     * 0x4004 was FAULT_AUDIO_DATA_SERVICE_META_BUFFER_NOT_ENOUGH_SPACE
     * Sadly, this can be emitted by production builds
     * so the fault code should not be reused.
     */
