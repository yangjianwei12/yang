 ...
 * \ingroup opmgr
 *
 * This file contains opmgr functions and types that are only exposed to stream.
 */
...
/**
 * \brief Add endpoint to the table of endpoints to be kicked.
 *
 * This function is used at connect, to cache information about the thing
 * on the other side of the connection if it wants to be kicked. If the other
 * side of the connection doesn't wish to receive kicks then it is expected
 * that this function is not called.
 *
 * \param endpoint_id The ID of the endpoint associated with the operator
 *                    terminal which is being connected and is required to
 *                    propagate kicks.
 * \param ep_id_to_kick Endpoint id to be kicked.
 *
 * \return TRUE the table was successfully updated. FALSE the table update
 *         failed likely reason is insufficient RAM.
 */
extern bool opmgr_kick_prop_table_add(unsigned endpoint_id,
                                      unsigned ep_id_to_kick);
...
/**
 * \brief  Returns the connection link used to create a given operator.
 *
 * \param  opid External operator identifier.
 *
 * \return The connection link used during creation, or CONN_LINK_INVALID
 *         in case of failure.
 */
extern CONNECTION_LINK opmgr_con_id_from_opid(EXT_OP_ID opid);
