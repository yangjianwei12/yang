                              ...
                              EXT_OP_ID *owned_op_array)
{
    ...
    op_id = EXT_TO_INT_OPID(ext_op_id);
     ...
     */
    cur_client_id = GET_CON_ID_OWNER_CLIENT_ID(con_id);
                                           ...
                                           EXT_OP_ID *op_array)
{
    unsigned i, j;
    EXT_OP_ID *tmp_array;
    CONNECTION_PEER original_client_id;
    EXT_OP_ID ext_op_id;

    original_client_id = GET_CON_ID_OWNER_CLIENT_ID(con_id);
                                         ...
                                         unsigned *msg)
{
    ...
    client_id = GET_CON_ID_RECV_ID(reversed_con_id);
    ...
    put_message_with_routing(dst_op->task_id, OPCMD_MESSAGE, client_msg, &new_rinfo);
                                  ...
                                  unsigned *msg)
{
    ...
    client = op_client_with_op_id(op_data->id);
    if (client == NULL)
    {
        return FALSE;
    }
           ...
           if ((*pop) != NULL)
           {
               ...
               else
               {
                   ...
                   *pop = op->next;
                   ...
                   {
                       /* Notify the client that it does not have any delegated
                        * operators now. Send "DELEGATED_OPERATORS" message with
                        * just the message ID and no payload
                        */
                       op_client_notify_promotion(op_data->con_id, client);
                                       ...
                                       OP_CLIENT *client)
{
    ...
    /* Memory freed here. */
    len = OPMSG_OP_CLIENT_DELEGATED_OPERATORS_OPERATORS_WORD_OFFSET;
    len += client->num_owned_ops;
    msg = pnewn(len, unsigned);
                                         ...
                                         unsigned *resp_payload)
{
    ...
    /* Lookup the client operator */
    client_id = GET_EXT_CON_ID_RECV_ID(reversed_con_id);
    dst_op = op_client_get_client_op_data(client_id);
    if (dst_op == NULL)
    {
        /* Invalid connection id. The client might have been unpromoted.
