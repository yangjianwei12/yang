...
DM_P1_RW_ZI fault_info fault_data1;
#endif

#ifndef FAULT_LEAN_AND_MEAN
#if defined(SUPPORTS_MULTI_CORE)
static fault_info* get_fault_data_struct(void)
{
    return(PROC_PRIMARY_CONTEXT() ? &fault_data: &fault_data1);
}
#else
static fault_info* get_fault_data_struct(void)
{
...
#define LOCKDB()   interrupt_block()
#define UNLOCKDB() interrupt_unblock()
#endif
...
static void record_fault(faultid f, DIATRIBE_TYPE arg, bool freshen)
{
    ...
    volatile fault_info *this_fault_info = get_fault_data_struct();

    this_fault_info->last_id = f;
    this_fault_info->last_arg = arg;
...
static void groan(const FAULTINFO *fi)
{
    ...
    fault_test_last_groan = MAX(time_get_time(), 1);
#endif
}
 ...
 * publish_faults
 */
void publish_faults_bg(void **ignored)
{
    FAULTINFO *fi;
    uint16 i;
    bool more = FALSE;
 ...
 * publish_fresh_faults
 */
void publish_fresh_faults_bg(void **ignored)
{
    FAULTINFO *fi;
    uint16 i;
...
bool fault_test_check_publishing_delay(TIME deadline)
{
    ...
    while (time_eq(fault_test_last_groan, 0) &&
            time_lt(time_get_time(), extended_deadline));
