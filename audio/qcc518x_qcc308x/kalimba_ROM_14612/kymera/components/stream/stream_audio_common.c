...
#define EOD_REQ_IN_BUFF_HEADROOM 5
#define NINETY_PERCENT FRACTIONAL(0.9)
...
static unsigned int get_audio_buffer_size(unsigned buffer_size)
{
    ...
    else if (buffer_size <= 1840)
    {
        return 2048;
    }
    else if (buffer_size <= 3680)
    {
        return 4096;
    }
    else
...
/**
 * \brief Set first ToA timestamp for continuous buffering
 *
 * \param endpoint head of sync endpoint
 */
static void initialise_toa_for_continuous_buffering(ENDPOINT *endpoint)
{
    ...
    if ((ep_audio->initialise_toa) && (endpoint->direction == SOURCE))
    {
        SID sid = stream_external_id_from_endpoint(endpoint);
        if (audio_vsm_get_aov_mode_from_sid(sid))
        {
            unsigned aov_buff_size = audio_vsm_get_max_entries_from_sid(sid) + 1;
            ep_audio->previous_toa = endpoint->deferred.interrupt_handled_time -
                                     (INTERVAL) (((uint48)aov_buff_size*SECOND)/ep_audio->sample_rate);
...
#ifdef AOV_EOD_ENABLED
/**
 * \brief Start Empty on Demand transfer for all the endpoints in sync
 *
 * \param endpoint head of sync endpoint
 */
static void request_data_from_aov_hardware(ENDPOINT *endpoint)
{
    patch_fn_shared(stream_audio);
    unsigned amt;
    unsigned num_eps_in_sync = 0;
    unsigned data_in_aov_buff;
    unsigned output_buff_size;
    unsigned input_buff_size;
    unsigned output_buff_data;
    unsigned input_buff_data;
    unsigned aov_instances[3] = {3, 3, 3};
    ENDPOINT* synced;
    SID sid = stream_external_id_from_endpoint(endpoint);

    data_in_aov_buff = audio_vsm_get_data_in_aov_buffer(audio_vsm_get_aov_instance_from_sid(sid));
    input_buff_data = cbuffer_calc_amount_data_in_words(endpoint->state.audio.source_buf);
    output_buff_data = cbuffer_calc_amount_data_in_words(endpoint->state.audio.sink_buf);
    input_buff_size = cbuffer_get_size_in_words(endpoint->state.audio.source_buf) - 1;
    output_buff_size = cbuffer_get_size_in_words(endpoint->state.audio.sink_buf) - 1;

    /*
     * If the input and output buffers are too full
     * don't request any more data
     */
    if (input_buff_data >= frac_mult(input_buff_size, NINETY_PERCENT) &&
        output_buff_data >= frac_mult(output_buff_size, NINETY_PERCENT))
    {
        return;
    }

    if (data_in_aov_buff == 0)
    {
        return;
    }
    /*
     * Calculate the amount to transfer on demand as
     * MIN(AoV buffer size, EP input buffer space).
     * AoV buffer size = AOV_RAM_CFG_MAX_ENTRIES + 1 */
    amt = audio_vsm_get_max_entries_from_sid(sid) + 1;
    for (synced = endpoint; synced != NULL; synced = synced->state.audio.nep_in_sync)
    {
        amt = MIN(amt, cbuffer_calc_amount_space_in_words(synced->state.audio.source_buf));
        aov_instances[num_eps_in_sync] = audio_vsm_get_aov_instance_from_sid(stream_external_id_from_endpoint(synced));
        num_eps_in_sync++;
    }

    if (amt > EOD_REQ_IN_BUFF_HEADROOM)
    {
        audio_vsm_aov_eod_start_transfer(num_eps_in_sync, aov_instances, amt - EOD_REQ_IN_BUFF_HEADROOM);
    }
}
#endif /* AOV_EOD_ENABLED */
/****************************************************************************
...
/*
 * One of the following functions will be passed to the timed playback module
 * and called to adjust the output rate.
 * The implementations are different depending on the rate adjustment scheme
 */
#ifdef INSTALL_DELEGATE_RATE_ADJUST_SUPPORT
/*
 * audio_adjust_rm_rate_delegated
 *
 * Rate adjustment function (delegated case)
 */
static void audio_adjust_rm_rate_delegated(ENDPOINT *ep, int adjust_val)
{
    /* Do rate adjustment using standalone
     * rate adjust operator.
     *
     * only send the warp rate to the operator if
     * it has changed since last time.
     */
    if(adjust_val != ep->state.audio.external_last_rate)
    {
        /* send the rate adjust value to the standalone operator */
        stream_delegate_rate_adjust_set_current_rate(ep->state.audio.external_rate_adjust_opid, adjust_val);
        ep->state.audio.external_last_rate = adjust_val;
    }
}
#endif /* INSTALL_DELEGATE_RATE_ADJUST_SUPPORT */

/*
 * audio_adjust_rm_rate_warp
 *
 * Rate adjustment function (HW warp case)
 */
static void audio_adjust_rm_rate_warp(ENDPOINT *ep, int adjust_val)
{
    ep->functions->configure(ep, EP_RATEMATCH_ADJUSTMENT, adjust_val);
}

/*
 * audio_adjust_rm_rate_sra
 *
 * Rate adjustment function (endpoint SRA case)
 */
static void audio_adjust_rm_rate_sra(ENDPOINT *ep, int adjust_val)
{
    cbops_op *rm = find_cbops_op(ep->cbops->graph, cbops_rate_adjust_table);

    if (rm != NULL)
    {
        cbops_rateadjust_passthrough_mode(rm, FALSE);

        /* Set the SRA */
        cbops_sra_set_rate_adjust(rm, adjust_val);
    }
}

/**
 * \brief Default callback to notify the rest of system of unachievable latency.
 *        This will get called by Timed Playback when a packet is late
 *        for a number of consecutive kicks.
 *
 * \param endpoint pointer to endpoint
 * \param error latency error to pass in message
 */
static void audio_unachievable_latency_cback(ENDPOINT *endpoint,
                                             unsigned error)
{
    ...
    msg->message[0] = stream_external_id_from_endpoint(endpoint);
    msg->message[1] = error & 0xffff;
    msg->message[2] = error >> 16;

    (void) adaptor_send_message(endpoint->con_id, AMSGID_FROM_FRAMEWORK,
                                length,
                                (ADAPTOR_DATA) msg);
    pdelete(msg);
}
 ...
 * audio_internal_kick
 *
 * Timer event function for the internal kick
 */
static void audio_internal_kick(void *priv)
{
    patch_fn_shared(stream_audio);
    ((ENDPOINT*)(priv))->state.audio.internal_kick_timer_id = TIMER_ID_INVALID;
    stream_audio_kick(priv, STREAM_KICK_INTERNAL);
}
...
void stream_audio_kick(ENDPOINT *endpoint, ENDPOINT_KICK_DIRECTION kick_dir)
{
...
#ifdef SUPPORTS_CONTINUOUS_BUFFERING
    initialise_toa_for_continuous_buffering(endpoint);
#endif /* SUPPORTS_CONTINUOUS_BUFFERING */
    ...
    {
        ENDPOINT_ID ext_ep_id =
                stream_toggle_ep_id_between_int_and_ext(endpoint->id);

        if (endpoint->deferred.kick_is_deferred)
        {
            timing_trace_record_kicked(ext_ep_id, endpoint->deferred.interrupt_handled_time);
        }
        timing_trace_record_begin_run(ext_ep_id);
    }
#endif /* INSTALL_TIMING_TRACE */

#ifdef AOV_EOD_ENABLED
    if ((endpoint->state.audio.aov_eod_enabled) ||
        (cbuffer_calc_amount_data_in_words(ep_audio->source_buf) > 0))
#else
    if (cbuffer_calc_amount_data_in_words(ep_audio->source_buf) > 0)
#endif
    {
        /**
         ...
         *  This is applicable to source endpoints which accept backwards kicks.
         */
        if ((SOURCE == endpoint->direction) && (kick_dir == STREAM_KICK_BACKWARDS))
        {
            if (endpoint->deferred.kick_is_deferred)
            {
                endpoint->deferred.kick_dir = STREAM_KICK_INTERNAL;
                raise_bg_int(endpoint->deferred.bg_task);
            }
            else
            {
                /* Schedule a new event only if there isn't one pending */
                if (ep_audio->internal_kick_timer_id == TIMER_ID_INVALID)
                {
                    timer_schedule_event_in_atomic(0, audio_internal_kick, (void *)(endpoint), &ep_audio->internal_kick_timer_id);
                }
            }
            return;
        }
    ...
    if ((SINK == endpoint->direction) && (!ep_audio->data_flow_started))
    {
        ...
        if( (samples>0)&&(tag != NULL)&&(IS_TIME_TO_PLAY_TAG(tag) || IS_VOID_TTP_TAG(tag)) )
        {
            void (*rate_adjust_func)(ENDPOINT *ep, int32 adjust_val);
            bool need_sra;
            ...
            /* Work out how we're doing rate matching at this endpoint */
#ifdef INSTALL_DELEGATE_RATE_ADJUST_SUPPORT
            if (0 != endpoint->state.audio.external_rate_adjust_opid)
            {
                 ...
                 * rate adjust operator.
                 */
                L2_DBG_MSG1("Timed Playback: using standalone rate adjust operator: opid=0x%04x",
                    endpoint->state.audio.external_rate_adjust_opid);

                rate_adjust_func = audio_adjust_rm_rate_delegated;
                need_sra = FALSE;
            }
            else
#endif /* INSTALL_DELEGATE_RATE_ADJUST_SUPPORT */
            ...
            {
                L2_DBG_MSG("Timed Playback: using HW warp");

                endpoint->functions->configure(endpoint, EP_RATEMATCH_ENACTING, TRUE);
                rate_adjust_func = audio_adjust_rm_rate_warp;
                need_sra = FALSE;
            }
            else
            {
                L2_DBG_MSG("Timed Playback: using endpoint SRA");

                cbops_mgr_rateadjust_passthrough_mode(endpoint->cbops, FALSE);
                rate_adjust_func = audio_adjust_rm_rate_sra;
                need_sra = TRUE;
            }
            ...
            if (!timed_playback_init(ep_audio->timed_playback, endpoint->cbops,
                    ...
                    stream_if_get_system_kick_period(), audio_unachievable_latency_cback,
                    ep_audio->endpoint_delay_us,
                    endpoint, rate_adjust_func, need_sra))
            {
    ...
    if(endpoint->connected_to != NULL)
    {
        ...
        else
        {
            ...
            {
                unsigned out_metadata_space = 0;
                /*
                 * If the output buffer has metadata, we want to check for
                 * metadata space because a kick from hardware can pre-empt
                 * the data process function of the operator connected
                 * to the endpoint which, if caught between updating the
                 * transform cbuffer pointer and removing the metadata tags
                 * for the data consumed, can result in data/metadata getting out of sync.
                 * If the endpoints are synchronised, we make sure we copy
                 * the data in sync (the checks for input data and output space
                 * via cbuffer pointers are done in cbops).
                 */
                for (synced = endpoint; synced != NULL; synced = synced->state.audio.nep_in_sync)
                {
                    if (buff_has_metadata(synced->state.audio.sink_buf))
                    {
                        out_metadata_space = buff_metadata_available_space(synced->state.audio.sink_buf) / OCTETS_PER_SAMPLE;
                        max_copy = MIN(max_copy, out_metadata_space);
                    }
                }
            }
    ...
    if (endpoint->state.audio.aov_eod_enabled)
    {
        request_data_from_aov_hardware(endpoint);
    }
#endif /* AOV_EOD_ENABLED */

#ifdef INSTALL_TIMING_TRACE
    timing_trace_instr_stream_record_end_ep_run(endpoint, FALSE);
#endif /* INSTALL_TIMING_TRACE */
...
unsigned int stream_audio_get_buffer_length(uint32 sample_rate, ENDPOINT_DIRECTION dir, bool get_hw_size)
{
    ...
    if (get_hw_size)
    {
        buffer_size = get_audio_buffer_size(buffer_size);
    }
...
/**
 * stream_audio_hw_rate_adjustment_supported
 */
bool stream_audio_hw_rate_adjustment_supported(ENDPOINT *endpoint)
{
#ifdef INSTALL_AUDIO_MODULE
...
/**
 * stream_audio_create_endpoint_cbops
 */
bool stream_audio_create_endpoint_cbops(ENDPOINT *endpoint)
{
    /*
     * Create single-channel cbop infrastructure .
     ...
     */
    unsigned cbops_flags = CBOPS_DISCARD | CBOPS_DC_REMOVE | CBOPS_UNDERRUN;
    bool need_sra = !stream_audio_hw_rate_adjustment_supported(endpoint);

    if (need_sra)
    {
        cbops_flags |= CBOPS_RATEADJUST;
    }

    L3_DBG_MSG2("stream_audio_create_endpoint_cbops EP = %04X sra = %d",
        stream_external_id_from_endpoint(endpoint), need_sra);

    if((endpoint->cbops = cbops_mgr_create(endpoint->direction, cbops_flags)) == NULL)
    {
        return FALSE;
    }
    else
 ...
 */
STREAM_ANC_INSTANCE stream_audio_anc_get_primary_instance_id(ENDPOINT *ep)
{
...
/*
 * stream_audio_get_secoandary_anc_secondary_instance_id
 */
STREAM_ANC_INSTANCE stream_audio_anc_get_secondary_instance_id(ENDPOINT *ep)
{
    if (ep->stream_endpoint_type == endpoint_audio)
    {
        return (ep->state.audio.anc.secondary_instance_id);
    }
    else
    {
        return STREAM_ANC_INSTANCE_NONE_ID;
 ...
 */
void stream_audio_anc_set_primary_instance_id(ENDPOINT *ep, STREAM_ANC_INSTANCE instance_id)
{
    if (ep->stream_endpoint_type == endpoint_audio)
    {
...
/*
 * stream_audio_set_anc_secondary_instance_id
 */
void stream_audio_anc_set_secondary_instance_id(ENDPOINT *ep, STREAM_ANC_INSTANCE instance_id)
{
    if (ep->stream_endpoint_type == endpoint_audio)
    {
        ep->state.audio.anc.secondary_instance_id = instance_id;
    }
}
 ...
 */
STREAM_ANC_PATH stream_audio_anc_get_primary_input_path_id(ENDPOINT *ep)
{
    if (ep->stream_endpoint_type == endpoint_audio)
    {
...
/*
 * stream_audio_get_anc_secondary_input_path_id
 */
STREAM_ANC_PATH stream_audio_anc_get_secondary_input_path_id(ENDPOINT *ep)
{
    if (ep->stream_endpoint_type == endpoint_audio)
    {
        return (ep->state.audio.anc.secondary_input_path_id);
    }
    else
    {
        return STREAM_ANC_PATH_NONE_ID;
 ...
 */
void stream_audio_anc_set_primary_input_path_id(ENDPOINT *ep, STREAM_ANC_PATH path_id)
{
    if (ep->stream_endpoint_type == endpoint_audio)
    {
...
/*
 * stream_audio_set_anc_secondary_input_path_id
 */
void stream_audio_anc_set_secondary_input_path_id(ENDPOINT *ep, STREAM_ANC_PATH path_id)
{
    if (ep->stream_endpoint_type == endpoint_audio)
    {
        ep->state.audio.anc.secondary_input_path_id = path_id;
    }
}
