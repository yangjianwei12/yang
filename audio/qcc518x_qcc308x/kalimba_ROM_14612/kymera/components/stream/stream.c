...
ENDPOINT *stream_endpoint_from_extern_id(unsigned int id)
{
    ...
    return endpoint_from_id(stream_toggle_ep_id_between_int_and_ext(id));
}
...
unsigned stream_external_id_from_endpoint(ENDPOINT *endpoint)
{
    ...
    if (endpoint != NULL)
    {
        ...
        id = stream_toggle_ep_id_between_int_and_ext(endpoint->id);
    }
...
/****************************************************************************
 *
 * stream_toggle_ep_id_between_int_and_ext
 *
 */
unsigned stream_toggle_ep_id_between_int_and_ext(unsigned id)
{
    /* Explicit common subexpression and shifting results in fewer/smaller instructions */
    /* Position of the lowest non-zero bit of STREAM_EP_TYPE_MASK */
#define STREAM_EP_TYPE_POSN (13)
    /* Guard against getting the shift wrong */
    STATIC_ASSERT(((STREAM_EP_TYPE_MASK >> STREAM_EP_TYPE_POSN) << STREAM_EP_TYPE_POSN) == STREAM_EP_TYPE_MASK,
                  shift_preserves_ep_type);

    const unsigned ep_type = (id & STREAM_EP_TYPE_MASK) >> STREAM_EP_TYPE_POSN;
    if (ep_type == (STREAM_EP_EXT_SOURCE >> STREAM_EP_TYPE_POSN))
    {
        id ^= SOURCE_EP_COOKIE;
    }
    else if (ep_type == (STREAM_EP_EXT_SINK >> STREAM_EP_TYPE_POSN))
    {
        id ^= SINK_EP_COOKIE;
    }
    return id;
...
bool stream_set_deferred_kick(ENDPOINT* ep, bool deferred)
{
    ...
    if (ep->deferred.kick_is_deferred != deferred)
    {
        ...
        {
            id = create_uncoupled_bgint(HIGHEST_PRIORITY,
                                        ep,
...
{
    if (ep->deferred.bg_task != NO_TASK)
    {
