...
static void a2dp_self_kick(void *priv)
{
    ...
    ENDPOINT_ID ext_ep_id = 0;

    if (ep->deferred.kick_is_deferred)
    {
        ext_ep_id = stream_toggle_ep_id_between_int_and_ext(ep->id);
        timing_trace_record_begin_run(ext_ep_id);
        timing_trace_record_kicked(ext_ep_id, get_last_fire_time());
    }
#endif /* INSTALL_TIMING_TRACE */

    ep->state.a2dp.self_kick_timer_id = TIMER_ID_INVALID;
...
{
    /* Note that kicks would be required if we ever have an A2DP sink  */
 ...
 */
static void a2dp_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir)
{
    int copied = 0;
    ...
    {
#ifdef INSTALL_TIMING_TRACE
        ext_ep_id = stream_external_id_from_endpoint(ep);
        timing_trace_record_begin_run(ext_ep_id);
#endif /* INSTALL_TIMING_TRACE */
        ...
        if (buff_has_metadata(a2dp->source_buf))
        {
            ...
            audio_data_service_queue_meta(a2dp->service_priv, NULL);
        }
    }
    else
    {
