...
void iso_kick(ENDPOINT *endpoint, ENDPOINT_KICK_DIRECTION kick_dir)
{
    ...
    /* At connect the endpoint asked not to be kicked.
     * For both source and sink endpoints the kick object kicks this endpoint.
     * In this manner this function is called for source and sink endpoints.
     */
...
bool iso_get_config(ENDPOINT *endpoint, unsigned int key, ENDPOINT_GET_CONFIG_RESULT* result)
{
    ...
    switch (key)
    {
        case EP_DATA_FORMAT:
        {
             ...
             * data format returned is not relevant to the transform buffer,
             * but the MMU buffer. Overwrite the ISO ep data format to set the
