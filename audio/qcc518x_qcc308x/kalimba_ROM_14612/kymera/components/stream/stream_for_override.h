...
****************************************************************************/
/**
 * \file stream_for_override.h
 * \ingroup stream
 *
 * Stream header file for function allowing to override endpoints.
 */
...
typedef enum OVERRIDE_CONFIG_KEYS
{
    ...
    OVERRIDE_RATE_ADJUST_OP = 0x010019,
    OVERRIDE_HW_WARP_APPLY_MODE = 0x01001A,
    OVERRIDE_CURRENT_HW_WARP = 0x01001B
...
extern bool stream_connected_to_endpoints_have_same_clock_source(ENDPOINT *ep1,
                                                                 ENDPOINT *ep2);
/**
 * \brief Function to tests whether there is a sidetone path enabled between
 *        two overridden endpoints.
 *
 * \param mic_ep  The handle for the operator endpoint which overrides a real
 *                mic endpoint.
 * \param spkr_ep The second handle for the operator endpoint which overrides
 *                a real speaker endpoint.
 *
 * \return TRUE if there is sidetone path between two overridden endpoints.
 */
