 ...
 */
static void operator_list_resp_handler(tRoutingInfo *rinfo,
                                       OP_STD_RSP *msg,
                                       OPCMD_ID req_msg_id,
                                       void (*specific_handler)(INT_OP_ID op_id),
                                       preproc_function preproc_func)
{
    MULTI_OP_REQ_DATA *data;
    ...
    data = opmgr_retrieve_in_progress_task(rinfo->dest_id, rinfo->src_id);
    if (data == NULL)
    {
    ...
    {
        operator_resp_handler_send_response_and_free_data(data,
                                                          rinfo,
                                                          msg,
                                                          STATUS_CMD_FAILED);
        return;
    ...
    if (data->num_ops == 1)
    {
        ext_op_id = data->id_info.id;
    }
    else
    {
        ext_op_id = data->id_info.op_list[data->cur_index];
    }
    int_op_id = EXT_TO_INT_OPID(ext_op_id);
    specific_handler(int_op_id);
    ...
    if (data->cur_index < data->num_ops)
    {
        ...
        int_op_id = EXT_TO_INT_OPID(data->id_info.op_list[data->cur_index]);
        ...
        new_rinfo.dest_id = int_op_id;
        if (!opmgr_store_in_progress_task(rinfo->dest_id,
                                          new_rinfo.dest_id,
                                          (void *) data))
        {
            operator_resp_handler_send_response_and_free_data(data,
 ...
 */
static void start_resp_handler(INT_OP_ID op_id)
{
 ...
 * \param  op_id the operator id to take action for/on.
 */
static void stop_connected_endpoints(INT_OP_ID op_id)
{
 ...
 * \param  op_id the operator id to take action for/on.
 */
static void stop_resp_handler(INT_OP_ID op_id)
{
    OPERATOR_DATA *cur_op = get_op_data_from_id(op_id);
 ...
 * \param  op_id the operator id to take action for/on.
 */
static void reset_resp_handler(INT_OP_ID op_id)
{
    OPERATOR_DATA *cur_op = get_op_data_from_id(op_id);
 ...
 *
 */
static void destroy_resp_handler(INT_OP_ID op_id)
{
...
static void create_resp_handler(tRoutingInfo *rinfo, OP_STD_RSP *msg)
{
    ...
    int_op_id = GET_OPID_FROM_SRC_ID(rinfo->src_id);
    rinfo->src_id = GET_SRC_ID_FROM_PACKED_SRC_ID((CONNECTION_LINK)rinfo->src_id);
    con_id = PACK_CON_ID((CONNECTION_PEER) rinfo->src_id,
                         (CONNECTION_PEER) rinfo->dest_id);

    /* Callback function pointer had better be valid; either path below will call it. */
    callback = (OP_CREATE_CBACK) opmgr_retrieve_in_progress_task(rinfo->dest_id, int_op_id);
    ...
    else
    {
        ...
        op_data = get_op_data_from_id(int_op_id);
        if (op_data == NULL)
        {
            panic_diatribe(PANIC_AUDIO_OPERATOR_NOT_FOUND, int_op_id);
        }
        delete_task(op_data->task_id);

        /* Remove the given operator's data structure from the local operator list. */
        remove_op_data_from_list(int_op_id, &oplist_head);
    }

    /* Notify the client of the result of the operation. */
    ext_op_id = INT_TO_EXT_OPID(int_op_id);
    callback(con_id, (STATUS_KYMERA) msg->status, ext_op_id);    
}
...
void opmgr_task_handler(void **msg_data)
{
    ...
    while (get_message_with_routing(&cmd_id, &msg_body, &rinfo))
    {
        switch (cmd_id)
        {
            case OPCMD_CREATE:
