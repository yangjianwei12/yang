 ...
 * Cbuffer structure
 *
 * The cbuffer structure can encapsulate a simple SW buffer or a local MMU buffer,
 * NB. remote MMU buffers no longer supported through cbuffer API. Neither Bluecore ports.
 *
...
/*@{*/
/*
 * Buffer flag definitions to pass to cbuffer_create, cbuffer_create_mmu_buffer and variations.
...
/* bit 1 : Remote buffers No longer supported in cbuffer
                   0 - if bit 0 is set, cbuffer struct wraps a local MMU buffer
...
#define BUF_DESC_WR_PTR_TYPE_MMU(x)         ((x) & BUF_DESC_WR_PTR_TYPE_MMU_MASK)
   ...
   /* bit 4: Remote buffers No longer supported in cbuffer
                   0 - AudioCPU can not modify the remote MMU handle pointed to by read_ptr
                   1 - AudioCPU can modify the remote MMU handle pointed to by read_ptr
#define BUF_DESC_REMOTE_RDH_MOD_SHIFT       4
    */
    
   /* bit 5: Remote buffers No longer supported in cbuffer
                   0 - AudioCPU can not modify the remote MMU handle pointed to by write_ptr
 ...
 * handle it is mirroring (read or write) */
   ...
   /* bit 12: Remote buffers No longer supported in cbuffer
                   0 - Remote read handle has no byte-swap
...
#define BUF_DESC_REMOTE_RDH_BSWAP_SHIFT     12
   ...
   /* bit 13: Remote buffers No longer supported in cbuffer
                   0 - Remote write handle has no byte-swap
 ...
 *
 * The user is responsible for allocating the buffer data space with malloc
 * preference for cbuffer and the underlying buffer resources.
 *
 ...
 * \param cbuf_preference  malloc preference for cbuffer
 *
 * \return If successful pointer to the created cbuffer structure else NULL
 *
 */
extern tCbuffer *cbuffer_create_with_cbuf_preference(void *cbuffer_data_ptr,
                                                     unsigned int buffer_size,
                                                     unsigned int descriptor,
                                                     unsigned cbuf_preference);
 ...
 * This function mallocs a buffer buffer_size words big for the cbuffer data storage,
 * creates a tCbuffer object and populates its fields.
 * This function allocates memory from EXTRA ram if it exists on the device.
 *
 * \param buffer_size is the requested size of the buffer in words.
 * \param descriptor is the bit field cbuffer descriptor.
 * \return pointer to tCbuffer object or NULL if unsuccessful. Could panic.
 */
extern tCbuffer *cbuffer_create_with_malloc_extra(unsigned int buffer_size, unsigned int descriptor);
...
/*
 * Returns the usable data read offset of the given buffer.
 * When the data and the metadata is aligned this is equal to
 * buff_metadata_get_read_offset(cbuffer).
 * This function only supports 32-bit packed and 16-bit unpacked data.
 *
 * \param cbuffer structure to extract information from.
 * \return The read offset in octets.
 */
extern unsigned int cbuffer_get_usable_data_read_offset(tCbuffer *cbuffer);
...
/**
 * Checks if the required amount of octets is present in the input buffer.
 *
 * \param cbuffer           Structure to extract information from.
 * \param data_size_octets  Number of octets which needs reading from the input.
 *
 * \returns TRUE if there is enough data, FALSE otherwise.
 */
extern bool cbuffer_enough_data_to_run_ex(tCbuffer *cbuffer,
                                          unsigned data_size_octets);

/**
 * Return amount of input data in words, allowing for available metadata.
 * This assumes whole-word data (typically audio samples) and should not
 * be used for buffers containing 16-bit unpacked data.
 *
 * \param cbuffer           Structure to extract information from.
 *
 * \returns samples available for processing.
 */
extern unsigned cbuffer_calc_avail_data_words(tCbuffer *buffer);

/**
 * Return amount of input data in octets, allowing for available metadata.
 *
 * \param cbuffer           Structure to extract information from.
 *
 * \returns octets available for processing.
 */
extern unsigned cbuffer_calc_avail_data_ex(tCbuffer *buffer);

/**
 * Return amount of output space in words, allowing for metadata.
 * This assumes whole-word data (typically audio samples) and should not
 * be used for buffers containing 16-bit unpacked data.
 *
 * \param cbuffer           Structure to extract information from.
 *
 * \returns samples that can be safely written.
 */
extern unsigned cbuffer_calc_avail_space_words(tCbuffer *buffer);

/**
 * Return amount of output space in octets, allowing for metadata.
 *
 * \param cbuffer           Structure to extract information from.
 *
 * \returns octets that can be safely written.
 */
extern unsigned cbuffer_calc_avail_space_ex(tCbuffer *buffer);
