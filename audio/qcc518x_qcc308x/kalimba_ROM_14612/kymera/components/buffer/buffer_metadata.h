...
typedef enum
{
    ...
    /* Status and other info for SCO/ISO packets/SDUs */
    META_PRIV_KEY_SCO_DATA_STATUS,

    /* Start of range for user-defined private tag types */
    META_PRIV_KEY_USER_DATA = 16
} METADATA_PRIV_KEY;
...
/* Tag list filter callback */
typedef struct metadata_tag_list_filter
{
    void *data;
    void (*filter)(void* data, metadata_tag* tag_list, unsigned* before, unsigned* after);
}
metadata_tag_list_filter;

/* Private data scan callback */
typedef void (*metadata_scan_private_data_fn)(void* cb_data, metadata_tag *tag, METADATA_PRIV_KEY item_key, unsigned item_length, void *item_data, bool *done);
...
struct metadata_list_struct
{
    ...
    unsigned buffer_size;     /**< cached size of the buffers using this metadata, in octets */
...
/**
 * \brief Initialise the buffer metadata system.
 *
...
/**
 * \brief Create a new (empty) metadata tag
 *
...
/**
 * \brief Check whether there are lots of metadata tags allocated
 *
...
/**
 * \brief Release a metadata tag that's no longer in use back to the store.
 *
 ...
 *
 * \note Most callers should set the process_eof parameter to TRUE, unless the
 *       tag is getting replaced by a new one carrying the same EOF information.
 */
 ...
 *       allocated and populated separately. If it is non-NULL it will be
 *       copied to the newly-added entry.
 */
extern void* buff_metadata_add_private_data(metadata_tag *tag,
                                            METADATA_PRIV_KEY key,
                                            unsigned length,
                                            void *data);
 ...
 */
extern bool buff_metadata_find_private_data(metadata_tag *tag,
                                            METADATA_PRIV_KEY key,
                                            unsigned *out_length,
                                            void **out_data);

/**
 * \brief Apply processing to all private data items in a tag
 *
 * \param tag The tag to look in
 * \param process_fn Callback for each private data item
 * \param cb_data Data for the processing function
 */
extern void buff_metadata_scan_private_data(metadata_tag *tag, metadata_scan_private_data_fn process_fn, void *cb_data);
 ...
 */
extern bool buff_metadata_add_eof_callback(metadata_tag *tag,
 ...
 *
 * \param tag       The tag to look in
 * \param cb_struct pointer to callback structure containing the
 *                  function pointer and data parameter
 *
 * \return TRUE if successful
 */
extern bool buff_metadata_get_eof_callback(metadata_tag *tag,
                                           metadata_eof_callback *cb_struct);
 ...
 * \param tag                 Pointer to (populated) metadata tag to attach
 *                            (index will be recalculated)
 * \param octets_pre_written  Number of octets that have been written to
 *                            buffer since last append
 * \param octets_post_written Number of octets yet to be written to buffer past
 *                            the current index
 *
...
extern bool buff_metadata_append_dbg(tCbuffer *buff,
                                     metadata_tag *tag,
                                     unsigned octets_pre_written,
                                     unsigned octets_post_written,
                                     unsigned caller_addr);
#else /* METADATA_DEBUG_TRANSPORT */
extern bool buff_metadata_append(tCbuffer *buff,
                                 metadata_tag *tag,
                                 unsigned octets_pre_written,
                                 unsigned octets_post_written);
#endif /* METADATA_DEBUG_TRANSPORT */
...
/**
 * \brief Gets the buffer size (as cached by metadata submodule).
 *
 * \param buff Pointer to cbuffer structure
 *
 * \return buffer size in octets
 *
 * \note. The returned buffer size is the value cached in the metadata
 *        structure might be different from the value stored in parent
 *        cbuffer structure (that one is in memory allocation units anyway).
...
/**
 * \brief Sets the buffer size (as cached by metadata submodule).
 *
 * \param buff Pointer to cbuffer structure
 ...
 */
extern void buff_metadata_set_buffer_size(tCbuffer *buff,
                                          unsigned usable_octets);

/**
 * \brief Look at first metadata tag attached to buffer without removing the tag from
 * the buffer.
 *
...
/**
 * \brief Look at first metadata tag attached to buffer
 *
 ...
 */
extern unsigned buff_metadata_peek_frame_bytes(tCbuffer *buff,
                                               unsigned *octets_b4idx);

/**
 * \brief Returns the available octets in the metadata.
 *
 * \param buff Pointer to buffer
 *
...
/**
 * \brief Returns the available space according to the metadata.
 *
 * \param buff Pointer to buffer
 *
...
/**
 * \brief Remove the first metadata tag from a buffer
 *
 * \param buff                 Pointer to buffer
 * \param octets_consumed      Number of octets consumed from the data buffer
 * \param[out] octets_b4idx    Pointer to number of octets not yet processed
 *                             before the index of the removed tag.
 * \param[out] octets_afteridx Pointer to number of octets already processed
 *                             after the index of the removed tag.
 *
 * \return Pointer to first removed tag, or
 *         NULL if none present or none removed
 */
#ifdef METADATA_DEBUG_TRANSPORT
#define buff_metadata_remove(B,C,P,A) buff_metadata_remove_dbg((B),(C),(P),(A),0)
extern metadata_tag *buff_metadata_remove_dbg(tCbuffer *buff,
                                              unsigned octets_consumed,
                                              unsigned *octets_b4idx,
                                              unsigned *octets_afteridx,
                                              unsigned caller_addr);
#else /* METADATA_DEBUG_TRANSPORT */
extern metadata_tag *buff_metadata_remove(tCbuffer *buff,
                                          unsigned octets_consumed,
                                          unsigned *octets_b4idx,
                                          unsigned *octets_afteridx);
#endif /* METADATA_DEBUG_TRANSPORT */
...
/**
 * \brief Returns the write offset of the buffer metadata.
 *
 * \param buff Pointer to buffer
 *
...
/**
 * \brief Returns the head tag of the buffer metadata.
 *
 * \param buff Pointer to buffer
 *
...
/**
 * \brief Sets up metadata structure for a new connection.
 *
 * Metadata is rationalised between existing connections that can share
 * multiple channels between the same entity share the same metadata structure,
 * which is reference counted. If the two sides are attempting to share
 * different metadata instances with the same connection then metadata will be
 * ignored on this connection. Where the source can share but the output
 * differs then a new linked metadata instances is created.
 *
 * This function is only called when the connection is made between an operator
 * and a SHADOW endpoint on P0 and the assumption is that the metadata list
 * is not NULL.
 * The metadata list is allocated on P1 when the shadow endpoint is connected to
 * the operator. This order of connections is fixed.
 *
 * \param buff          The buffer for the connection to setup metadata for.
 * \param src_metadata  The buffer provided by the source of the connection
 *                      that has the same metadata.
 * \param sink_metadata The buffer provided by the sink of the connection
 *                      that has the same metadata.
 *
 * \return TRUE if successful
 */
extern bool buff_metadata_connect_existing(tCbuffer *buff,
                                           tCbuffer *src_metadata,
                                           tCbuffer *sink_metadata);
 ...
 * \return TRUE if successful
 */
extern bool buff_metadata_connect(tCbuffer *buff,
                                  tCbuffer *src_metadata,
                                  tCbuffer *sink_metadata);
...
/**
 * \brief Transport metadata for operators that have an opinion about
 *        how much data they want to process.
 *
 *  Move all meta tags from src->metadata to dst->metadata list.
 *  All entries that reference data in src until the position of the read
 *  pointer (not including the read pointer) will have to be consumed.
 *
 * \param src         source data cbuffer
 * \param dst         destination data cbuffer
 * \param tran_octets number of octets to transport
 */
extern void metadata_strict_transport(tCbuffer *src,
                                      tCbuffer *dst,
                                      unsigned tran_octets);

/**
 * Filtering version of metadata_strict_transport.
 ...
 * \param src source data cbuffer
 * \param dst destination data cbuffer
 * \param trans_octets number of octets to transport
 * \param filter optional filter object
 */
extern void metadata_strict_transport_filter( tCbuffer *src, tCbuffer *dst, unsigned trans_octets, metadata_tag_list_filter* filter);


/**
 * \brief Helper function to identify if a buffer is configured
 *        with metadata support.
 *
 * \param buff Pointer to the buffer to test for metadata support.
 *             Pointer must be a valid buffer.
 *
 ...
 *
 * \note This function only works if the underlying buffer is
 *       a purely software buffer.
 */
 ...
 *
 * \param buff The buffer to align the metadata of
 * \note - This function only works if the underlying buffer is a
 *         purely software buffer.
 *       - Only the owner of write side of the buffer shall use this function.
 *       - Don't use this function if buffer has added delay to metadata,
 *         or fix the delay after using this function.
 */
 ...
 *
 * \return The head of a linked-list of metadata tags. The tail is also
 *         returned for convenience through the tail argument.
 */
extern metadata_tag* buff_metadata_pop_tags_from_KIP(tCbuffer* shared_buffer,
                                                     metadata_tag** tail);
 ...
 *
 * \param  shared_buffer The IPC channel buffer for moving metadata tags.
 * \param  first_tag     Pointer to the first tag being pushed to the
 *                       shared buffer.
 *
 * \return Returns the last tag the function attempted to push to the IPC
 *         buffer so that the caller can update the shadow endpoint linked-list
 *         accordingly.
 */
extern metadata_tag* buff_metadata_push_tags_to_KIP(tCbuffer* shared_buffer,
                                                    metadata_tag* first_tag);
#endif /* defined(SUPPORTS_MULTI_CORE) */

#ifdef DESKTOP_TEST_BUILD
/**
 * \brief Helper function for unit tests that displays the contents of
 *        a metadata_list structure.
 *
...
/**
 * \brief Helper function for unit tests that displays the contents of
 *        a metadata_tag private / user data.
 *
