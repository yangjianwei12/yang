...
#ifdef __KCC__
/* with saturating addition, 16 octets in minim */
inline asm int rate_second_interval_to_sec_frac_inl(RATE_SECOND_INTERVAL interval)
{
    @[
      .scratch p
      .restrict p:bank1
      .restrict interval:bank1
      .restrict :bank1
    ]
    @{p} = 0.048576;
    @{p} = @{p} * @{interval} (frac);
...
#else /* __KCC__ */
/* Without saturating addition, 14 octets in Kalimba minim */
static inline int rate_second_interval_to_sec_frac_inl(RATE_SECOND_INTERVAL interval)
{
    return interval +
            frac_mult(interval,
                      FRACTIONAL((1.0*(1u<<(DAWTH-1)))/RATE_SECOND_INTERVAL_SECOND-1.0));
}
#endif /* __KCC__ */
 ...
 */
RATE_SECOND_INTERVAL rate_samples_to_second_interval(
        ...
        RATE_SAMPLE_PERIOD sample_period)
{
    ...
    enum
    {
        RSH = (RATE_SAMPLE_PERIOD_EXTRA_RESOLUTION - RATE_SECOND_INTERVAL_EXTRA_RESOLUTION)
    };
    COMPILE_TIME_ASSERT( RSH > 0, RATE_SAMPLE_PERIOD_has_more_fractional_bits_than_RATE_SECOND_INTERVAL);

    if (num_samples * (sample_period >> RATE_SAMPLE_PERIOD_EXTRA_RESOLUTION) <= 50000)
    {
        RATE_SAMPLE_PERIOD t = sample_period * num_samples;

        return (RATE_SECOND_INTERVAL) ((t + (1 << (RSH-1))) >> RSH);
    }
    else
    {
        RATE_TIME t = ((RATE_TIME)sample_period) * num_samples;
 ...
 * \note TODO unit test
 */
int rate_second_interval_to_sec_frac(RATE_SECOND_INTERVAL interval)
{
    return rate_second_interval_to_sec_frac_inl(interval);
}
 ...
 * \note TODO unit test
 */
unsigned rate_second_interval_to_samples(
        RATE_SECOND_INTERVAL interval,
        unsigned sample_rate)
{
    return rate_time_to_samples(
            rate_second_interval_to_sec_frac_inl(pl_max(interval, 0)),
            sample_rate);
}
 ...
 * \note TODO unit test
 */
unsigned rate_second_interval_to_samples_trunc(
        RATE_SECOND_INTERVAL interval,
        unsigned sample_rate)
{
    return rate_time_to_samples_trunc(
            rate_second_interval_to_sec_frac_inl(pl_max(interval, 0)),
            sample_rate);
}
