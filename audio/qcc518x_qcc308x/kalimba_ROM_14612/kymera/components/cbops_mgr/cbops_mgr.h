 ...
 * \param cbops_mgr_obj pointer to cbops_mgr object previously allocated (create)
 */
extern void cbops_mgr_destroy(cbops_mgr* cbops_mgr_obj);
 ...
 * \param cbops_mgr_obj pointer to cbops_mgr object previously allocated (create)
 */
extern void cbops_mgr_disconnect(cbops_mgr* cbops_mgr_obj);
 ...
 */
extern void cbops_mgr_buffer_reinit(cbops_mgr* cbops_mgr_obj);
...
{
    return cbops_get_amount(cbops_mgr_obj->graph,
	                        cbops_mgr_obj->graph->num_io >> 1);
}


/**
 * \brief Set a channel to "not used" in the cbops chain (for all operators
 *        and framework).
 *
 * This sets the cbuffer pointers to NULL and clear the channel index entries
 * for the given channel.
 *
 * \param cbops_mgr_obj pointer to cbops_mgr object
 * \param channel       Channel number to set to unused (0...N-1, N is the
 *                      number of (equal) input and output channels. This can
 *                      be further generalised if ever needed by
 *                      cbops_mgr-owned chains.
 *
 * \return TRUE/FALSE indicates if the operation was successful.
 */
extern bool cbops_mgr_set_unused_channel(cbops_mgr *cbops_mgr_obj,
                                         unsigned channel);
 ...
 * \param out_buffs     sink buffers.
 *
 * \return indicates if the operation was successful.
 */
extern bool cbops_mgr_get_buffer_info(cbops_mgr *cbops_mgr_obj,
                                      unsigned nr_chans,
									  tCbuffer **in_buffs,
									  tCbuffer **out_buffs);
 ...
 * \return indicates if the operation was successful.
 */
extern bool cbops_mgr_alloc_buffer_info(cbops_mgr *cbops_mgr_obj,
                                        unsigned *nr_chans,
										tCbuffer ***in_buffs,
										tCbuffer ***outbuffs);
 ...
 */
extern void cbops_mgr_free_buffer_info(tCbuffer **in_buffs,
                                       tCbuffer **out_buffs);
 ...
 *
 * \param cbops_mgr_obj pointer to the cbops manager.
 * \param channel       the number of the channel (as input-to-output channel,
 *                      considering equal number of inputs and outputs)
 * \param in_buff       source buffer
 * \param out_buff      sink buffer
 * \return indicates if the operation was successful.
 */
extern bool cbops_mgr_get_channel_buffers(cbops_mgr *cbops_mgr_obj,
                                          unsigned channel,
										  tCbuffer **in_buff,
										  tCbuffer **out_buff);

/**
 * \brief Connect source/sink buffer information for a certain channel.
 *
 * \param cbops_mgr_obj pointer to the cbops manager.
 * \param channel       the number of the channel (as input-to-output channel,
 *                      considering equal number of inputs and outputs).
 * \param source_buf    source buffer.
 * \param sink_buf      sink buffer.
 *
 * \return indicates if the operation was successful.
 */
extern bool cbops_mgr_connect_channel(cbops_mgr *cbops_mgr_obj,
                                      unsigned channel,
                                      tCbuffer *source_buf,
									  tCbuffer *sink_buf);

/**
 * \brief Default channel indexes for a given channel.
 *
 * \param cbops_mgr_obj pointer to the cbops manager.
 * \param channel       the number of the channel (as input-to-output channel,
 *                      considering equal number of inputs and outputs).
 ...
 * \return indicates if the operation was successful.
 */
extern bool cbops_mgr_default_channel_idx(cbops_mgr *cbops_mgr_obj,
                                          unsigned channel);

/**
 * \brief Retrieve some alg parameters from the cbops chain.
 *
 * \param cbops_mgr_obj pointer to the cbops manager.
 * \param vals          pointer to a CBOP_VALS struct of parameters that will
 *                      get filled based on cbop chain's params
 *
 * \return indicates if the operation was successful.
 */
extern bool cbops_mgr_get_op_vals(cbops_mgr *cbops_mgr_obj,
                                  CBOP_VALS *vals);

/**
 * \brief Utility function for configuring the passthrough mode of the
 *        rateadjust and shift cbop in a cbops_mgr graph.
 *
 * \param cbops_mgr_obj The cbops_mgr containing the graph to configure
 *                      the mode of
 * \param enable        Boolean representing the state to set the passthrough
 *                      mode to.
 *
 * \return Indicates if the cbop operator was found and configured
 */
extern bool cbops_mgr_rateadjust_passthrough_mode(cbops_mgr *cbops_mgr_obj,
                                                  bool enable);
 ...
 */
extern cbops_op* find_cbops_op(cbops_graph *head,
                               unsigned func_table[]);
