...
#define current_time() time_get_time()
#else
#define current_time() hal_get_reg_timer_time()
...
static tTimerId get_new_timer_id(unsigned int queue_flag, TIME event_time)
{
     ...
     *
     * NOTE: We do this outside of the time_le macro to ensure current_time()
     * is only called once.
     */
    TIME curr_long_threshold = time_add(current_time(),LONG_TIMER_THRESHOLD);
 ...
 */
static bool timers_get_next_event_time_int(tEventsQueue *event_queue, TIME *next_time)
{
...
tTimerId asm_timer_schedule_event_in(
        ...
        void *data_pointer)
{
    return create_add_strict_event(time_add(current_time(), time_in),
                                   TimerEventFunction,
                                   data_pointer);
}
#endif
...
static inline void timers_service_expired_strict_events(void)
{
    ...
    while (NULL != (event = get_next_expired_strict_event()))
    {
        ...
        tTimerId timer_id = event->timer_id;
        ...
        CHECK_ARITHMETIC_MODE_SANITY("after strict timer event", timer_id);
...
void timers_service_expired_casual_events(void)
{
    ...
    while (NULL != (event = get_next_expired_casual_event()))
    {
        ...
        interrupt_unblock();
        event_function(event_data_pointer);
        CHECK_ARITHMETIC_MODE_SANITY("after casual timer event", timer_id);
        interrupt_block();
