...
.MODULE $M.error_sw_exception;
   ...
   // This register points exactly at the instruction that caused the
   // exception.
   r0 = M[$EXCEPTION_PC];
...
// *****************************************************************************
// MODULE:
//    $_check_arithmetic_mode_sanity
//
// DESCRIPTION:
//    Check that the ARITHMETIC_MODE register is sane for general-purpose
//    code (i.e., nonzero).
//
// INPUTS:
//    - r0 contains a string pointer and r1 contains an integer, which will
//      both be output in a log message if the check fails, to give some
//      context
//
// OUTPUTS:
//    - none
...
//
// NOTES:
//    This function has to function correctly if ARITHMETIC_MODE is
//    nonzero on entry.
//
// *****************************************************************************
.MODULE $M.error.check_arithmetic_mode_sanity;
   .CODESEGMENT ERROR_PM;

   $_check_arithmetic_mode_sanity:
   Null = M[$ARITHMETIC_MODE];
   if Z rts;
   push rLink;
   r2 = M[$ARITHMETIC_MODE];
   // make it safe to execute general-purpose code:
   M[$ARITHMETIC_MODE] = 0;
   // first two arguments come straight from caller
   call $_pl_arithmetic_mode_error;
   // never reached:
   pop rLink;
   rts;
