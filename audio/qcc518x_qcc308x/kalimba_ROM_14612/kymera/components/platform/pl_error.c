...
void pl_sw_exception_handler(unsigned code_address)
{
     ...
     * the exception occurred. On all current platforms, it contains the
     * exact PC value of the instruction that caused the exception
     * (see comments on definition of PANIC_AUDIO_SW_EXCEPTION for some
     * history).
     *
     ...
     * ambiguous. In audio subsystem logs etc it will retain its full width.
     */
...
#ifdef __KCC__
/* Handler for Kalimba's ARITHMETIC_MODE being nonzero */
void pl_arithmetic_mode_error(const char *reason, unsigned int context,
                              unsigned int mode)
{
    L0_DBG_MSG3("ARITHMETIC_MODE register is erroneously 0x%X (%s 0x%X)",
                mode, reason, context);
    panic_diatribe(PANIC_ARITHMETIC_MODE_NONZERO, mode);
}
#endif /* __KCC__ */
