...
/**
 * Does the referenced table start with an entry indicating exported
 * to filesystem?
 * 
...
#else
#define DynLoaderHasExternalRedirect(ptr) (FALSE)
#endif
 ...
 * \param pFile Pointer to variable to hold file ID.
 *              Only updated the function succeed.
 *
 * \returns TRUE if the descriptor references a file.
 */
#ifdef INSTALL_CAPABILITY_CONSTANT_EXPORT
bool DynLoaderGetExternalRedirectFile(void *ptr, uint16 *pFile);
#else
#define DynLoaderGetExternalRedirectFile(ptr, pFile) (FALSE)
