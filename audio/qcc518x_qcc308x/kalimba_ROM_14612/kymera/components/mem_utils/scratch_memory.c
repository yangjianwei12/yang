...
static void deregister_update(unsigned int priority)
{
    ...
    scratch_per_task_data *ptd;
    scratch_per_prio_data *prio_data;
    ...
    prio_data = &per_prio_data[priority];
    if (max_dm1 < prio_data->alloc_info_dm1.max_reserved)
    {
        realloc_base(&prio_data->alloc_info_dm1, max_dm1, MALLOC_PREFERENCE_DM1);
    }
    if (max_dm2 < prio_data->alloc_info_dm2.max_reserved)
    {
        realloc_base(&prio_data->alloc_info_dm2, max_dm2, MALLOC_PREFERENCE_DM2);
    }
    if (max_none < prio_data->alloc_info_none.max_reserved)
    {
        realloc_base(&prio_data->alloc_info_none, max_none, MALLOC_PREFERENCE_FAST);
    }
}
...
bool scratch_deregister(void)
{
    ...
    if (*pptd != NULL)
    {
        ...
        *pptd = (*pptd)->next;
        ...
        prio_data->refcount -= 1;
        if (prio_data->refcount == 0)
        {
            /* Free the scratch block if it was */
            free_base_alloc(&prio_data->alloc_info_dm1);
            free_base_alloc(&prio_data->alloc_info_dm2);
            free_base_alloc(&prio_data->alloc_info_none);
        }
        else
        {
            deregister_update(priority);
        }
        prio_data->last_task = NO_TASK;
...
bool scratch_reserve(unsigned int size, unsigned int preference)
{
    ...
    prio_data = &per_prio_data[ptd->task_priority];
    ...
    switch (preference)
    {
        ...
        {
            alloc_info = &prio_data->alloc_info_dm1;
            reserved = &ptd->total_reserved_dm1;
            break;
        }
        case MALLOC_PREFERENCE_DM2:
        {
            alloc_info = &prio_data->alloc_info_dm2;
        ...
        default:
        {
            alloc_info = &prio_data->alloc_info_none;
            reserved = &ptd->total_reserved_none;
            break;
        }
    }

    if ((task == prio_data->last_task) &&
        (alloc_info->total_committed != 0))
    {
...
void scratch_free(void)
{
    ...
    prio_data->alloc_info_dm1.total_committed = 0;
    prio_data->alloc_info_dm2.total_committed = 0;
    prio_data->alloc_info_none.total_committed = 0;
    prio_data->last_task = NO_TASK;
}
