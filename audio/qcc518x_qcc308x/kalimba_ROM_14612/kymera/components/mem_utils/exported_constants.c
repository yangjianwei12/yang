 ...
 *
 * This is pre-filled for P0, and read only on P1
 */
static DM_P0_RW_ZI EXP_CONST_IPC exp_const_config = {   exp_const_mapping_table,
...
void external_constant_handle_kip_px(unsigned length,uint16 *msg_data)
{
    ...
    put_message_with_routing(cb->task_id, cb->id,cb->msg, &cb->rinfo);
    pfree(cb);
}
#endif /* defined(SUPPORTS_MULTI_CORE) */
...
static void process_activity_queue(void)
{
    ...
    case EXP_CONST_MSG_CALLBACK_ID:
        ...
        put_message_with_routing(cb_msg->task_id, cb_msg->id,cb_msg->msg, &cb_msg->rinfo);
        pfree(cb_msg);
        break;
    }
...
void mem_utils_task_init(void **data)
{
    ...
    if (PROC_PRIMARY_CONTEXT())
    {
         ...
         * The task should only run on P0 */
