...
void ringtone_generator_process_data(OPERATOR_DATA *op_data, TOUCHED_TERMINALS *touched)
{
    ...
    if (generated_words > 0)
    {
        ...
        {
            /* Make a copy of the output buffer structure, 
             * in case we need to use it more than once 
             */
            copy_buffer = *rgop_data->out_cbuffer;

            for (i=0; i<MAX_OUTPUT_CHANS; i++)
            {
                if(active_chans & 01)
                {
                    *rgop_data->out_cbuffer = copy_buffer;
