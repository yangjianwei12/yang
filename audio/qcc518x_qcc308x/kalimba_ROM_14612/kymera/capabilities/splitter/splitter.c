...
static inline void splitter_process_data_clone(OPERATOR_DATA *op_data, TOUCHED_TERMINALS *touched)
{
    ...
    if (min_new_data > 0)
    {
        ...
        {
            /* Only copy whole tags. When a new output is connected
             * the metadata read and write are aligned the the input metadata read index.
             * If the operator only copies whole tags this input read index will
             * point to a tag. This will make the transitions correct.
             */
            if ((splitter->data_format == AUDIO_DATA_FORMAT_FIXP) && (splitter->frame_size != 0))
            {
                unsigned octets_b4idx;
                unsigned available_octets;
                unsigned remaining_octets;
                unsigned max_tag_size;
                metadata_tag *input_tag;

                octets_moved = 0;
                max_tag_size = buff_metadata_get_buffer_size(metadata_ip_buffer) - 1;
                available_octets = buff_metadata_available_octets(metadata_ip_buffer);
                remaining_octets = available_octets;
                input_tag=buff_metadata_peek_ex(metadata_ip_buffer, &octets_b4idx);

                while (input_tag != NULL)
                {
                    unsigned cur_tag_lenght = input_tag->length;

                    if (max_tag_size < cur_tag_lenght)
                    {
                        SPLITTER_ERRORMSG2("Splitter: Tag bigger than buffer size, cannot consume whole tags! "
                                "max_tag_size=%d, cur_tag_lenght=%d",
                                max_tag_size, cur_tag_lenght);
                        /* Just transport everything. */
                        octets_moved = available_octets;
                        break;
                    }
                    if(octets_b4idx != 0)
                    {
                        SPLITTER_ERRORMSG1("Splitter: Incomplete tag consumed at input. Try to align anyways. "
                                "octets_b4idx=%d",octets_b4idx);
                        remaining_octets -= octets_b4idx;
                        octets_moved += octets_b4idx;
                        octets_b4idx = 0;
                    }
                    if (remaining_octets < cur_tag_lenght)
                    {
                        SPLITTER_MSG3("Splitter: Incomplete tag.  "
                                "octets_moved=%d cur_tag_lenght=%d, remaining_octets=%d ",
                                octets_moved, cur_tag_lenght, remaining_octets);
                        break;
                    }

                    SPLITTER_MSG3("Splitter: octets_moved %d += %d, remaining_octets = %d ",
                            octets_moved, cur_tag_lenght, remaining_octets);
                    remaining_octets -= cur_tag_lenght;
                    octets_moved += cur_tag_lenght;
                    /* Go to next tag. */
                    input_tag = input_tag->next;
                }

                SPLITTER_MSG2("Splitter: Limiting min_new_data %d to octets_moved %d ",
                        min_new_data, octets_moved);
                min_new_data = octets_moved;
            }
            else
            {
...
#ifdef SPLITTER_OFFLOAD
    if (offload_enabled && thread_offload_is_active())
    {
...
#ifdef SPLITTER_OFFLOAD
    if (splitter->offload_enabled && thread_offload_is_active())
    {
        thread_offload_rpc_data rpc_data =
        {
            opid,
...
static bool splitter_transition_to_active(SPLITTER_OP_DATA *splitter, unsigned output_index)
{
    ...
    if (output_state == HOLD)
    {
        ...
        /* Go through the metadata until the transition point.
