...
/**
 * Scale the 16-bit config value in an operator message to the
 * word width. Positive values are integers, so do not need to be scaled;
 * negative values are Q6.n in 16 bits and (DAWTH) bits resp.
 */
static int src_sync_scale_buffer_size_config(SRC_SYNC_OP_DATA *op_extra_data, int16 msg_value)
{
    int msg_value_se = msg_value;

    if (msg_value_se < 0)
    {
        /* Qn.10 to Q6.n */
        msg_value_se <<= (DAWTH-16);
        SOSY_MSG1( SRC_SYNC_TRACE_ALWAYS, "set_(terminal_)buffer_size 0.%05d x 32 x kick period x fs",
                   frac_mult(100000, -msg_value_se));
    }
    else if (msg_value_se > 0)
    {
        SOSY_MSG1( SRC_SYNC_TRACE_ALWAYS, "set_(terminal_)buffer_size %d samples",
                   msg_value_se);
    }
    else
    {
        SOSY_MSG( SRC_SYNC_TRACE_ALWAYS, "set_buffer_size auto");
    }
...
/**
 * Apply a buffer size config value to terminal groups whose mask
 * overlaps the mask argument.
 */
static void src_sync_set_group_buffer_size(SRC_SYNC_TERMINAL_GROUP* grp,
                                           ...
                                           unsigned terminal_mask)
{
    while (grp != NULL)
    {
        if ((terminal_mask & grp->channel_mask) != 0)
        {
            grp->has_buffer_size = has_buffer_size;
            grp->buffer_size = buffer_size_config;
        }
        grp = grp->next;
    }
}
...
bool src_sync_opmsg_set_buffer_size(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    op_extra_data->buffer_size =
            src_sync_scale_buffer_size_config(
                    op_extra_data,
                    (int16)OPMSG_FIELD_GET( message_data,
                                            OPMSG_COMMON_SET_BUFFER_SIZE,
                                            BUFFER_SIZE));

    /* Clear all terminal group buffer sizes */
    src_sync_set_group_buffer_size(&op_extra_data->sink_groups->common,
                                   FALSE,
                                   SRC_SYNC_DEFAULT_BUFFER_SIZE_CONFIG,
                                   (unsigned)-1);
    src_sync_set_group_buffer_size(&op_extra_data->source_groups->common,
                                   FALSE,
                                   SRC_SYNC_DEFAULT_BUFFER_SIZE_CONFIG,
                                   (unsigned)-1);
 ...
 * The operator accepts it at any time but it only applies to
 * currently defined terminal groups.
 */
bool src_sync_opmsg_set_terminal_buffer_size(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    SRC_SYNC_OP_DATA *op_extra_data = get_instance_data(op_data);
    ...
    src_sync_set_group_buffer_size(
...
/**
 * The message is the standard OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD.
 * The operator accepts it at any time but it only applies to
 * currently defined terminal groups.
 */
        ...
        OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    {
        if ((sink_grp->common.channel_mask & sinks_mask) != 0)
        {
            sink_grp->back_kick_mode = back_kick_mode;
            sink_grp->back_kick_threshold = back_kick_threshold;
        }
    }
 ...
 * The operator accepts it when it is not running.
 */
bool src_sync_opmsg_set_sample_rate(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    unsigned new_sample_rate = SAMPLE_RATE_FROM_COMMON_OPMSG(message_data);
    if (new_sample_rate < 8000)
    {
        L2_DBG_MSG2( "src_sync 0x%04x set_sample_rate REJECTED: %d < minimum 8000",
                     op_extra_data->id, new_sample_rate);
        return (FALSE);
    }

    if (new_sample_rate != op_extra_data->sample_rate)
    {
        if (opmgr_op_is_running(op_data))
        {
            L2_DBG_MSG3( "src_sync 0x%04x set_sample_rate REJECTED: change %d -> %d while running",
                         op_extra_data->id, op_extra_data->sample_rate, new_sample_rate);
            return (FALSE);
        }
        op_extra_data->sample_rate = new_sample_rate;
        src_sync_update_derived(op_extra_data);
    }

    SOSY_MSG2( SRC_SYNC_TRACE_ALWAYS, "0x%04x set_sample_rate %d Hz",
               op_extra_data->id, op_extra_data->sample_rate);
...
void src_sync_clear_all_routes(SRC_SYNC_OP_DATA *op_extra_data)
{
    ...
    for (i = 0; i < SRC_SYNC_CAP_MAX_CHANNELS; ++ i)
    {
        ...
        if ((src_ptr != NULL) && src_sync_valid_route(&src_ptr->current_route))
        {
            ...
            src_ptr->switch_route.is_valid = FALSE;
        ...
        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
     ...
     */
    groups = cast_sink_group(
            src_sync_alloc_groups(msg->num_groups, sizeof(SRC_SYNC_SINK_GROUP)) );
    if (groups == NULL)
    {
        return FALSE;
    }
        ...
        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
     ...
     * without knowledge of the size of each entry
     */
    groups = cast_source_group(
            src_sync_alloc_groups(num_groups, sizeof(SRC_SYNC_SOURCE_GROUP)) );
    if (groups == NULL)
    {
        return FALSE;
    }
                        ...
                        OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    for ( i = 0; i < SRC_SYNC_CAP_MAX_CHANNELS; ++ i )
    {
        if ((routed_sources & (1 << i)) == 0)
        {
            ...
            if ( (src_data != NULL)
                 && src_data->current_route.is_valid )
            {
    ...
    /* Allow routes to change sample rate if:
     * - the operator is stopped
     * - the current SET_ROUTE replaces all existing routes
     *   (e.g. there are no routes set now)
     */
    for ( i = 0, route = msg->routes;
          i < msg->num_routes;
          i++, route++ )
    {
        unsigned sample_rate = 25 * route->sample_rate;
        if (sample_rate != 0)
        {
            if (sample_rate != op_extra_data->sample_rate)
            {
                if (retained_routes || opmgr_op_is_running(op_data))
                {
                    L2_DBG_MSG5("src_sync 0x%04x set_route REJECTED: "
                                "attempt to change sample rate from %d to %d, "
                                "entry %d, running %d",
                                op_extra_data->id,
                                op_extra_data->sample_rate, sample_rate,
                                i, opmgr_op_is_running(op_data));
                    return FALSE;
                }
                else
                {
                    SOSY_MSG2( SRC_SYNC_TRACE_ALWAYS,
                               "set_route changes sample rate %d -> %d",
                               op_extra_data->sample_rate, sample_rate);
                    op_extra_data->sample_rate = sample_rate;
                    src_sync_update_derived(op_extra_data);
                }
            }
            /* Check that any other routes set the same sample rate */
            retained_routes = TRUE;
        }
    }
          ...
          i++, route++ )
    {
        ...
        src_data->switch_route.is_valid         = (sample_rate != 0);
...
{
    SOSY_MSG1( SRC_SYNC_TRACE_ALWAYS, "SS_PERIOD                       = 0.%06d",
        rate_second_interval_to_signed_usec_round (src_sync_get_period(op_extra_data)));
    SOSY_MSG1( SRC_SYNC_TRACE_ALWAYS, "SS_MAX_PERIOD                   = 0.%06d",
        rate_second_interval_to_signed_usec_round (src_sync_get_max_period(op_extra_data)));
    SOSY_MSG1( SRC_SYNC_TRACE_ALWAYS, "SS_MAX_LATENCY                  = 0.%06d",
        rate_second_interval_to_signed_usec_round (src_sync_get_max_latency(op_extra_data)));
    SOSY_MSG1( SRC_SYNC_TRACE_ALWAYS, "SS_STALL_RECOVERY_DEFAULT_FILL  = 0.%06d",
        rate_second_interval_to_signed_usec_round (src_sync_get_stall_recovery_default_fill(op_extra_data)));
    SOSY_MSG1( SRC_SYNC_TRACE_ALWAYS, "SS_STALL_RECOVERY_CATCHUP_LIMIT = 0.%06d",
        frac_mult(SECOND, op_extra_data->cur_params.OFFSET_SS_STALL_RECOVERY_CATCHUP_LIMIT));
}
#endif /* SOSY_VERBOSE */
        ...
        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    /* 32-bit MSW first */
    uint32 prev_trace_enable = op_extra_data->trace_enable;
    ...
    SOSY_MSG2( SRC_SYNC_TRACE_ALWAYS, "set_trace_enable 0x%08x -> 0x%08x",
               prev_trace_enable, op_extra_data->trace_enable);
#endif /* SOSY_VERBOSE */
