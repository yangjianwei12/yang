        ...
        if (buffers[num_channels-1] != NULL)
        {
            ...
            if (enable_metadata_success)
            {
                ...
                if (cbops != NULL)
                {
                     ...
                     */
                    rm_state->block_size = op_extra_data->ss_period_w;
...
void src_sync_rm_process( SRC_SYNC_OP_DATA *op_extra_data, unsigned *back_kick )
{
          ...
          sink_grp = next_sink_group(sink_grp) )
    {
        ...
        {
            skip_if_less = op_extra_data->ss_period_w;
        }
                ...
                if (input_read_w > 0)
                {
                    ...
                    else
                    {
                        ...
                        do
                        {
                            ...
                            if (IS_TIME_TO_PLAY_TAG(consume_tag) || IS_TIME_OF_ARRIVAL_TAG(consume_tag))
                            {
                                SOSY_MSG4( SRC_SYNC_TRACE_RM_TRANSFER,
                            ...
                            if (IS_TIME_TO_PLAY_TAG(consume_tag))
                            {
                                if (! rm_state->valid_ttp_reconstruct)
                                {
                            ...
                            else if (IS_TIME_TO_PLAY_TAG(consume_tag))
                            {
