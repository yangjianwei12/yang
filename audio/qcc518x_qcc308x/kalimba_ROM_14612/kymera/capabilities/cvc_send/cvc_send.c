...
void cvc_send_process_data(OPERATOR_DATA *op_data, TOUCHED_TERMINALS *touched)
{
    ...
    if(NULL != op_extra_data->input_metadata_buffer)
    {
        /* Propagate any metadata to ALL the output. Null output_metadata_buffer
        * can also be handled by the metadata library */
        metadata_strict_transport(op_extra_data->input_metadata_buffer,
                                op_extra_data->output_metadata_buffer,
                                frame_size*OCTETS_PER_SAMPLE);
    }
    else if(NULL != op_extra_data->output_metadata_buffer)
    {
        /* Input doesn't have metadata but output does,
        * Normally this shouldn't be the case, but in this case,
        * we need to append an empty tag to the output metadata buffer.
        */
        ...
        /* Propagate the empty tag to all the output.*/
        buff_metadata_append(op_extra_data->output_metadata_buffer, out_mtag, 0, afteridx);
    }
...
bool cvc_send_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(resp)
    {
        ...
        *(stats[3])=0;
        ...
        resp = cpsPack1Word(*stats[22] , resp);
