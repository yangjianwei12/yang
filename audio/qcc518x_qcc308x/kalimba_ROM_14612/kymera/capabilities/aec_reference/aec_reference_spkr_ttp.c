...
static void aec_reference_spkr_ttp_error_control(OPERATOR_DATA *op_data,
                                                 TIME_INTERVAL error);
static void aec_reference_spkr_ttp_adjust_rate(OPERATOR_DATA *op_data, int warp);
/****************************************************************************
 ...
 */
void aec_reference_spkr_ttp_run(OPERATOR_DATA *op_data, unsigned *max_to_process)
{
    AEC_REFERENCE_OP_DATA  *op_extra_data = NULL;
    TIME_INTERVAL error = 0;
    bool error_valid;
    unsigned amount_can_process;
    /* only used for debug log */
    TIME current_time;

    patch_fn_shared(aec_reference_run);

    op_extra_data = base_op_get_instance_data(op_data);
    amount_can_process = *max_to_process;
       ...
       amount_can_process > 0)
    {
        ...
        if(samples_to_trash > 0)
        {
            ...
            opmgr_record_timing_trace_op_event(
                    base_op_get_ext_op_id(op_data) | STREAM_EP_OP_SINK,
                    TIMING_TRACE_TTP_DISCARD_SAMPLES,
                    samples_trashed,
                    error);
#endif
    ...
    /* Note: late tags have been processed above, so now either
     *       error is positive (error > - threshold) or,
     *       no sample left to process (amount_can_process == 0).
     */
    if (pl_abs_i32(error) < op_extra_data->spkr_error_threshold)
    {
         ...
         */
        aec_reference_spkr_ttp_error_control(op_data, error);
    ...
    else if (error > 0)
    {
        ...
        opmgr_record_timing_trace_op_event(
                base_op_get_ext_op_id(op_data) | STREAM_EP_OP_SINK,
                TIMING_TRACE_TTP_INSERT_SILENCE_SAMPLES,
                op_extra_data->spkr_in_threshold-amount_can_process,
                error);
#endif
    }
...
static bool aec_reference_spkr_ttp_get_error(AEC_REFERENCE_OP_DATA *op_extra_data, TIME_INTERVAL *error)
{
    ...
    met_buf = op_extra_data->spkr_input_metadata_buffer;
 ...
 */
static void aec_reference_spkr_ttp_error_control(OPERATOR_DATA *op_data, TIME_INTERVAL error)
{
    AEC_REFERENCE_OP_DATA *op_extra_data = NULL;
    int error_diff;
    patch_fn_shared(aec_reference);
    ...
    if(op_extra_data->spkr_ttp_error_acc_cnt == SPKR_TTP_ERROR_AVERAGING_PERIOD_MS)
    {
        ...
        aec_reference_spkr_ttp_adjust_rate(op_data, ttp_pid_controller_get_warp(op_extra_data->spkr_pid));
...
static void aec_reference_spkr_ttp_adjust_rate(OPERATOR_DATA *op_data, int warp)
{
    ...
    opmgr_record_timing_trace_op_event(
            base_op_get_ext_op_id(op_data) | STREAM_EP_OP_SINK,
            TIMING_TRACE_TTP_PLAYBACK_CONTROL,
            op_extra_data->spkr_ttp_error_acc,
            warp);
