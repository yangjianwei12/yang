...
typedef enum
{
    ...
    ACCMD_AUDIO_CHANNEL_SLOT_4 = 0x0004,
    ACCMD_AUDIO_CHANNEL_SLOT_5 = 0x0005,
    ACCMD_AUDIO_CHANNEL_SLOT_6 = 0x0006,
    ACCMD_AUDIO_CHANNEL_SLOT_7 = 0x0007,
...
/*******************************************************************************
  ...
  DESCRIPTION
    ...
    not reuse 0x030d. NOTE 0x031d ,0x031e,0x031f keys are available  to use
    0x0400 - 0x04FF typically used for FM keys 0x0500 - 0x05FF typically used
    for SPDIF keys 0x0600 - 0x0650 typically used for digital mic keys 0x0651
    - 0x06FF will be used for MON_OUT endpoint 0x0700 - 0x07FF typically used
    for keys applicable to all interfaces 0x0710 - 0x071F for end-point
    processing (Cbops) 0x0900 - 0x09FF typically used for FastPipe keys
    0x0902 formerly "STREAM_SCO_SRC_MAJORITY_VOTE_BYPASS", now unused. 0x0903
    formerly "STREAM_SCO_SRC_MAJORITY_VOTE_QUESTIONABLE_BITS_MAX", now
    unused. 0x1000 - 0x10FF typically used for PWM keys 0x1100 - 0x11FF
    typically used for ANC 0x1200 - 0x12FF for rate measurement/rate
    matching.
 ...
 VALUES
    ...
    STREAM_PCM_SLOT_COUNT
                   - PCM slot count 0: Derived from the clock and the sync
                     rates. 1..8: Number of slots present between sync pulses.
    ...
    STREAM_PCM_MASTER_CLK_SOURCE
                   - Set the source of the master clock for a PCM interface. By
                     ...
                     as the fourth PCM/I2S interface. 4 (BORROW_PCM1): Interface
                     clock derived from the same source as the second PCM/I2S
                     interface.
    ...
    STREAM_CODEC_DAC_QUIET_MODE
                   - 1: If DAC is active, mutes audio mux and disables the DAC
                     analog stage. 0: If DAC is active, enables DAC analog stage
                     and unmutes audio mux. DAC is used as a stereo pair. So
                     enabling quiet mode on one channel will disable both
                     channels if L & R channels are active
    ...
    STREAM_DIGITAL_MIC_AOV_MAX_ENTRIES
                   - Configure the threshold of aov buffer. After this threshold
                     is reached, an interrupt is raised by aov hardware.
    STREAM_DIGITAL_MIC_ROUTING_CONFIG
                   - Configures which of digital mic data to be routed to this
                     decimation chain
    STREAM_DIGITAL_MIC_ROUTING_ENABLE
                   - Enable routing of configured digital mic data to this
                     decimation chain
    STREAM_MON_OUT_SELECT_ANC_SIGNAL
                   - Configure the monitor output from different tap points of
                     ANC HW.
    STREAM_MON_OUT_SELECT_CODEC_LOOPBACK
                   - Configure the monitor output from different tap points of
                     DAC HW to ADC.
    STREAM_MON_OUT_SELECT_CODEC_DMIC_ROUTING
                   - Configure the monitor output by routing a codec or DMIC
                     input.
    ...
    STREAM_ANC_CONTROL
                   - Set ANC controls.
    STREAM_ANC_CONTROL_1
                   - Set ANC controls_1.
    STREAM_ANC_RX_MIX_FFA_GAIN
                   - Set the gain for ANC FFa Path to the Rx Mixer.
    STREAM_ANC_RX_MIX_FFA_SHIFT
                   - Set the gain shift for ANC FF path A to the Rx Mixer.
    STREAM_ANC_RX_MIX_FFB_GAIN
                   - Set the gain for ANC FFb Path to the Rx Mixer.
    STREAM_ANC_RX_MIX_FFB_SHIFT
                   - Set the gain shift for ANC FF path B to the Rx Mixer.
    STREAM_ANC_FFA_IIR_COEFFS
                   - Set the ANC IIR filter coeffs for FFA path
    STREAM_ANC_FFB_IIR_COEFFS
                   - Set the ANC IIR filter coeffs for FFB path
    STREAM_ANC_FB_IIR_COEFFS
                   - Set the ANC IIR filter coeffs for FB path
    STREAM_ANC_FFA_LPF_COEFFS
                   - Set the ANC LPF coeffs for FFA path
    STREAM_ANC_FFB_LPF_COEFFS
                   - Set the ANC LPF coeffs for FFB path
    STREAM_ANC_FB_LPF_COEFFS
                   - Set the ANC LPF coeffs for FB path
...
typedef enum
{
    ...
    ACCMD_CONFIG_KEY_STREAM_CODEC_DAC_QUIET_MODE = 0x0320,
    ...
    ACCMD_CONFIG_KEY_STREAM_DIGITAL_MIC_ROUTING_CONFIG = 0x060f,
    ACCMD_CONFIG_KEY_STREAM_DIGITAL_MIC_ROUTING_ENABLE = 0x0610,
    ACCMD_CONFIG_KEY_STREAM_MON_OUT_SELECT_ANC_SIGNAL = 0x0651,
    ACCMD_CONFIG_KEY_STREAM_MON_OUT_SELECT_CODEC_LOOPBACK = 0x0652,
    ACCMD_CONFIG_KEY_STREAM_MON_OUT_SELECT_CODEC_DMIC_ROUTING = 0x0653,
    ...
    ACCMD_CONFIG_KEY_STREAM_ANC_CONTROL_1 = 0x1112,
    ACCMD_CONFIG_KEY_STREAM_ANC_RX_MIX_FFA_GAIN = 0x1113,
    ACCMD_CONFIG_KEY_STREAM_ANC_RX_MIX_FFA_SHIFT  = 0x1114,
    ACCMD_CONFIG_KEY_STREAM_ANC_RX_MIX_FFB_GAIN = 0x1115,
    ACCMD_CONFIG_KEY_STREAM_ANC_RX_MIX_FFB_SHIFT = 0x1116,
    ACCMD_CONFIG_KEY_STREAM_ANC_FFA_IIR_COEFFS = 0x1117,
    ACCMD_CONFIG_KEY_STREAM_ANC_FFB_IIR_COEFFS = 0x1118,
    ACCMD_CONFIG_KEY_STREAM_ANC_FB_IIR_COEFFS = 0x1119,
    ACCMD_CONFIG_KEY_STREAM_ANC_FFA_LPF_COEFFS = 0x111A,
    ACCMD_CONFIG_KEY_STREAM_ANC_FFB_LPF_COEFFS = 0x111B,
    ACCMD_CONFIG_KEY_STREAM_ANC_FB_LPF_COEFFS = 0x111C,
...
/*******************************************************************************
  ...
  DESCRIPTION
    ...
    unused. 0x001C formerly "GET_SYSTEM_STREAM_RATE_REQ", now unused. 0x003D
    formerly "CAP_DOWNLOAD_ADD_KCS_REQ", now unused. 0x003E formerly
    "CAP_DOWNLOAD_COMPLETE_KCS_REQ", now unused. 0x0045 formerly
    "DDS_CLOCK_CONFIGURE_REQ", now unused. Response messages 0x4014 formerly
    "SET_SYSTEM_STREAM_RATE_RESP", now unused. 0x4015 formerly
    "GET_SYSTEM_STREAM_RATE_RESP", now unused. Test Request messages 0x2003
    formerly "SET_DEBUG_ENABLES_REQ", now unused. Special ID for discardable
    unsolicited message. This is never actually sent to the ACCMD client Test
    response messages Debug messages, sent as indications 0x8000 formerly
    "DEBUG_MESSAGE_IND", now unused. 0x8002 formerly "DEBUG_SEND_CONFIG", now
    unused.
 ...
 VALUES
    ...
    CALL_FUNCTION_REQ
                   -
    MESSAGE_FROM_OPERATOR_DISCARDABLE_REQ
                   -
    NO_CMD_RESP
                   -
...
typedef enum
{
    ...
    ACCMD_SIGNAL_ID_ANC_CONFIGURE_REQ = 0x0046,
    ...
    ACCMD_SIGNAL_ID_MESSAGE_FROM_OPERATOR_DISCARDABLE_REQ = 0x2007,
    ...
    ACCMD_SIGNAL_ID_ANC_CONFIGURE_RESP = 0x403E,
...
typedef enum
{
    ...
    ACCMD_STREAM_DEVICE_ISO = 0x0013,
    ACCMD_STREAM_DEVICE_MON_OUT = 0x0014,
...
/*******************************************************************************
  ...
  DESCRIPTION
    Indicates in which part of the audio subsystem memory space, a file
    stored into the application flash should be loaded into.

 VALUES
    UNUSED     -
    BIN        - The file will be copied in the main heap of the internal
                 memory.
    PUK        - Legacy. It is now equivalent to "BIN".
    EDF        - The file will be copied in the external memory.
    EXTRA_HEAP - The file will be copied in the extra heap of the internal
                 memory.
...
typedef enum
{
    ...
    ACCMD_TYPE_OF_FILE_EXTRA_HEAP = 4
} ACCMD_TYPE_OF_FILE;
/*******************************************************************************
  ...
  DESCRIPTION
    ...
    bit-field default value ANC FBMON_SEL bit-field position
...
typedef enum
{
    ...
    ACCMD_ANC_CONTROL_FBMON_SEL_MASK = 0x0030,
    ...
    ACCMD_ANC_CONTROL_FBMON_SEL_POSITION = 4,
...
/*******************************************************************************
  ...
  DESCRIPTION
    ANC control_1 bitfield

 VALUES
    SELF_RXMIX_EN_MASK            -
    CROSS_RXMIX_EN_MASK           -
    BORROW_ANC0_RX_PCM_INPUT_MASK -
    FFLE_EN_MASK                  -

*******************************************************************************/
typedef enum
{
    ACCMD_ANC_CONTROL_1_SELF_RXMIX_EN_MASK = 0x0001,
    ACCMD_ANC_CONTROL_1_CROSS_RXMIX_EN_MASK = 0x0002,
    ACCMD_ANC_CONTROL_1_BORROW_ANC0_RX_PCM_INPUT_MASK = 0x0004,
    ACCMD_ANC_CONTROL_1_FFLE_EN_MASK = 0x0008
} ACCMD_ANC_CONTROL_1;
/*******************************************************************************
...
/*******************************************************************************
  ...
  NAME
    Accmd_anc_iir_filter32

  DESCRIPTION
    ANC IIR filter details (updated ANC HW)
...
typedef enum
{
    ACCMD_ANC_IIR_FILTER32_OCTETS_PER_COEFF = 4,
    ACCMD_ANC_IIR_FILTER32_COEFF_MSW_SHIFT = 16,
    ACCMD_ANC_IIR_FILTER32_FFB_NUM_COEFFS = 17,
    ACCMD_ANC_IIR_FILTER32_FB_NUM_COEFFS = 17,
    ACCMD_ANC_IIR_FILTER32_FFA_NUM_COEFFS = 17
} ACCMD_ANC_IIR_FILTER32;
/*******************************************************************************
...
typedef enum
{
    ...
    ACCMD_ANC_INSTANCE_ANC01_ID = 0x0003
} ACCMD_ANC_INSTANCE;
/*******************************************************************************
...
typedef enum
{
    ...
    ACCMD_CLK_TYPE_BORROW_PCM1 = 0x0004,
    ACCMD_CLK_TYPE_MAX = 0x0004
} ACCMD_CLK_TYPE;
/*******************************************************************************

  NAME
    Accmd_mon_out_anc_signals

  DESCRIPTION
    Monitor_OUT signal tapped from ANC HW block

 VALUES
    FFA_IN_ANC0            -
    FB_IN_ANC0             -
    ANC_OUT_ANC0           -
    FB_MON_SIGNAL_OFF_ANC0 -
    FFA_IN_ANC1            -
    FB_IN_ANC1             -
    ANC_OUT_ANC1           -
    FB_MON_SIGNAL_OFF_ANC1 -

*******************************************************************************/
typedef enum
{
    ACCMD_MON_OUT_ANC_SIGNALS_FFA_IN_ANC0 = 0x0000,
    ACCMD_MON_OUT_ANC_SIGNALS_FB_IN_ANC0 = 0x0001,
    ACCMD_MON_OUT_ANC_SIGNALS_ANC_OUT_ANC0 = 0x0002,
    ACCMD_MON_OUT_ANC_SIGNALS_FB_MON_SIGNAL_OFF_ANC0 = 0x00FF,
    ACCMD_MON_OUT_ANC_SIGNALS_FFA_IN_ANC1 = 0x0100,
    ACCMD_MON_OUT_ANC_SIGNALS_FB_IN_ANC1 = 0x0101,
    ACCMD_MON_OUT_ANC_SIGNALS_ANC_OUT_ANC1 = 0x0102,
    ACCMD_MON_OUT_ANC_SIGNALS_FB_MON_SIGNAL_OFF_ANC1 = 0x01FF
} ACCMD_MON_OUT_ANC_SIGNALS;
/*******************************************************************************

  NAME
    Accmd_mon_out_codec_dmic_signals

  DESCRIPTION
    Monitor_OUT signal tapped at different adc or dmic interface

 VALUES
    CODEC_DMIC_INSTANC_0_CHAN_A   -
    CODEC_DMIC_INSTANC_0_CHAN_B   -
    CODEC_DMIC_INSTANC_1_CHAN_A   -
    CODEC_DMIC_INSTANC_1_CHAN_B   -
    CODEC_DMIC_INSTANC_2_CHAN_A   -
    CODEC_DMIC_INSTANC_2_CHAN_B   -
    CODEC_DMIC_INSTANC_3_CHAN_A   -
    CODEC_DMIC_INSTANC_3_CHAN_B   -
    CODEC_DMIC_INSTANC_4_CHAN_A   -
    CODEC_DMIC_INSTANC_4_CHAN_B   -
    CODEC_DMIC_SIGNAL_ROUTING_OFF -

*******************************************************************************/
typedef enum
{
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_0_CHAN_A = 0x0000,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_0_CHAN_B = 0x0001,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_1_CHAN_A = 0x0002,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_1_CHAN_B = 0x0003,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_2_CHAN_A = 0x0004,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_2_CHAN_B = 0x0005,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_3_CHAN_A = 0x0006,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_3_CHAN_B = 0x0007,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_4_CHAN_A = 0x0008,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_INSTANC_4_CHAN_B = 0x0009,
    ACCMD_MON_OUT_CODEC_DMIC_SIGNALS_CODEC_DMIC_SIGNAL_ROUTING_OFF = 0x00FF
} ACCMD_MON_OUT_CODEC_DMIC_SIGNALS;
/*******************************************************************************

  NAME
    Accmd_mon_out_codec_loopback_signals

  DESCRIPTION
    Monitor_OUT signal tapped through different tap points of DAC

 VALUES
    CODEC_LOOPBACK_OFF            -
    CODEC_SHORT_DIG_LOOPBACK_DAC0 -
    CODEC_LONG_DIG_LOOPBACK_DAC0  -
    CODEC_SHORT_ANA_LOOPBACK_DAC0 -
    CODEC_LONG_ANA_LOOPBACK_DAC0  -
    CODEC_SHORT_DIG_LOOPBACK_DAC1 -
    CODEC_LONG_DIG_LOOPBACK_DAC1  -
    CODEC_SHORT_ANA_LOOPBACK_DAC1 -
    CODEC_LONG_ANA_LOOPBACK_DAC1  -

*******************************************************************************/
typedef enum
{
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_LOOPBACK_OFF = 0x0000,
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_SHORT_DIG_LOOPBACK_DAC0 = 0x0001,
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_LONG_DIG_LOOPBACK_DAC0 = 0x0002,
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_SHORT_ANA_LOOPBACK_DAC0 = 0x0003,
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_LONG_ANA_LOOPBACK_DAC0 = 0x0004,
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_SHORT_DIG_LOOPBACK_DAC1 = 0x0101,
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_LONG_DIG_LOOPBACK_DAC1 = 0x0102,
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_SHORT_ANA_LOOPBACK_DAC1 = 0x0103,
    ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS_CODEC_LONG_ANA_LOOPBACK_DAC1 = 0x0104
} ACCMD_MON_OUT_CODEC_LOOPBACK_SIGNALS;
...
/*******************************************************************************
  ...
  NAME
    Accmd_ANC_configure_Req

  DESCRIPTION
    From AudioSS Configure ANC HW for the given set of configuration cmds.

  MEMBERS
    instance_id -
    Total_Len   - The length of the entire configuration payload, in uint16s
    data        - the actual data; length implicit in ACCMD payload length
...
/* The following macros take ACCMD_ANC_CONFIGURE_REQ *accmd_anc_configure_req_ptr or uint16 *addr */
#define ACCMD_ANC_CONFIGURE_REQ_INSTANCE_ID_WORD_OFFSET (3)
#define ACCMD_ANC_CONFIGURE_REQ_INSTANCE_ID_GET(addr) (*((addr) + 3))
#define ACCMD_ANC_CONFIGURE_REQ_INSTANCE_ID_SET(addr, instance_id) (*((addr) + 3) = (uint16)(instance_id))
#define ACCMD_ANC_CONFIGURE_REQ_TOTAL_LEN_WORD_OFFSET (4)
#define ACCMD_ANC_CONFIGURE_REQ_TOTAL_LEN_GET(addr) (*((addr) + 4))
#define ACCMD_ANC_CONFIGURE_REQ_TOTAL_LEN_SET(addr, total_len) (*((addr) + 4) = (uint16)(total_len))
#define ACCMD_ANC_CONFIGURE_REQ_DATA_WORD_OFFSET (5)
#define ACCMD_ANC_CONFIGURE_REQ_DATA_GET(addr) (*((addr) + 5))
#define ACCMD_ANC_CONFIGURE_REQ_DATA_SET(addr, data) (*((addr) + 5) = (uint16)(data))
#define ACCMD_ANC_CONFIGURE_REQ_WORD_SIZE (6)
#define ACCMD_ANC_CONFIGURE_REQ_PACK(addr, instance_id, Total_Len, data) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(instance_id)); \
        *((addr) + 4) = (uint16)((uint16)(Total_Len)); \
        *((addr) + 5) = (uint16)((uint16)(data)); \
    } while (0)

#define ACCMD_ANC_CONFIGURE_REQ_MARSHALL(addr, accmd_anc_configure_req_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_anc_configure_req_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_anc_configure_req_ptr)->instance_id); \
        *((addr) + 4) = (uint16)((accmd_anc_configure_req_ptr)->Total_Len); \
        *((addr) + 5) = (uint16)((accmd_anc_configure_req_ptr)->data); \
    } while (0)

#define ACCMD_ANC_CONFIGURE_REQ_UNMARSHALL(addr, accmd_anc_configure_req_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_anc_configure_req_ptr)->header)); \
        (accmd_anc_configure_req_ptr)->instance_id = ACCMD_ANC_CONFIGURE_REQ_INSTANCE_ID_GET(addr); \
        (accmd_anc_configure_req_ptr)->Total_Len = ACCMD_ANC_CONFIGURE_REQ_TOTAL_LEN_GET(addr); \
        (accmd_anc_configure_req_ptr)->data = ACCMD_ANC_CONFIGURE_REQ_DATA_GET(addr); \
    } while (0)
...
/*******************************************************************************
  ...
  NAME
    Accmd_ANC_configure_Resp

  DESCRIPTION
    From AudioSS Returns the number ANC configuration successfully applied.
    If the number of configuration done is less than "Count" in the previous
    Accmd_ANC_configure_Req then an error reason code will also be returned

  MEMBERS
    Status      -
    Count       - The number of ANC configure cmds executed successfully
    reason_code - Error reason code if not all configurations were applied. In
                  case of partial success, reason_code will be the key id of
                  first failed configuration. Error code will zero is
                  Accmd_ANC_configure_Req have non-compatible payload
...
/* The following macros take ACCMD_ANC_CONFIGURE_RESP *accmd_anc_configure_resp_ptr or uint16 *addr */
#define ACCMD_ANC_CONFIGURE_RESP_STATUS_WORD_OFFSET (3)
#define ACCMD_ANC_CONFIGURE_RESP_STATUS_GET(addr) ((ACCMD_STATUS)*((addr) + 3))
#define ACCMD_ANC_CONFIGURE_RESP_STATUS_SET(addr, status) (*((addr) + 3) = (uint16)(status))
#define ACCMD_ANC_CONFIGURE_RESP_COUNT_WORD_OFFSET (4)
#define ACCMD_ANC_CONFIGURE_RESP_COUNT_GET(addr) (*((addr) + 4))
#define ACCMD_ANC_CONFIGURE_RESP_COUNT_SET(addr, count) (*((addr) + 4) = (uint16)(count))
#define ACCMD_ANC_CONFIGURE_RESP_REASON_CODE_WORD_OFFSET (5)
#define ACCMD_ANC_CONFIGURE_RESP_REASON_CODE_GET(addr) (*((addr) + 5))
#define ACCMD_ANC_CONFIGURE_RESP_REASON_CODE_SET(addr, reason_code) (*((addr) + 5) = (uint16)(reason_code))
#define ACCMD_ANC_CONFIGURE_RESP_WORD_SIZE (6)
#define ACCMD_ANC_CONFIGURE_RESP_PACK(addr, Status, Count, reason_code) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(Status)); \
        *((addr) + 4) = (uint16)((uint16)(Count)); \
        *((addr) + 5) = (uint16)((uint16)(reason_code)); \
...
#define ACCMD_ANC_CONFIGURE_RESP_MARSHALL(addr, accmd_anc_configure_resp_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_anc_configure_resp_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_anc_configure_resp_ptr)->Status); \
        *((addr) + 4) = (uint16)((accmd_anc_configure_resp_ptr)->Count); \
        *((addr) + 5) = (uint16)((accmd_anc_configure_resp_ptr)->reason_code); \
    } while (0)

#define ACCMD_ANC_CONFIGURE_RESP_UNMARSHALL(addr, accmd_anc_configure_resp_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_anc_configure_resp_ptr)->header)); \
        (accmd_anc_configure_resp_ptr)->Status = ACCMD_ANC_CONFIGURE_RESP_STATUS_GET(addr); \
        (accmd_anc_configure_resp_ptr)->Count = ACCMD_ANC_CONFIGURE_RESP_COUNT_GET(addr); \
        (accmd_anc_configure_resp_ptr)->reason_code = ACCMD_ANC_CONFIGURE_RESP_REASON_CODE_GET(addr); \
    } while (0)
