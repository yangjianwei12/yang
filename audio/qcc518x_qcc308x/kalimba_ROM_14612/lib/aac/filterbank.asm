...
.MODULE $M.aacdec.filterbank;
   ...
   // save output channel write pointer before windowing modifies it
   ...
   left_channel:
      ...
      if Z jump skip_get_write_address;
      jump channel_select_done;
   right_channel:
   ...
   // Add 2.4dB of gain to compensate for reduction in windowing scale factor
   r5 = M[r9 + $aac.mem.codec_struc];
   NULL = M[r9 + $aac.mem.CURRENT_CHANNEL];
   if NZ jump post_overlap_right_channel;
      r0 = M[r5 + $codec.DECODER_OUT_LEFT_BUFFER_FIELD];
      if Z jump $pop_rLink_and_rts;
      jump post_overlap_channel_select_done;
   post_overlap_right_channel:
      r0 = M[r5 + $codec.DECODER_OUT_RIGHT_BUFFER_FIELD];
      if Z jump $pop_rLink_and_rts;
   post_overlap_channel_select_done:

   // Pop the buffer info from before windowing
   pop r0;
   pop L1;
#ifdef BASE_REGISTER_MODE
   pop B1;
   ...
   r10 = $aacdec.MAX_AUDIO_FRAME_SIZE_IN_WORDS;
   r0 = M[I1,M3];
   //Add gain of 2.35dB to output PCM
   do gain_compensation_loop; 
      r1 = r0 * 0.65534502 (frac);
      r1 = r1 ASHIFT 1;
      r0 = M[I1,M3], M[I4,M3] = r1;
   gain_compensation_loop:

   // Restore base and length registers we use, otherwise we panic
   pop B4;
   pop L4;
   pop B1;
   pop L1;
