...
//#endif
   ...
   // ***********************************************
   // call coeffs initialization for f2 if in 3-band
   // ***********************************************
#ifdef INSTALL_OPERATOR_XOVER_3BAND
   r0 = M[r8 + $audio_proc.xover_2band.CAP_CONFIG_FIELD];
   Null = r0 - $audio_proc.xover_2band.XOVER_3BAND;
   if NZ jump select_peq_core;

   r5 = M[r7 + $audio_proc.xover_2band.parameter.LP_TYPE_F2];
   r6 = 1;
   call $audio_proc.xover.bw_lr_filter.initialize_f2;

   r5 = M[r7 + $audio_proc.xover_2band.parameter.HP_TYPE_F2];
   r6 = 2;
   call $audio_proc.xover.bw_lr_filter.initialize_f2;
#endif
...
init_peq_filter:
...
#ifdef INSTALL_OPERATOR_XOVER_3BAND
   // *******************************************************
   // If 3-band initialize peq low_freq and high_freq for f2
   // *******************************************************
   r0 = M[r8 + $audio_proc.xover_2band.CAP_CONFIG_FIELD];
   Null = r0 - $audio_proc.xover_2band.XOVER_3BAND;
   if NZ jump init_peq_filter_xover_params;

   r7 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_LOW_FREQ_F2];
   push r8;
   call r4;
   pop r8;

   r7 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_HIGH_FREQ_F2];
   push r8;
   call r4;
   pop r8;

   // *************************************************************
   // 3-band APC has a 5th PEQ object that needs to be initialized
   // *************************************************************
   r7 = M[r8 + $audio_proc.xover_2band.PARAM_PTR_FIELD];
   r5 = M[r7 + $audio_proc.xover_2band.parameter.LP_TYPE];
   Null = r5 - $audio_proc.xover_2band.filter.APC;
   ...
   r7 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_APC_COMP_F2];
   push r8;
   call r4;
   pop r8;
...
headroom_bits_high_pass:
   ...
   // *******************************************************************
   // set "headroom_bits" for f2 filter if 3-band
   // Note: APC is controlled by LP_TYPE for f1 so no checking done here
   // *******************************************************************
#ifdef INSTALL_OPERATOR_XOVER_3BAND
   r0 = M[r8 + $audio_proc.xover_2band.CAP_CONFIG_FIELD];
   Null = r0 - $audio_proc.xover_2band.XOVER_3BAND;
...
headroom_bits_3band:
   r0 = M[r7 + $audio_proc.xover_2band.parameter.LP_ORDER_F2];
   r1 = M[r7 + $audio_proc.xover_2band.parameter.LP_TYPE_F2];
   r0 = r0 - 1;        // calculate index in the headroom_bits table
   Words2Addr(r0);     // arch4: headroom _bits index in addr
   r1 = r1 - 1;        // index for picking bw/lr filter
   Words2Addr(r1);     // arch4: bw/lr index in addr
   r3 = M[r5 + r1];    // index into the bw or lr table
   r4 = M[r3 + r0];    // extract the "headroom_bits" value for the f2 "low_pass" filter
   r3 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_LOW_FREQ_F2];
   M[r3	+ $audio_proc.peq.HEADROOM_BITS] = r4;

   r0 = M[r7 + $audio_proc.xover_2band.parameter.HP_ORDER_F2];
   r1 = M[r7 + $audio_proc.xover_2band.parameter.HP_TYPE_F2];
   r0 = r0 - 1;        // calculate index in the headroom_bits table
   Words2Addr(r0);     // arch4: headroom _bits index in addr
   r1 = r1 - 1;        // index for picking bw/lr table
   Words2Addr(r1);     // arch4: bw/lr index in addr
   r3 = M[r5 + r1];    // index into the bw or lr table
   r4 = M[r3 + r0];    // extract the "headroom_bits" value for the f2 high_pass filter
   r3 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_HIGH_FREQ_F2];
   M[r3	+ $audio_proc.peq.HEADROOM_BITS] = r4;
#endif //INSTALL_OPERATOR_XOVER_3BAND
...
// ***************************************************************************************
// MODULE:
//    $audio_proc.xover.bw_lr_filter.initialize_f2
//
// DESCRIPTION:
//   Generates the coeffs for BW/LR filter by calling the "peq_cap_gen_coeffs" function for f2
//
// INPUTS:
//    - r5 = flag indicating butterworth/linkwitz_riley filter (1/2)
//    - r6 = flag indicating low_pass_filter/high_pass_filter (1/2)
...
// ***************************************************************************************
#ifdef INSTALL_OPERATOR_XOVER_3BAND
.MODULE $M.audio_proc.xover.bw_lr_filter.initialize_f2;
   .CODESEGMENT   PM;
...
//#endif
   ...
   r3 = r7;
   Null = r6 - 2;
   if Z jump high_pass_filter_coeff_gen_f2;

   // *********************************************************************
   //                   LOW-PASS FILTER COEFFICIENT GENERATION
   // *********************************************************************

low_pass_filter_coeff_gen_f2:
   // ********************************************************************************
   // set the peq_cap_params values based on the xover_params for the low_pass filter
   // ********************************************************************************
   r0 = M[r3 + $audio_proc.xover_2band.parameter.LP_ORDER_F2];
   r1 = M[r3 + $audio_proc.xover_2band.parameter.LP_FC_F2];
   r2 = M[r8 + $audio_proc.xover_2band.PEQ_CAP_PARAMS_LOW_FREQ_F2];
   call $audio_proc.xover.set_peq_params;
   ...
   r7 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_LOW_FREQ_F2];
   r7 = M[r7 + $audio_proc.peq.PARAM_PTR_FIELD];                          // pointer to coeff_params
   r8 = M[r8 + $audio_proc.xover_2band.PEQ_CAP_PARAMS_LOW_FREQ_F2];
   r8 = r8 + $M.PEQ.PARAMETERS.OFFSET_NUM_BANDS;                          // pointer to peq_cap_params with num_bands offset
   pushm <r3,r7>;
   call $peq_cap_gen_coeffs;
   popm <r3,r7>;

   jump filter_coeff_gen_done_f2; // No filter inversion for this filter

   // *********************************************************************
   //                   HIGH-PASS FILTER COEFFICIENT GENERATION
   // *********************************************************************

high_pass_filter_coeff_gen_f2:
   // ********************************************************************************
   // set the peq_cap_params values based on the xover_params for the high_pass filter
   // ********************************************************************************
   r0 = M[r3 + $audio_proc.xover_2band.parameter.HP_ORDER_F2];
   r1 = M[r3 + $audio_proc.xover_2band.parameter.HP_FC_F2];
   r2 = M[r8 + $audio_proc.xover_2band.PEQ_CAP_PARAMS_HIGH_FREQ_F2];
   call $audio_proc.xover.set_peq_params;
   ...
   r7 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_HIGH_FREQ_F2];
   r7 = M[r7 + $audio_proc.peq.PARAM_PTR_FIELD];                          // pointer to coeff_params
   r8 = M[r8 + $audio_proc.xover_2band.PEQ_CAP_PARAMS_HIGH_FREQ_F2];
   r8 = r8 + $M.PEQ.PARAMETERS.OFFSET_NUM_BANDS;                          // pointer to peq_cap_params with num_bands offset
   pushm <r3,r7>;
   call $peq_cap_gen_coeffs;
   popm <r3,r7>;
   ...
   // ***************************************************************
   // invert <b2,b1,b0> for stage1 if "INV_BAND3(high_freq)" is set
   // ***************************************************************
   r0 = M[r3 + $audio_proc.xover_2band.parameter.XOVER_CONFIG];
   r1 = r0 AND $M.XOVER.CONFIG.INV_BAND3;
   if Z jump filter_coeff_gen_done_f2;

invert_stage1_b_coeffs_f2:
   r0 = M[r7 + $audio_proc.xover_2band.PEQ_STAGE1_COEFF_OFFSET];
   r0 = -r0;
   M[r7 + $audio_proc.xover_2band.PEQ_STAGE1_COEFF_OFFSET] = r0;
   r0 = M[r7 + ($audio_proc.xover_2band.PEQ_STAGE1_COEFF_OFFSET + 1*ADDR_PER_WORD)];
...
//#endif
   ...
   r0 = M[r8 + $audio_proc.xover_2band.CAP_CONFIG_FIELD];
   Null = r0 - $audio_proc.xover_2band.XOVER_3BAND;
   if NZ jump apc_initialize_done;

   // *********************************
   // 3-band PEQ object initialization
   // *********************************
   pushm <r7,r8>;
   r4 = M[r7 + $audio_proc.xover_2band.parameter.LP_FC_F2];           // cutoff_frequency
   ...
   r2 = M[r8 + $audio_proc.xover_2band.PEQ_CAP_PARAMS_LOW_FREQ_F2];
   r7 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_APC_COMP_F2];
   r3 = 3;                                                         // f2 APC compensation filter
   call $audio_proc.xover.apc_filter.coeff_gen;
...
//#endif
   ...
   // ***************************************************************************************
   // Set the mid_freq output cbuffers from the corresponding output frame buffer for 3-band
   // ***************************************************************************************
#ifdef INSTALL_OPERATOR_XOVER_3BAND
   r0 = M[r8 + $audio_proc.xover_2band.CAP_CONFIG_FIELD];
   Null = r0 - $audio_proc.xover_2band.XOVER_3BAND;
   if NZ jump set_samples_to_process;

   r0 = M[r8 + $audio_proc.xover_2band.OUTPUT_MID_FRAME_ADDR_FIELD];
   r0 = M[r0 + $frmbuffer.CBUFFER_PTR_FIELD];
   M[r8 + $audio_proc.xover_2band.OUTPUT_ADDR_FIELD_MID] = r0;

set_samples_to_process:
#endif //INSTALL_OPERATOR_XOVER_3BAND
   // ************************************
...
xover_processing:
   ...
   // ***************************************************
   // check if 3-band and do 3-band processing for BW/LR
   // ***************************************************
#ifdef INSTALL_OPERATOR_XOVER_3BAND
   r0 = M[r8 + $audio_proc.xover_2band.CAP_CONFIG_FIELD];
   Null = r0 - $audio_proc.xover_2band.XOVER_3BAND;
   if Z jump bw_lr_3band;
#endif //INSTALL_OPERATOR_XOVER_3BAND
...
bw_lr_3band:
   ...
   // get write pointer, size and start address of high_freq output cbuffer
   // **********************************************************************
   r0 = M[r8 + $audio_proc.xover_2band.INTERNAL_ADDR_FIELD_0];
   call $audio_proc.xover_2band.get_output_cbuffer_details;
   ...
   // ****************************************************************
   // ****************************************************************
   //                     F2 LOW FREQUENCY OUTPUT
   //
   // (input is intermediate PROC_F2 buffer)
   //
   // ****************************************************************
   // ****************************************************************
   ...
   // get write pointer, size and start address of low_freq output cbuffer
   // **********************************************************************
   r0 = M[r8 + $audio_proc.xover_2band.OUTPUT_ADDR_FIELD_MID];
   call $audio_proc.xover_2band.get_output_cbuffer_details;

   // ****************************************************************
   // call the peq process function for f2 low_frequency component
   // ****************************************************************
   r4 = M[r8 + $audio_proc.xover_2band.SAMPLES_TO_PROCESS];
   r7 = M[r8 + $audio_proc.xover_2band.PEQ_OBJECT_PTR_LOW_FREQ_F2];
   pushm <r3,r8>;
...
apc_2band:
   ...
   // **********************************************************************
   // get write pointer, size and start address of internal cbuffer 0
   // **********************************************************************
   r0 = M[r8 + $audio_proc.xover_2band.INTERNAL_ADDR_FIELD_0];
   call $audio_proc.xover_2band.get_output_cbuffer_details;
   ...
   // **********************************************************************
   // get write pointer, size and start address of internal cbuffer 1
   // **********************************************************************
   r0 = M[r8 + $audio_proc.xover_2band.INTERNAL_ADDR_FIELD_1];
   call $audio_proc.xover_2band.get_output_cbuffer_details;
   ...
   // *************************************************************
   // set the band multipliers based on the the user input: Band 1
   // *************************************************************
   r2 = -1.0;
   r5 = 1.0;
   r6 = 1.0;
   ...
   // **************************************
   // Don't invert band 2 if in 3-band mode
   // **************************************
#ifdef INSTALL_OPERATOR_XOVER_3BAND
   r0 = M[r8 + $audio_proc.xover_2band.CAP_CONFIG_FIELD];
   Null = r0 - $audio_proc.xover_2band.XOVER_3BAND;
   if Z jump apc_2band_processing;
#endif //INSTALL_OPERATOR_XOVER_3BAND

   // *************************************************************
   // set the band multipliers based on the the user input: Band 2
   // *************************************************************
   r0 = M[r7 + $audio_proc.xover_2band.parameter.XOVER_CONFIG];
   r1 = r0 AND $M.XOVER.CONFIG.INV_BAND2;
   if NZ r6 = r2;

apc_2band_processing:
   // *********************************************
   // generate the low_freq and high_freq outputs
   //  internal cbuffer 0 corresponds to low freq
   //  internal cbuffer 1 corresponds to high freq
   // *********************************************
   push r3; // Preserve PEQ function pointer for 3-band processing
   ...
   // ***************************
   // Check for 3-band operation
   // ***************************
#ifdef INSTALL_OPERATOR_XOVER_3BAND
   r0 = M[r8 + $audio_proc.xover_2band.CAP_CONFIG_FIELD];
   Null = r0 - $audio_proc.xover_2band.XOVER_3BAND;
   if Z jump apc_3band_processing;
#endif //INSTALL_OPERATOR_XOVER_3BAND

   // ********************************************
   // 2-band: copy internal buffers to the output
   // ********************************************
   r0 = M[r8 + $audio_proc.xover_2band.INTERNAL_ADDR_FIELD_0];
   call $audio_proc.xover_2band.get_input_cbuffer_details;
...
#ifdef INSTALL_OPERATOR_XOVER_3BAND
apc_3band_processing:
   // ***************************************************************
   //
   //              3-BAND APC FILTERING
   //
   // - Take the low output and pass it through the compensation PEQ
   // - Take the high output and do the same APC split on it again
   //
   // ***************************************************************

   // ************************************************************
   // 3-band F1 delay compensation
   // ************************************************************
   ...
   // **********************************************************************
   // get write pointer, size and start address of internal buffer 2
   // **********************************************************************
   r0 = M[r8 + $audio_proc.xover_2band.INTERNAL_ADDR_FIELD_2];
   call $audio_proc.xover_2band.get_output_cbuffer_details;
   ...
   // **********************************************************************
   // get write pointer, size and start address of internal buffer 3
   // **********************************************************************
   r0 = M[r8 + $audio_proc.xover_2band.INTERNAL_ADDR_FIELD_3];
   call $audio_proc.xover_2band.get_output_cbuffer_details;
   ...
   // ******************************************************************
   // set the band multipliers based on the the user input: Bands 2 & 3
   // ******************************************************************
   r2 = -1.0;
   r5 = 1.0;
   r6 = 1.0;
   ...
   // ************************************
   // Copy internal buffers to the output
   // ************************************
   r0 = M[r8 + $audio_proc.xover_2band.INTERNAL_ADDR_FIELD_2];
   call $audio_proc.xover_2band.get_input_cbuffer_details;
