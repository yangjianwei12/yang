...
$sco_decoder.wbs.validate:
   ...
   if NZ jump havesync;
      // go back to previous word, this will allow sync-words to cross SCO boundaries
      // Note: This is with the assumption that the buffer doesn't convey SCO
      // in band meta-data, i.e. suitable for new SCO framework only.
      r0 = M[I0, -MK1];
      r5 = r5 + 2;
      ...
      // Before validate is called the wrapper verifies if there is enough data
      // however, if the read offset is 1 findsync assumes that a word is missing
      // since it sees the sync in odd (not word aligned) place.
      // Account for the read offset.
      r5 = r5 + r8;
...
// *****************************************************************************
// FUNCTION:
//    $sco_decoder.wbs.octets_to_sync
//
// DESCRIPTION:
//    Rerturns the distance in octets to the next WBS sync.
//    It only looks in the next WBS_ENCODED_FRAME_SIZE_B octets
//    so if the functions return WBS_ENCODED_FRAME_SIZE_B no sync was found.
//
// INPUT:
//    r0 : Pointer to Cbuffer/
//
// OUTPUT:
//    r0: octets to next sync words.
//
// TRASHED REGISTERS:
//    r1,r2,r3,r10,I1,L1,B1,loop
//
// CPU USAGE:
//    D-MEMORY: xxx
//    P-MEMORY: xxx
...
// NOTES:
// *****************************************************************************
.MODULE $M.sco_decoder.wbs.octets_to_sync;

   .DATASEGMENT DM;
   .CODESEGMENT SBCDEC_WBS_OCTETS_TO_SYNC_PM;

$_wbs_octets_to_sync:

   pushm <r4, r5, r6, r9, rLink>;
   pushm <I1, M1, M2, M3>;
   ...
   r6 = r1; // save the offset for later use.
   L1 = r2; // set the length register
   I1 = r0; // set the index register.
#ifdef BASE_REGISTER_MODE
   push r3;
   pop B1; // The base register needs to be popped.
#endif
   M1 = ADDR_PER_WORD;
   r5 = $wbs.decoder.WBS_ENCODED_FRAME_SIZE_B;
   ...
   // no sync
   r0 = $wbs.decoder.WBS_ENCODED_FRAME_SIZE_B;
   jump exit_wbs_octets_to_sync;


foundsync_wbs:
   // have sync. word.
   // recalculate number of bytes, including wbs header sync words
   r1 = r10 LSHIFT 1; // convert words left to octets.
   r1 = r1 - r4; // substract the sync alignment.
   r0 = $wbs.decoder.WBS_ENCODED_FRAME_SIZE_B - r1; // calculate the octets to sync.
   r0 = r0 - r6; // account for the read offset.

exit_wbs_octets_to_sync:
   // clear the length and base register.
   L1 = 0;
#ifdef BASE_REGISTER_MODE
   push Null;
   pop  B1;
...
//    r1    : cloned cbuffer structure for payload buffer 
