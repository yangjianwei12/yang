...
typedef struct __attribute__((__packed__))
{
    ...
    */
    uint64_t serial;
    /* The position of dfu_reboot is reserved here, and any change of this
     * position would lead to compilation error because of the static assert
     * placed below
     */
#ifdef DFU_EARBUD_IF
    uint8_t dfu_reboot;
#else
    /* This field is currently reserved for CB build variant because of the
     * limitation in build system, and because of no support of DFU_EARBUD_IF
     * in CB variant.
     */
    uint8_t reserved;
#endif
...
/*
 * The serial number must be the first item stored as this persists between changes in CONFIG_DATA
 */
_Static_assert(offsetof(CONFIG_DATA, serial) == 0, "Serial number must be the first member stored in the flash configuration.");

#ifdef DFU_EARBUD_IF

/* This dfu_reboot byte addr is reserved currently, and if it modified, it will
 * lead to compilation error
 */
_Static_assert(FLASH_CFG_START + offsetof(CONFIG, data.dfu_reboot) == DFU_REBOOT_BYTE_ADDR, "Change of dfu_reboot byte addr not allowed");
#endif 
/*-----------------------------------------------------------------------------
------------------ PROTOTYPES -------------------------------------------------
-----------------------------------------------------------------------------*/
...
* Get dfu reboot mode.
*/
uint8_t config_get_dfu_reboot(void)
{
    return config.data.dfu_reboot;
}

/*
* Set dfu reboot mode.
*/
bool config_set_dfu_reboot(uint8_t mode)
{
    config.data.dfu_reboot = mode;
    return config_write();
}
#endif
