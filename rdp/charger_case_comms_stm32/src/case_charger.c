...
/**
 * Time in microsecond to wait for a USB enumeration after a charger has been
 * connected.
 */
#define CASE_CHARGER_USB_ENUMERATION_TIMEOUT (2000 * 1000)

/**
 * Period in microseconds to check the battery temperature and adjust the charge
 * current as needed
 */
#define CASE_CHARGER_MONITOR_PERIOD (2000 * 1000)
 ...
 */
static bool case_charger_delay_finished(void)
{
    return global_time_us >= case_charger_status.delay_time;
}
 ...
 */
static void case_charger_next_state(CASE_CHARGER_STATE state,
                                      uint64_t us_to_wait)
{
    case_charger_status.state = state;
    case_charger_status.delay_time = global_time_us + us_to_wait; 
}
 ...
 */ 
bool case_charger_can_trust(void)
{
    /* Wait at least 400ms from power up before trusting the charger. */
    return global_time_us > (400 * 1000);
}
...
static void case_charger_monitor_battery_temp(void)
{
     ...
     */
    if (ntc >= BATTERY_THERMISTOR_MIN_VALUE)
    {
        /* < 0C or > 45C */
        if (ntc > BATTERY_THERMISTOR_VOLTAGE_0_C || ntc < BATTERY_THERMISTOR_VOLTAGE_45_C)
        {
            mode = CHARGER_CURRENT_MODE_STANDBY;
            temperature_ok = false;
        }
        /* < 15C and > 0C */
        else if (ntc >= BATTERY_THERMISTOR_VOLTAGE_15_C && ntc < BATTERY_THERMISTOR_VOLTAGE_0_C)
        {
...
static void charger_detect_inserted_periodic(void)
{
    switch(case_charger_status.state)
    {
        ...
        case CASE_CHARGER_CONNECTED:
            ...
            case_charger_next_state(CASE_CHARGER_WAITING, CASE_CHARGER_USB_ENUMERATION_TIMEOUT);
            break;
        ...
        case CASE_CHARGER_DISCONNECTED:
            ...
            case_charger_next_state(CASE_CHARGER_IDLE, 0);
        ...
        case CASE_CHARGER_WAITING:
            if (case_charger_delay_finished() || usb_has_enumerated())
            {
                case_charger_next_state(CASE_CHARGER_FINISH, 0);
            }
            break;
        ...
        case CASE_CHARGER_FINISH:
            if (charger_connected())
            {
                ...
                case_charger_next_state(CASE_CHARGER_MONITORING, 0);
            }
            break;
        case CASE_CHARGER_MONITORING:
            if (case_charger_delay_finished())
            {
                /* Monitor the battery temperature and adjust the charger if needed. */
                case_charger_monitor_battery_temp();

                /* Stay in this state and repeat after a delay. */
                case_charger_next_state(CASE_CHARGER_MONITORING, CASE_CHARGER_MONITOR_PERIOD);
            }
            break;
    }
}
