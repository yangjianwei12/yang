...
int main(void)
{
    ...
    if (vta->image_count==0xFFFFFFFF)
    {
        ...
        {
            if(DFU_REBOOT_BYTE == 0x01 || DFU_REBOOT_BYTE == 0x02)
            {
                /* If it is a DFU reboot, then load from the alternate image bank
                 * whose image count is 0xFFFFFFFF
                 */
                use_a = true;
            }
            else
    ...
    else if (vtb->image_count==0xFFFFFFFF)
    {
        if(DFU_REBOOT_BYTE == 0x01 || DFU_REBOOT_BYTE == 0x02)
        {
           /* If it is a DFU reboot, then load from the alternate bank whose 
            * image count is 0xFFFFFFFF
            */
            use_b = true;
        }
        else
