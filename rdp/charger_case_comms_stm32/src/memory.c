...
typedef struct
{
    ...
    struct
    {
        ...
        /** Either enable or disable the voltage regulator after STOP. */
        bool enable_vbus_after_low_power : 1;
...
void mem_cfg_vbus_enabled_set(bool enabled)
{
    ram_cfg.data.enable_vbus_after_low_power = enabled;
    mem_cfg_update();
}

bool mem_cfg_vbus_should_be_enabled(void)
{
    return ram_cfg.data.enable_vbus_after_low_power;
}
...
void mem_cfg_clear_low_power(void)
{
    ram_cfg.data.enter_stop = false;
    ram_cfg.data.enter_standby = false;
    mem_cfg_update();
}
