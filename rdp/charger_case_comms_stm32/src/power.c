...
static VBUS_AFTER_STOP power_vbus_after_stop = VBUS_AFTER_STOP_ENABLED;
...
void power_enter_stop_after_reset(void)
{
    ...
    /* Additional setup depending on if we want the voltage regulator to
     * be enabled during STOP mode. */
    if (mem_cfg_vbus_should_be_enabled())
    {
        vreg_enable();
    }
    else
...
void power_reset_to_stop(VBUS_AFTER_STOP vbus_state)
{
    rtc_disable_alarm();
    mem_cfg_vbus_enabled_set(vbus_state == VBUS_AFTER_STOP_ENABLED);
...
void power_set_vbus_after_stop(VBUS_AFTER_STOP vbus_state)
{
    power_vbus_after_stop = vbus_state;
}
...
void power_periodic(void)
{
    if (!power_reason_to_run)
    {
        ...
        */
        ...
        {
            power_reset_to_stop(power_vbus_after_stop);
        }
