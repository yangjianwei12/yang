...
/*
* LED_BATTERY_LOW - any percentage below which we consider the battery level
* to be low.
*/
#define LED_BATTERY_LOW 30

/**
 * The number of microseconds between each LED tick.
 */
#define LED_TICK_DURATION_US 10000
...
static const LED_SEQUENCE led_seq_battery_charging_normal =
{
    LED_SEQ_FOREVER, 2,
    {
        { LED_COLOUR_GREEN, 50 },
...
/**
 * \brief Reset the tick which drives all LED sequences.
 */
static void led_reset_tick(void)
{
    next_tick = global_time_us + LED_TICK_DURATION_US;
}
            ...
            if (charger_connected())
            {
                ...
                else
                {
                    ...
                    if (charger_is_charging())
                    {
                        ...
                        else if (bpc >= LED_BATTERY_LOW)
                        {
                            wanted_seq = &led_seq_battery_charging_normal;
                        }
                        else
                        {
