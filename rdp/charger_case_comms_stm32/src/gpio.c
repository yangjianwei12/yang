...
static void gpio_input_pu(uint16_t pin)
{
    GPIO_InitTypeDef init_struct;
...
void gpio_prepare_for_stop(void)
{
     ...
     */
    GPIOA->MODER = GPIO_MODER_IN(GPIO_MAG_SENSOR) & GPIO_MODER_OUT(GPIO_VREG_EN);
    GPIOB->MODER = GPIO_MODER_OUT(GPIO_VREG_MOD);
     ...
     * Keep
     * - GPIO_A0 as input (GPIO_MAG_SENSOR)
     * - GPIO_A1 as input (GPIO_LOAD_FAULT_N)
     * - GPIO_A5 as output (GPIO_CHG_CE_N)
     ...
     * All other GPIOs put into analogue mode
     */
    GPIOA->MODER = GPIO_MODER_IN(GPIO_MAG_SENSOR, GPIO_LOAD_FAULT_N)
                 & GPIO_MODER_OUT(GPIO_CHG_CE_N, GPIO_VREG_ISO);
    GPIOB->MODER = GPIO_MODER_OUT(GPIO_DOCK_PULL_EN, GPIO_VREG_EN, GPIO_VREG_SEL);
...
#elif defined(HAVE_EARBUD_SWITCHES)
    /* Same as above + switches */
    GPIOA->MODER = GPIO_MODER_IN(GPIO_MAG_SENSOR, GPIO_LOAD_FAULT_N, GPIO_SWITCH_L,
                                 GPIO_SWITCH_R)
                 & GPIO_MODER_OUT(GPIO_CHG_CE_N, GPIO_VREG_ISO);

    GPIOB->MODER = GPIO_MODER_OUT(GPIO_DOCK_PULL_EN, GPIO_SWITCH_EN, GPIO_VREG_EN,
                                  GPIO_VREG_SEL);
...
void gpio_init_after_stop(void)
{
    ...
    */
    ...
    /* Output from load switch*/
    gpio_input_pu(GPIO_LOAD_FAULT_N);
#endif
    ...
    // switches
    gpio_input_pu(GPIO_SWITCH_L);
    gpio_input_pu(GPIO_SWITCH_R);
    gpio_output(GPIO_SWITCH_EN);
#endif
}
