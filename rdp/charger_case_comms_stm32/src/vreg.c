...
{
#if defined(HAVE_LOAD_SWITCH)
    return !gpio_active(GPIO_LOAD_FAULT_N); 
#else
    return vreg_is_switched_on();
#endif
}
    
bool vreg_is_switched_on(void)
{
    return gpio_active(GPIO_VREG_EN);
}

CLI_RESULT atq_regulator(uint8_t cmd_source)
{
    PRINTF("%u,%u", vreg_is_switched_on(), vreg_is_enabled());
    return CLI_OK;
}
