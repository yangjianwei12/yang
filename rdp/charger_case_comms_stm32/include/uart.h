...
/**
 * \brief Set the baud rate of a particular UART.
 *
 * \param uart_no The UART to transmit on.
 * \param rate The baud rate to set this port to.
 */
void uart_set_baud_rate(UART_ID uart_no, uint32_t rate);

/**
 * \brief Set the baud rate of a particular UART.
 * \param cmd_source unused.
 *
 * \return CLI_OK if the command was accepted, otherwise CLI_ERROR.
 */
