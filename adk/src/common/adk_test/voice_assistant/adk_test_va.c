...
{
    DEBUG_LOG_DEBUG("appTestSetVaPrivacyMode %d", enabled);
    VoiceUi_SetPrivacyModeEnable(enabled);
}

bool appTestIsVaPrivacyModeEnabled(void)
{
    bool enabled = VoiceUi_IsPrivacyModeEnabled();
    DEBUG_LOG_DEBUG("appTestIsVaPrivacyModeEnabled %d", enabled);
    return enabled;
}
