...
\file
\defgroup   cc_case_channel Case Communications Case Channel
...
/*! \brief Send Earbud Command to enter handset pairing mode.
    \param dest Left or right earbud destinations only are valid.
    \return bool TRUE if packet was accepted for TX to the Earbud, FALSE otherwise. 

    \note This command is for use on the Case side only.
 */
bool CcCaseChannel_HandsetPairingCmdTx(cc_dev_t dest);

/*! \brief Send an Earbud Command Response message to the handset pairing command.
    \param cmd_accepted TRUE if Earbud will enter handset pairing mode.
                        FALSE otherwise.
...
/*!
 * \brief Send a case command which is equivalent to a no-operation in response
 *        to a case command request.
 * \return TRUE if packet was accepted for TX to the Case, FALSE otherwise.
 *
 * \note Expected usage is on the Earbuds only.
 */
...
/*!
 * \brief Send a case command to request the case serial number in response
 *        to a case command request.
 * \return TRUE if packet was accepted for TX to the Case, FALSE otherwise.
 *
 * \note Expected usage is on the Earbuds only.
