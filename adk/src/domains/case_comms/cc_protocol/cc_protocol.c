...
/* Prevent dormant from ccProtocol */
static void ccProtocol_SetDormantFlag(cc_dormant_inhibit_t req)
{
    cc_protocol_t* td = CcProtocol_GetTaskData();
    if (req & cc_dormant_mask)
    {
        td->cc_allow_dormant |= req;
        appPowerClientProhibitSleep(CcProtocol_GetTask());
    }
}

/* Potentially re-allow dormant from ccProtocol */
static void ccProtocol_ClearDormantFlag(cc_dormant_inhibit_t req)
{
    cc_protocol_t* td = CcProtocol_GetTaskData();
    if (req & cc_dormant_mask)
    {    
        td->cc_allow_dormant &= ~req;
        if (td->cc_allow_dormant == 0)
        {
            /* Allow dormant if all CC users are happy */
            appPowerClientAllowSleep(CcProtocol_GetTask());
        }
    }
}

static void ccProtocol_ResetVeto(void)
{
    MessageCancelFirst(CcProtocol_GetTask(), CC_PROTOCOL_INTERNAL_TIMEOUT_PROHIBIT_SLEEP);
    ccProtocol_ClearDormantFlag(cc_comms_timeout_dorm);
}

/* Prevent sleep for a period to allow any possible comms messages to complete */
static void ccProtocol_StartSleepVetoTimeout(void)
{
    MessageCancelFirst(CcProtocol_GetTask(), CC_PROTOCOL_INTERNAL_TIMEOUT_PROHIBIT_SLEEP);
    ccProtocol_SetDormantFlag(cc_comms_timeout_dorm);
    MessageSendLater(CcProtocol_GetTask(), CC_PROTOCOL_INTERNAL_TIMEOUT_PROHIBIT_SLEEP,
                     NULL, CcProtocol_ConfigProhibitSleepTimeout());    
}

static void ccProtocol_RxInd(void)
{
    ccProtocol_StartSleepVetoTimeout();
    /* Any other RX processing */
}
    ...
    if (ccProtocol_TransRequiresReset())
    {
        switch (status)
        {
            ...
            case CASECOMMS_TX_TIMEOUT:
                /* don't reset for a simple timeout, which happens
                   when earbud is not present */
                break;
            case CASECOMMS_TX_FAIL:
                if (++td->tx_fail_count == CcProtocol_ConfigNumFailsToReset())
                {
                    DEBUG_LOG_V_VERBOSE("ccProtocol_CheckAckNakReset SeqNum Reset");
                    if (!ccProtocol_TransReset())
                    {
                        DEBUG_LOG_WARN("ccProtocol_CheckAckNakReset reset fail, already in progress?");
                    }
                    td->tx_fail_count = 0;
                }
                break;
...
static void ccProtocol_HandlePhyStateChangedInd(const PHY_STATE_CHANGED_IND_T *ind)
{
    cc_protocol_t* td = CcProtocol_GetTaskData();

    /* when earbuds come out of the case and we're using transport that requires
       reset (scheme B) send a sequence number reset message, which will be sent
       to the case when earbud is put back in the case */
    if (   ind->event == phy_state_event_out_of_case
        && ccProtocol_TransRequiresReset()
        && td->mode == CASECOMMS_MODE_EARBUD)
    {
        if (!ccProtocol_TransReset())
        {
            DEBUG_LOG_WARN("ccProtocol_TransReset reset attempt failure, already in progress?");
        }
    }
}
...
static void CcProtocol_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case MESSAGE_CHARGERCOMMS_UART_STATUS:
            ...
            break;
        case MESSAGE_CHARGERCOMMS_UART_PRI_STATUS:
            DEBUG_LOG_ALWAYS("CcProtocol_HandleMessage MESSAGE_CHARGERCOMMS_UART_PRIORITY_STATUS %d", ((const MessageChargerCommsUartPriStatus*)message)->status);
            break;
#endif
        ...
        case PHY_STATE_CHANGED_IND:
            ccProtocol_HandlePhyStateChangedInd((const PHY_STATE_CHANGED_IND_T*)message);
            break;

        case CC_PROTOCOL_INTERNAL_TIMEOUT_PROHIBIT_SLEEP:
           ccProtocol_ClearDormantFlag(cc_comms_timeout_dorm);
           break;

        case APP_POWER_SLEEP_PREPARE_IND:
            DEBUG_LOG_INFO("CcProtocol_HandleMessage APP_POWER_SLEEP_PREPARE_IND");
            appPowerSleepPrepareResponse(CcProtocol_GetTask());
            break;

        case APP_POWER_SHUTDOWN_PREPARE_IND:
            DEBUG_LOG_INFO("CcProtocol_HandleMessage APP_POWER_SHUTDOWN_PREPARE_IND");
            appPowerShutdownPrepareResponse(CcProtocol_GetTask());
            break;
...
void CcProtocol_Init(cc_mode_t mode, cc_trans_t trans)
{
    ...
    appPowerClientRegister(CcProtocol_GetTask());
    appPowerClientAllowSleep(CcProtocol_GetTask());  /* The transports are initialised after this, so sleep can still be vetoed */

    if (mode == CASECOMMS_MODE_EARBUD)
    {
        /* Register for notification if the earbud is out of the case */
        appPhyStateRegisterClient(CcProtocol_GetTask());
    }
...
{
    ccProtocol_SetDormantFlag(cc_transport_dorm);
}
...
void CcProtocol_ConfigureAsWakeupSource(void)
{
    ...
    {    
        uint16 bank = PioCommonPioBank(CcProtocol_ConfigSchemeBTxRxPio());
        uint32 mask = PioCommonPioMask(CcProtocol_ConfigSchemeBTxRxPio());
        
        PanicFalse(DormantConfigure(LED_WAKE_MASK, 1 << (CASE_COMMS_LED)));
        /* We want to wake on some activity on the Charger Comms line.
         * While making this decision, the charging case may be providing
         * a 5V power source, or it may have removed this in order to
         * save current.
         * We need to configure the charger comms line to wake us when its
         * state changes. The line is by default high (as it is shared with
         * VCHG, so we need to invert it to be woken on comms activity.
         */
        if (PioGet32Bank(bank) & mask) 
        {
