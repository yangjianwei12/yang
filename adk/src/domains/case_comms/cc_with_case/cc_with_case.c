...
/*! \brief Clear the pending case command.
 */
static void CcWithCase_ClearPendingCaseCommand(void)
{
    cc_with_case_t* case_td = CcWithCase_GetTaskData();
    free(case_td->current_command);
    case_td->current_command = NULL;
}
...
bool CcWithCase_Init(Task init_task)
{
    ...
    /* Initialise the current earbud->case command */
    CcWithCase_ClearPendingCaseCommand();
...
{
    cc_with_case_t* case_td = CcWithCase_GetTaskData();
    bool generated_pairing_request = FALSE;

    if (   appPhyStateGetState() == PHY_STATE_IN_CASE
        && case_td->lid_state == CASE_LID_STATE_OPEN)
    {
        Ui_InjectUiInput(ui_input_sm_pair_handset);
        generated_pairing_request = TRUE;
    }

    CcCaseChannel_HandsetPairingCmdRespTx(generated_pairing_request);
}

bool ccWithCase_HavePendingCommand(void)
{
    cc_with_case_t* case_td = CcWithCase_GetTaskData();
    return case_td->current_command != NULL;
}

static bool ccWithCase_RequestCaseCommand(Task task, case_cmd_t cmd, void *params, unsigned params_length)
{
    cc_with_case_t* case_td = CcWithCase_GetTaskData();
    cc_with_case_current_command_t *pending_cmd;

    if (case_td->current_command)
    {
        return FALSE;
    }

    pending_cmd = malloc(sizeof(cc_with_case_current_command_t));
    if(!pending_cmd)
    {
        return FALSE;
    }

    pending_cmd->requester = task;
    pending_cmd->command = cmd;
    pending_cmd->command_params = params;
    pending_cmd->params_length = params_length;

    case_td->current_command = pending_cmd;

    return TRUE;
}

bool ccWithCase_RequestCaseSerialNumber(Task task)
{
    return ccWithCase_RequestCaseCommand(task, CASE_CMD_RX_CASE_SERIAL_NUMBER, NULL, 0);
}
...
void ccWithCase_CaseCommandRequestRx(void)
{
    ...
    DEBUG_LOG_VERBOSE("ccWithCase_CaseCommandRequestRx");

    if (case_td->current_command)
    {
        command = case_td->current_command->command;
    }
    else
    {
        command = CASE_CMD_NONE;
    }

    switch(command)
    {
    case CASE_CMD_RX_CASE_SERIAL_NUMBER:
        CcCaseChannel_CaseCommandSerialNumberTx();
        break;
    case CASE_CMD_NONE:
    default:
        CcCaseChannel_CaseCommandNoneTx();
        break;
    }
}

/* \brief Notify the last task which requested a case command the response.
    \param task The task to send the response to.
    \param serial_number The serial number data from the case.
    \param length The size of the serial number data in octets.
    \return None
*/
static void CcWithCase_ClientMsgSerialNumber(Task task, const uint8_t *serial_number, unsigned length)
{
    if (length != CASE_SERIAL_NUMBER_SIZE)
    {
        return;
    }

    MAKE_CCWC_MESSAGE(CASE_SERIAL_NUMBER);
    memcpy(message->serial_number, serial_number, CASE_SERIAL_NUMBER_SIZE);

    MessageSend(task, CASE_SERIAL_NUMBER, message);
}
...
void CcWithCase_CaseSerialNumberCmdRx(const uint8_t *serial_number, unsigned length)
{
    ...
    DEBUG_LOG_VERBOSE("CcWithCase_CaseSerialNumberCmdRx");

    if (!case_td->current_command)
    {
        DEBUG_LOG_WARN("CcWithCase_CaseSerialNumberCmdRx: Unexpected response");
        return;
    }

    if (case_td->current_command->requester)
    {
        CcWithCase_ClientMsgSerialNumber(case_td->current_command->requester, serial_number, length);
    }
    CcWithCase_ClearPendingCaseCommand();
}
