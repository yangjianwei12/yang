...
\file       cc_with_case.h
\defgroup   cc_with_case Case Communications With Case
\ingroup    case_comms_domain
...
/*! The total size of the Case serial number used in the #CASE_SERIAL_NUMBER_T message. */
#define CASE_SERIAL_NUMBER_SIZE    (8)
...
typedef enum
{
    ...
    /*! Notification about the case serial number */
    CASE_SERIAL_NUMBER,
...
typedef struct
{
    uint8_t serial_number[CASE_SERIAL_NUMBER_SIZE];
} CASE_SERIAL_NUMBER_T;

/*! \brief Types of Case Command. */
typedef enum
{
    /*! No command to send, abort the request. */
    CASE_CMD_NONE                   = 0,

    /*! Request the serial number from the case. */
    CASE_CMD_RX_CASE_SERIAL_NUMBER  = 1,
} case_cmd_t;
...
/*! \brief Handle command to start handset pairing.

    \note Earbud will send a handset pairing command response to inficate
          to the case if the handset pairing command was accepted and the
          earbud(s) will enter handset pairing mode.

          The earbud must be in the case and with the lid open to accept
          the handset pairing command, and a ui_input_sm_pair_handset
          event will be generated, to trigger the application to start
          handset pairing.
*/
void CcWithCase_HandsetPairingCmdRx(void);

/*! \brief Determine if the earbud wishes to transmit a case command to a
           charger case
    \return bool TRUE if a case command is pending, FALSE otherwise.
*/
bool ccWithCase_HavePendingCommand(void);

/*! \brief Handle a case command request from the case.
    
    \note The case will usually send this if this earbud has requested a
          command to be sent by setting the 
          #EARBUD_STATUS_FLAGS_COMMAND_REQUESTED_MASK flag in the simple earbud
          status response.
*/
void ccWithCase_CaseCommandRequestRx(void);

/*! \brief Request the serial number of a case.
    \param client_task The task to send the serial number response to.
    \return bool TRUE if this request has been accepted, FALSE otherwise.

    \note Only one request can be made at a time.
*/
bool ccWithCase_RequestCaseSerialNumber(Task client_task);

/*! \brief Handle a serial number response from the case.
    
    \note This is the respsonse to the #CASE_CMD_RX_CASE_SERIAL_NUMBER command.
*/
void CcWithCase_CaseSerialNumberCmdRx(const uint8_t *serial_number, unsigned length);
...
#define CcWithCase_HandsetPairingCmdRx()
#define ccWithCase_HavePendingCommand()                (FALSE)
#define CcWithCase_CaseSerialNumberCmdRx(_serial_number, \
                                         _length) (UNUSED(_serial_number), UNUSED(_length))
#define ccWithCase_RequestCaseSerialNumber(_client_task) (UNUSED(_client_task))
#define ccWithCase_CaseCommandRequestRx()
