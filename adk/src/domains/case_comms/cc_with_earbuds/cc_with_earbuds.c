...
\file       cc_with_earbuds.c
\ingroup    cc_with_earbud
\brief      
*/
/*! @{ */
...
{
    CcCaseChannel_HandsetPairingCmdTx(CASECOMMS_DEVICE_LEFT_EB);
    CcCaseChannel_HandsetPairingCmdTx(CASECOMMS_DEVICE_RIGHT_EB);
}

void CcWithEarbuds_HandsetPairingResponseRx(cc_dev_t source, bool handset_pairing_accepted)
{
    DEBUG_LOG_INFO("CcWithEarbuds_HandsetPairingResponseRx enum:cc_dev_t:%d handset_pairing_accepted %d", source, handset_pairing_accepted);
}
