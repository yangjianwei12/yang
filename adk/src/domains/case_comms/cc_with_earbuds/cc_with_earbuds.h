...
\file       cc_with_earbuds.h
\defgroup   cc_with_earbud Case Communications With Earbud
\ingroup    case_comms_domain
\brief      Public interface to the case domain, managing interactions with an earbud.
@{
*/
...
/*! \brief Send indication to case about state of request to enter handset pairing mode.
    \param source Casecomms device address.
    \param handset_pairing_accepted TRUE if #source Earbud has accepted command and will start handset pairing.
                                  FALSE otherwise.
*/
void CcWithEarbuds_HandsetPairingResponseRx(cc_dev_t source, bool handset_pairing_accepted);

/*! \brief Initiate handset pairing on Earbuds by sending command over case comms.
 
    \note Earbuds must be in the case and the case lid open for the handset
          pairing command to be accepted and them to enter handset pairing
          mode. A handset pairing command is sent to both left and right
          earbuds, only the primary Earbud will perform handset pairing, the
          seconadry will forward the command to the primary. Duplicate command
          is handled gracefully by the earbuds and avoids the case having to
          identify which is currently the primary earbud.

          The CcWithEarbuds_HandsetPairingResponseRx() callback will be called
          by the case channel component for each response from the left and right
          earbuds indicating accept/reject of the handset pairing command.
*/
void CcWithEarbuds_HandsetPairing(void);
...
#define CcWithEarbuds_HandsetPairingResponseRx(_source, _handset_pairing_accepted)  (UNUSED(_source), UNUSED(_handset_pairing_accepted))
#define CcWithEarbuds_HandsetPairing()
