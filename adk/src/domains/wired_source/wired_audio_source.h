...
/*! Maximum number of Analogue Audio Source (Line-in), can be connected, at the same time */
#if defined(INCLUDE_WIRED_ANALOG_AUDIO) || defined(INCLUDE_A2DP_ANALOG_SOURCE) || defined(INCLUDE_LE_AUDIO_ANALOG_SOURCE)
#define WIRED_AUDIO_MAX_ANALOG_SOURCE_SUPPORTED  1
#else
#define WIRED_AUDIO_MAX_ANALOG_SOURCE_SUPPORTED  0
#endif
...
/*! \brief Check if any Wired audio device is routed.
...
#else
#define  WiredAudioSource_IsAudioRouted(source) (FALSE)
#endif

/*! \brief Get a handle to the wired audio device .

    \return The device handle if a wired audio device is found, NULL otherwise.
*/
#if defined(INCLUDE_WIRED_ANALOG_AUDIO) || defined(INCLUDE_A2DP_ANALOG_SOURCE) || defined(INCLUDE_LE_AUDIO_ANALOG_SOURCE)
device_t WiredAudioSource_GetDevice(audio_source_t source);
#else
#define  WiredAudioSource_GetDevice(source) (NULL)
#endif
