...
/*! \brief A table of domain component names.
    ...
    X(MYGROUP,2) will allocate 2 groups of messages to the group, for example.
*/
#define FOREACH_DOMAINS_MESSAGE_GROUP(X) \
    ...
    X(ACCESSORY_TRACKING)   /* Accessory tracking messages */\
    X(TMAP_SERVER)          /* TMAP server messages */\
    X(MICP_SERVER)          /* MICP server messages for mute/unmute */\
    X(LE_DEBUG_SECONDARY)   /* See #le_debug_secondary_msg_t */\
    X(GATT_QSS_SERVER)      /* GATT QSS Server Message base see #gatt_qss_server_msg_t */\
...
#define FOREACH_APPS_MESSAGE_GROUP(X) \
    ...
    X(CHARGER_CASE) \
    X(USB_DONGLE) \
    X(GAMING_HEADSET_ADDON) \

#ifdef INCLUDE_VOICE_UI
#define VOICE_UI_MSG_GRP(x) x
#else
#define VOICE_UI_MSG_GRP(x)
#endif
...
#define FOREACH_UI_INPUTS_MESSAGE_GROUP(X) \
    ...
    VOICE_UI_MSG_GRP(X(UI_INPUTS_VOICE_UI)) \
    X(UI_INPUTS_APP)\
    X(UI_INPUTS_ACCESSORY_TRACKING)\
