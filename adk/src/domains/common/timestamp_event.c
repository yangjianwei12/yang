...
{
    uint16 delta = (uint16)(timestampEvent_Get(end_id) - timestampEvent_Get(start_id));
    DEBUG_LOG_VERBOSE("TimestampEvent_Delta (%d) end_id %d - start_id %d = delta %d",
            timestampEvent_Get(end_id) > timestampEvent_Get(start_id), timestampEvent_Get(end_id), timestampEvent_Get(start_id), delta);
