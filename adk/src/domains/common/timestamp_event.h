...
typedef enum
{
    ...
    /*! Acceptance of HFP call */
    TIMESTAMP_EVENT_HFP_ACCEPT_CALL,

    /*! Used to time code execution (e.g. EXEC_AND_LOG_TIMEDELTA) */
    TIMESTAMP_EVENT_EXEC_TIME,

    /*! A2DP started time */
    TIMESTAMP_EVENT_A2DP_KYMERA_STREAMING,

    /*! Lid Opened time */
    TIMESTAMP_EVENT_LID_OPENED,
...
/*! \brief Execute and log the time it took to execute a code section (block, function etc)
           Example (time function):
               EXEC_AND_LOG_TIMEDELTA(my_function(params);, DEBUG_LOG, "my_function");
               Prints: my_function execution time was 12ms
           Example (time random code section):
               EXEC_AND_LOG_TIMEDELTA(
                   if (!exception)
                   {
                       for(unsigned i; i < ARRAY_DIM(values); i++)
                       {
                           sum+= values[i];
                       }
                   }
                   ...
                   {
                       status = captureException(exception);
                   }
               , DEBUG_LOG_ALWAYS, "Sum values or catch exception");
               Prints: Sum values or catch exception execution time was 5ms
*/
#define EXEC_AND_LOG_TIMEDELTA(code, logger, log_prefix) do{TimestampEvent(TIMESTAMP_EVENT_EXEC_TIME); code## logger##(log_prefix " execution time was %ums", TimestampEvent_DeltaFrom(TIMESTAMP_EVENT_EXEC_TIME));}while(0)
...
/*! \brief Calculate the time difference between two timestamped events
    ...
    \param start_id The id of the first timestamped event (start for timedelta)
    \param end_id The id of the second timestamped event (end for timedelta)

    \return The elapsed time between the first and second events in milli-seconds.
*/
uint32 TimestampEvent_Delta(timestamp_event_id_t start_id, timestamp_event_id_t end_id);

/*! \brief Calculate the time since a timestamped event

    \param start_id The id of the timestamped event (start for timedelta)

    \return The elapsed time since the event in milli-seconds.
*/
...
#define EXEC_AND_LOG_TIMEDELTA(code, logger, log_prefix) do{code##}while(0)

/*! \brief Not implemented */
#define TimestampEvent(id)

/*! \brief Not implemented */
#define TimestampEvent_Delta(start_id, end_id) (0)

/*! \brief Not implemented */
#define TimestampEvent_DeltaFrom(start_id) (0)

/*! \brief Not implemented */
#define TimestampEvent_GetTime(id) (0)
