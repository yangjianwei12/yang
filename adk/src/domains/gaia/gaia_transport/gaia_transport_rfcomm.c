...
static const uint8 *gaiaTransport_RfcommAllocateServiceRecord(const uint8 *record, uint16 size_record, uint8 channel_offset, uint8 channel)
{
    ...
    if (channel == record[channel_offset])
        return record;
#else
    /* ULCONV_TODO: Need understand how record freeing is not attempted for non synergy case.
     *              Synergy case panics with PANIC_HYDRA_INVALID_PRIVATE_MEMORY_POINTER (IPC PFREE)
     */
#endif
...
#ifdef USE_SYNERGY
    link_cfg.trans_info.non_gatt_trans.trans_link_id = 0; /* Automatic server channel allocation */
#else
...
static void gaiaTransport_RfcommError(gaia_transport *t)
{
    ...
    /* Initiate disconnect */
    TransportMgrDisconnect(transport_mgr_type_rfcomm, tr->u.sink);
}
...
#ifdef USE_SYNERGY
        /* Associate transport task with connection id inside synergy connection manager. */
        TransportMgrRegisterTaskWithConnectionId(conn_id);
#endif
...
static void gaiaTransport_RfcommHandleCmPrims(gaia_transport_rfcomm_t *tr, Message message)
{
    ...
    switch (*prim)
    {
        case CSR_BT_CM_SDS_REGISTER_CFM:
            gaiaTransport_RfcommHandleSdpRegisterCfm(tr, (uint16) ((CsrBtCmSdsRegisterCfm *) message)->resultCode,
                                                     ((CsrBtCmSdsRegisterCfm *) message)->serviceRecHandle);
            break;

        case CSR_BT_CM_SDS_UNREGISTER_CFM:
            gaiaTransport_RfcommHandleSdpUnregisterCfm(tr, (uint16) ((CsrBtCmSdsUnregisterCfm *) message)->resultCode);
            break;

        default:
            DEBUG_LOG_ERROR("gaiaTransport_RfcommHandleCmPrims, unhandled message 0x%04x", *prim);
            break;
    }
...
static void gaiaTransport_RfcommHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case CM_PRIM:
            gaiaTransport_RfcommHandleCmPrims(tr, message);
            break;
