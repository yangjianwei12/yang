...
/* Macros necessary for individual commands */
#define SIZE_DEVICE_BD_ADDR (6)

/* System Information size definitions */
/* TX header format is 
 * 00 : Bit 0, 'more info'
 * 01 : Token, current system info "id" being returned
 * 02 : Token, index into "id" being returned (bits 0 to 7)
 * 03 : Token, index into "id" being returned (bits 8 to 15)
 * 04 : Token, unused
 */
#define SYSTEM_INFO_HEADER_SIZE (5)
...
static void gaiaCorePlugin_GetSystemInformation(GAIA_TRANSPORT *t, uint16 payload_length, const uint8 *payload);
...
static gaia_framework_command_status_t gaiaCorePlugin_MainHandler(GAIA_TRANSPORT *t, uint8 pdu_id, uint16 payload_length, const uint8 *payload)
{
    ...
    switch (pdu_id)
    {
        ...
        case get_system_information:
            gaiaCorePlugin_GetSystemInformation(t, payload_length, payload);
            break;
...
static void gaiaCorePlugin_GetSystemInformation(GAIA_TRANSPORT *t, uint16 payload_length, const uint8 *payload)
{
    bool more_data;
    uint32 value;
    uint16 offset, payload_size;
    uint8 *response = NULL, *dest;
    const char *info_str;
    int info_len, copy_len, buf_space;
    gaia_system_information_id_t id_req;
    ...
    if (payload_length > 0)
    {
        ...
        payload_size = MIN(USER_FEATURE_RESPONSE_MAX_TX_PKT_SIZE, ((uint16)value));
        response = (uint8*) PanicNull(malloc(payload_size));
        buf_space = payload_size;
        
        /* Meaning of TOKEN:
        byte  0  : Information ID being fetched (00 = BuildID, 01 = ...
        bytes 1,2: Offset into information
        byte  3  : Unused
        */

        id_req = payload[0];
        offset =(uint16)payload[1] | 
                (uint16)payload[2] << 8;
    
        dest = response+SYSTEM_INFO_HEADER_SIZE;
        buf_space -= SYSTEM_INFO_HEADER_SIZE;
        copy_len = buf_space;
    
        bool consume = TRUE;  /* Keep going until there are no more system_ids to get */
        more_data = TRUE;     /* Inform GAIA Client that we have more data to give */
        /* Build up payload until we reach payload_size; set more_data if necessary */
        while(consume && buf_space > 0)
        {
            switch (id_req)
            {
                case application_build_id:
                    info_str = DeviceInfo_GetBuildId();
                    info_len = strlen(info_str);
                    if (offset == 0)   /* New "system id", fill in the info in the buffer */
                    {
                        *dest++ = id_req;
                        *dest++ = info_len;
                        buf_space -=2;
                    }
                    /* Limit the copy length to remaining size of the dest buffer */
                    if (copy_len > buf_space)
                    {
                        copy_len = buf_space;
                    }
                    /* Limit the copy length to the length remaining in the build ID */
                    if (copy_len + offset > info_len)
                    {
                        copy_len = info_len - offset;
                    }
                    /* Copy from src + offset into response */
                    memcpy(dest, info_str + offset, copy_len);
                    offset += copy_len;
                    dest += copy_len;
                    buf_space -= copy_len;
                    /* If we have consumed all the data for this system id then move to the next */
                    if (offset >= info_len)
                    {
                        id_req++;
                        offset = 0;
                ...
                break;
                default:
                    /* If all system id list data has been copied then break out of the loop;
                     * We also tell the GAIA client to stop asking. */
                    consume = FALSE;
                    more_data = FALSE;
                break;
            }
        ...
        /* Handle the case where the final info ID was totally consumed.
           If we don't catch it here, we send more_data = TRUE but then
           the next request from the client will just receive an empty
           payload. */
        if (id_req == system_information_max)
        {
            more_data = FALSE;
            payload_size -= buf_space;
        }

        /* Fill in the response header */
        response[0] = more_data ? 1 : 0;
        response[1] = id_req;  /* First byte of the token */
        response[2] = offset & 0xff;
        response[3] = (offset >> 8) & 0xff;
        response[4] = 0;

        GaiaFramework_SendResponse(t, GAIA_CORE_FEATURE_ID, get_system_information, payload_size, response);
        free(response);    
    }
    else
    {
        GaiaFramework_SendError(t, GAIA_CORE_FEATURE_ID, get_system_information, 0);
    }    
}
