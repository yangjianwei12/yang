...
typedef struct
{
    ...
    bool_t                      role_change_in_progress;
    ...
    switch (upgradeGaiaPlugin_task.state)
    {
        case UPGRADE_STATE_CONNECTING:
        {
            if (cfm->status == upgrade_status_success)
            {
                ...
                /* In LE DFU usecases, if a handover happens while upgrade is throttled, ensure that the new
                 * PEB throttles the upgrade, otherwise DFU may resume without waiting for it to unthrottle */
                if ((upgradeGaiaPlugin_task.throttled) &&
                    (upgradeGaiaPlugin_task.transport->type == gaia_transport_gatt))
                {
                    /* Throttle it once again */
                    upgrade_BandwithThrottle(upgradeGaiaPlugin_task.throttled);
                }
...
static gaia_framework_command_status_t upgradeGaiaPlugin_UpgradeControl(GAIA_TRANSPORT *transport, uint16 payload_length, const uint8 *payload)
{
    ...
    {
        /* Replying handled to avoid error in some scenario such as when the transport is already disconnected
         * and some packets come after that because of delay
         */
        DEBUG_LOG_VERBOSE("upgradeGaiaPlugin_UpgradeControl, from different transport");
...
static void upgradeGaiaPlugin_RoleChangeStart(GAIA_TRANSPORT *t)
{
    ...
    upgradeGaiaPlugin_task.role_change_in_progress = TRUE;
}
...
#if defined(ENABLE_GAIA_DYNAMIC_HANDOVER) && defined(ENABLE_LE_HANDOVER)
    /* If we are secondary, clear the throttled flag as only primary is responsible for throttle and unthrottle */
    if(is_primary != TRUE)
    {
        upgradeGaiaPlugin_task.throttled = 0;
    }
#endif /* ENABLE_GAIA_DYNAMIC_HANDOVER && ENABLE_LE_HANDOVER */

    upgradeGaiaPlugin_task.role_change_in_progress = FALSE;
}
...
static void upgrade_BandwithThrottle(bool throttle_required)
{
    bool_t role_change_in_progress = upgradeGaiaPlugin_task.role_change_in_progress;
    DEBUG_LOG("upgrade_BandwithThrottle, throttle_required %u, role_change_in_progress %d",
                                         throttle_required, role_change_in_progress);
    upgradeGaiaPlugin_task.throttled = throttle_required;

    /* Check if upgrade transport is connected and any role change is in progress */
    if (upgradeGaiaPlugin_task.transport && !role_change_in_progress)
    {
