...
#ifdef USE_SYNERGY
#define  GAIA_DEBUG_L2CAP_PEER_LINK_SEC_REQ    ((SECL4_IN_SSP | SECL_IN_AUTHENTICATION | SECL_IN_ENCRYPTION) | (SECL4_OUT_SSP | SECL_OUT_AUTHENTICATION | SECL_OUT_ENCRYPTION))

static CsrBtUuid128 service_uuid_gaia_debug_l2cap_peer_link = {UUID_GAIA_DEBUG_PEER_LINK_SERVICE};
#endif
...
static l2cap_manager_status_t gaiaDebugPlugin_GetSdpSearchPattern(const tp_bdaddr *tpaddr, l2cap_manager_sdp_search_pattern_t *sdp_search_pattern)
{
    ...
    memcpy(sdp_search_pattern->service_uuid128, service_uuid_gaia_debug_l2cap_peer_link, sizeof(CsrBtUuid128));
#else
...
#ifdef USE_SYNERGY
    config->security_level = GAIA_DEBUG_L2CAP_PEER_LINK_SEC_REQ;
    config->conftab_length = sizeof(gaia_debug_l2cap_peer_link_conftab);
#else
...
#ifdef USE_SYNERGY
            rsp->conftab_length = sizeof(gaia_debug_l2cap_peer_link_conftab_response);
#else
