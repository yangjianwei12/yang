...
\file       
\ingroup    device_info
\brief      Device Info component to provide context on the device information relevant to the particular 
...
static const char device_build_id[] = _DIC_STRINGIFY_(DEVICE_BUILDID);
...
const char * DeviceInfo_GetBuildId(void)
{
    return device_build_id;
}
