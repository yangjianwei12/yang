...
#else
void appAuthHandleCmEncryptionChangeInd(Task task, const CsrBtCmEncryptChangeInd *ind)
{
    if (ind->transportType == TRANSPORT_BREDR_ACL)
    {
        bdaddr bdAddr;
        BdaddrConvertBluestackToVm(&bdAddr, &ind->deviceAddr);

        if (ind->encryptType == CSR_BT_CM_ENC_TYPE_BREDR_AES_CCM)
        {
            TP_BD_ADDR_T tpAddr;

            DEBUG_LOG("appAuthHandleCmEncryptionChangeInd: link_mode SECURE_CONNECTION");

            tpAddr.addrt.addr = ind->deviceAddr;
            tpAddr.addrt.type = ind->deviceAddrType;
            tpAddr.tp_type    = ind->transportType;

            appDeviceSetLinkMode(&bdAddr, DEVICE_LINK_MODE_SECURE_CONNECTION);

            /* Write the APT value to the controller for the link
             * NB: Timeout is in units of 10ms */
            CmWriteAuthPayloadTimeoutReqSend(task,
                                             tpAddr,
                                             (DEFAULT_BR_EDR_AUTHENTICATED_PAYLOAD_TIMEOUT / 10),
                                             DM_SM_APT_APPLICATION);
        }
        else if (ind->encryptType != CSR_BT_CM_ENC_TYPE_KEY_REFRESH)
        {
            DEBUG_LOG("appAuthHandleCmEncryptionChangeInd: link_mode NO_SECURE_CONNECTION");
            appDeviceSetLinkMode(&bdAddr, DEVICE_LINK_MODE_NO_SECURE_CONNECTION);
        }
    }
}
#endif /* !USE_SYNERGY */
