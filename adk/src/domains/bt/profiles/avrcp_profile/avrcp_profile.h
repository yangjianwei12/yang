...
/*! Convert the AVRCP event ID to a mask usable by the libraries */
#define appAvrcpEventIdToMask(event)                (1 << ((event) - 1))
...
#else
void appAvrcpRemoteControl(avInstanceTaskData *theInst, CsrBtAvrcpPTOpId op_id, CsrBtAvrcpPTState rstate, bool beep, uint16 repeat_ms);
