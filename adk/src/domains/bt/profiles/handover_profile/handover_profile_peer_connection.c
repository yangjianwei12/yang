 ...
 ******************************************************************************/
#ifdef USE_SYNERGY
#define HANDOVER_PROFILE_SEC_REQ    ((SECL4_IN_SSP | SECL_IN_AUTHENTICATION | SECL_IN_ENCRYPTION) | (SECL4_OUT_SSP | SECL_OUT_AUTHENTICATION | SECL_OUT_ENCRYPTION))
#endif /* USE_SYNERGY */
...
#define handoverProfile_IsConnecting(ho_inst) (HandoverProfile_GetState(ho_inst) >= HANDOVER_PROFILE_STATE_CONNECTING_SDP_SEARCH && \
                                                HandoverProfile_GetState(ho_inst) <= HANDOVER_PROFILE_STATE_CONNECTING_LOCAL)
#else
#define handoverProfile_IsConnecting(ho_inst) (HandoverProfile_GetState(ho_inst) >= HANDOVER_PROFILE_STATE_CONNECTING_SDP_SEARCH && \
...
#ifdef USE_SYNERGY
    CsrBtUtilRfcConCancel(ho_inst, ho_inst->sdp_search_data);
    CsrBtUtilSdcRfcDeinit(&ho_inst->sdp_search_data);
#endif /* USE_SYNERGY */
...
static void handoverProfile_EnterInitialising(void)
{
    ...
    /* Register a Protocol/Service Multiplexor (PSM) that will be
       ...
       ends. */
    ConnectionL2capRegisterRequest(&ho_inst->task, L2CAP_PSM_HANDOVER_PROFILE, 0);
}
...
#ifdef USE_SYNERGY
static const CsrBtUuid128 service_uuid_handover = {UUID_HANDOVER_PROFILE_SERVICE};

static uint16 handoverProfile_ExtractPsm(CmnCsrBtLinkedListStruct *sdpTagList)
{
                                               ...
                                               &dummy2))
    {
        if (tmpResult == SDR_SDC_SEARCH_SUCCESS &&
            !memcmp(tmpUuid,
                    &service_uuid_handover,
                    sizeof(service_uuid_handover)))
        {
...
*/
static void handoverProfile_sdpResultHandler(void *inst,
                                             ...
                                             CsrBtSupplier resultSupplier)
{
    ...
    DEBUG_LOG("handoverProfile_sdpResultHandler: SDP query completed, result(0x%04x) supplier(0x%04x)", resultCode, resultSupplier);
    ...
    switch (HandoverProfile_GetState(ho_inst))
    {
        case HANDOVER_PROFILE_STATE_CONNECTING_SDP_SEARCH:
        {
            ...
            if (resultCode == CSR_BT_RESULT_CODE_CM_SUCCESS &&
                sdpTagList)
            {
                psm = handoverProfile_ExtractPsm(sdpTagList);
            }

            if (psm != L2CA_PSM_INVALID)
            { /* L2CAP PSM found */
                ho_inst->remote_psm = psm;
                handoverProfile_ConnectL2cap(&ho_inst->peer_addr);
                HandoverProfile_SetState(HANDOVER_PROFILE_STATE_CONNECTING_LOCAL);
            }
            else
            {
                if (ConManagerIsConnected(&ho_inst->peer_addr) &&
                    ho_inst->sdp_search_attempts < HandoverProfile_GetSdpSearchTryLimit())
                {
                    DEBUG_LOG("sdpResultHandler, retry attempts %d",
                              ho_inst->sdp_search_attempts);

                    CsrBtUtilRfcConCancel(inst, ho_inst->sdp_search_data);
        ...
        default:
        {
            DEBUG_LOG("HandoverProfile_HandleClSdpServiceSearchAttributeCfm, unexpected state 0x%x status 0x%x",
...
static void handoverProfile_ResultHandler(CsrSdcOptCallbackType cbType, void *context)
{
    CsrSdcResultFuncType *params = (CsrSdcResultFuncType *)context;

    if(cbType == CSR_SDC_OPT_CB_SEARCH_RESULT)
    {
        handoverProfile_sdpResultHandler(params->instData, params->sdpTagList, params->deviceAddr, params->resultCode, params->resultSupplier);
    }
}
#endif /* USE_SYNERGY */
...
static void handoverProfile_EnterSdpSearch(void)
{
    ...
    if (!ho_inst->sdp_search_data)
    {
        ho_inst->sdp_search_data = CsrBtUtilSdcInit(handoverProfile_ResultHandler,
                                                  TrapToOxygenTask(&ho_inst->task));
    }
    ...
    BdaddrConvertVmToBluestack(&addr, &ho_inst->peer_addr);

    /* Start the SDP search */
    CsrBtUtilSdcSearchStart((void *) ho_inst,
...
#ifdef USE_SYNERGY
    else if(ho_inst->connect_req_sent)
    {
        DEBUG_LOG("handoverProfile_EnterDisconnecting cancelling pending connect request");
        CsrBtDeviceAddr addr;
        BdaddrConvertVmToBluestack(&addr, &ho_inst->peer_addr);
        CmL2caCancelConnectReqSend(&ho_inst->task, addr, ho_inst->local_psm);
    }
#endif
}
    ...
    switch (HandoverProfile_GetState(ho_inst))
    {
        case HANDOVER_PROFILE_STATE_DISCONNECTED:
        {
            ...
            if (ConManagerIsConnected(&req->peer_addr))
            {
                ...
                if(L2CAP_PSM_HANDOVER_PROFILE == L2CA_PSM_INVALID)
                {
                ...
                {
                    ho_inst->remote_psm = L2CAP_PSM_HANDOVER_PROFILE;
                    handoverProfile_ConnectL2cap(&ho_inst->peer_addr);
                    HandoverProfile_SetState(HANDOVER_PROFILE_STATE_CONNECTING_LOCAL);
                }
            }
...
{
    handover_profile_task_data_t *ho_inst = Handover_GetTaskData();
    DEBUG_LOG("HandoverProfile_HandleInternalCloseSdpReq, state %u", HandoverProfile_GetState(ho_inst));

    if (HandoverProfile_GetState(ho_inst) == HANDOVER_PROFILE_STATE_DISCONNECTED &&
        ho_inst->sdp_search_data)
    {
        CsrBtUtilRfcConCancel(ho_inst, ho_inst->sdp_search_data);
        CsrBtUtilSdcRfcDeinit(&ho_inst->sdp_search_data);
...
{
    uint16_t status = cfm->resultCode, local_psm = cfm->localPsm;
#else
...
{
    uint16_t status = cfm->resultCode;
#else
...
#ifdef USE_SYNERGY
static void handoverProfile_Connected(CsrBtConnId btConnId,
                                      CsrBtResultCode result)
{
    ...
    switch (HandoverProfile_GetState(ho_inst))
    {
        ...
        case HANDOVER_PROFILE_STATE_CONNECTING_REMOTE:
        {
             ...
             * to connected state */
            if (result == CSR_BT_RESULT_CODE_CM_SUCCESS && SinkIsValid(sink))
            {
                MessageMoreData mmd;

                DEBUG_LOG("HandoverProfile_Connected, connected, conn ID %u",
                                btConnId) ;

                ho_inst->link_sink = sink;
                ho_inst->link_source = StreamSourceFromSink(sink);

                /* Set the sink in the marshal_common module */
                MarshalCommon_SetSink(ho_inst->link_sink);

                MessageStreamTaskFromSink(ho_inst->link_sink, &ho_inst->task);
                MessageStreamTaskFromSource(ho_inst->link_source,
                                            &ho_inst->task);

                PanicFalse(SinkConfigure(ho_inst->link_sink,
                                         VM_SINK_MESSAGES,
                                         VM_MESSAGES_ALL));
                PanicFalse(SourceConfigure(ho_inst->link_source,
                                           VM_SOURCE_MESSAGES,
                                           VM_MESSAGES_ALL));
            ...
            {
                DEBUG_LOG("HandoverProfile_Connected, failed, go to disconnected state");
                HandoverProfile_SetState(HANDOVER_PROFILE_STATE_DISCONNECTED);
            }
            break;
        }
        ...
        case HANDOVER_PROFILE_STATE_DISCONNECTING:
        {
            ...
            DEBUG_LOG("HandoverProfile_Connected, cancelled");
        ...
        default:
        {
            ...
            DEBUG_LOG("HandoverProfile_Connected, failed");
...
void HandoverProfile_HandleCmL2caConnectAcceptInd(const CsrBtCmL2caConnectAcceptInd *ind)
{
    ...
    DEBUG_LOG("HandoverProfile_HandleCmL2caConnectAcceptInd, state %u, psm %u, local_psm %u",
              HandoverProfile_GetState(ho_inst), ind->localPsm, ho_inst->local_psm);

    /* If the PSM doesn't match, then send l2cap failure and put the device in disconnected state */
    PanicFalse(ind->localPsm == ho_inst->local_psm);
    ...
    switch (HandoverProfile_GetState(ho_inst))
    {
        case HANDOVER_PROFILE_STATE_DISCONNECTED:
        {
            ...
            {
                DEBUG_LOG("HandoverProfile_HandleCmL2caConnectAcceptInd, accepted");
            ...
            {
                DEBUG_LOG("HandoverProfile_HandleCmL2caConnectAcceptInd, rejected, unknown peer");
            }
        }
        break;
        ...
        default:
        {
            DEBUG_LOG("HandoverProfile_HandleCmL2caConnectAcceptInd, rejected, state %u", HandoverProfile_GetState(ho_inst));
        }
        break;
    }

    /* Send a response accepting or rejecting the connection. */
    CmL2caConnectAcceptRspSend(&ho_inst->task,
                               accept,
                               ind->btConnId,
                               ind->localPsm,
                               ind->deviceAddr,
...
{
    DEBUG_LOG("HandoverProfile_HandleCmL2caConnectAcceptCfm, status 0x%04x supplier 0x%04x psm %u bdaddr %04x,%02x,%06lx", 
              cfm->resultCode,
              cfm->resultSupplier,
              cfm->localPsm,
              cfm->deviceAddr.nap,
    ...
    handoverProfile_Connected(cfm->btConnId, cfm->resultCode);
}
...
void HandoverProfile_HandleCmL2caConnectCfm(const CsrBtCmL2caConnectCfm *cfm)
{
    ...
    DEBUG_LOG("HandoverProfile_HandleCmL2caConnectCfm, status 0x%04x supplier 0x%04x psm %u bdaddr %04x,%02x,%06lx", 
              cfm->resultCode,
              cfm->resultSupplier,
              cfm->localPsm,
              cfm->deviceAddr.nap,
    ...
    ho_inst->connect_req_sent = FALSE;
    handoverProfile_Connected(cfm->btConnId, cfm->resultCode);
}
...
void HandoverProfile_HandleCmL2caDisconnectInd(const CsrBtCmL2caDisconnectInd *ind)
{
    ...
    DEBUG_LOG("HandoverProfile_HandleL2capDisconnectInd, status 0x%04x supplier 0x%04x", ind->reasonCode, ind->reasonSupplier);
        ...
        if (!handoverProfile_IsDisconnected(ho_inst))
        {
           ...
           if (sink == ho_inst->link_sink)
           {
               ...
               if (ind->reasonSupplier == CSR_BT_SUPPLIER_L2CAP_DISCONNECT &&
                   ...
                   !BdaddrIsZero(&ho_inst->peer_addr))
               {
