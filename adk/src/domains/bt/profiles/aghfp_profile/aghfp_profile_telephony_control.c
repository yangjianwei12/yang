...
static void aghfpProfile_ToggleMicMute(voice_source_t source)
{
    DEBUG_LOG_FN_ENTRY("aghfpProfile_ToggleMicMute");

    aghfpInstanceTaskData * instance = AghfpProfileInstance_GetInstanceForSource(source);

    if (!instance)
    {
        DEBUG_LOG_INFO("aghfpProfile_ToggleMicMute: No aghfpInstanceTaskData instance found");
        return;
    }

    MAKE_AGHFP_MESSAGE(AGHFP_INTERNAL_HFP_TOGGLE_MIC_MUTE_REQ);
    message->instance = instance;
    MessageSend(AghfpProfile_GetInstanceTask(instance), AGHFP_INTERNAL_HFP_TOGGLE_MIC_MUTE_REQ, message);
}
