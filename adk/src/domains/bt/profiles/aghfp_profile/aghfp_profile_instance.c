...
static void aghfpProfileInstance_InitTaskData(aghfpInstanceTaskData* instance)
{
    ...
    instance->bitfields.auto_out_audio_conn_enabled = TRUE;
    instance->bitfields.retain_instance_after_disconnect = FALSE;
...
static void aghfpProfileInstance_HandleInternalCallAccept(AGHFP_INTERNAL_HFP_CALL_ACCEPT_REQ_T* req)
{
    ...
    {   
		...
		AghfpProfileAbstract_SendCallStatusIndication(req->instance);
    }
}
...
static void aghfpProfileInstance_HandleInternalCallReject(AGHFP_INTERNAL_HFP_CALL_REJECT_REQ_T* req)
{
    ...
    else if (req->instance->bitfields.call_setup == aghfp_call_setup_incoming)
    {
        ...
        if (req->instance->bitfields.call_status == aghfp_call_none)
        {
            AghfpProfileInstance_DestroyIfAllowed(req->instance);
        }
    }
}
...
#ifdef USE_SYNERGY
static void aghfpProfileInstance_ResetCallStatusIndicators(aghfpInstanceTaskData *instance)
{
    DEBUG_LOG("aghfpProfileInstance_ResetCallStatusIndicators(%p): enum:aghfp_call_status:%d, enum:aghfp_call_setup:%d, enum:aghfp_call_held_status:%d",
              instance, instance->bitfields.call_status, instance->bitfields.call_setup, instance->bitfields.call_hold);
...
aghfpInstanceTaskData * AghfpProfileInstance_Create(const bdaddr *bd_addr, bool allocate_source)
{
    ...
    if (AghfpProfileInstance_ReachedMaxInstances())
    {
        /* If the retain_instance_after_disconnect flag is set, the AGHFP instance shall be retained after HF disconnection.
         * If a new HF is getting connected and maximum instance limit is reached, disconnected instance (if any), shall be destroyed. */
        for_all_aghfp_instances(instance, &iterator)
        {
            if (AghfpProfile_IsDisconnected(instance))
            {
                DEBUG_LOG("AghfpProfileInstance_Create: Removing disconnected instance(%p)", instance);
                AghfpProfileInstance_Destroy(instance);
                break;
            }
        }
    ...
    device = PanicNull(BtDevice_GetDeviceForBdAddr(bd_addr));
...
#ifdef USE_SYNERGY
        /* The status indicators values are cached in synergy.
         * So, before destroying the instance, synergy need to be updated, even if SLC is disconnected. */
        aghfpProfileInstance_ResetCallStatusIndicators(instance);
#endif /* USE_SYNERGY */
...
{
    if (!instance->bitfields.retain_instance_after_disconnect)
    {
        AghfpProfileInstance_Destroy(instance);
    }
}
