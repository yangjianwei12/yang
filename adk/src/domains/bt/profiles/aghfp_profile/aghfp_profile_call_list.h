 ...
 * \note Helper function that only works with first call it finds.
 *       Does not work if multiple calls with same status
 * \returns TRUE if a call was found, FALSE otherwise.
 */
bool AghfpProfileCallList_TerminateActiveCall(call_list_t *call_list);
...
/*! \brief Finds the number of calls in the call list provuded with the requested call status 
 */
int AghfpProfileCallList_GetCallStatusCount(call_list_t *call_list, aghfp_call_state call_state);
