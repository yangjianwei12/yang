...
static void extractSbcParameters(sbc_parameters *sbc, uint8 sbc_format)
{
    ...
    sbc->mode = (a2dpChannelMode) ((sbc_format & SBC_HEADER_CHANNEL_MASK) >> SBC_HEADER_CHANNEL_SHIFT);
}
...
/**************************************************************************/
void appA2dpSelectOptimalSbcCapsSink(const uint8 *local_codec_caps, uint8 *remote_codec_caps)
{
...
/**************************************************************************/
void appA2dpSelectOptimalSbcCapsSource(const uint8 *local_codec_caps, uint8 *remote_codec_caps)
{
    /*
...
void appA2dpGetSbcConfigSettings(const uint8 *service_caps, a2dp_codec_settings *codec_settings)
{
    if (!service_caps)
    {
        ...
        codec_settings->channel_mode = A2DP_CHANNEL_MODE_MONO;
        return;
    }
    ...
    {   /* Stereo - dual channel */
        codec_settings->channel_mode = A2DP_CHANNEL_MODE_DUAL;
    }
    else if (service_caps[4] & 0x02)
    {   /* Stereo */
        codec_settings->channel_mode = A2DP_CHANNEL_MODE_STEREO;
    }
    else
    {   /* Stereo - joint */
        codec_settings->channel_mode = A2DP_CHANNEL_MODE_JOINT_STEREO;
    }
}
...
uint8 appA2dpSbcFormatFromConfig(const uint8 *config)
{
    ...
    if (config[4] & 0x08)
    {
        sbc_format |= ((uint8) A2DP_CHANNEL_MODE_MONO) << SBC_HEADER_CHANNEL_SHIFT;
    }
    else if (config[4] & 0x04)
    {
        sbc_format |= ((uint8) A2DP_CHANNEL_MODE_DUAL) << SBC_HEADER_CHANNEL_SHIFT;
    }
    else if (config[4] & 0x02)
    {
        sbc_format |= ((uint8) A2DP_CHANNEL_MODE_STEREO) << SBC_HEADER_CHANNEL_SHIFT;
    }
    else
    {
        sbc_format |= ((uint8) A2DP_CHANNEL_MODE_JOINT_STEREO) << SBC_HEADER_CHANNEL_SHIFT;
    }
