...
/*! Subrating is disabled completely when CIG delegation is active */
#define SNIFF_SUBRATE_CIG_DELEGATION_ACTIVE 1
#define SNIFF_SUBRATE_TIMEOUT_CIG_DELEGATION_ACTIVE 0
#endif
...
#ifdef ENABLE_LEA_CIS_DELEGATION
static const subrate_params_t sniff_sub_rate_req_cig_delegation_active = {
    .max_remote_latency = SNIFF_SUBRATE_CIG_DELEGATION_ACTIVE * SNIFF_INVERVAL,
    .min_remote_timeout = SNIFF_SUBRATE_TIMEOUT_CIG_DELEGATION_ACTIVE * SNIFF_INVERVAL,
    .min_local_timeout = SNIFF_SUBRATE_TIMEOUT_CIG_DELEGATION_ACTIVE * SNIFF_INVERVAL,
};
#endif
...
void MirrorProfile_PeerLinkPolicySetCisDelegationActive(void)
{
    ...
    if (MirrorProfile_IsAudioSyncL2capConnected() &&
        MirrorProfile_Get()->peer_lp_mode != PEER_LP_CIS_DELEGATION)
    {
        mirrorProfile_PeerSniffSubrate(&sniff_sub_rate_req_cig_delegation_active);
        MirrorProfile_Get()->peer_lp_mode = PEER_LP_CIS_DELEGATION;

        /* When taken out of case, mirror profile moves directly into CIS connected(if unicast is already streaming),
           bye passing ACL transitioning.In such scenario, set the peer link policy to initialised to avoid getting 
           re-configured again.
        */
        MirrorProfile_SetLinkPolicyInitialised();
    }
...
#ifdef USE_SYNERGY
void MirrorProfilePeerMode_HandleCmSniffSubRatingInd(const CsrBtCmSniffSubRatingInd *ind)
{
    bdaddr bd_addr;

    DEBUG_LOG("MirrorProfilePeerMode_HandleCmSniffSubRatingInd");

    BdaddrConvertBluestackToVm(&bd_addr, &ind->deviceAddr);

    if (appDeviceIsPeer(&bd_addr))
    {
        DEBUG_LOG("MirrorProfilePeerMode_HandleCmSniffSubRatingInd resultCode:0x%x,supplier=0x%x,tl=%u,rl=%u,rt=%u,lt=%u",
            ind->resultCode, ind->resultSupplier, ind->maxTxLatency, ind->maxRxLatency, ind->minRemoteTimeout, ind->minLocalTimeout);
...
uint32 MirrorProfilePeerLinkPolicy_GetExpectedTransmissionTime(void)
{
    ...
    case PEER_LP_TRANSITIONING:
        ...
        break;
#ifdef ENABLE_LEA_CIS_DELEGATION
    case PEER_LP_CIS_DELEGATION:
        peer_relay_time_usecs = SNIFF_INTERVAL_US(SNIFF_SUBRATE_CIG_DELEGATION_ACTIVE * SNIFF_INVERVAL * NUM_SNIFF_INSTANCES);
        break;
#endif
    default:
        Panic();
