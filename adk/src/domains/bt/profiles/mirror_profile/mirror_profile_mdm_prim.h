...
/*! \brief Create CIS mirror/delegate or transfer information to peer.

    Only the Primary should call this.
    This function sends a MDM_LE_CIS_CREATE_REQ.

    \param cis_handle CIS handle which needs to mirrored/delegated/inform to peer.
    \param cis_ownership Informs CIS to be mirrored or delegated or informed (NO_MIRROR_NO_DELEGATE) to peer.
*/
...
/*! \brief Disconnect mirror CIS link.
    ...
    Only the Primary should call this.
    This function sends a MDM_LE_CIS_DISCONNECT_REQ.

    \param conn_handle The CIS mirror connection handle to disconnect.
    \param reason Reason for disconnection.
*/
void MirrorProfile_MirrorCisDisconnectReq(hci_connection_handle_t conn_handle, hci_reason_t reason);

/*! \brief Handles confirmation of data path setup on mirrored CIS.

    \param cfm Message body.
*/
void MirrorProfile_HandleSetupIsoDataPathCfm(const CmIsocSetupIsoDataPathCfm *cfm);

/*! \brief Handles confirmation of data path removal on mirrored CIS.
