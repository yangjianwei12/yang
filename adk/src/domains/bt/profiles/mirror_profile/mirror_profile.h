...
typedef enum
{
    ...
    */
    ...
    /*! In Q2Q mode, this is used by the primary when the primary and
        secondary earbuds are connected but audio is already streaming when
        the earbuds start to mirror the A2DP stream. In this case, both earbuds
        mute their outputs and the secondary defines a unmute instant which it
        sends to the primary. Both earbuds unmute at this instant.
    */
    MIRROR_PROFILE_A2DP_START_Q2Q_MODE_PRIMARY_UNMUTE,
    
    /*! In Q2Q mode, this is used by the secondary when the primary and
        secondary earbuds are connected but audio is already streaming when
        the earbuds start to mirror the A2DP stream. In this case, both earbuds
        mute their outputs and the secondary defines a unmute instant which it
        sends to the primary. Both earbuds unmute at this instant.
    */
    MIRROR_PROFILE_A2DP_START_Q2Q_MODE_SECONDARY_UNMUTE,
...
/*! \brief Test if any CIS delegation is connected.
    ...
    \return TRUE if any CIS delegation is connected, FALSE otherwise.
*/
...
/*! \brief Get Peer and Own Cis Handles from Mirror profile.
    \param own_handle The own cis handle filled by mirror profile.
    \param peer_handle The peer cis handle filled by mirror profile.
*/
void MirrorProfile_GetCisHandle(hci_connection_handle_t *own_handle,
                                hci_connection_handle_t *peer_handle);

/*! \brief return CIS mirrored device address.
    \return Delegated device address
*/
bdaddr* MirrorProfile_GetCisMirroredDeviceAddress(void);

/*! \brief Send Mic mute to the secondary */
