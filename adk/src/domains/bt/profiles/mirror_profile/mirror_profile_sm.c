...
static void mirrorProfile_EnterAclConnecting(void)
{
    ...
    /* Sniff mode is entered when peer connects, wait for CL_DM_MODE_CHANGE_EVENT
       then start mirroring */
    PanicFalse(ConManagerGetPowerMode(&tpbdaddr, &lp_mode));
    ...
    {
        MirrorProfile_UpdatePeerLinkPolicy(CSR_BT_SNIFF_MODE);
    }
...
static void mirrorProfile_ExitAclConnected(void)
{
    ...
    else
    {
        ...
        /* Don't set the property if secondary doesn't 
           have information on mirrored device, which will be the 
           case with TWM speakers */
        if(MirrorProfile_GetMirroredDevice())
...
static void mirrorProfile_EnterA2dpConnecting(void)
{
    ...
    if (MirrorProfile_IsPrimary())
    {
        /* Update the mirrored A2DP source and cid to match the target device */
        audio_source_t target_source = MirrorProfile_GetTargetA2dpSource();
        mirror_profile_cached_context_t* context = MirrorProfile_GetCachedContext(target_source);
        
        a2dp->audio_source = target_source;
...
/*! \brief Enter CIS_CONNECTING state */
static void mirrorProfile_EnterCisConnecting(void)
{
    ...
    MIRROR_LOG("mirrorProfile_EnterCisConnecting");

    /* We want to be out of sniff mode during CIS delegation */
    ...
    {
        if (!MirrorProfile_GetLeUnicastConfigSent())
        {
            MirrorProfile_SendUnicastConfigData(Multidevice_GetPairSide());
        }

        cis_ownership = lea_unicast->own_cis_handle != lea_unicast->peer_cis_handle ?
                            CIS_DELEGATE_SECONDARY : CIS_MIRRORING_SECONDARY;
        cis_handle = lea_unicast->peer_cis_handle;
    }
    else if (lea_unicast->own_cis_state == MIRROR_PROFILE_CIS_SUB_STATE_READY_TO_CONNECT)
    {
        if (MirrorProfile_IsStereoCisMirroring(lea_unicast) && !MirrorProfile_GetLeUnicastConfigSent())
        {
            MirrorProfile_SendUnicastConfigData(Multidevice_GetPairSide());
        }
    ...
    {
        MIRROR_LOG("mirrorProfile_EnterCisConnecting: Invalid cis state");
        Panic();
        return;
    }


    MirrorProfile_MirrorCisCreateReq(cis_handle, cis_ownership);
}

/*! \brief Exit CIS_CONNECTING state */
static void mirrorProfile_ExitCisConnecting(void)
{
    MIRROR_LOG("mirrorProfile_ExitCisConnecting");
}

/*! \brief Enter CIS_CONNECTED parent state */
static void mirrorProfile_EnterCisConnected(void)
{
    MIRROR_LOG("mirrorProfile_EnterCisConnected");

    MirrorProfile_PeerLinkPolicySetCisDelegationActive();
}

/*! \brief Exit CIS_CONNECTED parent state */
static void mirrorProfile_ExitCisConnected(void)
{
    MIRROR_LOG("mirrorProfile_ExitCisConnected");
...
/*! \brief Enter CIS_DISCONNECTING state */
static void mirrorProfile_EnterCisDisconnecting(void)
{
    MIRROR_LOG("mirrorProfile_EnterCisDisconnecting");
}

/*! \brief Exit CIS_DISCONNECTING state */
static void mirrorProfile_ExitCisDisconnecting(void)
{
    MIRROR_LOG("mirrorProfile_ExitCisDisconnecting");
}
...
void MirrorProfile_SetOwnCisState(mirror_profile_cis_sub_state_t new_state)
{
    ...
    MIRROR_LOG("MirrorProfile_SetOwnCisState enum:mirror_profile_cis_sub_state_t:%d to enum:mirror_profile_cis_sub_state_t:%d",
               lea_unicast->own_cis_state, new_state);
    lea_unicast->own_cis_state = new_state;

    if (lea_unicast->own_cis_state == MIRROR_PROFILE_CIS_SUB_STATE_CONNECTED)
    {
        TaskList_MessageSendId(MirrorProfile_Get()->client_tasks, MIRROR_PROFILE_UNICAST_CIS_CONNECT_IND);
    }
    else if(lea_unicast->own_cis_state == MIRROR_PROFILE_CIS_SUB_STATE_IDLE)
    {
        lea_unicast->own_cis_handle = MIRROR_PROFILE_CONNECTION_HANDLE_INVALID;
        TaskList_MessageSendId(MirrorProfile_Get()->client_tasks, MIRROR_PROFILE_UNICAST_CIS_DISCONNECT_IND);
    }
}
...
void MirrorProfile_SetPeerCisState(mirror_profile_cis_sub_state_t new_state)
{
    ...
    MIRROR_LOG("MirrorProfile_SetPeerCisState enum:mirror_profile_cis_sub_state_t:%d to enum:mirror_profile_cis_sub_state_t:%d",
               lea_unicast->peer_cis_state, new_state);
    lea_unicast->peer_cis_state = new_state;
...
#ifdef ENABLE_LEA_CIS_DELEGATION
        /* Active mode speeds up CIS delegation */
...
#ifdef ENABLE_LEA_CIS_DELEGATION
    case MIRROR_PROFILE_STATE_CIS_CONNECTING:
        mirrorProfile_ExitCisConnecting();
        break;
    case MIRROR_PROFILE_STATE_CIS_DISCONNECTING:
        mirrorProfile_ExitCisDisconnecting();
        break;
#endif
    default:
        break;
...
#ifdef ENABLE_LEA_CIS_DELEGATION
    if (MirrorProfile_IsSubStateCisConnected(old_state) && !MirrorProfile_IsSubStateCisConnected(state))
        mirrorProfile_ExitCisConnected();
#endif
...
#ifdef ENABLE_LEA_CIS_DELEGATION
    if (!MirrorProfile_IsSubStateCisConnected(old_state) && MirrorProfile_IsSubStateCisConnected(state))
        mirrorProfile_EnterCisConnected();
#endif
...
#ifdef ENABLE_LEA_CIS_DELEGATION
    case MIRROR_PROFILE_STATE_CIS_CONNECTING:
        mirrorProfile_EnterCisConnecting();
        break;
    case MIRROR_PROFILE_STATE_CIS_CONNECTED:
        break;
    case MIRROR_PROFILE_STATE_CIS_DISCONNECTING:
        mirrorProfile_EnterCisDisconnecting();
        break;
#endif
    default:
        break;
...
static mirror_profile_state_t mirrorProfile_SmTransition(void)
{
    ...
    case MIRROR_PROFILE_STATE_DISCONNECTED:
        ...
        case MIRROR_PROFILE_STATE_CIS_CONNECTING:
        case MIRROR_PROFILE_STATE_CIS_CONNECTED:
            return MIRROR_PROFILE_STATE_CIS_DISCONNECTING;
#endif
        default:
            break;
        }
    ...
    case MIRROR_PROFILE_STATE_CIS_CONNECTED:
        ...
        case MIRROR_PROFILE_STATE_A2DP_CONNECTED:
        case MIRROR_PROFILE_STATE_A2DP_ROUTED:
            return MIRROR_PROFILE_STATE_CIS_CONNECTING;
        case MIRROR_PROFILE_STATE_CIS_CONNECTED:
            if (MirrorProfile_AnyCisMirroringPending(MirrorProfile_GetLeaUnicastState()))
            {
                return MIRROR_PROFILE_STATE_CIS_CONNECTING;
            }
        default:
