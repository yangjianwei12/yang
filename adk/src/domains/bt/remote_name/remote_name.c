...
static void remoteName_PrintName(uint8 *name, uint16 size)
{
    ...
    {
        uint8 ch = *name++;
        DEBUG_LOG_VERBOSE("  %02X %c", ch, ch < 32 || ch > 126 ? '.' : ch);
    }
}
...
static void remoteName_MessageHandler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case CM_PRIM:
        {
            if (*(CsrBtCmPrim *) message == CSR_BT_CM_READ_REMOTE_NAME_CFM)
            {
                bdaddr addr = {0};
                CsrBtCmReadRemoteNameCfm * msg = (CsrBtCmReadRemoteNameCfm*)message;
                BdaddrConvertBluestackToVm(&addr, &msg->deviceAddr);

                DEBUG_LOG_VERBOSE("CSR_BT_CM_READ_REMOTE_NAME_CFM status 0x%x, supplier 0x%x, lap 0x%x",
                                msg->resultCode, msg->resultSupplier, addr.lap);
                ...
                {
                    uint8* rname = msg->friendlyName;
                    uint16 size_rname = CsrStrLen((char *)msg->friendlyName);
                    remoteName_PrintName(rname, size_rname);

                    remoteName_Set(device, (char *)rname, size_rname);
                }
...
#ifdef USE_SYNERGY
    /* Subscribe for remote name notification */
    CmSetEventMaskReqSend((Task)&remote_name.task_data,
