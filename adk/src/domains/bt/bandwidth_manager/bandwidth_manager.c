...
static void bandwidthManager_throttleBandwidthOfAllLowPrioFeatures(bandwidth_manager_priority_t higher_feature_prio)
{
    ...
    FOR_EACH_REGISTERED_FEATURE(feature_info)
    {
        ...
        {            
            ...
            {
                DEBUG_LOG_VERBOSE("bandwidthManager_throttleBandwidthOfAllLowPrioFeatures, enum:bandwidth_manager_feature_id_t:%d at enum:bandwidth_manager_priority_t:priority[%d] requesting to throttle",
            ...
            {
                DEBUG_LOG_VERBOSE("bandwidthManager_throttleBandwidthOfAllLowPrioFeatures, enum:bandwidth_manager_feature_id_t:%d at enum:bandwidth_manager_priority_t:priority[%d] already throttling",
                    feature_info->bitfields.identifier, feature_info->bitfields.priority);
            }
...
static void bandwidthManager_unthrottleBandwidthOfFeatures(bandwidth_manager_priority_t feature_prio)
{
    ...
    FOR_EACH_REGISTERED_FEATURE(feature_info)
    {
        if ((feature_prio == feature_info->bitfields.priority) && (feature_info->bitfields.running))
        {
            if (feature_info->bitfields.throttle_required)
            {
                DEBUG_LOG_VERBOSE("bandwidthManager_unthrottleBandwidthOfFeatures, enum:bandwidth_manager_feature_id_t:%d at  enum:bandwidth_manager_priority_t:priority[%d] requesting to unthrottle",
                    feature_info->bitfields.identifier, feature_info->bitfields.priority);
            ...
            {
                DEBUG_LOG_VERBOSE("bandwidthManager_unthrottleBandwidthOfFeatures, enum:bandwidth_manager_feature_id_t:%d at  enum:bandwidth_manager_priority_t:priority[%d] was not throttled",
                    feature_info->bitfields.identifier, feature_info->bitfields.priority);
            }
        }
    }
    ...
    bandwidth_manager_action_msg_t action_msg)
{
    ...
    FOR_EACH_FEATURE_PRIORITY(priority)
    {
        DEBUG_LOG_VERBOSE("bandwidthManager_handleAction: features[uint8:%d] running at enum:bandwidth_manager_priority_t:priority[%d]", running_at_priority[priority], priority);
    ...
    switch (action_msg)
    {
        case BANDWIDTH_MGR_ACTION_MSG_START:
            ...
            {
                DEBUG_LOG_VERBOSE("bandwidthManager_handleAction: only one feature running");
            }
            break;
        ...
        case BANDWIDTH_MGR_ACTION_MSG_STOP:
            ...
            {
                DEBUG_LOG_VERBOSE("bandwidthManager_handleAction: last feature stopped");
            }
            break;
...
static bandwidth_manager_feature_info_t* bandwidthManager_ArrangeFeatureSlot(bandwidth_manager_feature_info_t *current_feature_slot)
{
    ...
    if (current_feature_slot->bitfields.running)
    {
        ...
        {
            DEBUG_LOG_VERBOSE("bandwidthManager_ArrangeFeatureSlot: No need to arrange");
    ...
    else
    {
            ...
            )
        {
            ...
            DEBUG_LOG_VERBOSE("bandwidthManager_ArrangeFeatureSlot: move number of elements[%d]", move_num_of_elements);
