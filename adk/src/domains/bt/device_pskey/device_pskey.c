...
static pdd_size_t devicePsKey_GetPdduSize(device_t device)
{
...
static void devicePsKey_Serialise(device_t device, void *buf, pdd_size_t offset)
{
    void *value = NULL;
...
static void devicePsKey_Deserialise(device_t device, void *buf, pdd_size_t data_length, pdd_size_t offset)
{
    UNUSED(device);
    UNUSED(buf);
    UNUSED(data_length);
    ...
    {
        DEBUG_LOG_ERROR("devicePsKey_Deserialise invalid pskey id %u", entry.ps_key);
    }
}
