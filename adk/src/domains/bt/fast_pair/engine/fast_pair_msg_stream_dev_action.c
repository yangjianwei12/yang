...
static void fastPair_HandleRingTone(bool is_ring)
{
    if (is_ring)
    {
        ...
        {
            appKymeraTonePlay(ringtone_vol32, 0, TRUE, FALSE, NULL, 0);
        ...
        {
            appKymeraTonePlay(ringtone_vol64, 0, TRUE, FALSE, NULL, 0);
            ring_device.ringtimes--;
            if (ring_device.ringtimes == 0) 
            {
        ...
        {
            appKymeraTonePlay(ringtone_vol128, 0, TRUE, FALSE, NULL, 0);
            ring_device.ringtimes--;
            if (ring_device.ringtimes == 0)
            {
        ...
        {
            appKymeraTonePlay(ringtone_volmax, 0, TRUE, FALSE, NULL, 0);
        }
...
bool FastPair_RingMuteDevice(uint8 fp_seeker_number, uint8 ring_component, uint8 ring_volume, uint16 ring_timeout)
{
    ...
    if(fp_seeker_number == FASTPAIR_SEEKER_INVALID)
    {
        ...
        fastPair_MsgStreamSendDataToAll(FASTPAIR_MESSAGESTREAM_MESSAGE_GROUP_DEVCIE_ACTION_EVENT,
                                        FASTPAIR_MESSAGESTREAM_DEVACTION_RING_EVENT,
    ...
    if (dev_action_data.ring_component == FASTPAIR_DEVICEACTION_RING_RIGHT_MUTE_LEFT)
    {
        ...
        /* Mute left earbud if the opertaion is performed via message stream*/
        else
        {
            if(fp_seeker_number != FASTPAIR_SEEKER_INVALID)
            {
