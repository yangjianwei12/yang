...
void FastPair_LocalBatteryValInit(void)
{
    bool local_is_left = Multidevice_IsLeft();
    uint8 battery_percentage = Soc_GetBatterySoc();

    DEBUG_LOG("FastPair_LocalBatteryValInit: Local battery value = %d", battery_percentage);

    if(local_is_left)
    {
        fp_battery_ntf_data[FP_BATTERY_NTF_DATA_LEFT_STATE_OFFSET] = battery_percentage;
    }
    else
    {
        fp_battery_ntf_data[FP_BATTERY_NTF_DATA_RIGHT_STATE_OFFSET] = battery_percentage;
    }
}
#endif
