...
void fastPair_ProviderPasskey(bool isPasskeySame, uint32 provider_passkey, tp_bdaddr handset_tpaddr)
{
    ...
    args.handset_tpaddr = handset_tpaddr;
...
void fastPair_PairingActivity(PAIRING_ACTIVITY_T *message)
{
    ...
    switch(message->status)
    {
        ...
        {
            fastPair_AdvNotifyPairingSuccess();

            /* Store the handset public address in FP Data structure.
            ...
            /* Set retroactively writing account key flag only if we have not done FP process
               and store the handset public address in FP Data structure. */
