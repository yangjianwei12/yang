...
static bool fastpair_ReadRandomAddressEventHandler(fast_pair_state_event_rpa_addr_args_t* args)
{
...
#ifdef USE_SYNERGY
    result = args->rpa_cfm->resultCode;
    flag = args->rpa_cfm->flag;
#else
    ...
    {
#ifdef USE_SYNERGY
        BdaddrConvertBluestackToVm(&theFastPair->rpa_bd_addr, &args->rpa_cfm->rpa);
        BdaddrConvertBluestackToVm(&peer_bd_addr, &args->rpa_cfm->idAddress.addr);
#else
