...
static pairing_user_confirmation_rsp_t fastpair_handle_user_confirmation_req(const CL_SM_USER_CONFIRMATION_REQ_IND_T* ind)
{
    ...
    if(fp_if_data->seeker_passkey)
    {
        ...
        fastPair_ProviderPasskey(isPasskeySame, numeric_value, ind->tpaddr);
    }
    else
    {
...
#ifdef USE_SYNERGY
/*! \brief Handle Pairing indication
 */

/*! \brief Handle Bond indication
 */
static void fastpair_handle_bond_indication(const bond_indication_ind_t *ind)
{
...
#ifdef USE_SYNERGY
    fastPair_exchange.handle_bond_indication = fastpair_handle_bond_indication;
#endif
