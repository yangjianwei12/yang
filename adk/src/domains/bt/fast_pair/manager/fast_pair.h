...
#if defined(INCLUDE_FAST_PAIR) && !defined(USE_RPA)
	#error "Fast pair feature requires RPA to be enabled"
#endif
...
/* Randomly generated nonce length */
#define FAST_PAIR_RANDOM_NONCE_LEN    (8)
...
{
#ifdef USE_SYNERGY
    const bond_indication_ind_t* auth_cfm;
#else
...
/*! \brief Get the "owner" account key

    The "owner" account is the first Fast pair user account to be associated with a device,
    it should be considered special and should not be overwritten

    The Google finder feature uses this account.

    \return Pointer to The owner account key if one exists, else NULL. the calling function is responsible 
    for freeing the memory at the returned address
 */
...
/*! \brief Helper function to generate 8 octets random nonce
    \param None
    \return uint8* Pointer to the randomly generated nonce
 */
...
/*! \brief Check fastpair in progress

    \param device Handle to device

    \return bool TRUE if Fastpair is ongoing, FALSE otherwise.
*/
