...
#ifdef USE_SYNERGY
    MAKE_MARSHAL_MEMBER(key_sync_req_t, uint8, link_key_type),
#else
...
static uint32 key_sync_paired_list_req_data_size(const void *parent,
                                                   const marshal_member_descriptor_t *member_descriptor,
                                                   uint32 array_element)
{
    const key_sync_paired_list_req_t* req = parent;
    UNUSED(member_descriptor);
    UNUSED(array_element);
    return req->num_handsets;
}

const marshal_member_descriptor_t key_sync_paired_list_req_member_descriptors[] =
{
    MAKE_MARSHAL_MEMBER(key_sync_paired_list_req_t, uint8, num_handsets),
    MAKE_MARSHAL_MEMBER_ARRAY(key_sync_paired_list_req_t, bdaddr, bd_addrs, 0),
};

const marshal_type_descriptor_dynamic_t marshal_type_descriptor_key_sync_paired_list_req_t =
    MAKE_MARSHAL_TYPE_DEFINITION_HAS_DYNAMIC_ARRAY(key_sync_paired_list_req_t,
                                                   key_sync_paired_list_req_member_descriptors,
                                                   key_sync_paired_list_req_data_size);

/*----------------------------------------------------------------------------*/

static uint32 key_sync_paired_list_cfm_data_size(const void *parent,
                                                   const marshal_member_descriptor_t *member_descriptor,
                                                   uint32 array_element)
{
    const key_sync_paired_list_cfm_t* cfm = parent;
    UNUSED(member_descriptor);
    UNUSED(array_element);
    return cfm->num_handsets;
}

const marshal_member_descriptor_t key_sync_paired_list_cfm_member_descriptors[] =
{
    MAKE_MARSHAL_MEMBER(key_sync_paired_list_cfm_t, uint8, num_handsets),
    MAKE_MARSHAL_MEMBER_ARRAY(key_sync_paired_list_cfm_t, bdaddr, bd_addrs, 0),
};

const marshal_type_descriptor_dynamic_t marshal_type_descriptor_key_sync_paired_list_cfm_t =
    MAKE_MARSHAL_TYPE_DEFINITION_HAS_DYNAMIC_ARRAY(key_sync_paired_list_cfm_t,
                                                   key_sync_paired_list_cfm_member_descriptors,
                                                   key_sync_paired_list_cfm_data_size);
