...
/*! Convert a connection interval in ms into a \b valid connection
    ...
    Valid intervals start at 7.5ms.

    This macro produces the setting that is equal to or greater than
    the value requested */
#define CONN_INT_MS(_ms)  ((unsigned)(((_ms)+1.20)/1.25))
...
/*! Settings used on an LE link when it is idle. */
...
#define SHORT_STREAMING_CONNECTION_INTERVAL CONN_INT_MS(16.25)

/* Low interval/latency for use when already connected and/or streaming */
#define LE_SHORT_STREAMING_CONNECTION_PARAMS \
    .conn_interval_min  = SHORT_STREAMING_CONNECTION_INTERVAL, \
    .conn_interval_max  = SHORT_STREAMING_CONNECTION_INTERVAL, \
    .conn_latency       = 0

static const ble_connection_params short_streaming_connection_params = 
{
    LE_FIXED_PARAMS, LE_SHORT_STREAMING_CONNECTION_PARAMS
};
...
const ble_connection_params* const cm_qos_params[cm_qos_max] =
{
    ...
    [cm_qos_short_while_streaming]  = &short_streaming_connection_params,
