...
static con_manager_filter_notification_cb_t filter_notification_cb = NULL;
...
/******************************************************************************/
static void conManagerMsgAuthoriseInd(const typed_bdaddr* bd_addr, dm_protocol_id protocol_id, uint32 channel, bool incoming)
{
    MAKE_CONMAN_MESSAGE(CON_MANAGER_AUTHORISE_IND);
    message->bd_addr = *bd_addr;
    message->protocol_id = protocol_id;
    message->channel = channel;
    message->incoming = incoming;
    TaskList_MessageSend(TaskList_GetFlexibleBaseTaskList(ConManager_GetBredrObserverTasks()), CON_MANAGER_AUTHORISE_IND, message);
}

void ConManager_SetNotificationFilter(const con_manager_filter_notification_cb_t callback)
{
    filter_notification_cb = callback;
}

/******************************************************************************/
void conManagerNotifyObservers(const tp_bdaddr *tpaddr, cm_notify_message_t notify_message, hci_status reason)
{
    con_manager_notify_group_type_t notify_group_type = con_manager_notify_group_type_disconnect;

    if (notify_message == cm_notify_message_connected_incoming || notify_message == cm_notify_message_connected_outgoing)
    {
        notify_group_type = con_manager_notify_group_type_connect;
    }

    /* Filter out connection manager notifications from registered observers if notifications are handled by callback */
    if (filter_notification_cb != NULL && filter_notification_cb(tpaddr, notify_group_type, reason))
    {
        DEBUG_LOG("conManagerNotifyObservers: Filtering notifications enum:cm_notify_message_t:%d from device: %04x %02x %06x Transport: %02x",
                   notify_message, tpaddr->taddr.addr.nap, tpaddr->taddr.addr.uap,
                   tpaddr->taddr.addr.lap, tpaddr->transport);
        return;
    }
...
{
    conManagerMsgAuthoriseInd(bd_addr, protocol_id, channel, incoming);
}
