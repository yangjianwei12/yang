...
static void gattServerBattery_ResendBatteryNotificationsIfChanged(void)
{
    ...
    {
        gattServerBattery_SendBatteryLevelIfChanged(GetGattServerBatteryInstance(battery_type));
    }
...
#ifdef USE_SYNERGY
    DEBUG_LOG("gattServerBattery_MessageHandler Synergy Task Id:0x%x", id);
    if (id == (SYNERGY_EVENT_BASE + BAS_SERVER_PRIM))
    {	
        id = *((CsrBtCmPrim *) message);
    }
#endif
...
#ifdef USE_SYNERGY
                                   TrapToOxygenTask((Task)GetGattServerBatteryTask()),
#else
