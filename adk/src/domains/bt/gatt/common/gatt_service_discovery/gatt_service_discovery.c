...
#ifdef USE_SYNERGY
static inline bool gattServiceDiscovery_CheckServiceDiscoveryContinueConditions(CsrBtResultCode status, bool more_to_come)
{
    bool multiple_services_discovered = (status == CSR_BT_GATT_RESULT_SUCCESS) && more_to_come;
#else
...
#ifdef USE_SYNERGY
static void gattServiceDiscovery_HandleRegisterCfm(const CsrBtGattRegisterCfm *cfm)
{
    if (cfm->resultCode == CSR_BT_GATT_RESULT_SUCCESS)
    {
        discovery_state.gatt_id = cfm->gattId;
    }
}

static void gattServiceDiscovery_HandlePrimaryServiceDiscoveryInd(const CsrBtGattDiscoverServicesInd* ind)
{
    /* Attach client to the discovered service */
    client_priority_list.num_service_instances[discovery_state.current_service_index]++;
    GattClient_AttachClient(discovery_state.current_client_id, ind->btConnId , ind->startHandle,ind->endHandle);
}

static void gattServiceDiscovery_HandlePrimaryServiceDiscoveryCfm(const CsrBtGattDiscoverServicesCfm* cfm)
{
    if(gattServiceDiscovery_CheckServiceDiscoveryContinueConditions(cfm->resultCode, FALSE))
    {
        /* Continue to iterate over client list */
        discovery_state.current_service_index++;
        gattServiceDiscovery_ProcessClientPriorityList(cfm->btConnId);
    }
}
...
static void GattServiceDisovery_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (*primType)
    {
        ...
        case CSR_BT_GATT_REGISTER_CFM:
            gattServiceDiscovery_HandleRegisterCfm((CsrBtGattRegisterCfm*)message);
            break;

        case CSR_BT_GATT_DISCOVER_SERVICES_IND:
            gattServiceDiscovery_HandlePrimaryServiceDiscoveryInd((const CsrBtGattDiscoverServicesInd *)message);
            break;

        case CSR_BT_GATT_DISCOVER_SERVICES_CFM:
            gattServiceDiscovery_HandlePrimaryServiceDiscoveryCfm((const CsrBtGattDiscoverServicesCfm *)message);
            break;
...
#ifdef USE_SYNERGY
    GattDiscoverPrimaryServicesByUuidReqSend(discovery_state.gatt_id,
                                             cid,
...
#ifdef USE_SYNERGY
    if (discovery_state.gatt_id == CSR_BT_GATT_INVALID_GATT_ID)
    {
        GattRegisterReqSend(&gsd_taskdata, 1234);
    }
#endif /* USE_SYNERGY */
