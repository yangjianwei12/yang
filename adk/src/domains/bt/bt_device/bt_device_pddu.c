...
static uint16 BtDevice_SerialisePddu(bt_device_pdd_t *pdd, uint8* buf)
{
    ...
    btDevice_SerialiseVariable(pdd->google_finder_location_tracking_disabled, sizeof(pdd->google_finder_location_tracking_disabled), buf, &index);
...
static pdd_size_t btDevice_GetDeviceDataLen(device_t device)
{
    UNUSED(device);
...
static void btDevice_SerialisePersistentDeviceData(device_t device, void *buf, pdd_size_t offset)
{
...
static bool btDevice_IsEnoughDataFor222Properties(bt_device_pdd_t *device_data, uint16 index, uint8 data_length)
{
    uint8 size_of_properties = sizeof(device_data->handset_account_key_index) + sizeof(device_data->sass_switch_disabled) + 
                               sizeof(device_data->gsd_completed) + sizeof(device_data->google_finder_location_tracking_disabled);
    return index + size_of_properties <= data_length ? TRUE : FALSE;
}
...
static void btDevice_Load222Properties(bt_device_pdd_t *device_data, uint8 *buf, uint16 *index)
{
    ...
    btDevice_DeserialisePddAndIncrementIndexU8(&device_data->google_finder_location_tracking_disabled, buf, index);
}
...
static void btDevice_Set222Defaults(bt_device_pdd_t *device_data)
{
    ...
    device_data->google_finder_location_tracking_disabled = 0;
}
...
static void btDevice_DeserialisePersistentDeviceData(device_t device, void *buffer, pdd_size_t data_length, pdd_size_t offset)
{
