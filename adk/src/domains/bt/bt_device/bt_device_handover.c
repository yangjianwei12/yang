...
static bool btDevice_MarshalLe(const typed_bdaddr *tbdaddr,
                               marshal_type_t type,
                               void **marshal_obj);

static app_unmarshal_status_t btDevice_UnmarshalLe(const typed_bdaddr *tbdaddr,
                                                   marshal_type_t type,
                                                   void *unmarshal_obj);

static void btDevice_CommitLe(bool is_primary);

/*! \brief Marshal data type to use for LE device */
static bt_device_handover_le_t marshal_data_le;
...
#ifdef ENABLE_LE_HANDOVER
    MARSHAL_TYPE_INFO(bt_device_handover_le_t, MARSHAL_TYPE_CATEGORY_PER_INSTANCE),
#endif
};
...
#ifdef ENABLE_LE_HANDOVER
REGISTER_HANDOVER_INTERFACE_LE(BT_DEVICE, &bt_device_marshal_types_list, NULL, btDevice_MarshalLe, btDevice_UnmarshalLe, btDevice_CommitLe);
#endif /* ENABLE_LE_HANDOVER */
...
#ifdef USE_SYNERGY
    ConManagerUpdatePeerAddress(&self_addr, &peer_addr);
#endif
...
/*!
    ...
    \param[in] tbdaddr      Bluetooth address of the link to be marshalled.
    \param[in] type         Type of the data to be marshalled.
    \param[out] marshal_obj Holds address of data to be marshalled.
    \return TRUE: The component has another object to marshal of that type.
            FALSE: The component does not have another object to marshal of that type.
...
static bool btDevice_MarshalLe(const typed_bdaddr *tbdaddr,
                               ...
                               void **marshal_obj)
{
    DEBUG_LOG("btDevice_MarshalLe");
    *marshal_obj = NULL;
    bool status = FALSE;

    switch (type)
    {
        case MARSHAL_TYPE(bt_device_handover_le_t):
        {
            tp_bdaddr tpaddr;

            tpaddr.transport = TRANSPORT_BLE_ACL;
            memcpy(&tpaddr.taddr, tbdaddr, sizeof(typed_bdaddr));
            device_t device = BtDevice_GetDeviceFromTpAddr(&tpaddr);
            PanicNull(device);

            /* Marshalling is only needs to be done for LE handset devices */
            if (BtDevice_GetDeviceType(device) == DEVICE_TYPE_HANDSET_LE)
            {
                BtDevice_GetDeviceData(device, &marshal_data_le.pdd);
                BtDevice_GetRandomTpAddrForDevice(device, &marshal_data_le.random_tp_bdaddr);

                *marshal_obj = &marshal_data_le;
                status = TRUE;
            }
        }
        break;
...
/*!
    ...
    \param[in] tbdaddr      Bluetooth address of the link to be unmarshalled.
    \param[in] type         Type of the unmarshalled data.
    \param[in] unmarshal_obj Address of the unmarshalled object.
    \return unmarshalling result. Based on this, caller decides whether to free
            the marshalling object or not.
...
static app_unmarshal_status_t btDevice_UnmarshalLe(const typed_bdaddr *tbdaddr,
                                 ...
                                 void *unmarshal_obj)
{
    DEBUG_LOG("btDevice_UnmarshalLe");
    app_unmarshal_status_t result = UNMARSHAL_FAILURE;
    PanicNull(unmarshal_obj);
    ...
    switch (type)
    {
        ...
        {
            device_t device;
            bt_device_handover_le_t *unmarshalled = NULL;
            tp_bdaddr tpaddr;

            /* Unmarshal device data for handset address */
            unmarshalled = (bt_device_handover_le_t*)unmarshal_obj;
            ...
            if (device == NULL)
            {
                /* Device will not be existing in secondary for a handset type 'DEVICE_TYPE_HANDSET_LE' which is not paired.
                   Create a device with unmarshalled tp address. */
                deviceType dev_type = DEVICE_TYPE_HANDSET_LE;

                device = PanicNull(BtDevice_GetDeviceCreateIfNewWithTpAddr(&unmarshalled->random_tp_bdaddr, dev_type));
