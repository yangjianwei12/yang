...
/*! \brief Device supports LE audio profiles */
#define DEVICE_PROFILE_LE_AUDIO (1 << 14)
...
/*! This flag is no longer used; keep it in the bitmap for backwards compatibility. */
...
#ifdef USE_SYNERGY
/*! Bit in flags indicating that the key type is BLE and the same shall be used for ignoring/processing
 *  CSR_BT_CM_DATABASE_CFM at the time of key sync for secondary device. */
#define DEVICE_FLAGS_KEY_TYPE                    (1 << 5)
#endif /* USE_SYNERGY */
...
/*! \brief Get the number of BT devices in the list.

    \return The number of BT devices currently in the list.
*/
...
/*! \brief To set maximum supported trusted BT devices.

    \param Number of devices supported .
*/
void BtDevice_SetMaxTrustedDevices(uint8 num_devices);

/*! \brief To get maximum supported trusted BT devices.

    \return Number of devices supported
*/
...
/*! \brief Determine if a given device supports a particular profile
...
/*! \brief Get QSS Server client config.

    \param bd_addr Pointer to read-only device BT address.
    \param[out] config The client configuration for QSS.
    \note The config should only be requested for handsets.
*/
bool appDeviceGetQssServerConfig(const bdaddr *bd_addr, uint16* config);

/*! \brief Set QSS Server client config.
...
/*! \brief Get the Peer Device.

    This function will query the Device List and find the Peer device,
    if it exists

    \return a device_t which will either be the Peer device or NULL.
*/
...
/*! \brief Check if the BD Addr belongs to a handset connected over LE ACL.

    \param bd_addr BD Addr of the client
    \return TRUE if the bd_addr belongs to Handset connected over LE ACL, FALSE otherwise.
*/
