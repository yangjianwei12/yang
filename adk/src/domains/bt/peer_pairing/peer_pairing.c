                                      ...
                                      const PEER_PAIRING_LONG_TERM_KEY_T *bredr)
{
    ...
    memset(&bredrKey, 0, sizeof(bredrKey));
    bredrKey.linkkeyType = DM_SM_LINK_KEY_AUTHENTICATED_P256;
    bredrKey.linkkeyLen = sizeof(bredr->key);
    memcpy(&bredrKey.linkkey, bredr->key, sizeof(bredrKey.linkkey));
    bredrKey.authorised = TRUE;

    BdaddrConvertVmToBluestack(&deviceAddr, addr);

    /* Add the bredr keys to PS */
    result = CmWriteKeysNow(&deviceAddr,
                            CSR_BT_ADDR_PUBLIC,
                            CSR_BT_TD_DB_SC_KEY_BREDR_KEY,
                            &bredrKey);

    return (result == CSR_BT_RESULT_CODE_TD_DB_SUCCESS) ? TRUE : FALSE;
}
                                       ...
                                       const PEER_PAIRING_LONG_TERM_KEY_T *le)
{
    ...
    CsrBtScLeKeyEncCentral le_key = {0};
    CsrBtDeviceAddr deviceAddr = {0};

    BdaddrConvertVmToBluestack(&deviceAddr, addr);

    memset(&leKeys, 0, sizeof(leKeys));

    if (VmGetLocalIrk(&pirk))
    {
        memmove(&leKeys.id.irk, &pirk, sizeof(packed_irk));
    }
    else
    {
        return FALSE;
    ...
    memcpy(&leKeys.encCentral, &le_key, sizeof(leKeys.encCentral));
    leKeys.secReq = DM_SM_SECURITY_BONDING | DM_SM_SECURITY_MITM_PROTECTION | DM_SM_SECURITY_SECURE_CONNECTIONS;
    leKeys.keySize = sizeof(le_key.ltk);
    leKeys.keyValid = CSR_BT_TD_DB_LE_KEY_ENC_CENTRAL | CSR_BT_TD_DB_LE_KEY_ID;
