...
#define APP_PEER_SEC_REQ    ((SECL4_IN_SSP | SECL_IN_AUTHENTICATION | SECL_IN_ENCRYPTION) | (SECL4_OUT_SSP | SECL_OUT_AUTHENTICATION | SECL_OUT_ENCRYPTION))
#endif
...
static void appPeerSigConnectL2cap(const bdaddr *bd_addr)
{
    ...
    CmL2caConnectReqConftabSend(&peer_sig->task,
                                addr,
                                peer_sig->local_psm,
                                peer_sig->remote_psm,
                                APP_PEER_SEC_REQ,
    ...
    peer_sig->connect_req_sent = TRUE;
#else
...
#ifdef USE_SYNERGY
    CsrBtUtilRfcConCancel(PeerSigGetTaskData(), PeerSigGetTaskData()->sdp_search_data);
    CsrBtUtilSdcRfcDeinit(&PeerSigGetTaskData()->sdp_search_data);
#endif
...
static void appPeerSigEnterInitialising(void)
{
    ...
    /* Register a Protocol/Service Multiplexor (PSM) that will be
       ...
       ends. */
    ConnectionL2capRegisterRequest(&peer_sig->task, L2CAP_PSM_PEER_SIGNALING, 0);
}
...
#ifdef USE_SYNERGY
static const CsrBtUuid128 service_uuid_sync = {UUID_PEER_SYNC_SERVICE};

static uint16 appPeerSigExtractPsm(CmnCsrBtLinkedListStruct *sdpTagList)
{
                                               ...
                                               &dummy2))
    {
        if (tmpResult == SDR_SDC_SEARCH_SUCCESS &&
            !memcmp(tmpUuid, &service_uuid_sync, sizeof(service_uuid_sync)))
        {
...
static void appPeerSigSdpResultHandler(void *inst,
                                       CmnCsrBtLinkedListStruct *sdpTagList,
                                       CsrBtDeviceAddr deviceAddr,
                                       CsrBtResultCode resultCode,
                                       CsrBtSupplier resultSupplier)
{
    peerSigTaskData *peer_sig = (peerSigTaskData *) inst;

    DEBUG_LOG("appPeerSigSdpResultHandler, result:0x%04x supplier:0x%04x", resultCode, resultSupplier);
    ...
    switch (appPeerSigGetState())
    {
        case PEER_SIG_STATE_CONNECTING_SDP_SEARCH:
        {
            ...
            if (resultCode == CSR_BT_RESULT_CODE_CM_SUCCESS &&
                sdpTagList)
            {
                psm = appPeerSigExtractPsm(sdpTagList);
            }

            if (psm != L2CA_PSM_INVALID)
            { /* L2CAP PSM found */
                peer_sig->remote_psm = psm;
                appPeerSigConnectL2cap(&peer_sig->peer_addr);
                appPeerSigSetState(PEER_SIG_STATE_CONNECTING_LOCAL);
            }
            else
            {
                if (ConManagerIsConnected(&peer_sig->peer_addr) &&
                    peer_sig->sdp_search_attempts < appConfigPeerSigSdpSearchTryLimit())
                {
                    DEBUG_LOG("sdpResultHandler, retry attempts %d",
                              peer_sig->sdp_search_attempts);

                    CsrBtUtilRfcConCancel(inst, peer_sig->sdp_search_data);
                    appPeerSigSetState(PEER_SIG_STATE_CONNECTING_SDP_SEARCH);
                }
                else
                {
                    DEBUG_LOG("sdpResultHandler, SDP query failed");
                    appPeerSigSetState(PEER_SIG_STATE_DISCONNECTED);
                    MessageSend(&peer_sig->task,
        ...
        case PEER_SIG_STATE_DISCONNECTING:
            ...
            DEBUG_LOG("sdpResultHandler, cancelled");
            appPeerSigSetState(PEER_SIG_STATE_DISCONNECTED);
            break;

        default:
            DEBUG_LOG("sdpResultHandler, unexpected state 0x%x status 0x%x",
                      appPeerSigGetState(),
                      resultCode);
            Panic();
            break;
...
static void appPeerSigResultHandler(CsrSdcOptCallbackType cbType, void *context)
{
    CsrSdcResultFuncType *params = (CsrSdcResultFuncType *)context;

    if(cbType == CSR_SDC_OPT_CB_SEARCH_RESULT)
    {
        appPeerSigSdpResultHandler(params->instData, params->sdpTagList, params->deviceAddr, params->resultCode, params->resultSupplier);
    }
}
...
static void appPeerSigEnterSdpSearch(void)
{
    ...
    if (!peer_sig->sdp_search_data)
    {
        peer_sig->sdp_search_data = CsrBtUtilSdcInit(appPeerSigResultHandler,
                                                   TrapToOxygenTask(&peer_sig->task));
    }
    ...
    BdaddrConvertVmToBluestack(&addr, &peer_sig->peer_addr);
/* Start the SDP search */
    CsrBtUtilSdcSearchStart((void *) peer_sig,
                            peer_sig->sdp_search_data,
                            sdpTagList,
                            addr);
#else
    /* Perform SDP search */
...
#ifdef USE_SYNERGY
    else if(peer_sig->connect_req_sent && (peer_sig->pending_connects > 0))
    {
        DEBUG_LOG("appPeerSigEnterDisconnecting cancelling pending connect request");
        CsrBtDeviceAddr addr;
        BdaddrConvertVmToBluestack(&addr, &peer_sig->peer_addr);
        CmL2caCancelConnectReqSend(&peer_sig->task, addr, peer_sig->local_psm);
    }
#endif
}
    ...
    switch (appPeerSigGetState())
    {
        case PEER_SIG_STATE_DISCONNECTED:
        {
            ...
            if (ConManagerIsConnected(&req->peer_addr))
            {
                ...
                if(L2CAP_PSM_PEER_SIGNALING == L2CA_PSM_INVALID)
                {
                ...
                {
                    peer_sig->remote_psm = L2CAP_PSM_PEER_SIGNALING;
                    appPeerSigConnectL2cap(&peer_sig->peer_addr);
                    appPeerSigSetState(PEER_SIG_STATE_CONNECTING_LOCAL);
                }
            }
        ...
        case PEER_SIG_STATE_CONNECTING_SDP_SEARCH:
            {
                /* Cancel SDP search */
#ifdef USE_SYNERGY
                CsrBtUtilRfcConCancel(peer_sig, peer_sig->sdp_search_data);
                CsrBtUtilSdcRfcDeinit(&peer_sig->sdp_search_data);
#else
...
#ifdef USE_SYNERGY
static void appPeerSigHandleInternalCloseSdp(void)
{
    peerSigTaskData *peer_sig = PeerSigGetTaskData();
    DEBUG_LOG("appPeerSigHandleInternalCloseSdp, state %u", appPeerSigGetState());
...
#ifdef USE_SYNERGY
static void appPeerSigConnected(CsrBtConnId btConnId, CsrBtResultCode result)
{
    peerSigTaskData *peer_sig = PeerSigGetTaskData();
    ...
    switch (appPeerSigGetState())
    {
        ...
        case PEER_SIG_STATE_CONNECTING_REMOTE:
        {
            ...
            if (result == CSR_BT_RESULT_CODE_CM_SUCCESS)
            {
                ...
                DEBUG_LOG("appPeerSigConnected, connected, conn ID %u",
                                btConnId) ;

                PanicNull(sink);
                peer_sig->link_sink = sink;
                peer_sig->link_source = StreamSourceFromSink(sink);

                /* Set the sink in the marshal_common module */
                MarshalCommon_SetSink(peer_sig->link_sink);

                MessageStreamTaskFromSink(peer_sig->link_sink, &peer_sig->task);
                MessageStreamTaskFromSource(peer_sig->link_source,
                                            &peer_sig->task);

                PanicFalse(SinkConfigure(peer_sig->link_sink,
                                         VM_SINK_MESSAGES,
                                         VM_MESSAGES_ALL));
                PanicFalse(SourceConfigure(peer_sig->link_source,
                                           VM_SOURCE_MESSAGES,
                                           VM_MESSAGES_SOME));
            ...
            else
            {
                ...
                {
                    DEBUG_LOG("appPeerSigConnected, failed, go to disconnected state");
                ...
                {
                    DEBUG_LOG("appPeerSigConnected, failed, wait");
                }
            }
            break;
        }
        ...
        case PEER_SIG_STATE_DISCONNECTING:
        {
            ...
            DEBUG_LOG("appPeerSigConnected, cancelled, pending %u",
                      peer_sig->pending_connects);
        ...
        default:
        {
            ...
            DEBUG_LOG("appPeerSigConnected, failed, pending %u",
                            peer_sig->pending_connects) ;
...
#ifdef USE_SYNERGY
static void appPeerSigHandleCmL2caConnectAcceptInd(const CsrBtCmL2caConnectAcceptInd *ind)
{
    peerSigTaskData *peer_sig = PeerSigGetTaskData();
    DEBUG_LOG("appPeerSigHandleCmL2caConnectAcceptInd, state %u, psm %u", appPeerSigGetState(), ind->localPsm);
    PanicFalse(ind->localPsm == peer_sig->local_psm);
    bool accept = FALSE;
    const uint16 conftab[] =
    {
     ...
     /* Local Flush Timeout  - Accept Non-default Timeout*/
     L2CAP_AUTOPT_FLUSH_OUT,
     BKV_UINT32R(DEFAULT_L2CAP_FLUSH_TIMEOUT,
                 0),
                 L2CAP_AUTOPT_TERMINATOR
    };
    ...
    switch (appPeerSigGetState())
    {
        case PEER_SIG_STATE_DISCONNECTED:
        {
            ...
            {
                DEBUG_LOG("appPeerSigHandleCmL2caConnectAcceptInd, accepted");
            ...
            {
                DEBUG_LOG("appPeerSigHandleCmL2caConnectAcceptInd, rejected, unknown peer");

                /* Not a known peer, remember it just in case we're in the middle of pairing */
                peer_sig->peer_addr = bd_addr;
            }
        }
        break;
        ...
        default:
        {
            DEBUG_LOG("appPeerSigHandleCmL2caConnectAcceptInd, rejected, state %u", appPeerSigGetState());
        }
        break;
    }
    ...
    CmL2caConnectAcceptRspSend(&peer_sig->task,
                               accept,
                               ind->btConnId,
                               ind->localPsm,
                               ind->deviceAddr,
...
static void appPeerSigHandleCmL2caConnectAcceptCfm(const CsrBtCmL2caConnectAcceptCfm *cfm)
{
    DEBUG_LOG("appPeerSigHandleCmL2caConnectAcceptCfm, status %u supplier %u psm %u bdaddr %04x,%02x,%06lx", 
              cfm->resultCode,
              cfm->resultSupplier,
              cfm->localPsm,
              cfm->deviceAddr.nap,
    ...
    appPeerSigConnected(cfm->btConnId, cfm->resultCode);
}

static void appPeerSigHandleCmL2caConnectCfm(const CsrBtCmL2caConnectCfm *cfm)
{
    peerSigTaskData *peer_sig = PeerSigGetTaskData();

    DEBUG_LOG("appPeerSigHandleCmL2caConnectCfm, status %u supplier %u psm %u bdaddr %04x,%02x,%06lx pending_connects %d", 
              cfm->resultCode,
              cfm->resultSupplier,
              cfm->localPsm,
              cfm->deviceAddr.nap,
    ...
    peer_sig->connect_req_sent = FALSE;
    appPeerSigConnected(cfm->btConnId, cfm->resultCode);
}
#else
...
static void appPeerSigHandleCmL2caDisconnectInd(const CsrBtCmL2caDisconnectInd *ind)
{
    peerSigTaskData *peer_sig = PeerSigGetTaskData();

    DEBUG_LOG("appPeerSigHandleL2capDisconnectInd, status 0x%04x supplier 0x%04x", ind->reasonCode, ind->reasonSupplier);
    ...
    if(!ind->localTerminated)
    {
        ...
        if (sink == peer_sig->link_sink)
        {
            ...
            if (ind->reasonSupplier == CSR_BT_SUPPLIER_L2CAP_DISCONNECT &&
                ...
                !BdaddrIsZero(&peer_sig->peer_addr))
            {
                    ...
                    void *msg_ptr)
{
    ...
    switch (appPeerSigGetState())
    {
        ...
        {
#ifdef USE_SYNERGY
            /* ULCONV_TODO: Workaround to avoid panic if sink is not valid and 
             * peer signalling is still in connected state. This issue will be
             * revisited under ULCONV-203.
             */
            if(!SinkIsValid(peer_sig->link_sink))
            {
...
#ifdef USE_SYNERGY
static void appPeerSigHandleCmPrim(Message message)
{
    CsrBtCmPrim *prim = (CsrBtCmPrim *) message;

    if (*prim == CSR_BT_CM_SDS_REGISTER_CFM)
    {
        appPeerSigHandleClSdpRegisterCfm(((const CsrBtCmSdsRegisterCfm *) message)->resultCode);
    }
    else if (CsrBtUtilSdcVerifyCmMsg(prim))
    {
        peerSigTaskData *peer_sig = PeerSigGetTaskData();

        CsrBtUtilSdcCmMsgHandler(peer_sig,
                                 peer_sig->sdp_search_data,
                                 prim);
    }
    else
    {
        switch (*prim)
        {
            case CSR_BT_CM_L2CA_REGISTER_CFM:
            {
                ...
                appPeerSigHandleCmL2caRegisterCfm(cfm->resultCode, cfm->localPsm);
            }
            break;

            case CSR_BT_CM_L2CA_CONNECT_ACCEPT_IND:
                appPeerSigHandleCmL2caConnectAcceptInd((const CsrBtCmL2caConnectAcceptInd *) message);
                break;

            case CSR_BT_CM_L2CA_CONNECT_ACCEPT_CFM:
                appPeerSigHandleCmL2caConnectAcceptCfm((const CsrBtCmL2caConnectAcceptCfm *) message);
                break;

            case CSR_BT_CM_L2CA_CONNECT_CFM:
                appPeerSigHandleCmL2caConnectCfm((const CsrBtCmL2caConnectCfm *) message);
                break;

            case CSR_BT_CM_L2CA_DISCONNECT_IND:
                appPeerSigHandleCmL2caDisconnectInd((const CsrBtCmL2caDisconnectInd *) message);
                break;

            default:
                DEBUG_LOG("appPeerSigHandleCmPrim Unhandled CM Prim 0x%04x",
                          *prim);
                break;
        }
    }
...
#ifdef USE_SYNERGY
        case CM_PRIM:
            appPeerSigHandleCmPrim(message);
            break;
#else
...
#ifdef USE_SYNERGY
        case PEER_SIG_INTERNAL_CLOSE_SDP:
            appPeerSigHandleInternalCloseSdp();
            break;
#endif
        default:
