...
typedef enum scan_manager_messages
{
    ...
    /*! Extended Scanning result */
    LE_SCAN_MANAGER_START_EXTENDED_SCAN_CFM,
#endif
...
/*! \brief Data sent with LE_SCAN_MANAGER_START_EXTENDED_SCAN_CFM message. */
typedef struct {
    /*! Scan Start Status */
    le_scan_result_t status;
}LE_SCAN_MANAGER_START_EXTENDED_SCAN_CFM_T;

/*! Extended advertisement report to be sent to Application. */
typedef CL_DM_BLE_EXT_SCAN_FILTERED_ADV_REPORT_IND_T LE_SCAN_MANAGER_EXTENDED_ADV_REPORT_IND_T;
...
#ifdef ENABLE_LE_EXTENDED_SCANNING
/*! \brief Start BLE Extended Scanning using the provided filter parameters.

           Once scan is started, LE LE_SCAN_MANAGER_START_EXTENDED_SCAN_CFM message is sent
           as response to requester. \ref LE_SCAN_MANAGER_START_EXTENDED_SCAN_CFM_T for
           corresponding message data.
    ...
    \param[in] task   Requester's Task \ref Task
    \param[in] filter pointer to a advertising filter \ref le_extended_advertising_filter_t
