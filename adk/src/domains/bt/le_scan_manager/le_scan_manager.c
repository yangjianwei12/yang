...
#ifdef USE_SYNERGY
static void leScanManager_HandleAdverts(const CsrBtCmLeReportInd *scan);
static void leScanManager_HandleCmMessages(void *msg);
#else
...
#ifdef USE_SYNERGY
    CmLeScanReqSend(LeScanManagerGetTask(),
                    LE_SCAN_MANAGER_SCAN_REQ_CONTEXT_ENABLE,
                    is_active? CSR_BT_CM_LE_MODE_MODIFY: CSR_BT_CM_LE_MODE_OFF,
                    CSR_BT_CM_LE_SCANTYPE_ACTIVE,
                    sm_data->scan_parameters.scan_interval,
                    sm_data->scan_parameters.scan_window,
                    HCI_ULP_SCAN_FP_ALLOW_ALL,
...
static void leScanManager_FreeScanSettings(le_scan_settings_t *scan_settings)
{
    if(scan_settings)
    {
        if(scan_settings->filter.find_tpaddr)
        {
            free(scan_settings->filter.find_tpaddr);
        }

        if(scan_settings->filter.pattern)
        {
...
static le_scan_settings_t* leScanManager_AcquireScan(le_scan_interval_t scan_interval, le_advertising_report_filter_t* filter, Task task)
{
    ...
    if (settings_index < MAX_ACTIVE_SCANS)
    {
        ...
        memset(sm_data->active_settings[settings_index], 0 , sizeof(*scan_settings));
...
#ifdef USE_SYNERGY
    CmLeScanReqSend(LeScanManagerGetTask(),
                    LE_SCAN_MANAGER_SCAN_REQ_CONTEXT_PARAM,
...
static void leScanManager_HandleExtendedStopCfm(LE_SCAN_MANAGER_EXTENDED_STOP_CFM_T *message)
{
    scanCommand scan_command = LeScanManagerGetCurrentCommand();
    le_scan_manager_data_t * sm_data = LeScanManagerGetTaskData();
    ...
    if (scan_command == LE_SCAN_MANAGER_CMD_STOP)
    {
        if (message->result.status == LE_SCAN_MANAGER_RESULT_BUSY)
        {
            PanicFalse(LeScanManager_ExtendedScanStop(LeScanManagerGetTask(), message->scan_task));
        }
        else
        {
            sm_data->is_busy = FALSE;
            leScanManager_SendStopCfm(sm_data->requester,message->result);
        }
    }
}

static void leScanManager_HandleExtendedDisableCfm(LE_SCAN_MANAGER_EXTENDED_DISABLE_CFM_T *message)
{
    scanCommand scan_command = LeScanManagerGetCurrentCommand();
    le_scan_manager_data_t * sm_data = LeScanManagerGetTaskData();
    ...
    if (scan_command == LE_SCAN_MANAGER_CMD_DISABLE)
    {
        if (message->result.status == LE_SCAN_MANAGER_RESULT_BUSY)
        {
            PanicFalse(leScanManager_ExtendedScanDisable(LeScanManagerGetTask()));
        }
        else
        {
            sm_data->is_busy = FALSE;
...
static void leScanManager_HandleExtendedEnableCfm(LE_SCAN_MANAGER_EXTENDED_ENABLE_CFM_T *message)
{
    scanCommand scan_command = LeScanManagerGetCurrentCommand();
    le_scan_manager_data_t * sm_data = LeScanManagerGetTaskData();
    ...
    if  (scan_command == LE_SCAN_MANAGER_CMD_ENABLE)
    {
        if (message->result.status == LE_SCAN_MANAGER_RESULT_BUSY)
        {
            PanicFalse(leScanManager_ExtendedScanEnable(LeScanManagerGetTask()));
        }
        else
        {
            sm_data->is_busy = FALSE;
...
void leScanManager_Handler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
#ifdef USE_SYNERGY
        case CM_PRIM:
            leScanManager_HandleCmMessages((void *) message);
            break;
#else
        ...
        case LE_SCAN_MANAGER_PERIODIC_ENABLE_CFM:
            ...
            break;
#ifdef ENABLE_LE_EXTENDED_SCANNING
        case LE_SCAN_MANAGER_EXTENDED_STOP_CFM:
            leScanManager_HandleExtendedStopCfm((LE_SCAN_MANAGER_EXTENDED_STOP_CFM_T*)message);
        break;
        case LE_SCAN_MANAGER_EXTENDED_DISABLE_CFM:
            leScanManager_HandleExtendedDisableCfm((LE_SCAN_MANAGER_EXTENDED_DISABLE_CFM_T*)message);
        break;
        case LE_SCAN_MANAGER_EXTENDED_ENABLE_CFM:
            leScanManager_HandleExtendedEnableCfm((LE_SCAN_MANAGER_EXTENDED_ENABLE_CFM_T*)message);
        break;
#endif
        default:
            break;
...
#ifdef USE_SYNERGY
static bool leScanManager_FilterAddress(const TYPED_BD_ADDR_T* scan_taddr)
#else
...
#ifdef USE_SYNERGY
            BdaddrConvertTypedBluestackToVm(&(current_addr.taddr), scan_taddr);
#else
            memcpy(&(current_addr.taddr), scan_taddr, sizeof(typed_bdaddr));
#endif
            
            /* Retrieve the permanent address if current_addr is a RPA, or  if
               it is already a public address, copy it to public_addr. */
            if (!ConManagerResolveTpaddr(&current_addr, &public_addr))
            {
                return FALSE;
            }
...
static void leScanManager_HandleAdverts(const CsrBtCmLeReportInd *scan)
{
    ...
    le_scan_manager_data_t *sm_data = LeScanManagerGetTaskData();
    size_t size = sizeof(LE_SCAN_MANAGER_ADV_REPORT_IND_T)+(sizeof(uint8) * (scan->lengthData - 1));
    if(current_state == LE_SCAN_MANAGER_STATE_SCANNING)
    {

        DEBUG_LOG("leScanManager_HandleAdverts. Advert: rssi:%d bdaddr %04X:%02X:%06lX",
                   scan->rssi,
                   scan->address.addr.nap,scan->address.addr.uap,scan->address.addr.lap);

        MAKE_CL_MESSAGE_WITH_LEN(LE_SCAN_MANAGER_ADV_REPORT_IND,size);

        message->num_reports = 1;
        message->event_type =  scan->eventType;
        message->rssi = scan->rssi;
        message->size_advertising_data = scan->lengthData;
        
        memcpy(message->advertising_data, scan->data, scan->lengthData);

        message->current_taddr.type =  scan->address.type;
        message->current_taddr.addr.nap =  scan->address.addr.nap;
        message->current_taddr.addr.uap =  scan->address.addr.uap;
        message->current_taddr.addr.lap =  scan->address.addr.lap;

        message->permanent_taddr.type =  scan->permanentAddress.type;
        message->permanent_taddr.addr.nap =  scan->permanentAddress.addr.nap;
        message->permanent_taddr.addr.uap =  scan->permanentAddress.addr.uap;
        message->permanent_taddr.addr.lap =  scan->permanentAddress.addr.lap;
        
        TaskList_MessageSendWithSize(sm_data->client_list, LE_SCAN_MANAGER_ADV_REPORT_IND, message,size);

        /* If client task is interested in finding specific device then resolve all RPA devices to check for a match */
        if((sm_data->confirmation_settings != NULL) &&
           leScanManager_FilterAddress(&(scan->address)))
        {
            MAKE_CL_MESSAGE_WITH_LEN_TO_TASK(LE_SCAN_MANAGER_ADV_REPORT_IND,size);

            message_task->num_reports = 1;
            message_task->event_type =  scan->eventType;
            message_task->rssi = scan->rssi;
            message_task->size_advertising_data = scan->lengthData;
            
            memcpy(message_task->advertising_data, scan->data, scan->lengthData);

            message_task->current_taddr.type =  scan->address.type;
            message_task->current_taddr.addr.nap =  scan->address.addr.nap;
            message_task->current_taddr.addr.uap =  scan->address.addr.uap;
            message_task->current_taddr.addr.lap =  scan->address.addr.lap;
...
#ifdef ENABLE_LE_EXTENDED_SCANNING
        else if (leScanManager_ExtendedScanEnable(LeScanManagerGetTask()))
        {
            /* Wait for confirmation message from Extended enable */
            sm_data->requester = task;
            sm_data->is_busy = TRUE;
            LeScanManagerSetCurrentCommand(LE_SCAN_MANAGER_CMD_ENABLE);
        }
...
#ifdef ENABLE_LE_EXTENDED_SCANNING
        else if (leScanManager_ExtendedScanDisable(LeScanManagerGetTask()))
        {
            /* Wait for confirmation message from Extended disable */
            sm_data->requester = task;
            sm_data->is_busy = TRUE;
            LeScanManagerSetCurrentCommand(LE_SCAN_MANAGER_CMD_DISABLE);
        }
...
#ifdef ENABLE_LE_EXTENDED_SCANNING
        else if (LeScanManager_ExtendedScanStop(LeScanManagerGetTask(), task))
        {
            /* Wait for confirmation message from Extended stop */
            sm_data->requester = task;
            sm_data->is_busy = TRUE;
            LeScanManagerSetCurrentCommand(LE_SCAN_MANAGER_CMD_STOP);
        }
...
#ifdef USE_SYNERGY
    CmRegisterHandlerReqSend(CSR_BT_CM_HANDLER_TYPE_LE, LeScanManagerGetTask(), 0);
#endif
    leScanManager_PeriodicScanInit();

#ifdef ENABLE_LE_EXTENDED_SCANNING
    LeScanManager_ExtendedScanInit();
#endif
...
#ifdef USE_SYNERGY
static void leScanManager_HandleCmScanCfm(const CsrBtCmLeScanCfm *cfm)
{
    DEBUG_LOG("leScanManager_HandleCmScanCfm context: %d, scanMode: %d, resultCode: 0x%04x, supplier: 0x%04x",
              cfm->context,
              cfm->scanMode,
              cfm->resultCode,
              cfm->resultSupplier);

    if (cfm->context == LE_SCAN_MANAGER_SCAN_REQ_CONTEXT_ENABLE)
    {
        leScanManager_HandleScanEnable((cfm->resultCode == CSR_BT_RESULT_CODE_CM_SUCCESS)?success:fail);
    }
}
...
static void leScanManager_HandleCmMessages(void *msg)
{
    ...
    switch (*primType)
    {
        case CSR_BT_CM_LE_SCAN_CFM:
            leScanManager_HandleCmScanCfm((const CsrBtCmLeScanCfm *) msg);
            break;

        case CSR_BT_CM_LE_REPORT_IND:
            leScanManager_HandleAdverts((const CsrBtCmLeReportInd *) msg);
            break;

        default:
            if (leScanManager_HandlePeriodicCmMessages(msg))
            {
                return;
            }
            DEBUG_LOG("leScanManager_HandleCmMessages Unhandled CM Primitive: 0x%x", *primType);
            break;
    }
...
#ifdef ENABLE_LE_EXTENDED_SCANNING
    else if (LeScanManager_IsExtendedTaskScanning(task))
    {
        return TRUE;
    }
#endif
...
#ifdef ENABLE_LE_EXTENDED_SCANNING
/****************** EXTENDED SCANNING ADDITIONS **************************************/

void LeScanManager_StartExtendedScan(Task task, le_extended_advertising_filter_t* filter)
{
    LeScanManager_ExtendedScanStart(task, filter);
}
#endif
