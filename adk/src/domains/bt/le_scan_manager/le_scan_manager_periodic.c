...
#ifdef USE_SYNERGY
    UNUSED(id);
    leScanManager_HandlePeriodicCmMessages((void *) message);
#else
...
bool leScanManager_HandlePeriodicCmMessages(void *msg)
{
    ...
    switch (*primType)
    {
        case CSR_BT_CM_PERIODIC_SCAN_START_FIND_TRAINS_CFM:
            {
                CmPeriodicScanStartFindTrainsCfm *cmMessage = (CmPeriodicScanStartFindTrainsCfm *)msg;
                CL_DM_BLE_PERIODIC_SCAN_START_FIND_TRAINS_CFM_T clMessage;
                clMessage.scan_handle = cmMessage->scanHandle;
                clMessage.status = (cmMessage->resultCode != CSR_BT_RESULT_CODE_SUCCESS) ? fail : success;
                leScanManager_handleConnectionDmBlePeriodicScanStartFindTrainsCfm(&clMessage);
            }
            break;
        case CSR_BT_CM_EXT_SCAN_FILTERED_ADV_REPORT_IND:
            {
                CmExtScanFilteredAdvReportInd *cmMessage = (CmExtScanFilteredAdvReportInd *)msg;
        ...
        case CSR_BT_CM_PERIODIC_SCAN_STOP_FIND_TRAINS_CFM:
            {
                CmPeriodicScanStopFindTrainsCfm *cmMessage = (CmPeriodicScanStopFindTrainsCfm *)msg;
                CL_DM_BLE_PERIODIC_SCAN_STOP_FIND_TRAINS_CFM_T clMessage;
                clMessage.status = (cmMessage->resultCode != CSR_BT_RESULT_CODE_SUCCESS) ? fail : success;
                leScanManager_handleConnectionDmBlePeriodicScanStopFindTrainsCfm(&clMessage);
            }
            break;
