...
static void uiLeds_DoSetPattern(const ui_led_data_t *config, uint16 led_index)
{
    ...
    DEBUG_LOG_VERBOSE("uiLeds_DoSetPattern enum:led_priority_t:%d indicate=%d led_index=%04X", config->priority, indicate, led_index);
...
static void uiLeds_DoFlashIndication(uint16 led_index)
{
    ...
    DEBUG_LOG_VERBOSE("uiLeds_DoFlashIndication index=%04x", led_index);
...
static bool uiLeds_CheckForSupercedingContext(uint16 led_index)
{
    ...
    {
        DEBUG_LOG_V_VERBOSE("uiLeds_CheckForSupercedingContext initial_index=%04x, index_to_indicate=%04x",
        ...
        do
        {
                ...
                curr_context == Ui_GetUiProviderContext(curr_provider))
            {
                ...
                DEBUG_LOG_V_VERBOSE("uiLeds_CheckForSupercedingContext was superceded by enum:ui_providers_t:%d", curr_provider);
                break;
            }
            curr_context_index++;
        }
...
static void uiLeds_DoContextUpdate(UI_PROVIDER_CONTEXT_UPDATED_T * msg)
{
    ...
    DEBUG_LOG_VERBOSE("uiLeds_DoContextUpdate enum:ui_providers_t:%d Context=%02x", msg->provider, msg->context);
    ...
    if (uiLeds_getMatchingContextIndex(msg->provider, msg->context, &led_index))
    {
                 ...
                 msg->provider == the_leds.curr_indicated_provider)
        {
            ...
            if (!was_superceded)
            {
                DEBUG_LOG("uiLeds_DoContextUpdate not superceded.");
                uiLeds_DoContextFlashIndication(led_index, msg->provider);
            }
        }
    } else {
        ...
        if (the_leds.curr_indicated_provider == msg->provider)
        {
            ...
            {
                DEBUG_LOG("uiLeds_DoContextUpdate no matching contexts");
...
static bool uiLeds_HasActiveContextFlash(void)
{
    return !(the_leds.curr_indicated_context_index == LED_NONE && the_leds.curr_indicated_provider == LED_NONE);
}
...
void UiLeds_NotifyUiIndication(uint16 led_index)
{
    ...
    DEBUG_LOG_VERBOSE("UiLeds_NotifyUiIndication led_index=%04X", led_index);
...
void UiLeds_RefreshLedContextIndication(void)
{
    DEBUG_LOG_VERBOSE("UiLeds_RefreshLedContextIndication");

    if (uiLeds_HasActiveContextFlash())
    {
        uiLeds_DoContextFlashIndication(the_leds.curr_indicated_context_index,
                                        the_leds.curr_indicated_provider);
    }
}
