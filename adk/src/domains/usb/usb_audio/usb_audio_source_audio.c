...
static source_state_t usbAudio_GetState(audio_source_t source);
static device_t usbAudio_GetDevice(audio_source_t source);
...
static const audio_source_audio_interface_t usb_audio_interface =
{
    ...
    .GetState = usbAudio_GetState,
    .Device = usbAudio_GetDevice,
};
                                       ...
                                       source_defined_params_t * source_params)
{
    ...
    if(usbAudio_IsAudioAvailable(source))
    {
        ...
        usb_audio->headphone->spkr_streaming_config_index = streaming_info->streaming_desc_index;
...
static source_state_t usbAudio_GetState(audio_source_t source)
{
    ...
    if(usb_audio != NULL)
    {
        state = usb_audio->headphone->source_state;
        DEBUG_LOG_DEBUG("usbAudio_GetState(%p) enum:source_state_t:%d", usb_audio, state);
    }
