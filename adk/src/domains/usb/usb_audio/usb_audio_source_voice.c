...
static source_state_t usbAudio_GetVoiceState(voice_source_t source);
...
static const voice_source_audio_interface_t usb_voice_interface =
{
    ...
    .GetState = usbAudio_GetVoiceState
};
                                       ...
                                       source_defined_params_t * source_params)
{
    ...
    if(usbAudio_IsVoiceAvailable(source))
    {
        ...
        usb_audio->headset->mic_streaming_config_index = streaming_info->streaming_desc_index;
                ...
                (!usb_audio->headset->spkr_active && headphone_audio && headphone_audio->headphone->spkr_active))
        {
            ...
            PanicNull(streaming_info);
            headphone_audio->headphone->spkr_streaming_config_index = streaming_info->streaming_desc_index;
...
#endif
        {
            ...
            PanicNull(streaming_info);
            usb_audio->headset->spkr_streaming_config_index = streaming_info->streaming_desc_index;
...
static source_state_t usbAudio_GetVoiceState(voice_source_t source)
{
    ...
    if(usb_audio != NULL)
    {
        state = usb_audio->headset->source_state;
        DEBUG_LOG_DEBUG("usbAudio_GetVoiceState(%p) enum:source_state_t:%d", usb_audio, state);
    }
