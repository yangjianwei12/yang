...
static void usbAudioHandleAudioConfigChanged(usb_audio_info_t *usb_audio, usb_audio_status_message_t msg_id, bool reconnect_immediately)
{
    DEBUG_LOG_DEBUG("usbAudioHandleAudioConfigChanged");

    if (usb_audio_client_cb[USB_AUDIO_REGISTERED_CLIENT_CONFIG_CHANGE])
    {
        ...
        USB_AUDIO_MSG_SEND(usb_audio_client_cb[USB_AUDIO_REGISTERED_CLIENT_CONFIG_CHANGE], msg_id,
                           (Message)(&usb_audio->headphone->audio_source));
    }
...
static void usbAudioHandleVoiceConfigChanged(usb_audio_info_t *usb_audio, usb_audio_status_message_t msg_id, bool reconnect_immediately)
{
    DEBUG_LOG_DEBUG("usbAudioHandleVoiceConfigChanged");
...
static void usbAudioUpdateConnections(void)
{
    ...
    if (headset_audio)
    {
        if (headset_audio->headset->mic_active || headset_audio->headset->spkr_active)
        {
            ...
            {
                usbAudioHandleVoiceConfigChanged(headset_audio, USB_AUDIO_STREAMING_INTERFACE_CHANGED_VOICE_IND, FALSE);
            }
#endif
...
static void usbAudioUpdateDeviceStatus(usb_audio_info_t *usb_audio, uint8 interface_index)
{
    ...
    switch(intf_config->type)
    {
        ...
        case USB_AUDIO_DEVICE_TYPE_AUDIO_SPEAKER:
            ...
            if(alt_setting != INTF_ALT_SETTING_ZERO)
            {
                ...
                {
                    usbAudioHandleVoiceConfigChanged(headset_audio, USB_AUDIO_STREAMING_CONFIG_CHANGED_VOICE_IND, FALSE);
    ...
    if(alt_setting != INTF_ALT_SETTING_ZERO)
    {
        ...
        {
            usbAudioHandleVoiceConfigChanged(usb_audio, USB_AUDIO_STREAMING_CONFIG_CHANGED_VOICE_IND, FALSE);
            /* If disconnected, connection message will send from usbAudioUpdateConnections() */
        }

        if(headphone_streaming_config_changed)
        {
            usbAudioHandleAudioConfigChanged(usb_audio, USB_AUDIO_STREAMING_CONFIG_CHANGED_AUDIO_IND, FALSE);
            /* If disconnected, connection message will send from usbAudioUpdateConnections() */
        }
...
static void usbAudioUpdateHeadphoneVolume(usb_audio_info_t *usb_audio)
{
    if(usb_audio->headphone != NULL)
    {
        ...
        /* notify clients of mute status */
        USB_AUDIO_MSG_SEND(usb_audio_client_cb[USB_AUDIO_REGISTERED_CLIENT_STATUS],
                           (in_mute)?USB_AUDIO_HEADPHONE_MUTE_ACTIVE:USB_AUDIO_HEADPHONE_MUTE_INACTIVE,
                           (Message)(&usb_audio->headphone->audio_source));
    }
}
...
static void usbAudioSetDeviceSamplingRate(usb_audio_info_t *usb_audio, uint8 interface_index)
{
    ...
    {
        usbAudioHandleVoiceConfigChanged(usb_audio, USB_AUDIO_SAMPLE_RATE_VOICE_IND, TRUE);
    }

    if(is_headphone_rate_modified)
    {

        usbAudioHandleAudioConfigChanged(usb_audio, USB_AUDIO_SAMPLE_RATE_AUDIO_IND, TRUE);

#ifdef USB_SUPPORT_HEADPHONE_SPKR_IN_VOICE_CHAIN
        usb_audio_info_t *headset_audio = UsbAudioFd_GetHeadsetInfo(voice_source_usb);
        if(headset_audio && headset_audio->headset->alt_spkr_connected)
        {
            usbAudioHandleVoiceConfigChanged(headset_audio, USB_AUDIO_SAMPLE_RATE_VOICE_IND, TRUE);
        }
#endif
    }
}
