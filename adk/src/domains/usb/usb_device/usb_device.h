...
/*! Maximum number of USB Hosts, can be connected, at the same time */
#define USB_DEVICE_MAX_USB_HOSTS_SUPPORTED   1

/*! \brief Result of USB framework function call */
typedef enum
{
    /*! Call was a success */
    USB_RESULT_OK,
...
/*! \brief Unique index of USB device instance */
 ...
 *
 * \param index_\ptr Location to store unique USB device index assigned to
...
/*! \brief Function that is called when device is fully released.
 *
 * \param index USB device index */
typedef void (*usb_device_released_handler_t)(usb_device_index_t index);

/*! \brief  Delete USB device instance created previously
 *
...
/*! \brief Callback for configuring device
 *
...
/*! \brief Register device configuration callback
 *
 * \param index USB device index
...
/*! \brief Enable automatic generation of USB serial number strings
 *
...
/*! \brief Allocates unique endpoint address
 *
...
/*! \brief Create USB class callback, should return context data
 *
 * \param index USB device index
...
/*! \brief Delete USB class deallocating context data.
 *
...
/*! \brief Handle SetInterface control request from the host
 *
...
/*! \brief Class interface callbacks */
typedef struct
{
...
/*! \brief Class interface structure
 *
...
/*! \brief Register USB class with the framework.
 *
 * \param index USB device index
...
/*! \brief Notify USB device framework that device class has been fully released
...
/*! \brief Callback for handling USB events
 *
...
/*! \brief Attach USB device to the host
 *
...
/*! \brief Detach USB device from the host
 *
 * \param index USB device index
 * \return result code indicating either success or reason for an error.
 */
...
/*! \brief Handle USB messages received by the system task
 *
...
/*! \brief Register USB events handler
 *
...
/*! \brief Unregister USB events handler
 *
...
/*! \brief USB Device messages sent to registered clients */
typedef enum
{
...
/*! \brief Initialise USB device framework
 *
...
/*! \brief check whether USB attached to host or not
 *
...
/*! \brief Register client task to receive USB device messages
 *
...
/*! \brief UnRegister client task to stop receiving USB device messages
 *
 * \param client_task client task data */
#ifdef INCLUDE_USB_DEVICE
