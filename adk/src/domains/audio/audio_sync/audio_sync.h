...
\file       audio_sync.h
\defgroup   audio_sync Audio Sync
...
typedef enum
{
    ...
    AUDIO_SYNC_DESTROY_IND,
    AUDIO_SYNC_DESTROY_RES,
...
/*! \brief Notification that a source will be destroyed

    Sent by the audio source to notify that it is about to be destroyed

    The audio sink must send a AUDIO_SYNC_DESTROY_RES once it has cleared up
    any stale references to the sync interface.
*/
typedef struct
{
    Task task; /*!< The task to send the response, or NULL for no response. */
    audio_source_t source_id; /*!< The audio source. */
    uint8 sync_id; /*!< The sequence ID for this sync indication. */
} AUDIO_SYNC_DESTROY_IND_T;

/*! \brief Response to a AUDIO_SYNC_DESTROY_IND */
typedef struct
{
    uint8 sync_id; /*!< The sequence ID for this sync indication. */
} AUDIO_SYNC_DESTROY_RES_T;
...
/*! \brief Indicate that an audio sink must prepare for the source to be destroyed

    The indication is sent as a AUDIO_SYNC_DESTROY_IND to an audio sink instance.

    The audio sink instance represented by #sync_inst must clear any stale references
    to the sync interface and send AUDIO_SYNC_DESTROY_RES to #reply_task. 
