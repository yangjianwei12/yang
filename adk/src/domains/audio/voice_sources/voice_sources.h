...
/*! \brief Calls the OnVoiceRouted observer function of a sources registered observer interface.
    \param source The voice source
    \param change Indicates whether the voice source became routed or unrouted
 */
void VoiceSources_OnVoiceRoutingChange(voice_source_t source, source_routing_change_t change);
...
/*! \brief Check if the voice source has a ringing incoming call.
    \param source The voice source
 */
...
/*! \brief Get the state of a voice source

    \param source The voice source

    \return The source's routed state
 */
source_state_t VoiceSources_GetState(voice_source_t source);
