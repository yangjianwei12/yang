...
source_state_t VoiceSources_GetState(voice_source_t source)
{
    ...
    DEBUG_LOG_FN_ENTRY("VoiceSources_GetState enum:voice_source_t:%d", source);

    if(voiceSources_IsSourceRegistered(source) && (voice_source_audio_interfaces[source]->GetState))
    {
        state = voice_source_audio_interfaces[source]->GetState(source);
    }
    return state;
}
