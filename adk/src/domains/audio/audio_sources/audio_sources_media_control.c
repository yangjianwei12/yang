...
static const media_context_override_interface_t *audio_sources_media_context_override_interface = NULL;
...
unsigned AudioSources_GetSourceContext(audio_source_t source)
{
    ...
    if ((interface != NULL) && (interface->Context))
    {
        ...
        if ((audio_sources_media_context_override_interface != NULL) &&
            (audio_sources_media_context_override_interface->ContextOverride))
        {
            audio_sources_media_context_override_interface->ContextOverride(source, &context);
        }
    }

    DEBUG_LOG_V_VERBOSE("AudioSources_GetSourceContext enum:audio_source_t:%d enum:audio_source_provider_context_t:%d",
              source, context);
...
device_t AudioSources_GetMediaControlSourceDevice(audio_source_t source)
{
    ...
    DEBUG_LOG_VERBOSE("AudioSources_GetMediaControlSourceDevice enum:audio_source_t:%d", source);
...
bool AudioSources_RegisterMediaControlContextOverrideInterface(const media_context_override_interface_t * interface)
{
    ...
    if (audio_sources_media_context_override_interface == NULL)
    {
        audio_sources_media_context_override_interface = interface;
        success = TRUE;
    }
