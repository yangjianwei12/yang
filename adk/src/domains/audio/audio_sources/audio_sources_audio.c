...
void AudioSources_RegisterAudioInterface(audio_source_t source, const audio_source_audio_interface_t * interface)
{
    ...
    DEBUG_LOG("AudioSources_RegisterAudioInterface source enum:audio_source_t:%d interface=%p", source, interface);
...
bool AudioSources_IsAudioRouted(audio_source_t source)
{
    ...
    {
        is_routed = interface->IsAudioRouted(source);
        DEBUG_LOG("AudioSources_IsAudioRouted source enum:audio_source_t:%d, routed=%d", source, is_routed);
    }
    return is_routed;
}
...
bool AudioSources_IsAudioAvailable(audio_source_t source)
{
    ...
    if ((interface != NULL) && (interface->IsAudioAvailable))
    {
        ...
        DEBUG_LOG("AudioSources_IsAudioAvailable source enum:audio_source_t:%d, available=%d", source, is_available);
    }
    return is_available;
}
...
source_state_t AudioSources_GetState(audio_source_t source)
{
    DEBUG_LOG_FN_ENTRY("AudioSources_GetState enum:audio_source_t:%d", source);

    audio_source_audio_interface_t * interface = audioSources_GetRegisteredInterface(source);
    source_state_t state = source_state_invalid;

    if ((interface != NULL) && (interface->GetState))
    {
        state = interface->GetState(source);
    }
    return state;
}
