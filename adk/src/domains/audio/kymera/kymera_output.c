...
#define KICK_PERIOD_FAST_VOL_CNTRL_BUFFER 1000U
#else
...
static struct
{
    ...
    unsigned bypass_limiter:1;
} state;

static bool kymeraOutput_GetDspClockConfig(audio_dsp_clock_configuration * user_config, audio_power_save_mode * user_mode);
static const kymera_dsp_clock_user_if_t kymera_output_dsp_clock_user_if = { .get_dsp_clock_config = kymeraOutput_GetDspClockConfig };
...
static void kymeraConfigureVolumeControlAuxBuffer(kymera_chain_handle_t chain, const kymera_output_chain_config *config)
{
    ...
    switch(config->kick_period)
    {
        ...
        /*
            ...
            The buffer size in KICK_PERIOD_FAST case is increased to 1000 words for LEA support build as 480 words is not large enough 
            to hold enough samples for the kick to occur on time (for aptX Adaptive LEA codec).
         */
...
static void kymeraOutput_ConfigureGeq(kymera_chain_handle_t chain, const kymera_output_chain_config *config)
{
    Operator op_geq;
    op_geq = ChainGetOperatorByRole(chain,OPR_GEQ);
    if(op_geq)
    {
        Kymera_SetOperatorUcid(chain, OPR_GEQ, UCID_GEQ_BYPASS);
        DEBUG_LOG("kymeraOutput_ConfigureGeq: rate %d", config->rate);
        OperatorsStandardSetSampleRate(op_geq, config->rate);
        OperatorsGeqSetSysmodeCtrl(op_geq, geq_sysmode_full);
    }
}

Operator kymeraOutput_GetGeqOpId(void)
{
    kymera_chain_handle_t chain = KymeraGetTaskData()->chain_output_handle;
    Operator geq_op = ChainGetOperatorByRole(chain,OPR_GEQ);
    return geq_op;
}
#endif
...
static void kymeraOutput_ConfigureOperators(kymera_chain_handle_t chain, const kymera_output_chain_config *config)
{
...
#ifdef ENABLE_CONTINUOUS_EARBUD_FIT_TEST
    kymeraOutput_ConfigureGeq(chain, config);
#endif
    ...
    Kymera_SetOperatorUcid(chain, OPR_SOURCE_SYNC, UCID_SOURCE_SYNC);
#ifdef INCLUDE_KYMERA_COMPANDER
    if(!state.bypass_limiter)
    {
...
static const chain_config_t * kymeraOutput_GetChainConfig(output_chain_t chain_type)
{
    ...
    switch(chain_type)
    {
        ...
        case output_chain_common:
            ...
            break;
        case output_chain_mono_le:
            config = Kymera_GetChainConfigs()->chain_output_volume_mono_le_config;
            break;
        case output_chain_stereo_le:
            config = Kymera_GetChainConfigs()->chain_output_volume_stereo_le_config;
            break;
    }
...
static void kymeraOutput_SetOverallSampleRate(uint32 rate)
{
    ...
    state.input_rates.background = rate;
}
...
static void kymeraOutput_StartInputResamplers(void)
{
    ...
    kymeraOutput_StartResampler(state.resamplers.background_input);
}
...
static bool kymeraOutput_GetDspClockConfig(audio_dsp_clock_configuration * user_config, audio_power_save_mode * user_mode)
{
    ...
    if(KymeraGetTaskData()->chain_output_handle && GET_OP_FROM_CHAIN(mixer, KymeraGetTaskData()->chain_output_handle, OPR_MIXER))
    {
        /* Mixer output chains always require a base clock config as a minimum */
        user_config->active_mode = AUDIO_DSP_BASE_CLOCK;
        status = TRUE;
    }
...
uint32 KymeraOutput_GetBackgroundSampleRate(void)
{
    // Currently other modules create output chains as well (Voice Chains), in those cases the API should return the commonly used output rate
    return (state.input_rates.background) ? state.input_rates.background : KymeraGetTaskData()->output_rate;
}
...
void KymeraOutput_SetBackgroundSampleRate(uint32 rate)
{
    // Can only set after chain creation
    PanicFalse(KymeraGetTaskData()->chain_output_handle != NULL);
    state.input_rates.background = rate;
...
void KymeraOutput_ConnectToBackgroundInput(Source left, Source right)
{
    input_t connections[] =
    {
        {left, ChainGetInput(KymeraGetTaskData()->chain_output_handle, EPR_SINK_STEREO_MIXER_BACKGROUND_L)},
        {right, ChainGetInput(KymeraGetTaskData()->chain_output_handle, EPR_SINK_STEREO_MIXER_BACKGROUND_R)},
    };

    kymeraOutput_ConnectViaResampler(connections, ARRAY_DIM(connections), &state.resamplers.background_input, KymeraOutput_GetBackgroundSampleRate());
}
...
void KymeraOutput_DisconnectBackgroundInput(void)
{
    chain_endpoint_role_t in_roles[] =
    {
    ...
    kymeraOutput_DisconnectViaResampler(in_roles, ARRAY_DIM(in_roles), &state.resamplers.background_input);
}
...
void KymeraOutput_Init(void)
{
    appKymeraRegisterDspClockUser(&kymera_output_dsp_clock_user_if);
}

#ifdef INCLUDE_KYMERA_COMPANDER
void KymeraOutput_EnableSoundLimiter(void)
{
    state.bypass_limiter = FALSE;
}

void KymeraOutput_DisableSoundLimiter(void)
{
    if(KymeraOutput_GetOutputHandle())
    {
        DEBUG_LOG_WARN("KymeraOutput_DisableSoundLimiter: Cannot disable - chain already instantiated");
    }
    state.bypass_limiter = TRUE;
}
#endif
