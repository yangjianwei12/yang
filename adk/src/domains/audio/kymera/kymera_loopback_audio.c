...
void appKymeraCreateLoopBackAudioChain(uint16 mic_number, uint32 sample_rate)
{
    ...
    if (!appKymeraIsBusy())
    {
        ...
        Microphones_SetMicRate(mic_number, sample_rate, high_priority_user);
        Source mic = Microphones_TurnOnMicrophone(mic_number, high_priority_user);
...
void appKymeraDestroyLoopbackAudioChain(uint16 mic_number)
{
    ...
    {
        KymeraOutput_SetAuxVolume(0);
        Microphones_TurnOffMicrophone(mic_number, high_priority_user);
