...
typedef struct
{
    ...
    /*! Low latency codec streaming active */
    unsigned ll_stream_state:2;

    /*! High Bandwidth (ie 96K) stream active */
    unsigned hbw_stream_state:1;

    /* Stream modifications options to set for the BT */
    unsigned stream_modifier:2;
...
typedef enum
{
    ...
    LOW_LATENCY_STREAM_ACTIVE_APTX_ADAPTIVE,
    LOW_LATENCY_STREAM_ACTIVE_GAMING_MODE
}ll_stream_state_t;
...
typedef enum
{
    STREAMING_MODIFIER_NONE,
    STREAMING_MODIFIER_LL_HBW,
    STREAMING_MODIFIER_GAMING
}streaming_modifier_t;

typedef struct
{
    streaming_modifier_t modifier;
}KYMERA_STREAM_MODIFIER_CHANGED_IND_T;
...
#define Kymera_LatencyMangerIsLLStreamActive() (KymeraGetLatencyData()->ll_stream_state != LOW_LATENCY_STREAM_INACTIVE)
#define Kymera_LatencyMangerIsHBStreamActive() (KymeraGetLatencyData()->hbw_stream_state == HIGH_BANDWIDTH_STREAM_ACTIVE)
#define Kymera_LatencyMangerISStreamModActive() (KymeraGetLatencyData()->stream_modifier != STREAMING_MODIFIER_NONE)
...
#define Kymera_LatencyManagerA2dpPrepare(params) UNUSED(params)
