...
#define USERS_ENABLED_MAX_COUNT (255)

static struct
{
    uint8_t output_prepare_count;
    uint8_t users_enabled_count;
...
void Kymera_OutputCommonChainEnable(void)
{
    ...
    if(state.users_enabled_count == 0)
        Kymera_OutputCommonChainConfigEnable();

    PanicFalse(state.users_enabled_count < USERS_ENABLED_MAX_COUNT);
    state.users_enabled_count++;
}

void Kymera_OutputCommonChainDisable(void)
{
    PanicFalse(state.users_enabled_count > 0);
    state.users_enabled_count--;
