...
/*! @brief This structure contains the LE configuration used by the audio graph
 */
typedef struct
{
    uint32 sample_rate;                 /*! Sampling rate value in Hz */
    uint16 frame_length;                /*! frame length value in [octets] */
    uint16 frame_duration_us;           /*! frame duration value in [microseconds] */
    uint32 presentation_delay_us;       /*! Presentation delay in [microseconds] */
    appKymeraLeAudioCodec codec_type;   /*! Codec type used by the audio graph such as LC3/aptX */
} kymera_common_le_config_params_t;

/*! @brief This structure contains the LE configuration used by the audio graph for from air path (RX)
 */
typedef kymera_common_le_config_params_t kymera_le_audio_from_air_config_params_t;

/*! @brief This structure contains the LE configuration used by the audio graph for to air path (TX)
 */
typedef kymera_common_le_config_params_t kymera_le_audio_to_air_config_params_t;

/*! @brief This structure contains the LE configuration used by the audio graph when rendering LE Voice call
 */
typedef kymera_common_le_config_params_t kymera_le_voice_config_params_t;
...
typedef struct
{
    ...
    void (*ConfigureLeAudioFromAirChain)(kymera_chain_handle_t chain_handle, kymera_le_audio_from_air_config_params_t *params);
    void (*ConfigureLeAudioToAirChain)(kymera_chain_handle_t chain_handle, kymera_le_audio_to_air_config_params_t *params);
    void (*ConfigureLeVoiceChain)(kymera_chain_handle_t chain_handle, kymera_le_voice_config_params_t *params);
