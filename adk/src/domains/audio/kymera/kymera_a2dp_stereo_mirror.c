...
static void appKymeraA2dpPopulateOutputChainConfig(a2dp_params_getter_t a2dp_params, kymera_output_chain_config *config)
{
    ...
    if(Kymera_IsMusicProcessingPresent() && appKymera_IsSpeakerMirroring(KymeraGetTaskData()))
    ...
    config->chain_type = appKymera_IsSpeakerMonoMirroring(KymeraGetTaskData()) ? output_chain_mono : output_chain_stereo;
}
...
static void appKymeraCreateInputChain(kymeraTaskData *theKymera, uint8 seid)
{
    ...
    switch (seid)
    {
        case AV_SEID_SBC_SNK:
            ...
            if(appKymera_IsSpeakerMonoMirroring(theKymera))
        ...
        case AV_SEID_APTX_SNK:
            ...
            if(appKymera_IsSpeakerMonoMirroring(theKymera))
            {
                ...
                {
                    config = appKymera_IsSeakerMirroringLeft(theKymera) ? Kymera_GetChainConfigs()->chain_forwarding_input_aptx_left_config :
                                       Kymera_GetChainConfigs()->chain_forwarding_input_aptx_right_config;
                }
            }
            else if(appKymera_IsSpeakerMirroring(theKymera))
            {
                                         ...
                                         aptx_adaptive_ttp_latencies_t nq2q_ttp)
{
    ...
    if(appKymera_IsSpeakerMirroring(theKymera))
    {
    ...
    if(appKymera_IsSpeakerMirroring(theKymera))
        appKymeraConfigureLeftRightMixer(chain_handle, rate, theKymera->enable_left_right_mix, appKymera_IsSeakerMirroringLeft(theKymera));
...
static void appKymeraCreateOutputChain(uint8 seid, uint32 rate)
{
    ...
    a2dp_params_getter_t a2dp_params;
    output_users_t output_user = appKymera_IsSpeakerMonoMirroring(KymeraGetTaskData()) ? output_user_a2dp_spk_mono : output_user_a2dp;
    a2dp_params.seid = seid;
    a2dp_params.rate = rate;
...
static void appKymeraConfigureAndStartPacketiserTransform(kymeraTaskData *theKymera, Source source)
{
    ...
    if(theKymera->q2q_mode)
    {
        ...
        if (packetiser)
        {
            ...
            /* R2.2 Requires a slightly different packetiser configuration for standalone config */
            ...
            if(appKymera_IsSpeakerMirroring(theKymera))
            {
                ...
                appKymeraSetAptxADMixerModes(INVALID_OPERATOR, appKymera_IsSeakerMirroringLeft(theKymera), FALSE);
                                            ...
                                            mirror_profile_a2dp_start_mode_t a2dp_start_mode)
{
    ...
    case MIRROR_PROFILE_A2DP_START_Q2Q_MODE_PRIMARY_UNMUTE:
        theKymera->sync_info.mode = KYMERA_AUDIO_SYNC_START_Q2Q_PRIMARY_UNMUTE;
         break;

    case MIRROR_PROFILE_A2DP_START_Q2Q_MODE_SECONDARY_UNMUTE:
        theKymera->sync_info.mode = KYMERA_AUDIO_SYNC_START_Q2Q_SECONDARY_UNMUTE;
        break;
...
static void appKymeraStartChains(kymeraTaskData *theKymera)
{
    ...
    if(appKymera_IsSpeakerMirroring(theKymera))
    {
        ...
        {
            if (theKymera->sync_info.mode == KYMERA_AUDIO_SYNC_START_Q2Q_SECONDARY_UNMUTE)
            {
                rtime_t sync_time;
                sync_time = VmGetTimerTime();
                sync_time += (appConfigQ2QSynchronisedUnmuteDelayMs() * US_PER_MS);
                MirrorProfile_SendA2pdUnmuteTimeToPrimary(sync_time);
                appKymeraSendAudioSynchronisedMessage(sync_time);
            }
            else if (theKymera->sync_info.mode == KYMERA_AUDIO_SYNC_START_Q2Q_PRIMARY_UNMUTE)
            {
...
{
    kymeraTaskData *theKymera = KymeraGetTaskData();
    output_users_t output_user = appKymera_IsSpeakerMonoMirroring(theKymera) ? output_user_a2dp_spk_mono : output_user_a2dp;
...
void appKymeraSetStereoLeftRightMix(bool stereo_lr_mix)
{
    ...
    if (theKymera->enable_left_right_mix != stereo_lr_mix)
    {
        ...
        if (Kymera_A2dpIsStreaming())
        {
            appKymeraSetLeftRightMixerMode(theKymera->chain_input_handle, stereo_lr_mix, appKymera_IsSeakerMirroringLeft(theKymera));
        }
    }
}
...
void appKymeraA2dpSetSyncUnmuteTime(rtime_t unmute_time)
{
    ...
    DEBUG_LOG("appKymeraA2dpSetSyncUnmuteTime, theKymera->sync_info.state %d, unmute time %x", theKymera->sync_info.state, unmute_time);
    if (Kymera_A2dpIsForwarding() && (theKymera->sync_info.state != KYMERA_AUDIO_SYNC_STATE_COMPLETE))
    {
