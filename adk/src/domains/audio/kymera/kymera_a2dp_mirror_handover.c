...
const handover_interface kymera_a2dp_mirror_handover_if =
        MAKE_BREDR_HANDOVER_IF(&appKymeraA2dpMirrorVeto,
                                        ...
                                        uint16 *written)
{
    ...
    UNUSED(tp_bd_addr);
#else
    /* Only marshal this state if mirrored device is BR/EDR.Otherwise, this can be ignored.
        Latency states will be restored/initialized upon start of a2dp music
    */
    ...
    DEBUG_LOG("appKymeraA2dpMirrorMarshal: enum:kymera_a2dp_state_t:%d, enum:appKymeraState:%d", appKymeraA2dpGetState(), appKymeraGetState());

    if (Kymera_A2dpIsForwarding())
    {
...
static void appKymeraA2dpMirrorHandoverComplete(const bool is_primary)
{
    ...
    if (Kymera_A2dpIsForwarding() && (!theKymera->q2q_mode))
    {
        PanicZero(theKymera->sync_info.source);
        Operator op_rtp;
...
static void appKymeraA2dpMirrorHandoverAbort(void)
{
    ...
    DEBUG_LOG("appKymeraA2dpMirrorHandoverAbort: enum:kymera_a2dp_state_t:%d, enum:appKymeraState:%d", appKymeraA2dpGetState(), appKymeraGetState());
    if (Kymera_A2dpIsForwarding() && (!theKymera->q2q_mode))
    {
        PanicZero(theKymera->sync_info.source);
        switch(theKymera->sync_info.mode)
