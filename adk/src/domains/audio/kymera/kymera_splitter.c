...
#define MAX_INPUTS_ROM_SPLITTER 4
...
static void kymera_CreateSplitter(splitter_handle_t handle, uint8 splitter_index)
{
    ...
    if (handle->config)
    {
        if (handle->config->cap_id_splitter)
        {
            if((handle->config->cap_id_splitter == capability_id_splitter) && (handle->num_of_inputs > MAX_INPUTS_ROM_SPLITTER))
            {
                DEBUG_LOG_PANIC("kymera_CreateSplitter: ROM splitter only supports %d channels. Use downloadable splitter instead",
                                MAX_INPUTS_ROM_SPLITTER);
            }
            handle->splitters[splitter_index].op = CustomOperatorCreate(handle->config->cap_id_splitter, OPERATOR_PROCESSOR_ID_0,
                                                                        operator_priority_lowest, NULL);
        }
        else
        {
            DEBUG_LOG_PANIC("kymera_CreateSplitter: cap_id_splitter not available.");
        }
