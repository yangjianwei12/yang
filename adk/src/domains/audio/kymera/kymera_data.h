...
typedef enum
{
    ...
    KYMERA_AUDIO_SYNC_START_Q2Q_PRIMARY_UNMUTE,
    KYMERA_AUDIO_SYNC_START_Q2Q_SECONDARY_UNMUTE,
...
typedef struct
{
    ...
    */
    ...
    /*! The prompt file which is playing */
    FILE_INDEX prompt;
    ...
    appKymeraAudioType  audio_type;
#endif
#if defined (INCLUDE_LE_AUDIO_BROADCAST_SOURCE) && defined (ENABLE_SIMPLE_SPEAKER)
    kymera_chain_handle_t chain_to_air_handle;
    le_media_config_t * lea_broadcast_params;
    bool is_audio_broadcasting:1;
#endif
} kymeraTaskData;
...
#define appKymera_IsSpeakerMirroring(inst)   (inst->audio_type != KYMERA_AUDIO_STANDALONE_STEREO)
#define appKymera_IsSpeakerMonoMirroring(inst)   ((inst->audio_type == KYMERA_AUDIO_MIRROR_MONO_LEFT) || (inst->audio_type == KYMERA_AUDIO_MIRROR_MONO_RIGHT))
#define appKymera_IsSpeakerStereo(inst) ((inst->audio_type == KYMERA_AUDIO_STANDALONE_STEREO) || (inst->audio_type == KYMERA_AUDIO_MIRROR_STEREO))
#define appKymera_IsSeakerMirroringLeft(inst) (inst->audio_type == KYMERA_AUDIO_MIRROR_MONO_LEFT)
