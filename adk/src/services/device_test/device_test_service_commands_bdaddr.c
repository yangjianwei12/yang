...
/*! \brief Structure to hold information for this command handler, including 
   the Task */
static struct
{
...
static void deviceTestService_BdaddrMessageHandler(Task task, MessageId id, Message message)
{
    ...
    {
#ifdef USE_SYNERGY
        const CsrBtCmReadLocalBdAddrCfm *cfm = (const CsrBtCmReadLocalBdAddrCfm *) message;

        if ((id == CM_PRIM) && (cfm->type == CSR_BT_CM_READ_LOCAL_BD_ADDR_CFM))
        {
            BdaddrConvertBluestackToVm(&local_addr, &cfm->deviceAddr);
...
/*! \brief Command handler for AT+LOCALBDADDR?
