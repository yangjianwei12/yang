...
void DeviceTestService_HandleCmRfTestPrim(Message message)
{
    ...
    switch (*prim)
    {
        ...
        {
            CsrBtCmLeTransmitterTestCfm *tx_test_cfm = (CsrBtCmLeTransmitterTestCfm *) message;
            DEBUG_LOG_ALWAYS("deviceTestService_HandleBleTxStartCfm enum:resultCode:%d resultSupplier:%d",
                            tx_test_cfm->resultCode, tx_test_cfm->resultSupplier);
            DeviceTestService_CommandResponseOkOrError(LE_RESPONSE_TASK(),
                                                       tx_test_cfm->resultCode == CSR_BT_RESULT_CODE_CM_SUCCESS);
        }
        break;

        case CSR_BT_CM_LE_RECEIVER_TEST_CFM:
        {
            CsrBtCmLeTransmitterTestCfm *rx_test_cfm = (CsrBtCmLeTransmitterTestCfm *) message;
            DEBUG_LOG_ALWAYS("deviceTestService_HandleBleRxStartCfm enum:resultCode:%d resultSupplier:%d",
                                rx_test_cfm->resultCode, rx_test_cfm->resultSupplier);
            DeviceTestService_CommandResponseOkOrError(LE_RESPONSE_TASK(),
                                                       rx_test_cfm->resultCode == CSR_BT_RESULT_CODE_CM_SUCCESS);
        }
        break;
        ...
        {
            Task stashed_task = LE_RESPONSE_TASK();
            CsrBtCmLeTestEndCfm *stop_cfm = (CsrBtCmLeTestEndCfm *) message;

            DEBUG_LOG_ALWAYS("deviceTestService_HandleBleStopCfm enum:resultCode:%d resultSupplier:%d %u packets",
                                stop_cfm->resultCode, stop_cfm->resultSupplier, stop_cfm->numberOfPackets);
            deviceTestService_RfTest_TearDownOnTestCompletion();

            if (stop_cfm->resultCode == CSR_BT_RESULT_CODE_CM_SUCCESS)
            {
                char response[FULL_LE_RXTEST_RESPONSE_LEN];
                // Make use of C combining two adjacent strings
                sprintf(response, LE_RXTEST_RESPONSE "%u", stop_cfm->numberOfPackets);
                DeviceTestService_CommandResponse(stashed_task, response, FULL_LE_RXTEST_RESPONSE_LEN);
                DeviceTestService_CommandResponseOk(stashed_task);
            }
            else
...
{
#ifdef USE_SYNERGY
    ConnectionBleTransmitterTest(DeviceTestServiceGetSppTask(), SETTING(le_channel), SETTING(le_length), SETTING(le_pattern));
#else
...
{
#ifdef USE_SYNERGY
    ConnectionBleReceiverTest(DeviceTestServiceGetSppTask(), SETTING(le_channel));
#else
