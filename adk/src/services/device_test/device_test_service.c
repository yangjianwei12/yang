...
#ifdef USE_SYNERGY
#define DTS_SERVICE_NAME "DTS"
#define DTS_SPP_INST_INDEX (0) /* Using first SPP instance for DTS */
#endif
...
#else
    DEBUG_LOG_FN_ENTRY("deviceTestService_ContinueDisconnect. Disconnecting:%d Active:%d Auth:%d Registered:%d",
                    dts->disconnecting, dts->active, dts->authenticated,
                    dts->spp_service_registered);
#endif
...
#ifdef USE_SYNERGY
    if (!BdaddrIsZero(&dts->connected_address) && SinkIsValid(dts->rfc_sink))
    {
        DEBUG_LOG_DEBUG("deviceTestService_ContinueDisconnect. Deactivate SPPS");
        SppDeactivateReqSend(DeviceTestServiceGetSppInstQueueId(), DeviceTestServiceGetSppTask()); /* SPP Deactivate will disconnect if connected */
        return TRUE;
    }
#else
...
static void deviceTestService_Disconnect(void)
{
    ...
    DEBUG_LOG_FN_ENTRY("deviceTestService_Disconnect. Disconnecting:%d Active:%d Auth:%d Registered:%d SPP:%d",
                    ...
                    dts->spp_service_registered, !!dts->spp);
#else
    DEBUG_LOG_FN_ENTRY("deviceTestService_Disconnect. Disconnecting:%d Active:%d Auth:%d Registered:%d",
                    dts->disconnecting, dts->active, dts->authenticated,
                    dts->spp_service_registered);
#endif
...
#ifdef USE_SYNERGY
static void deviceTestService_HandleCmPrim(Task task, Message message)
{
    UNUSED(task);
    ...
    switch (*prim)
    {
        ...
        case CSR_BT_CM_LE_TEST_END_CFM:
            DeviceTestService_HandleCmRfTestPrim(message);
            break;
        ...
        default:
            DEBUG_LOG("deviceTestService_HandleCmPrim Unexpected CM primitive = 0x%04x\n",
                      *prim);
            break;
    }
...
static void deviceTestService_HandleSppGetInstancesQidCfm(Task task, CsrBtSppGetInstancesQidCfm *cfm)
{
    UNUSED(task);
    DEBUG_LOG_FN_ENTRY("deviceTestService_HandleSppGetInstancesQidCfm, phandlesListSize:%d",
                       cfm->phandlesListSize);

    if (cfm->phandlesListSize && cfm->phandlesList)
    {
        size_t length = CsrStrLen(DTS_SERVICE_NAME) + 1;
        CsrCharString *serviceName = (CsrCharString *)PanicUnlessMalloc(length); 
        CsrStrNCpyZero((CsrCharString *)serviceName, (CsrCharString *)DTS_SERVICE_NAME, length);
        
        device_test_service_data_t *dts = DeviceTestServiceGetData();
        dts->spp_queue_id = cfm->phandlesList[DTS_SPP_INST_INDEX]; 

        DEBUG_LOG_FN_ENTRY("deviceTestService_HandleSppGetInstancesQidCfm, Queue index(%d), id:%d",
                           DTS_SPP_INST_INDEX, dts->spp_queue_id);

        SppActivateReqSend(DeviceTestServiceGetSppInstQueueId(),
                           DeviceTestServiceGetSppTask(),
...
static void deviceTestService_HandleSppActivateCfm(Task task, CsrBtSppActivateCfm *cfm)
{
    UNUSED(task);
    DEBUG_LOG_FN_ENTRY("deviceTestService_HandleSppActivateCfm, serverChannel:%d",
                       cfm->serverChannel);
    ...
    dts->local_server_channel = cfm->serverChannel;
    dts->pairing_requested = TRUE;
    dts->spp_service_registered = TRUE;
    
}

static void deviceTestService_HandleSppDeactivateCfm(Task task, CsrBtSppDeactivateCfm *cfm)
{
    UNUSED(task);
    device_test_service_data_t *dts = DeviceTestServiceGetData();
    
    DEBUG_LOG_FN_ENTRY("deviceTestService_HandleSppDeactivateCfm, reason=0x%04X,Supplier=0x%04X", 
...
static void deviceTestService_ConnectHandler(Task task,
                                             ...
                                             CsrBtDeviceAddr deviceAddr)
{
        ...
        DEBUG_LOG_FN_ENTRY("deviceTestService_ConnectHandler, btConnId=0x%X",btConnId);
        ...
        if (SinkIsValid(sink))
        {
            ...
            deviceTestService_SetConnectedAddress(&addr);
            dts->rfc_sink = sink;
            dts->rfc_source = StreamSourceFromSink(dts->rfc_sink);
        ...
        {
            DEBUG_LOG_ERROR("deviceTestService_ConnectHandler. sink invalid.");
            Panic();
        }
}

static void deviceTestService_HandleSppConnectInd(Task task, CsrBtSppConnectInd *ind)
{
    DEBUG_LOG_FN_ENTRY("deviceTestService_HandleSppConnectInd, resultCode:0x04X, Supplier=0x04X",
                       ind->resultCode, ind->resultSupplier);

    if (ind->resultCode == CSR_BT_RESULT_CODE_SPP_SUCCESS)
    {
        device_test_service_data_t *dts = DeviceTestServiceGetData();
        dts->spp_service_registered = FALSE; /* SPP SDP record is unregistered on connection */
        deviceTestService_ConnectHandler(task,
                                         ind->btConnId,
                                         ind->deviceAddr);
    }
}

static void deviceTestService_HandleSppDisconnectInd(Task task, CsrBtSppDisconnectInd *ind)
{
    UNUSED(task);
    DEBUG_LOG_FN_ENTRY("deviceTestService_HandleSppDisconnectInd, reason=0x%04X,Supplier=0x%04X", 
    ...
    if (!deviceTestService_ContinueDisconnect())
    {
        DEBUG_LOG_DEBUG("deviceTestService_HandleSppDisconnectInd, Sending CSR_BT_SPP_ACTIVATE_REQ");
        size_t length = CsrStrLen(DTS_SERVICE_NAME) + 1;
        CsrCharString *serviceName = (CsrCharString *)PanicUnlessMalloc(length); 
        CsrStrNCpyZero((CsrCharString *)serviceName, (CsrCharString *)DTS_SERVICE_NAME, length);
...
static void deviceTestService_HandleSppPortnegInd(Task task, CsrBtSppPortnegInd *ind)
{
    UNUSED(task);
    DEBUG_LOG_FN_ENTRY("deviceTestService_HandleSppPortnegInd");

    if (ind->request)
    {
        RFC_PORTNEG_VALUES_T portPar;
        CsrBtPortParDefault(&portPar);
        SppPortnegResSend(DeviceTestServiceGetSppInstQueueId(), 
                          ind->serverChannel, 
                          &portPar);
    }
    else
...
static void deviceTestService_HandleSppPrim(Task task, Message message)
{
    ...
    switch (*prim)
    {
        case CSR_BT_SPP_GET_INSTANCES_QID_CFM:
            deviceTestService_HandleSppGetInstancesQidCfm(task, (CsrBtSppGetInstancesQidCfm*)message);
            break;

        case CSR_BT_SPP_ACTIVATE_CFM:
            deviceTestService_HandleSppActivateCfm(task, (CsrBtSppActivateCfm*)message);
            break;

        case CSR_BT_SPP_DEACTIVATE_CFM:
            deviceTestService_HandleSppDeactivateCfm(task, (CsrBtSppDeactivateCfm*)message);
            break;

        case CSR_BT_SPP_CONNECT_IND:
            deviceTestService_HandleSppConnectInd(task, (CsrBtSppConnectInd*)message);
            break;

        case CSR_BT_SPP_DISCONNECT_IND:
            deviceTestService_HandleSppDisconnectInd(task, (CsrBtSppDisconnectInd*) message);
            break;

        case CSR_BT_SPP_PORTNEG_IND:
            deviceTestService_HandleSppPortnegInd(task, (CsrBtSppPortnegInd*) message);
            break;

        default:
            DEBUG_LOG("deviceTestService_HandleSppPrim Unexpected CM primitive = 0x%04x\n",
                      *prim);
            break;
    }
...
#ifdef USE_SYNERGY
        case SPP_PRIM:
            deviceTestService_HandleSppPrim(task, message);
            break;

        case CM_PRIM:
            deviceTestService_HandleCmPrim(task, message);
            break;

        case MESSAGE_MORE_DATA:
        {
            DEBUG_LOG("deviceTestService_SppsMessageHandler RFCOMM MESSAGE_MORE_DATA");
            if (((MessageMoreData *)message)->source == dts->rfc_source)
            {
                deviceTestService_HandleDataFromSource(dts->rfc_source);
            }
            else
            {
                DEBUG_LOG_WARN("deviceTestService_MessageHandler MESSAGE_MORE_DATA mismatched source");
#ifdef DEVELOPMENT_DEBUG
                Panic();
#endif
            }
...
#ifdef USE_SYNERGY
            SppGetInstancesQidReqSend(DeviceTestServiceGetSppTask());
#else
