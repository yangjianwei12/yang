...
/*! \brief Tone sequence used for fixed tone sequence
 */
...
/*! \brief Structure used for playing a variable length tone. Pre-populated
...
/*! \brief Helper macros for updating deviceTestService_fixedtone
...
/*! \brief Play a very long continual tone 
...
/*! \brief Command handler for AT + AUDIOPLAYTESTTONE
...
void DeviceTestServiceCommand_HandleAudioPlayTestTone(Task task)
{
    ...
    appKymeraTonePlay(deviceTestService_testtone, 0, TRUE, FALSE, NULL, 0);
...
static void deviceTestService_PlayContinualTone(uint16 note, uint16 duration_ms)
{
    ...
    appKymeraTonePlay(deviceTestService_fixedtone, 0, TRUE, FALSE,
                      &deviceTestService_ctt_tone_active, 1);
}
...
/*! \brief Command handler for AT + AUDIOPLAYTONE = %d:tone , %d:speakerSelection , %d:durationMs
