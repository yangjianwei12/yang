...
/*! \brief Enumerated type (bit mask) to record scanning operations.
...
/*! \brief Enumerated type (bit mask) to record when media is busy, that
...
typedef enum
{
    ...
    PEER_FIND_ROLE_CONFIRMING_PRIMARY   = (1 << 2),
        ...
        /*! BLE and/or BR/EDR connection with a non-peer device has been established */
    PEER_FIND_ROLE_NON_PEER_DEVICE_CONNECTED   = (1 << 3),
#endif
        /*! Requested scan block  */
    PEER_FIND_ROLE_SCANNING_DISABLED    = (1 << 8),
} peerFindRoleMediaBusyStatus_t;

/* \brief Enumerated type describing current LE scan mode */
typedef enum
{
...
/*! \brief Structure to hold information used by the peer find role service */
typedef struct 
{
    ...
    uint32                      outcomes;
    /*! flag to remember if a connection indication has been handled by PFR and avoid races in the gatt role decision */
    bool                        peer_connected_handled;

    /*! Cached value of preserved role setting. */
    peer_find_role_preserved_role_t preserved_role;
} peerFindRoleTaskData;


/*! \brief Global data for the find role service.
...
/*! \brief Message identifiers used for internal messages */
typedef enum
{
...
/*! \brief Is this device in the central role ?
...
/*! \brief Indicate whether peer find role is running.

    \return TRUE if running, FALSE otherwise
*/
bool peer_find_role_is_running(void);

/*! \brief Record that we are advertising in the find role task 
*/
void peer_find_role_advertising_activity_set(void);


/*! \brief Clear the fact that we are advertising in the find role task 
...
/*! \brief Send the peer_find_role service a message when there are no longer 
...
/*! \brief Cancel messages queued for the peer_find_role service if 
