...
/* Enforce during compilation that a case comms must be defined */
#if !defined(INCLUDE_CASE_COMMS) || !defined(HAVE_CC_MODE_EARBUDS)
#error "Case comms not defined, must define INCLUDE_CASE_COMMS and HAVE_CC_MODE_EARBUDS for lid events"
#endif
#endif /* ENABLE_SKIP_PFR */
...
/*! Type representing the setting of a preserved role for the device. */
typedef enum {
    peer_find_role_preserved_role_primary,
    peer_find_role_preserved_role_secondary,
    peer_find_role_preserved_role_invalid
} peer_find_role_preserved_role_t;
...
#define PEER_FIND_ROLE_CONTEXT                              0x0001
void peer_find_role_handle_dm_sm_security_cfm(uint8 resultCode);
#else
...
/*! \brief Query if a preserved role has been assigned to the device.
    ...
    \return The current preserved role assigend to this device
*/
