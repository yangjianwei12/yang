...
#ifdef USE_SYNERGY
static void peer_find_role_handle_server_init_ind(const GATT_ROLE_SELECTION_SERVER_INIT_IND_T *init_ind);
#endif
...
void peer_find_role_completed(peer_find_role_message_t role)
{
    ...
    {
        DEBUG_LOG_INFO("peer_find_role_completed. Role is a primary one enum:peer_find_role_message_t:0x%x", role);
    ...
    {
        DEBUG_LOG_INFO("peer_find_role_completed. Role is not a primary one enum:peer_find_role_message_t:0x%x. Wait for links to go.", role);
...
/*! Handler for the connection manager connection/disconnection indications

    This message is received for creation of a connection as well as
    disconnection. We act upon disconnection, using the
...
static void peer_find_role_handle_connection_ind(const CON_MANAGER_CONNECTION_IND_T *ind)
{
    ...
    DEBUG_LOG("peer_find_role_handle_connection_ind ble[%d] connected[%d] is_peer[%d]",
              ind->ble,
              ind->connected,
              BdaddrIsSame(&ind->bd_addr, &pfr->peer_connection_typed_bdaddr.addr));
#endif

    /* Handle disconnection indication from peer device */
...
#if defined(ENABLE_GBSS_RDP_DEMO)
    /* Handle connection/disconnection indication from non-peer device */
    else if(!BdaddrIsSame(&ind->bd_addr, &pfr->peer_connection_typed_bdaddr.addr)
            && BtDevice_IsPairedWithPeer()
            && !appDeviceIsPeer(&ind->bd_addr))
    {
        DEBUG_LOG("peer_find_role_handle_connection_ind from non-peer device");

        if(ind->connected)
        {
            peer_find_role_update_media_flag(TRUE, PEER_FIND_ROLE_NON_PEER_DEVICE_CONNECTED);
        }
        else if( (!ind->connected)
                 && (!ConManagerGetConnectedDeviceCount(TRANSPORT_BLE_ACL)))
        {
            ...
            {
                peer_find_role_update_media_flag(FALSE, PEER_FIND_ROLE_NON_PEER_DEVICE_CONNECTED);
            }
        }
    }
#endif /* defined(ENABLE_GBSS_RDP_DEMO) */
}
...
/*! Handle confirmation of the BLE security request (for encryption)
    ...
    If we receive a failure, setting our state to DISCOVER to restart the find role process
        ...
        \param[in] cfm  The confirmation to our call to CmSmLeSecurityReqSend()
 */
void peer_find_role_handle_dm_sm_security_cfm(uint8 status)
{
    DEBUG_LOG("peer_find_role_handle_dm_sm_security_cfm status:%d", status);
    ...
    switch (status)
    {
        ...
        case HCI_ERROR_KEY_MISSING:
            break;
        default:
            DEBUG_LOG("peer_find_role_handle_dm_sm_security_cfm. FAILED sts:%d",status);
...
static void peer_find_role_handle_con_manager_connection(const CON_MANAGER_TP_CONNECT_IND_T *conn)
{
    ...
    if (conn->tpaddr.transport == TRANSPORT_BLE_ACL)
    {
        ...
        {
            pfr->peer_connected_handled = TRUE;
            ...
            DEBUG_LOG("peer_find_role_handle_con_manager_connection. Store addr 0x%06x, cid %x",
                    pfr->peer_connection_typed_bdaddr.addr.lap, pfr->gatt_cid);


            if (   PEER_FIND_ROLE_STATE_CONNECTING_TO_DISCOVERED == peer_find_role_get_state() ||
                   PEER_FIND_ROLE_STATE_DISCOVERED_DEVICE == peer_find_role_get_state() ||
                   PEER_FIND_ROLE_STATE_DISCOVER_CONNECTABLE == peer_find_role_get_state() ||
                   PEER_FIND_ROLE_STATE_DISCOVER == peer_find_role_get_state())
            {
                if(!conn->incoming)
                {
                ...
                else if(pfr->gatt_cid != INVALID_CID)
                {
...
static void peer_find_role_handle_con_manager_disconnection(const CON_MANAGER_TP_DISCONNECT_IND_T *conn)
{
    ...
    DEBUG_LOG("peer_find_role_handle_con_manager_disconnection. 0x%06x state:%d",
                    conn->tpaddr.taddr.addr.lap, peer_find_role_get_state());

    if (conn->tpaddr.transport == TRANSPORT_BLE_ACL && peer_find_role_is_peer_le_address(&conn->tpaddr))
    {
        pfr->peer_connected_handled = FALSE;
    }
}
...
#ifdef USE_SYNERGY
static void peerFindRole_HandleCmEncryptChangeInd(CsrBtCmEncryptChangeInd *ind)
{
    peerFindRoleTaskData *pfr = PeerFindRoleGetTaskData();
    tp_bdaddr bd_addr = { 0 };
    ...
    /* ToDo: Work around to avoid role change issue during handover */
    if (ind->encryptType == CSR_BT_CM_ENC_TYPE_KEY_REFRESH &&
        appDeviceIsPeer(&bd_addr.taddr.addr))
    { /* Key refresh may be due to attempt to change the role. Allow role switch just in case it is disabled. */
        DEBUG_LOG("peerFindRole_HandleCmEncryptChangeInd. CsrBtCmEncryptChangeInd %d",
                  ind->encryptType);
        appLinkPolicySetMode(&bd_addr.taddr.addr, ENABLE_MS_SWITCH | ENABLE_SNIFF);
    }

    if (BtDevice_LeDeviceIsPeer(&bd_addr))
    {
        bool encrypted = (ind->encryptType != CSR_BT_CM_ENC_TYPE_NONE);
        DEBUG_LOG("peerFindRole_HandleCmEncryptChangeInd. CsrBtCmEncryptChangeInd %d state %d",
                  encrypted,
                  peer_find_role_get_state());

        pfr->gatt_encrypted = encrypted;
...
static void PeerFindRole_HandleCmPrim(Message message)
{
    ...
    switch (*prim)
    {
        ...
        case CSR_BT_CM_ENCRYPT_CHANGE_IND:
            peerFindRole_HandleCmEncryptChangeInd((CsrBtCmEncryptChangeInd *) message);
            break;

        default:
            DEBUG_LOG("PeerFindRole_HandleCmPrim, Unexpected CM primitive");
            break;
    }
...
#ifdef USE_SYNERGY
        case CM_PRIM:
            PeerFindRole_HandleCmPrim(message);
            return;
#endif
        default:
            break;
...
#ifdef USE_SYNERGY
        /* ---- GATT Role Selection messages ---- */
        case GATT_ROLE_SELECTION_SERVER_INIT_IND:
            peer_find_role_handle_server_init_ind((const GATT_ROLE_SELECTION_SERVER_INIT_IND_T*)message);
            break;
#endif
        /* ---- Connection Manager messages ---- */
...
#ifdef USE_SYNERGY
   /* Store address of the connected peer device */
   CsrBtTypedAddr peerAddr = GattConnect_GetPeerBDAddr();
   tpaddr.transport = TRANSPORT_BLE_ACL;
   BdaddrConvertTypedBluestackToVm(&tpaddr.taddr, &peerAddr);
#endif
    switch (state)
    {
        case PEER_FIND_ROLE_STATE_DISCOVER:
...
#endif
            {
                DEBUG_LOG_INFO("peer_find_role_GattConnect. addr:%06x, peer_connected_handled: %d", tpaddr.taddr.addr.lap, pfr->peer_connected_handled);

                if (peer_find_role_is_peer_le_address(&tpaddr))
                {
                    pfr->gatt_cid = cid;

                    if(pfr->peer_connected_handled)
                    {
                        pfr->peer_connected_handled = FALSE;
...
static inline void peer_find_role_init(void)
{
    ...
    /* invalidate the preserved role before reading it from PS key */
    peer_find_role.preserved_role = peer_find_role_preserved_role_invalid;
}

#ifdef USE_SYNERGY
static void peer_find_role_handle_server_init_ind(const GATT_ROLE_SELECTION_SERVER_INIT_IND_T *init_ind)
{
    DEBUG_LOG("peer_find_role_handle_server_init_ind. sts:%d", init_ind->status);

    if (gatt_role_selection_server_init_success != init_ind->status)
    {
        DEBUG_LOG("peer_find_role_gatt_role_selection_server_init. Server init failed");
        Panic();
    }
    ...
    CmSetEventMaskReqSend(PeerFindRoleGetTask(),
                          CSR_BT_CM_EVENT_MASK_SUBSCRIBE_ENCRYPT_CHANGE,
                          CSR_BT_CM_EVENT_MASK_COND_ALL);

    MessageSend(SystemState_GetTransitionTask(), PEER_FIND_ROLE_INIT_CFM, NULL);
}
#endif
