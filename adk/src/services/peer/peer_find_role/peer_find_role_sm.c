...
#ifdef USE_SYNERGY
    handles->handle_service_start = HANDLE_ROLE_SELECTION_SERVICE;
    handles->handle_service_end = HANDLE_ROLE_SELECTION_SERVICE_END;
#endif
}
...
static void peer_find_role_enter_client(void)
{
    ...
    PanicFalse(GattRoleSelectionClientInit(&pfr->role_selection_client,
                                           ...
                                           &cached_handles,
                                           pfr->gatt_cid,
                                           &pfr->peer_connection_typed_bdaddr));
#else
    PanicFalse(GattRoleSelectionClientInit(&pfr->role_selection_client,
                                            PeerFindRoleGetTask(),
                                            &cached_handles,
 ...
 static void peer_find_role_enter_server_awaiting_encryption(void)
{
    ...
    BdaddrConvertTypedVmToBluestack(&addr, &pfr->peer_connection_typed_bdaddr);

    DEBUG_LOG("peer_find_role_enter_server_awaiting_encryption 0x%06x",
              addr.addr.lap);

    CsrBtCmSmLeSecurityReqSend(addr,
                               L2CA_CONFLAG_ENUM(L2CA_CONNECTION_LE_MASTER_DIRECTED),
                               PEER_FIND_ROLE_CONTEXT,
