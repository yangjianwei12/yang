...
static void peer_find_role_start_scanning(scan_state_mode_t mode)
{
    ...
    le_advertising_report_filter_t * ptr_to_filter = NULL;

#ifndef ENABLE_PEER_FIND_ROLE_DIRECTED_ADVERTISING

    tp_bdaddr tp_addr;
    BdaddrTpSetEmpty(&tp_addr);

    le_advertising_report_filter_t filter;
    memset(&filter, 0, sizeof(le_advertising_report_filter_t));
    ...
    if(mode == scan_state_low_duty)
    {
        ...
        DEBUG_LOG_INFO("peer_find_role_start_scanning: use fast params for demo");
        LeScanManager_Start(PeerFindRoleGetTask(),le_scan_interval_fast, ptr_to_filter);
#else
        LeScanManager_Start(PeerFindRoleGetTask(),le_scan_interval_slow, ptr_to_filter);
#endif
