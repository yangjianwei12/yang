...
bool peer_pair_le_can_stop(void)
{
    /* only if PeerPair is still scanning/advertising, we can stop. Else wait for peer to pair */
    switch (peer_pair_le_get_state())
    {
        case PEER_PAIR_LE_STATE_DISCOVERY:
        case PEER_PAIR_LE_STATE_SELECTING:
...
static void peer_pair_le_exit_discovery(PEER_PAIR_LE_STATE new_state)
{
    ...
    switch (new_state)
    {
        ...
        case PEER_PAIR_LE_STATE_IDLE:
            DEBUG_LOG("peer_pair_le_exit_discovery: exiting LE peer pair and entering IDLE");
            peer_pair_le_stop_advertising();
            peer_pair_le_stop_scanning();
            break;
...
static void peer_pair_le_exit_rediscovery(PEER_PAIR_LE_STATE new_state)
{
    if(new_state == PEER_PAIR_LE_STATE_IDLE)
    {
        DEBUG_LOG("peer_pairr_le_exit_rediscovery: exiting LE peer pair and entering IDLE");
        /* no need to stop scanning, because as part of entering re-discovery we have already triggered stop scanning */
        peer_pair_le_stop_advertising();
    }
}
...
static void peer_pair_le_enter_negotiate_p_role(void)
{
	...
	GattRootKeyClientInit(&ppl->root_key_client, PeerPairLeGetTask(), ppl->gatt_cid, &peer_taddr);
#else
...
static void peer_pair_le_exit_completed(void)
{
    ...
    PeerPairLe_UnregisterAdvertisingItem();
}
...
void peer_pair_le_set_state(PEER_PAIR_LE_STATE new_state)
{
    ...
    switch (old_state)
    {
        ...
        case PEER_PAIR_LE_STATE_REDISCOVERY:
            peer_pair_le_exit_rediscovery(new_state);
            break;
