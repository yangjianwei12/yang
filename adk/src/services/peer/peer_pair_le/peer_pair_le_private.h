...
/*! In case of peer pairing based on bd-address, we don't care
    if its Left or Right */
#define PeerPairLeIsLeft()  (FALSE)
#define PeerPairLeIsRight() (FALSE)

#else /* ENABLE_FIXED_HIGHER_ADDR_PRIMARY */
...
#define PeerPairLeCanStop() peer_pair_le_can_stop()
...
typedef enum
{
    ...
    PEER_PAIR_LE_INTERNAL_ENABLE_ADVERTISING,
       ...
       /*! To stop find peer on-going request */
    PEER_PAIR_LE_INTERNAL_STOP_FIND_PEER,
