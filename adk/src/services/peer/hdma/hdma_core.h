...
#if !defined(INCLUDE_LE_AUDIO_UNICAST) || !defined(ENABLE_LEA_CIS_DELEGATION)
#error "INCLUDE_HDMA_CIS_RSSI_EVENT defined without CIS delegation support"
#endif
#endif

typedef uint32 hdma_timestamp; // All timestamps are in ms

/*! \brief Enum used by event messge to invidate the event type */
typedef enum{
...
/*! \brief Structure describing rssi quality of the earbud */
typedef struct {
...
/*! \brief Structure describing CIS RSSI quality of the earbud */
typedef struct {
    /*! absolute RSSI of bud (dBm)*/
    int8 rssi;
...
/*! \brief Enum of HDMA handover reasons*/
typedef enum {
...
/*! \brief Enum describing handover urgency */
typedef enum {
...
/*! \brief Enum Current state of the device battery. */
typedef enum
{
...
/*! \brief Structure to hold all information pertinent to handover */
typedef struct {
...
/*! \brief Structure to hold all information pertinent to a single earbud */
typedef struct{
...
#endif
#ifdef INCLUDE_HDMA_CIS_RSSI_EVENT
    /*! Queue contains CIS RSSI of the earbud */
    queue_t phoneCisRSSI;
#endif
}hdma_bud_info_t;

/*! \brief Structure to hold all information pertinent to HDMA core state */
typedef struct{
...
#if defined(INCLUDE_HDMA_CIS_RSSI_EVENT) && defined(INCLUDE_LE_AUDIO_UNICAST)

/*! \brief Handle Le audio unicast connection indication. */
void Hdma_HandleLeAudioUnicastCisConnectionInd(void);

/*! \brief Handle the CIS RSSI event from HDMA
