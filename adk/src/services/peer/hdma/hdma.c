...
/*! Stub Functionality: Service Handover Request */
bool Hdma_ExternalHandoverRequest(void)
{
    DEBUG_LOG("HDMA Module Not included: Hdma_ExternalHandoverRequest");
    return FALSE;
}
...
bool Hdma_Init(Task client_task)
{
    ...
    HDMA_EVENTS_REGISTER |= state_proxy_event_type_cis_rssi;
#endif
...
#endif
#ifdef INCLUDE_HDMA_CIS_RSSI_EVENT
    HDMA_DEBUG_LOG_INFO("INCLUDE_HDMA_CIS_RSSI_EVENT = ENABLED");
#else
    ...
    StateProxy_StateProxyEventRegisterClient(&hdma->task);
#if defined(INCLUDE_HDMA_RSSI_EVENT) || defined(INCLUDE_HDMA_MIC_QUALITY_EVENT) || defined(INCLUDE_HDMA_CIS_RSSI_EVENT)
...
bool Hdma_Destroy(void)
{
    ...
    if(hdma && hdma->initialised == HDMA_INIT_COMPLETED_MAGIC)
    {
        ...
        StateProxy_StateProxyEventUnregisterClient(&hdma->task);
#if defined(INCLUDE_HDMA_RSSI_EVENT) || defined(INCLUDE_HDMA_MIC_QUALITY_EVENT) || defined(INCLUDE_HDMA_CIS_RSSI_EVENT)
...
/*! \brief Handle state proxy initial state received event
    \param[in] timestamp Time at which event is received.
*/
static void hdma_HandleStateProxyInitialStateReceivedEvent(hdma_timestamp timestamp)
{
    Hdma_CoreUpdateSecondaryDeviceState(timestamp);
}
    ...
    switch(sp_event->type)
    {
       ...
       case state_proxy_event_type_mic_quality:
        ...
        case state_proxy_event_type_cis_rssi:
            HDMA_DEBUG_LOG_VERBOSE("hdma_HandleStateProxyEvent Timestamp [%u] cis rssi value [%d] source [%d]", sp_event->timestamp, sp_event->event.cis_rssi.cis_rssi, is_this_bud);
            hdma_HandleCisRssi(is_this_bud, sp_event->timestamp, &(sp_event->event.cis_rssi));
            break;
#endif
...
void hdma_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        case STATE_PROXY_EVENT_INITIAL_STATE_RECEIVED:
            hdma_HandleStateProxyInitialStateReceivedEvent(timestamp);
            break;
...
#endif
#ifdef INCLUDE_HDMA_CIS_RSSI_EVENT
        case MIRROR_PROFILE_UNICAST_CIS_CONNECT_IND:
            Hdma_HandleLeAudioUnicastCisConnectionInd();
            break;

        case MIRROR_PROFILE_UNICAST_CIS_DISCONNECT_IND:
            break;
#endif
...
{
    hdma_core_cis_rssi_t rssi_val;
    rssi_val.rssi = cis_rssi->cis_rssi;
    Hdma_CoreHandleCisRssi(timestamp, is_this_bud, rssi_val);
}
#endif
