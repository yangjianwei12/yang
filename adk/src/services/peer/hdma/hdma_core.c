...
static hdma_core_handover_urgency_t hdma_ValidateCisLink(hdma_timestamp timestamp);
#endif
...
/*! \brief Updates the state of the secondary device
    \param[in] timestamp Time at which event is received.
*/
void Hdma_CoreUpdateSecondaryDeviceState(hdma_timestamp timestamp)
{    
    PanicFalse(StateProxy_IsPrimary()); // Must only be called on primary
#ifdef INCLUDE_HDMA_BATTERY_EVENT
    hdma_core_battery_state_t dummy_local_battery_state;
    StateProxy_GetLocalAndRemoteBatteryStates(&dummy_local_battery_state,&(hdma_core_data->remote_bud.batteryStatus));
#endif
    hdma_core_data->remote_bud.inCase = StateProxy_IsPeerInCase();
    hdma_core_data->remote_bud.inEar = StateProxy_IsPeerInEar();
    hdma_StateUpdate(timestamp);
...
static void hdma_StateUpdate(hdma_timestamp timestamp)
{
...
#ifdef INCLUDE_HDMA_CIS_RSSI_EVENT
    /*  Case (7) CIS RSSI: skip this if the secondary is unable to be handed over to    */
    uint8 skipCisRSSIChk = ((hdma_BudGetInEar(hdma_core_data->local_bud) && (!hdma_BudGetInEar(hdma_core_data->remote_bud))) || (hdma_core_data->remote_bud.inCase)
#ifdef INCLUDE_HDMA_BATTERY_EVENT
                            || (hdma_core_data->remote_bud.batteryStatus == HDMA_CORE_BATTERY_CRITICAL)
#endif
                            );
    if (!skipCisRSSIChk)
    {
        /* Check urgency and suppress urgency levels for CIS RSSI quality based handover. */
        urgency = hdma_ValidateCisLink(timestamp);
        if (urgency != HDMA_CORE_HANDOVER_URGENCY_INVALID)
        {
            result = hdma_MergeResult(result, hdma_NewResult(HDMA_CORE_HANDOVER_REASON_CIS_RSSI, urgency));
        }
    }
#endif /* INCLUDE_HDMA_CIS_RSSI_EVENT */
...
static void hdma_SetHandoverEvent(hdma_core_handover_result_t newResult)
{
    ...
    #ifndef DEBUG_HDMA_UT
    hdma_NotifyHandoverClients(HDMA_HANDOVER_NOTIFICATION, hdma_core_data->timestamp, (hdma_handover_reason_t)hdma_core_data->hdma_result.reason, (hdma_handover_urgency_t)hdma_core_data->hdma_result.urgency);
    #endif
}
...
#endif
#ifdef INCLUDE_HDMA_CIS_RSSI_EVENT
    Hdma_QueueCreate(&(bud_info->phoneCisRSSI));
#endif
...
/*! \brief Filter an CIS RSSI for a single set of urgency settings and determine if a handover is necessary.

    \param[in] timestamp Time at which event is received.
    \param[in] rssiHalfLife_ms CIS RSSI data older than half life will be down weighted
    \param[in] rssiMaxAge_ms CIS RSSI data older than Max life will be discarded
    \param[in] absRSSIThreshold Absolute CIS RSSI threshold
    \param[in] relRSSIThreshold Relative CIS RSSI threshold
    \param[out] Whether handover is required
*/
static uint8 hdma_CheckCisRSSILevel( hdma_timestamp timestamp, int16 rssiHalfLife_ms,int16 rssiMaxAge_ms, int16 absRSSIThreshold)
{
    int16 thisCisRSSI = 0;
    int16 otherCisRSSI = 0;
    if(hdma_core_data->local_bud.phoneCisRSSI.size > 0)
        thisCisRSSI = hdma_Filter(timestamp, &(hdma_core_data->local_bud.phoneCisRSSI), rssiHalfLife_ms, rssiMaxAge_ms, HDMA_QUEUE_RSSI);
    if(hdma_core_data->remote_bud.phoneCisRSSI.size > 0)
        otherCisRSSI = hdma_Filter(timestamp, &(hdma_core_data->remote_bud.phoneCisRSSI), rssiHalfLife_ms, rssiMaxAge_ms, HDMA_QUEUE_RSSI);

    HDMA_DEBUG_LOG_INFO("hdma_CheckCisRSSILevel otherRSSI = %d, thisRSSI = %d", otherCisRSSI, thisCisRSSI);

    if(otherCisRSSI < 0 && thisCisRSSI < 0)
    {
        /* Handover suggestion if RSSI on the primary earbuds CIS falls below
           a critical value. The absolute threshold is used for this purpose. The
           relative threshold is not used.
        */
        return (thisCisRSSI < absRSSIThreshold) && (otherCisRSSI > thisCisRSSI);
    }
...
static hdma_core_handover_urgency_t hdma_ValidateCisLink(hdma_timestamp timestamp)
{
    /*  validate the RF link determinng if a handover is generated at any urgency level */
    hdma_core_handover_urgency_t urgency = HDMA_CORE_HANDOVER_URGENCY_INVALID;
    if (hdma_CheckCisRSSILevel(timestamp,
                                cis_rssi.halfLife_ms.critical, cis_rssi.maxAge_ms.critical,
                                cis_rssi.absThreshold.critical))
    {
        urgency = HDMA_CORE_HANDOVER_URGENCY_CRITICAL;
    }

#if 0
    /* Currently below code are not required as only when once primary CIS RSSI falls below critical 
       threshold level, handover is triggered. If required in future, this can be enabled */
    else if (hdma_CheckCisRSSILevel(timestamp,
                                 cis_rssi.halfLife_ms.high, cis_rssi.maxAge_ms.high,
                                 cis_rssi.absThreshold.high))
    {
        urgency = HDMA_CORE_HANDOVER_URGENCY_HIGH;
    }
    else if (hdma_CheckCisRSSILevel(timestamp,
                                    cis_rssi.halfLife_ms.low, cis_rssi.maxAge_ms.low,
                                    cis_rssi.absThreshold.low))
    {
        urgency = HDMA_CORE_HANDOVER_URGENCY_LOW;
    }
#endif
...
void Hdma_HandleLeAudioUnicastCisConnectionInd(void)
{
    HDMA_DEBUG_LOG_INFO("Hdma_HandleLeAudioUnicastCisConnectionInd");
    /* Reset both local and remote buds' RSSI entries */
    Hdma_QueueCreate(&(hdma_core_data->local_bud.phoneCisRSSI));
    Hdma_QueueCreate(&(hdma_core_data->remote_bud.phoneCisRSSI));

    /* If HDMA had decisioned to handover based on previous CIS deligation stats,
     * it's required to cancel it and new CIS deligation stats shall be considered */
    if (HDMA_CORE_HANDOVER_REASON_CIS_RSSI == hdma_core_data->hdma_result.reason)
    {
        HDMA_DEBUG_LOG_INFO("Hdma_HandleLeAudioUnicastCisConnectionInd: Cancelling handover");
#ifndef DEBUG_HDMA_UT
        hdma_NotifyHandoverClients(HDMA_CANCEL_HANDOVER_NOTIFICATION, hdma_core_data->timestamp, HDMA_HANDOVER_REASON_INVALID, HDMA_HANDOVER_URGENCY_INVALID);
#endif
    }
...
void Hdma_CoreHandleCisRssi(hdma_timestamp timestamp,uint8 isThisBud, hdma_core_cis_rssi_t cis_rssi)
{
    HDMA_DEBUG_LOG("Hdma_CoreHandleCisRssi: Timestamp = %u", timestamp);

    if(isThisBud)
    {
        HDMA_DEBUG_LOG("Hdma_CoreHandleCisRssi: timestamp: %u, RSSI: %d(ThisBud)", timestamp, cis_rssi.rssi);
        Hdma_QueueInsert(&(hdma_core_data->local_bud.phoneCisRSSI), cis_rssi.rssi, timestamp);
        hdma_StateUpdate(timestamp);
    }
    else
    {
        HDMA_DEBUG_LOG("Hdma_CoreHandleCisRssi: timestamp: %u, RSSI: %d (other)", timestamp, cis_rssi.rssi);
        Hdma_QueueInsert(&(hdma_core_data->remote_bud.phoneCisRSSI), cis_rssi.rssi, timestamp);
        hdma_StateUpdate(timestamp);
    }
}
#endif /* INCLUDE_HDMA_CIS_RSSI_EVENT */
...
static int16 hdma_Filter(hdma_timestamp timestamp, queue_t *queue, int16 halfLife_ms,int16 maxAge_ms, queue_type_t type)
{
    ...
    if(!Hdma_IsQueueEmpty(queue))
    {
        ...
        for(index = queue->rear; i < queue->size; i++, index =((index - 1)%(queue->capacity)))
        {
             ...
             *  and if it is between 0.25 and 0.75 when we divide by sqrt(2) or
...
#endif
#ifdef INCLUDE_HDMA_CIS_RSSI_EVENT
    Hdma_PopulateQueueResult(&(hdma_core_data->remote_bud.phoneCisRSSI), &data.other_bud.phoneCisRSSI,HDMA_QUEUE_RSSI);
    Hdma_PopulateQueueResult(&(hdma_core_data->local_bud.phoneCisRSSI), &data.local_bud.phoneCisRSSI,HDMA_QUEUE_RSSI);
#endif
