...
static void stateProxy_HandleMarshalledMsgChannelRxInd(PEER_SIG_MARSHALLED_MSG_CHANNEL_RX_IND_T* ind)
{
    switch (ind->type)
    {
        ...
        case MARSHAL_TYPE_STATE_PROXY_CIS_RSSI_T:
            stateProxy_HandleRemoteCisRssiUpdate((const STATE_PROXY_CIS_RSSI_T *)ind->msg);
            break;
#endif
...
#ifdef INCLUDE_HDMA_CIS_RSSI_EVENT
        case STATE_PROXY_INTERNAL_TIMER_CIS_RSSI_QUALITY:
            stateProxy_HandleIntervalTimerCisRssiQuality();
            break;

        case MIRROR_PROFILE_UNICAST_CIS_CONNECT_IND:
             stateProxy_HandleLeAudioUnicastCisConnected();
            break;

        case MIRROR_PROFILE_UNICAST_CIS_DISCONNECT_IND:
             stateProxy_HandleLeAudioUnicastCisDisconnected();
             break;
#endif
...
/*! \brief Unregister clients for the specified task. */
void StateProxy_StateProxyEventUnregisterClient(Task client_task)
{
    TaskList_RemoveTask(TaskList_GetFlexibleBaseTaskList(stateProxy_GetEvents()), client_task);
}
...
void StateProxy_GetLocalAndRemoteBatteryLevels(uint16 *battery_level, uint16 *peer_battery_level)
{
    ...
    *peer_battery_level = stateProxy_GetRemoteData() ? stateProxy_GetRemoteData()->battery_voltage : 0;
}
