...
static uint32 peerUi_ForwardUiEventToSecondary(ui_indication_type_t ind_type, uint16 ind_index, uint32 time_to_play)
{
    ...
    DEBUG_LOG("peerUi_ForwardUiEventToSecondary ind type=%d index=%04X timestamp=%d us", ind_type, ind_index, updated_ttp);
...
#if defined (ENABLE_ANC) || defined (ENABLE_AEC_LEAKTHROUGH) || defined(ENABLE_EARBUD_FIT_TEST) || defined(INCLUDE_LE_AUDIO_UNICAST)
...
static uint32 peerUi_ForwardAncUiInputToPeer(ui_input_t ui_input)
{
    ...
    switch(ui_input)
    {
        ...
        case ui_input_anc_noise_id_enable:
        case ui_input_anc_noise_id_disable:
        case ui_input_anc_adverse_acoustic_handler_enable:
        case ui_input_anc_adverse_acoustic_handler_disable:
        case ui_input_continuous_fit_test_start:
        case ui_input_continuous_fit_test_stop:
        case ui_input_continuous_fit_test_single_capture:
        case ui_input_anc_set_filter_topology_parallel:
        case ui_input_anc_set_filter_topology_dual:
...
static uint32 peerUi_ForwardLeAudioUiInputToPeer(ui_input_t ui_input)
{
    uint32 delay = 0;
    ...
    switch(ui_input)
    {
        ...
        case ui_input_le_audio_enable_anc:
            peerUi_ForwardUiInputToPeer(ui_input, delay);
            break;
...
static void peerUi_Interceptor_FuncPtr(ui_input_t ui_input, uint32 delay)
{
    if(appPeerSigIsConnected())
    {
        ...
        {
            DEBUG_LOG("peerUi_Interceptor_FuncPtr, received UI input for ANC");
        ...
        {
            DEBUG_LOG("peerUi_Interceptor_FuncPtr, received UI input for Software LK");
        ...
        {
            DEBUG_LOG("peerUi_Interceptor_FuncPtr, received UI input for EB Fit Test");
            delay = peerUi_ForwardFitTestUiInputToPeer(ui_input);
        }
#endif/*ENABLE_EARBUD_FIT_TEST*/
#ifdef INCLUDE_LE_AUDIO_UNICAST
        if((ui_input >= ui_input_le_audio_disable_anc) && (ui_input <= ui_input_le_audio_enable_anc))
        {
            DEBUG_LOG("peerUi_Interceptor_FuncPtr, received UI input for LEA Unicast");
            delay = peerUi_ForwardLeAudioUiInputToPeer(ui_input);
        }
...
static void peerUi_HandlePeerSigConnectInd(PEER_SIG_CONNECTION_IND_T * ind)
{
    DEBUG_LOG_VERBOSE("peerUi_HandlePeerSigConnectInd enum:peerSigStatus:%d", ind->status);

    if (ind->status == peerSigStatusConnected && StateProxy_IsPrimary())
    {
        UiLeds_RefreshLedContextIndication();
    }
}
...
static void peerUi_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case PEER_SIG_CONNECTION_IND:
            peerUi_HandlePeerSigConnectInd((PEER_SIG_CONNECTION_IND_T*)message);
            break;
