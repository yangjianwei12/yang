...
static void handleAncUiDomainInput(MessageId ui_input)
{
...
#ifdef INCLUDE_LE_AUDIO_UNICAST
    /* Do not inject ANC events if stereo recording is in progress */
    ...
    switch(ui_input)
    {
        ...
        case ui_input_anc_noise_id_enable:
            DEBUG_LOG_ALWAYS("handleAncUiDomainInput, noise id enable");
            AncNoiseId_UpdateTaskData(TRUE);
            AncNoiseId_Enable(TRUE);
            break;

        case ui_input_anc_noise_id_disable:
            DEBUG_LOG_ALWAYS("handleAncUiDomainInput, noise id disable");
            AncNoiseId_Enable(FALSE);
            AncNoiseId_UpdateTaskData(FALSE);
            break;
        ...
        case ui_input_anc_adverse_acoustic_handler_enable:
            DEBUG_LOG_ALWAYS("handleAncUiDomainInput, aah enable");
            AncStateManager_HandleAahEnable();
            break;

        case ui_input_anc_adverse_acoustic_handler_disable:
            DEBUG_LOG_ALWAYS("handleAncUiDomainInput, aah disable");
            AncStateManager_HandleAahDisable();
            break;
        ...
        case ui_input_continuous_fit_test_start:
            FitTest_ContinuousStart();
            break;

        case ui_input_continuous_fit_test_stop:
            FitTest_ContinuousStop();
            break;

        case ui_input_continuous_fit_test_single_capture:
            FitTest_ContinuousSingleCapture();
            break;

        case ui_input_anc_set_filter_topology_parallel:
            DEBUG_LOG_ALWAYS("handleAncUiDomainInput, set filter_topology: Parallel");
            AncStateManager_SetFilterTopology(anc_parallel_filter_topology);
            break;

        case ui_input_anc_set_filter_topology_dual:
            DEBUG_LOG_ALWAYS("handleAncUiDomainInput, set filter_topology: Dual");
            AncStateManager_SetFilterTopology(anc_dual_filter_topology);
            break;
