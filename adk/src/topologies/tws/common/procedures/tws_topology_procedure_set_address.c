...
#define OVERRIDE_BD_ADDR_MILESTONE_1    (OVERRIDE_BD_ADDR_MAX_ATTEMPTS/3)
#define OVERRIDE_BD_ADDR_MILESTONE_2    ((OVERRIDE_BD_ADDR_MAX_ATTEMPTS/3)*2)
...
twsTopProcSetAddressTaskData twstop_proc_set_address = {.task = {twsTopology_ProcSetAddressHandleMessage}, .addr = {0}, .complete_fn = NULL, .attempts_remaining = 0, .first_attempt = FALSE };
...
static bool twsTopology_ProcSetAddressOverrideAddress(void)
{
    ...
    // MASH TODO - this will fail unless handset has been disconnected
    // so need to send topology errors back to application
...
static void twsTopology_ProcSetAddressHandleInternalRetry(void)
{
    ...
    else
    {
        ...
        else if((td->attempts_remaining == OVERRIDE_BD_ADDR_MILESTONE_1) ||
                (td->attempts_remaining == OVERRIDE_BD_ADDR_MILESTONE_2))
        {
            /* we have reached a milestone number of attempts, retry peer disconnection */
            bdaddr bd_addr;
            if(appDeviceGetPeerBdAddr(&bd_addr) && ConManagerIsConnected(&bd_addr))
            {
                DEBUG_LOG_INFO("twsTopology_ProcSetAddressHandleInternalRetry retrying peer disconnection");
