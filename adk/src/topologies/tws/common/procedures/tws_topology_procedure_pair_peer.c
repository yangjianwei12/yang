...
twsTopProcPairPeerTaskData twstop_proc_pair_peer = {.task = {twsTopology_ProcPairPeerHandleMessage}, .result_task = 0, .complete_fn = NULL};
                                        ...
                                        Message goal_data)
{
    ...
    const peer_search_t* peer_search = &twsTopology_GetProductBehaviour()->peer_search;
    if(peer_search->bdaddr_list)
    {
        // TODO - only supporting one peer atm
        PanicFalse(peer_search->num_peers == 1);
        bdaddr *peer_addr = (bdaddr*)(peer_search->bdaddr_list);
        PeerPairLe_PairPeerWithAddress(TwsTopProcPairPeerGetTask(), &peer_addr[0]);
    }
    else
    {
