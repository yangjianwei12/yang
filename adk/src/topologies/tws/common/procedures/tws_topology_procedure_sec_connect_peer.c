...
twsTopProcSecConnectPeerTaskData twstop_proc_sec_connect_peer = {.task = {twsTopology_ProcSecConnectPeerHandleMessage}, .complete_fn = NULL, .connect_wait_lock = NULL };
                                        ...
                                        Message goal_data)
{
    ...
    if (appDeviceGetPrimaryBdAddr(&peer_addr))
    {
        ...
        else
        {
            ...
            if(TwsToplogy_IsRoleElectedByPeerFindRole()) /* after PFR use existing timer */
            {
                MessageSendLater(TwsTopProcSecConnectPeerGetTask(),
                            PROC_SEC_CONNECT_PEER_INTERNAL_ACL_CONNECT_TIMEOUT,
                            NULL, D_SEC(twsTopology_GetProductBehaviour()->timeouts.secondary_wait_for_primary_sec));
            }
            else /* during boot use 2s timer */
            {
                MessageSendLater(TwsTopProcSecConnectPeerGetTask(),
                            PROC_SEC_CONNECT_PEER_INTERNAL_ACL_CONNECT_TIMEOUT,
                            NULL, D_SEC(twsTopology_GetProductBehaviour()->timeouts.post_idle_secondary_wait_for_primary_sec));
            }
#else
            MessageSendLater(TwsTopProcSecConnectPeerGetTask(),
                        PROC_SEC_CONNECT_PEER_INTERNAL_ACL_CONNECT_TIMEOUT,
...
static void twsTopology_ProcSecConnectPeerCheckAclStatus(bool timeout)
{
    ...
    if (appDeviceGetPrimaryBdAddr(&peer_addr))
    {
        ...
        else
        {
            ...
            if(timeout)
            {
                /* Secondary timeout is only an app-level timeout, it is not the timeout used by the Controller for paging.
                   The peer-paging timeout is presently hard-coded (appConfigEarbudPageTimeout()) in Connection Manager.
                   So, releasing the peer ACL explicity here after a timeout though the no role idle takes care of it later */
...
{
    DEBUG_LOG_INFO("twsTopology_ProcSecConnectPeerHandleInternalAclConnectTimeout");
