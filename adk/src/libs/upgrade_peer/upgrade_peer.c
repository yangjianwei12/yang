...
const upgrade_response_functions_t UpgradePeer_fptr = {
    ...
    .SendCommitStatusCfm = UpgradePeerIFDataSendCommitStatus,
};
...
static void upgradePeer_SendTransferCompleteRes(upgrade_action_status_t status);
static void upgradePeer_SendCommitCFM(upgrade_action_status_t status);
static void upgradePeer_SendProceedToCommit(upgrade_action_status_t status);
...
static upgrade_peer_state_t UpgradePeerGetState(void);

/**
 * Set resume point as provided by Secondary device.
 */
void UpgradePeerSetResumePoint(upgrade_peer_resume_point_t point)
{
    if(upgradePeerInfo->UpgradePSKeys.upgradeResumePoint != point)
    {
        /* Set the upgrade peer resume point as peer device upgrade resume point */
        upgradePeerInfo->UpgradePSKeys.upgradeResumePoint = point;
        UpgradePeerSavePSKeys();
    }
}

/*!
 * Returns TRUE if the resume point is pre-validate, else FALSE
*/
bool UpgradePeerIsResumePoinPreValidate(void)
{
    return (upgradePeerInfo->UpgradePSKeys.upgradeResumePoint == UPGRADE_PEER_RESUME_POINT_PRE_VALIDATE);
}

/*!
 * Returns TRUE if the resume point is post reboot, else FALSE
*/
bool UpgradePeerIsResumePointPostReboot(void)
{
    return (upgradePeerInfo->UpgradePSKeys.upgradeResumePoint == UPGRADE_PEER_RESUME_POINT_POST_REBOOT);
}
                                   ...
                                   upgrade_action_status_t status)
{
    ...
    switch (type)
    {
        case UPGRADE_TRANSFER_COMPLETE:
            ...
            {
                upgradePeer_SendTransferCompleteRes(status);
            }
            else
            {
        ...
        case UPGRADE_IN_PROGRESS:
            ...
            {
                upgradePeer_SendProceedToCommit(status);
            }
            else
            {
                upgradePeer_AbortPeerDfu();
...
static void upgradePeer_AskForConfirmation(upgrade_confirmation_type_t type)
{
    ...
    switch (type)
    {
        case UPGRADE_TRANSFER_COMPLETE:
            ...
            break;
        case UPGRADE_COMMIT:
            /* Forward this event to application. */
            MessageSend(upgradePeerInfo->appTask,UPGRADE_PEER_COMMIT_REQ_IND , NULL);
            break;
        case UPGRADE_IN_PROGRESS:
...
static void upgradePeer_CtxDestroy(void)
{
     ...
     */
    if(peer_partition_reader_handle)
    {
...
static void upgradePeer_ReceiveStartCFM(UPGRADE_PEER_START_CFM_T *data)
{
    ...
    if (data->common.length >= UPRAGE_HOST_START_CFM_DATA_LENGTH)
    {
        ...
        if (data->status == UPGRADE_PEER_SUCCESS)
        {
            ...
            {
                case UPGRADE_PEER_RESUME_POINT_START:
                {
                    UpgradePeerSetState(UPGRADE_PEER_STATE_DATA_READY);
                    upgradePeer_SendStartDataReq();
                }
                break;
                case UPGRADE_PEER_RESUME_POINT_PRE_VALIDATE:
                {
                    UpgradePeerSetState(UPGRADE_PEER_STATE_DATA_TRANSFER );
                    upgradePeer_SendValidationDoneReq();
                }
                break;
                case UPGRADE_PEER_RESUME_POINT_PRE_REBOOT:
                {
                    UpgradePeerSetState(UPGRADE_PEER_STATE_PROCESS_COMPLETE);
                    /* Send the process complete info to dfu domain */
                    MessageSend(upgradePeerInfo->appTask, UPGRADE_PEER_UPGRADE_PROCESS_COMPLETE_IND, NULL);
                    /* Set the resume point on process complete */
                    UpgradePeerSetResumePoint(UPGRADE_PEER_RESUME_POINT_PRE_REBOOT);
                }
                break;
                default:
                {
                    DEBUG_LOG("upgradePeer_ReceiveStartCFM Not handled ");
                }
                break;
            }
        }
...
static void upgradePeer_ReceiveSyncCFM(UPGRADE_PEER_SYNC_CFM_T *update_cfm)
{
    ...
    UpgradePeerSetResumePoint(update_cfm->resume_point);
...
static bool upgradePeer_StartPeerData(uint32 dataLength, uint8 *packet, bool is_last_packet)
{
    ...
    if (is_last_packet)
    {
        ...
        if (upgradePeerInfo->UpgradePSKeys.upgradeResumePoint == UPGRADE_PEER_RESUME_POINT_START)
        {
            UpgradePeerSetResumePoint(UPGRADE_PEER_RESUME_POINT_PRE_VALIDATE);
            ...
            /* Send the IS_VALIDATION_DONE_REQ to peer device */
            DEBUG_LOG("upgradePeer_StartPeerData: upgradePeer_SendValidationDoneReq");
            upgradePeer_SendValidationDoneReq();
        }
    }
    return TRUE;
...
static void upgradePeer_ReceiveDataBytesREQ(UPGRADE_PEER_START_DATA_BYTES_REQ_T *data)
{
    ...
    if (data->common.length == UPGRADE_DATA_BYTES_REQ_DATA_LENGTH)
    {
        ...
        if(data->start_offset)
        {
            ...
            UpgradePartitionReader_SetPendingPartition(peer_partition_reader_handle, expected_state.pending_partition);
...
static bool upgradePeer_DelayParallelReadRequest(uint32 req_offset)
{
    ...
    uint16 prim_partn     = UpgradeCtxGetFWPartitionNum().partitionID;
    ...
    if(UpgradeIsDataTransferComplete())
    {
        /* Do not delay any peer data request if we have received all the data. */
        return FALSE;
    }
    if(UpgradeSmStateIsBeforeDataTransfer() && (UpgradePeerGetState() == UPGRADE_PEER_STATE_DATA_TRANSFER))
    {
        DEBUG_LOG_INFO("upgradePeer_DelayParallelReadRequest : as secondary state > primary state");
        return TRUE;
    }
    ...
    if(prim_partn == sec_partn )
    {
        ...
        {
            DEBUG_LOG_INFO("upgradePeer_DelayParallelReadRequest: Footer state");
            return  TRUE;
        }
        else
        {
...
{
    DEBUG_LOG("upgradePeer_ReceiveValidationDoneCFM");
...
static void upgradePeer_ReceiveCommitREQ(void)
{
    ...
    UpgradePeerSetResumePoint(UPGRADE_PEER_RESUME_POINT_COMMIT);
...
static void upgradePeer_SendTransferCompleteRes(upgrade_action_status_t status)
{
    ...
    DEBUG_LOG("upgradePeer_SendTransferCompleteRes");
...
/**
 * To send an UPGRADE_PROCEED_TO_COMMIT packet.
 */
static void upgradePeer_SendProceedToCommit(upgrade_action_status_t status)
{
    uint8* payload = NULL;
    ...
    DEBUG_LOG("upgradePeer_SendProceedToCommit");

    payload = PanicUnlessMalloc(UPGRADE_IN_PROGRESS_DATA_LENGTH +
                                UPGRADE_PEER_PACKET_HEADER);
    byteIndex += ByteUtilsSet1Byte(payload, byteIndex,
                                   UPGRADE_PEER_PROCEED_TO_COMMIT);
    byteIndex += ByteUtilsSet2Bytes(payload, byteIndex,
    ...
    switch(msgId)
    {
        ...
        case UPGRADE_PEER_DATA_BYTES_REQ:
            ...
            DEBUG_LOG("UpgradePeer_HandlePeerAppMsg UPGRADE_PEER_DATA_BYTES_REQ_T start_offset %lu, last closed partition %d, peer read partition %d",
                (local_msg->start_offset), UpgradeCtxGetPSKeysLastClosedPartition().partitionID  - 1, UpgradePartitionReader_GetPartitionNumber(peer_partition_reader_handle));
        ...
        case UPGRADE_PEER_TRANSFER_COMPLETE_IND:
            DEBUG_LOG("upgradePeer_HandlePeerAppMsg: Received TRANSFER_COMPLETE_IND");
            UpgradePeerSetState(UPGRADE_PEER_STATE_PROCESS_COMPLETE);
            /* Send the process complete info to Application */
            MessageSend(upgradePeerInfo->appTask, UPGRADE_PEER_UPGRADE_PROCESS_COMPLETE_IND, NULL);
            /* Set the resume point to pre reboot */
            UpgradePeerSetResumePoint(UPGRADE_PEER_RESUME_POINT_PRE_REBOOT);
            break;

        case UPGRADE_PEER_COMPLETE_IND:
            /* Inform the application that peer upgrade is successful so, it can also do the commit. */
            MessageSend(upgradePeerInfo->appTask,UPGRADE_PEER_UPGRADE_COMPLETE_IND , NULL);
            /* Peer upgrade is finished let disconnect the peer connection */
            upgradePeer_StopUpgrade();
            break;

        case UPGRADE_PEER_COMPLETE_IND_WITH_STATUS:
            {
            UPGRADE_PEER_UPGRADE_COMPLETE_IND_WITH_STATUS_T *complete_msg;
            complete_msg = (UPGRADE_PEER_UPGRADE_COMPLETE_IND_WITH_STATUS_T *)PanicUnlessMalloc(sizeof(UPGRADE_PEER_UPGRADE_COMPLETE_IND_WITH_STATUS_T));
            memmove(complete_msg, data, sizeof(UPGRADE_PEER_UPGRADE_COMPLETE_IND_WITH_STATUS_T));
            
            DEBUG_LOG("upgradePeer_HandlePeerAppMsg: Received UPGRADE_PEER_COMPLETE_IND_WITH_STATUS %d", complete_msg->commit_status);
            
            /* Inform the application that peer upgrade is successful so, it can also do the commit. */
            MessageSend(upgradePeerInfo->appTask,UPGRADE_PEER_UPGRADE_COMPLETE_IND , complete_msg);
            /* Peer upgrade is finished let disconnect the peer connection */
            upgradePeer_StopUpgrade();
            }
        ...
        case UPGRADE_PEER_TRANSFER_COMPLETE_RES:
            {
                ...
                if(UPGRADE_PEER_IS_PRIMARY)
                {
                    upgrade_peer->confirm_type = UPGRADE_TRANSFER_COMPLETE;
                    upgradePeer_SendConfirmationToPeer(upgrade_peer->confirm_type, status);
                }
            }
            break;

        case UPGRADE_PEER_PROCEED_TO_COMMIT:
                                   ...
                                   uint16 dataSize)
{
    ...
    switch(id)
    {
            ...
            if(state == UPGRADE_PEER_CONNECT_SUCCESS)
            {
                switch(upgradePeerInfo->SmCtx->peerState)
                {
                    ...
                    default:
                    {
                        ...
                        {
                             DEBUG_LOG("UpgradePeerProcessDataRequest: SendAbortReq");
        ...
        case UPGRADE_PEER_DISCONNECT_IND:
            if(upgradePeerInfo->SmCtx->peerState == UPGRADE_PEER_STATE_DATA_TRANSFER)
            {
...
static upgrade_peer_state_t UpgradePeerGetState(void)
{
    if(upgradePeerInfo->SmCtx == NULL)
    {
        DEBUG_LOG("UpgradePeerGetState: you shouldn't have done that");
        Panic();
    }
...
/****************************************************************************
NAME
    UpgradePeerSetVldtnCompleteStatus

DESCRIPTION
    Set the upgrade peer validation status
*/

void UpgradePeerSetVldtnCompleteStatus(bool status)
{
    if (upgradePeerInfo != NULL)
    {
        DEBUG_LOG("UpgradePeerSetVldtnCompleteStatus is_validation_complete:%d", status);
        upgradePeerInfo->is_validation_complete = status;
    }
}

/****************************************************************************
NAME
    UpgradePeerIsVldtnComplete

DESCRIPTION
    Get the upgrade peer validation status
*/

bool UpgradePeerIsVldtnComplete(void)
{
    return (upgradePeerInfo && upgradePeerInfo->is_validation_complete);
}

/****************************************************************************
NAME
    UpgradePeerIsStateProcessComplete

DESCRIPTION
    Get the upgrade peer process completion status
*/

bool UpgradePeerIsStateProcessComplete(void)
{
    return (UpgradePeerGetState() == UPGRADE_PEER_STATE_PROCESS_COMPLETE);
}
