...
typedef enum
{
    ...
    UPGRADE_PEER_PROCEED_TO_COMMIT                                = 0x0E, /* Not Used */
    ...
    UPGRADE_PEER_COMPLETE_IND_WITH_STATUS                         = 0x25
} upgrade_peer_msg_t;
...
typedef enum
{
    ...
    /*! Message sent to application to inform that the peer has sent a commit request */
    UPGRADE_PEER_COMMIT_REQ_IND,
    /*! Message sent to application to inform that the peer has completed the upgrade */
    UPGRADE_PEER_UPGRADE_COMPLETE_IND,
    /*! Message sent to application to inform that peer has completed the upgrade and sending commit status */
    UPGRADE_PEER_UPGRADE_COMPLETE_IND_WITH_STATUS,
    
    /*! Message sent to application to inform that the peer process has completed */
    UPGRADE_PEER_UPGRADE_PROCESS_COMPLETE_IND
...
typedef struct {
    UPGRADE_PEER_COMMON_CMD_T    common;
    uint8                        commit_status;
} UPGRADE_PEER_UPGRADE_COMPLETE_IND_WITH_STATUS_T;
...
/*!
    @brief UpgradePeerReconnectForReboot

    Set the upgrade peer resume point

    @param resume point to set
    @return none
*/
void UpgradePeerSetResumePoint(upgrade_peer_resume_point_t point);

/*!
    @brief check if the upgrade peer resume point is pre-validate
    returns TRUE if the resume point is pre-validate, else FALSE
*/
bool UpgradePeerIsResumePoinPreValidate(void);

/*!
    @brief check if the upgrade peer resume point is post reboot
    returns TRUE if the resume point is post reboot, else FALSE
*/
bool UpgradePeerIsResumePointPostReboot(void);

/*!
    @brief UpgradePeerSetVldtnCompleteStatus

    Set upgrade peer validation complete status

    @param status Set to TRUE/FALSE based on vldtn complete status
    @return none.
*/
void UpgradePeerSetVldtnCompleteStatus(bool status);

/*!
    @brief check if the upgrade peer validation status is set to TRUE
    @returns TRUE if it is set, else FALSE
*/
bool UpgradePeerIsVldtnComplete(void);

/*!
    @brief check if the upgrade peer state process complete
    @returns TRUE if the state is process complete, else FALSE
*/
