...
void UpgradePeerIFDataSendCommitStatus(uint8 status)
{
    UPGRADE_PEER_UPGRADE_COMPLETE_IND_WITH_STATUS_T *rsp = 
        (UPGRADE_PEER_UPGRADE_COMPLETE_IND_WITH_STATUS_T  *)PanicUnlessMalloc(sizeof(*rsp));
    DEBUG_LOG_INFO("UpgradePeerIFDataSendCommitStatus %u", status);
    
    rsp->common.op_code = UPGRADE_HOST_COMPLETE_IND_WITH_STATUS - UPGRADE_HOST_MSG_BASE;
    rsp->common.length = UPGRADE_HOST_COMPLETE_IND_WITH_STATUS_BYTE_SIZE;
    rsp->commit_status = status;
