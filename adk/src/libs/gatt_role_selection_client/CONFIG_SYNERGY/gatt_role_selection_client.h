...
typedef struct
{
    uint16 handle_service_start;        /*!< Start handle for the Role selection service */
    uint16 handle_service_end;          /*!< End Handle for the Role Selection service */
...
typedef struct
{
    ...
    typed_bdaddr peer_addr;             /*!< Peer Address */
    bool handles_cached;                /*!< TRUE if cached handles are there otherwise FALSE */
...
/*!
    ...
    it should register with the relevant client service library (by passing the relevant CID) which will then discover any supported services in 
    which it has an interest. For the role_selection client it will use this API.
