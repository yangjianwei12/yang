...
#define DEVICE_LIST_MRU_INDEX_NOT_SET        0xFF

typedef uint8 device_list_mru_index_t;
...
/*! \brief The given device of device list was used, update MRU index of devices

    \param device Device that most recently used.
*/
void DeviceList_DeviceWasUsed(device_t device);

/*! \brief Clear MRU index of a device in the list.
...
/*! \brief Get the MRU index of the device on the list.

    If the device is not in the list or is not valid this function will not
    alter the list and will not panic.

    \return The MRU device index, or 0xFF if the device was not found on the list.
*/
device_list_mru_index_t DeviceList_GetMruIndex(device_t device);

/*! \brief Get the device at the specified MRU index.
