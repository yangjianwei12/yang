...
typedef struct
{
    ...
    uint32 commit_cid;               /*!< The cid used when responding to a key update */
    CsrBtGattId gattId;             /*!< GATT Identifier */
    uint16 start_handle;            /*!< Start handle*/
    uint16 end_handle;              /*!< End handle*/
...
{
    const GATT_ROOT_KEY_SERVER *instance;
    gatt_root_key_server_init_status_t  status;

} GATT_ROOT_KEY_SERVER_INIT_IND_T;
...
typedef enum
{
        /*! Root Key Service initialisation has completed */
    GATT_ROOT_KEY_SERVER_INIT_IND = GATT_ROOT_KEY_SERVER_MESSAGE_BASE,
        /*! A root key service challenge has completed */
    GATT_ROOT_KEY_SERVER_CHALLENGE_IND,
