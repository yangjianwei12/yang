...
/****************************************************************************
DESCRIPTION
    returns True for microphone type PCM
*/
static bool audioPluginIsTypePcm(audio_mic_params params)
{
    return(params.type == mic_type_pcm);
}
...
{
    if(audioPluginIsTypePcm(params))
    {
        bias.refcount_pcm[params.channel]++;
        DEBUG_LOG_VERBOSE("audioPluginIncrementBiasConfigRefCount: pcm %d cnt %u",
                  params.channel, bias.refcount_pcm[params.channel]);
    }
    else
    {
        if(audioPluginIsBiasConfigPio(params))
        {
            ...
            DEBUG_LOG_VERBOSE("audioPluginIncrementBiasConfigRefCount: PIO %u cnt %u",
                      bias.pio[index].pin, bias.pio[index].refcount);
        }
        else
        {
            bias.refcount_analog[params.bias_config]++;
            DEBUG_LOG_VERBOSE("audioPluginIncrementBiasConfigRefCount: analog %d cnt %u",
...
static void audioPluginDecrementBiasConfigRefCount(const audio_mic_params params)
{
    if(audioPluginIsTypePcm(params))
    {
        PanicFalse(bias.refcount_pcm[params.channel] > 0);
        bias.refcount_pcm[params.channel]--;
        DEBUG_LOG_VERBOSE("audioPluginDecrementBiasConfigRefCount: pcm %d cnt %u",
                  params.channel, bias.refcount_pcm[params.channel]);
    }
    else
    {
        if(audioPluginIsBiasConfigPio(params))
        {
            ...
            bias.pio[index].refcount--;
            DEBUG_LOG_VERBOSE("audioPluginDecrementBiasConfigRefCount: PIO %u cnt %u",
                      bias.pio[index].pin, bias.pio[index].refcount);
        }
        else
        {
            ...
            bias.refcount_analog[params.bias_config]--;
            DEBUG_LOG_VERBOSE("audioPluginDecrementBiasConfigRefCount: analog %d cnt %u",
                      params.bias_config, bias.refcount_analog[params.bias_config]);
        }
    }
...
static uint8 audioPluginGetRefCount(const audio_mic_params params)
{
    if(audioPluginIsTypePcm(params))
    {
        DEBUG_LOG_VERBOSE("audioPluginGetRefCount: pcm %d cnt %u",
                  params.channel, bias.refcount_pcm[params.channel]);
        return bias.refcount_pcm[params.channel];
    }
    else
    {
        if(audioPluginIsBiasConfigPio(params))
        {
            uint8 index = audioPluginGetBiasPioIndex(params);
            DEBUG_LOG_VERBOSE("audioPluginGetRefCount: PIO %u cnt %u",
                      bias.pio[index].pin, bias.pio[index].refcount);
            return bias.pio[index].refcount;
        }
        else
        {
            DEBUG_LOG_VERBOSE("audioPluginGetRefCount: analog %d cnt %u",
                      params.bias_config, bias.refcount_analog[params.bias_config]);
...
DESCRIPTION
    Get audio source
*/
static Source getAudioSource(audio_instance instance, audio_channel channel, mic_type_t type)
{
    Source mic_source = NULL;
    switch(type)
    {
        case mic_type_analog:
            mic_source = StreamAudioSource(AUDIO_HARDWARE_CODEC, instance, channel);
        break;
        case mic_type_digital:
            mic_source = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, instance, channel);
        break;
        case mic_type_pcm:
            mic_source = StreamAudioSource(AUDIO_HARDWARE_PCM, instance, channel);
        break;
        default:
            Panic();
        break;
    }
    PanicFalse(mic_source);
    return mic_source;
}
...
{
    return getAudioSource(audioPluginGetInstance(audio_mic.instance), channel, audio_mic.type);
}
...
{
    return getAudioSource(audioPluginGetInstance(analogue_input.instance), channel, mic_type_analog);
}

/****************************************************************************
DESCRIPTION
    Configure Mic channel
    In case of PCM microphones: Configure the interface
*/
void AudioPluginSetMicRate(audio_channel channel, const audio_mic_params audio_mic, uint32 mic_rate)
{
    ...
    switch(audio_mic.type)
    {
        case mic_type_digital:
            PanicFalse(SourceConfigure(mic_source, STREAM_DIGITAL_MIC_INPUT_RATE, mic_rate));
        break;
        case mic_type_analog:
            PanicFalse(SourceConfigure(mic_source, STREAM_CODEC_INPUT_RATE, mic_rate));
        break;
        case mic_type_pcm:
            AudioPcmCommonConfigureSource(mic_source, mic_rate);
        break;
    }
}
...
void AudioPluginSetMicGain(audio_channel channel, const audio_mic_params audio_mic)
{
    ...
    {
        case mic_type_digital:
            SourceConfigure(mic_source, STREAM_DIGITAL_MIC_INPUT_GAIN, audio_mic.gain);
        break;
        case mic_type_analog:
            SourceConfigure(mic_source, STREAM_CODEC_INPUT_GAIN, audio_mic.gain);
...
DESCRIPTION
    ...
    In case of PCM microphones: Configure the device
*/
Source AudioPluginMicSetup(audio_channel channel, const audio_mic_params audio_mic)
{
    ...
    if(audioPluginIsTypePcm(audio_mic))
    {
        AudioPcmCommonConfigureDevice(TRUE);
    }
...
void AudioPluginMicShutdown(audio_channel channel, const audio_mic_params * params, bool close_mic)
{
    ...
    if(audioPluginGetRefCount(*params) == 0)
    {
        ...
        if(audioPluginIsTypePcm(*params))
        {
            AudioPcmCommonConfigureDevice(FALSE);
        }
    }
}
