...
/* Mask of valid MPEG-2, 4 AAC Object Types, see A2DP Specification V1.3.2 section 4.5.2.1 */
#define A2DP_VALID_AAC_OBJECT_TYPE_MASK (0xF0)
...
*/
static uint16 areSBCCodecsCompatible(const uint8 *local_caps, const uint8 *remote_caps, uint8 local_losc, uint8 remote_losc, bool initiating)
{
    ...
    if (initiating)
    {
        ...
        if (local_caps[2] > remote_caps[3])
            return avdtp_invalid_maximum_bitpool_value;

        /* local max is less than remote min */
        if (remote_caps[2] > local_caps[3])
            return avdtp_invalid_minimum_bitpool_value;
    }
    else
    {
...
/*
    ...
    Codec Specific Information
        ...
        Octet3              Bit Rate
*/
static uint16 areMPEG12AudioCodecsCompatible(const uint8 *local_caps, const uint8 *remote_caps, uint8 local_losc, uint8 remote_losc, bool initiating)
{
    ...
    if (!oneBitSet(remote_caps[0] & 0xe0) && !initiating)
        return avdtp_invalid_layer;

    /* layer bits overlap? */
    if ( !((local_caps[0] >> 5) & (remote_caps[0] >> 5)))
        return avdtp_not_supported_layer;
...
/*
    ...
    Codec Specific Information
        ...
        Octet5              Bit rate
*/
static uint16 areMPEG24AACCodecsCompatible(const uint8 *local_caps, const uint8 *remote_caps, uint8 local_losc, uint8 remote_losc, bool initiating)
{
    ...
    if (!oneBitSet(remote_caps[0]) && !initiating)
        return avdtp_invalid_object_type;

    /* check this is a valid object type */
    if (!(remote_caps[0] & A2DP_VALID_AAC_OBJECT_TYPE_MASK))
        return avdtp_invalid_object_type;

    /* Object Type bits overlap? */
    if (!(local_caps[0] & remote_caps[0]))
        return avdtp_not_supported_object_type;
...
RETURNS
    void
*/
const uint8* a2dpFindMatchingCodecSpecificInformation(const uint8 *local_caps, const uint8 *remote_caps, bool initiating, uint8 *error_code)
{
    const uint8 *local_codec = local_caps;
    const uint8 *remote_codec = remote_caps;

    *error_code = avdtp_unsupported_configuration;
        ...
        if (local_codec[2] == (AVDTP_MEDIA_TYPE_AUDIO<<2))
        {
            switch (local_codec[3])
            {
                case AVDTP_MEDIA_CODEC_SBC:
                    ...
                    *error_code = areSBCCodecsCompatible(local_codec+4, remote_codec+4, local_codec[1], remote_codec[1], initiating);
                    if(avdtp_ok == (*error_code))
                        return remote_codec;
                    break;
#ifndef A2DP_SBC_ONLY
                case AVDTP_MEDIA_CODEC_MPEG1_2_AUDIO:
                    /* check MPEG-1,2 Audio codecs are compatible. */
                    *error_code = areMPEG12AudioCodecsCompatible(local_codec+4,remote_codec+4,local_codec[1],remote_codec[1], initiating);
                    if(avdtp_ok == (*error_code))
                        return remote_codec;
                    break;
#endif
                ...
                case AVDTP_MEDIA_CODEC_MPEG2_4_AAC:
                    ...
                    *error_code = areMPEG24AACCodecsCompatible(local_codec+4,remote_codec+4,local_codec[1],remote_codec[1], initiating);
                    if(avdtp_ok == (*error_code))
                        return remote_codec;
                    break;
#endif
    ...
    else if(local_codec[3] != remote_codec[3])
    {
        if(remote_codec[3] & ~(AVDTP_MEDIA_CODEC_SBC |
                               AVDTP_MEDIA_CODEC_MPEG1_2_AUDIO |
                               AVDTP_MEDIA_CODEC_MPEG2_4_AAC |
                               AVDTP_MEDIA_CODEC_ATRAC))
        {
            /* remote codec is invalid */
            *error_code = avdtp_invalid_codec_type;
        }
        else
        {
            /* remote codec is not supported */
            *error_code = avdtp_not_supported_codec_type;
        }
    }
