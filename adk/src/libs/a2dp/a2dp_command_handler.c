...
static bool issueSetConfiguration (remote_device *device)
{
    ...
    case CONFIGURATION_BY_CLIENT:       
        ...
        DEBUG_LOG_INFO("issueSetConfiguration CONFIGURATION_BY_CLIENT, seid=0x%02x role=enum:a2dp_role_type:%d",
                       device->local_sep.seid, device->local_sep.role);

        if (device->local_sep.role == a2dp_source)
        {
            /* If we are a source device, need to send the remote (sink) device's
               capabilities up to the client, so that it has all the info required
               to select a configuration matching its own (known) capabilities. */
            remote_service_caps = (uint8 *)PanicNull(blockGetBase((uint8)device->bitfields.device_id, data_block_configured_service_caps));
            remote_service_caps_size = blockGetSize((uint8)device->bitfields.device_id, data_block_configured_service_caps);
            if (!a2dpFindCodecSpecificInformation((const uint8**)&remote_service_caps, &remote_service_caps_size))
                break;  /* Unable to locate AVDTP_SERVICE_MEDIA_CODEC section, return. */
        }
        else /* a2dp_sink */
        {
            /* If we are a sink device, send service_caps that match the selected
               SEID, i.e. the local (sink) capabilities, for the local SEID. */
...
void a2dpHandleCodecConfigureResponse(const A2DP_INTERNAL_CODEC_CONFIGURE_RSP_T *rsp)
{
    ...
    else
    {
        ...
        /* remove the existing caps from configured service caps data block
         * since the last set configuration command was rejected by handset */
