...
uint16 a2dpProcessSetConfigurationCommand (remote_device *device)
{
    ...
    else 
    {
        ...
        else if ((codec_data = a2dpFindMatchingCodecSpecificInformation(sep_config->caps, &ptr[4], 0, &error_code)) == NULL)
        {   /*  Check the codec specific attributes are compatible set config does not match our caps - reject */
            return (AVDTP_SERVICE_MEDIA_CODEC << 8) | error_code;
        }
        else
        {
...
uint16 a2dpProcessReconfigureCommand (remote_device *device)
{
    ...
    */
    ...
    else
    {
        ...
        else if (a2dpFindMatchingCodecSpecificInformation(sep_config->caps, reconfig_caps, 0, &error_code) == NULL)
...
bool a2dpProcessGetCapabilitiesResponse(remote_device *device)
{
    ...
    /* Codec Configuration. */
    if ( a2dpFindMatchingCodecSpecificInformation(sep_config->caps, remote_codec, TRUE, &error_code) == NULL)
