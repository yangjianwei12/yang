...
static bool isAppIdValid(const CsrBtGattClientNotificationInd *ind)
{
    return isStringNullTerminated(&ind->value[1], ind->valueLength - 1);
}
...
static uint16 getCharacteristicHandle(GANCS *ancs, uint8 characteristic)
{
    switch(characteristic)
    {
        ...
        default:
            PANIC("getCharacteristicHandle: unknown characteristic 0x%04X", characteristic);
...
static bool discoverCharacteristicDescriptors(GANCS *ancs, uint8 characteristic)
{
    ...
    startHandle = MIN(getCharacteristicHandle(ancs, characteristic) + 1, ancs->service_end_handle);
    endHandle = findEndHandleForCharDesc(ancs, startHandle, ancs->service_end_handle, characteristic);
...
void handleAncsNotification(GANCS *ancs, const CsrBtGattClientNotificationInd *ind)
{
    ...
    if (ind->valueLength) 
    {
        ...
        if(ind->valueHandle == ancs->notification_source)
        {
        ...
        * " As with a response to a Get Notification Attributes command, if the response to a Get App Attributes command
            ...
            */
        else if(ind->valueHandle == ancs->data_source)
        {
            if(isPendingCmdWriteControlPointCharacteristic(ancs))
                gattAncsSendWriteControlPointResponse(ancs, CSR_BT_GATT_ACCESS_RES_SUCCESS);
            ...
            switch(ind->value[0])
            {
                case gatt_ancs_notification_attr:
                {
                    ...
                    gattAncsSendDataSourceAttributesInd(ancs, notification_uid, &ind->value[5], ind->valueLength - 5);
                }
                break;
                ...
                {
                    if(isAppIdValid(ind))
                        gattAncsSendDataSourceAppAttributesInd(ancs, ind->valueLength - 1, &ind->value[1]);
                }
                break;
    ...
    switch(characteristic)
    {
        case GATT_APPLE_NOTIFICATION_NS:
            if (CHECK_VALID_HANDLE(ancs->notification_source))
            {
                ...
                {
                    DEBUG_LOG_INFO("ancsSetNotificationRequest: write Notification Source config");
                    writeClientConfigNotifyValue(ancs, notifications_enable, ancs->ns_ccd);
                    ancs->pending_cmd = ancs_pending_write_ns_cconfig;
                }
                else
                {
                    DEBUG_LOG_INFO("ancsSetNotificationRequest: discoverAllCharacteristicDescriptors for Notification Source");
                    if (discoverCharacteristicDescriptors(ancs, GATT_APPLE_NOTIFICATION_NS))
                    {
                        ...
                        gattAncsSendSetNotificationSourceNotificationResponse(ancs, CSR_BT_GATT_ACCESS_RES_UNLIKELY_ERROR);
                }
            }
            else
        ...
        case GATT_APPLE_NOTIFICATION_DS:
            if (CHECK_VALID_HANDLE(ancs->data_source))
            {
                ...
                {
                    DEBUG_LOG_INFO("ancsSetNotificationRequest: write Data Source config");
                    writeClientConfigNotifyValue(ancs, notifications_enable, ancs->ds_ccd);
                    ancs->pending_cmd = ancs_pending_write_ds_cconfig;
                }
                else
                {
                    DEBUG_LOG_INFO("ancsSetNotificationRequest: discoverAllCharacteristicDescriptors for Data Source");
                    if (discoverCharacteristicDescriptors(ancs, GATT_APPLE_NOTIFICATION_DS))
                    {
                        ...
                        gattAncsSendSetDataSourceNotificationResponse(ancs, CSR_BT_GATT_ACCESS_RES_UNLIKELY_ERROR);
                }
            }
            else
        ...
        default:
            PANIC("ancsSetNotificationRequest: not a valid characteristic 0x%04X", characteristic);
        break;
    }
}
