...
static void processDiscoveredDescriptor(GANCS *ancs, const GATT_MANAGER_DISCOVER_ALL_CHARACTERISTIC_DESCRIPTORS_CFM_T *cfm)
{
    switch (ancs->pending_cmd)
    {
        ...
        case ancs_pending_set_ns_notify_disable:
            if (cfm->uuid_type == gatt_uuid16)
            {
                if (cfm->uuid[0] == GATT_CLIENT_CHARACTERISTIC_CONFIGURATION_UUID)
                {
                    ...
                    DEBUG_LOG_INFO("processDiscoveredDescriptor: found Notification Source CCD handle=0x%04X", cfm->handle);
        ...
        case ancs_pending_set_ds_notify_disable:
            if (cfm->uuid_type == gatt_uuid16)
            {
                if (cfm->uuid[0] == GATT_CLIENT_CHARACTERISTIC_CONFIGURATION_UUID)
                {
                    ...
                    DEBUG_LOG_INFO("processDiscoveredDescriptor: found Data Source CCD handle=0x%04X", cfm->handle);
        ...
        default:
            DEBUG_PANIC("processDiscoveredDescriptor: wrong state; pending_cmd=enum:ancs_pending_cmd_t:%d, handle=0x%04X", ancs->pending_cmd, cfm->handle);
        break;
    }
}
    ...
    */
    ...
    if (cfm->status == gatt_status_success)
    {
        if (cfm->uuid_type == gatt_uuid128)
        {
            ...
            {
                DEBUG_LOG_INFO("handleAncsDiscoverAllCharacteristicsResp: found Notification Source handle=0x%04X, status=enum:gatt_status_t:%d", cfm->handle , cfm->status);
                ancs->notification_source = cfm->handle;
                charVal = GATT_APPLE_NOTIFICATION_NS;
            }
            else if(CHECK_ANCS_DS_UUID(cfm))
            {
                DEBUG_LOG_INFO("handleAncsDiscoverAllCharacteristicsResp: found Data Source handle=0x%04X, status=enum:gatt_status_t:%d", cfm->handle , cfm->status);
                ancs->data_source = cfm->handle;
                charVal = GATT_APPLE_NOTIFICATION_DS;
            }
            else if(CHECK_ANCS_CP_UUID(cfm))
            {
                DEBUG_LOG_INFO("handleAncsDiscoverAllCharacteristicsResp: found Control Point handle=0x%04X, status=enum:gatt_status_t:%d", cfm->handle , cfm->status);
