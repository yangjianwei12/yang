                          ...
                          uint8 volume_settting_operand)
{
    ...
    if (vcp_inst)
    {
        switch(opcode)
        {
            ...
            default:
            {
                VCP_DEBUG_PANIC(("Invalid VCS control point opcode\n"));
                                  ...
                                  uint8 volume_setting)
{
    ...
    MessageSendConditionally((Task)&vcp_inst->lib_task,
                           ...
                           int16 volume_offset_operand)
{
    ...
    if (vcp_inst)
    {
        switch(opcode)
        {
            ...
            default:
            {
                VCP_DEBUG_PANIC(("Invalid VOCS control point opcode\n"));
...
{
    if (msg->status == (gatt_status_t) VCP_VOCS_INVALID_CHANGE_COUNTER_ERR)
    {
                           ...
                           int8 gain_settting_operand)
{
    ...
    if (vcp_inst)
    {
        switch(opcode)
        {
            ...
            default:
            {
                VCP_DEBUG_PANIC(("Invalid AICS control point opcode\n"));
                vcpSendAudioInputControlPointOpCfm(vcp_inst,
                                                   srvc_hndl,
                                                   gatt_status_failure,
                                                   vcpGetAicsMessageIdFromOpcode(opcode));
                                        ...
                                        gatt_status_t status)
{
    ...
    MessageSend(vcp_inst->app_task, VCP_SET_AUDIO_INPUT_DESC_CFM, message);
}
