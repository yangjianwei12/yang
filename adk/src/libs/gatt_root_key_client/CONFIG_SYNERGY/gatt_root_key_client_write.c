...
void rootKeyWriteClientConfigValue(GATT_ROOT_KEY_CLIENT *instance)
{
    ...
    config_value[0] = ROOT_KEY_SERVICE_NOTIFICATION_VALUE;
    config_value[1] = 0;


    GattWriteReqSend(instance->gattId, instance->cid,
                     instance->handle_challenge_control_config, 0, 
                     config_value_len, config_value);
}


/****************************************************************************/
void handleRootKeyWriteValueResp(GATT_ROOT_KEY_CLIENT *instance, const CsrBtGattWriteCfm *write_cfm)
{
    ...
    {
        GATT_ROOT_KEY_CLIENT_DEBUG("handleRootKeyWriteValueResp: response:0x%04x supplier:0x%04x not expected in state %d",
                                   write_cfm->resultCode, write_cfm->resultSupplier, state);
        gattRootKeyClientSetState(instance, root_key_client_error);
    }
}
                                    ...
                                    const bdaddr   *remote_address)
{
    ...
    DEBUG_LOG("GattRootKeyClientChallengePeer: LAddress%04x %02x %06x RAddress%04x %02x %06x",
...
{
    /* Allocate memory*/
    uint8 *value = (uint8*)CsrPmemAlloc(GRKS_KEY_SIZE_128BIT_OCTETS + 1);
    ...
    GattWriteReqSend(instance->gattId, instance->cid,
                     instance->handle_challenge_control, 0, 
                                           ...
                                           const GRKS_KEY_T *content)
{
    ...
    GattWriteReqSend(instance->gattId, instance->cid,
                     instance->handle_keys_control, 0, 
                     length, value);
}
