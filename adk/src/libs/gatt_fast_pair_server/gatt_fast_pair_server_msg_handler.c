...
/******************************************************************************/
static void fpsHandleBeaconActionAccess(
        const GFPS *fast_pair_server,
        const GATT_MANAGER_SERVER_ACCESS_IND_T *access_ind)
{
    /* If ONLY flag is ATT_ACCESS_READ */
    if (access_ind->flags == ATT_ACCESS_READ)
    {
        MAKE_FPS_MESSAGE(
        ...
        message->fast_pair_server = fast_pair_server;
        message->btConnId = access_ind->cid;

        MessageSend(fast_pair_server->app_task,
                    GATT_FAST_PAIR_SERVER_READ_MODEL_ID_IND,
                    message);
    }

    /* If ONLY flag is ATT_ACCESS_WRITE */
    else if (access_ind->flags == ATT_ACCESS_WRITE)
    {
        MAKE_FPS_MESSAGE_WITH_LEN(
        ...
        )
{
    ...
    else
    {
        switch (access_ind->handle)
        {
            ...
            {
                fpsHandleBeaconActionAccess(fast_pair_server,
                                            access_ind);
            }
            break;
