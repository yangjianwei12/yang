...
#ifdef SYNERGY_FOR_VM
    /* Associate the task with the synergy connection manager. */
    CmRegisterApplicationHandleRequest(task, CM_CREATE_L2CA_CONN_ID(cid), CM_PROTOCOL_L2CAP);
#endif /* SYNERGY_FOR_VM */
}

#ifdef SYNERGY_FOR_VM
void configureRfcommTaskToConnectionId(uint16 conn_id, Task task)
{
    /* Associate the task with the synergy connection manager. */
    CmRegisterApplicationHandleRequest(task, CM_CREATE_RFC_CONN_ID(conn_id), CM_PROTOCOL_RFCOMM);
}
#endif /* SYNERGY_FOR_VM */
