...
/* Sent by host
   ...
   UPGRADE_HOST_PROCEED_TO_COMMIT
...
typedef enum {
    ...
    UPGRADE_HOST_COMPLETE_IND_WITH_STATUS,
} UpgradeMsgHost;
...
typedef enum {
    ...
    UPGRADE_HOST_ERROR_SFS_VALIDATION_FAILED_DEPRECATED,
...
typedef enum {
    UPGRADE_MSG_ID_SIZE =                      sizeof(uint8),
    UPGRADE_MSG_LENGTH_SIZE =                  sizeof(uint16),
    UPGRADE_MSG_RESUME_POINT_SIZE =            sizeof(uint8),
    UPGRADE_MSG_ERROR_CODE_SIZE =              sizeof(uint16),
    UPGRADE_MSG_NUMBER_OF_BYTES_SIZE =         sizeof(uint32),
    UPGRADE_MSG_START_OFFSET_SIZE =            sizeof(uint32),
    UPGRADE_MSG_SILENT_COMMIT_SUPPORTED_SIZE = sizeof(uint8),
    UPGRADE_MSG_BACK_OFF_TIME_SIZE =           sizeof(uint16),
    UPGRADE_MSG_LAST_PACKET_FLAG_SIZE =        sizeof(uint8)
} upgrade_msg_element_size_t;
...
typedef UPGRADE_HOST_GENERIC_ACTION_T UPGRADE_HOST_PROCEED_TO_COMMIT_T;
#define UPGRADE_HOST_PROCEED_TO_COMMIT_BYTE_SIZE (1)
...
/*!
    Definition of the UPGRADE_HOST_COMPLETE_IND_WITH_STATUS message.
    Sent by the device to the host in response to the
    UPGRADE_HOST_COMMIT_CFM message.
 */
typedef struct {
    uint8 commitStatus;
} UPGRADE_HOST_COMPLETE_IND_WITH_STATUS_T;
#define UPGRADE_HOST_COMPLETE_IND_WITH_STATUS_BYTE_SIZE (1)
