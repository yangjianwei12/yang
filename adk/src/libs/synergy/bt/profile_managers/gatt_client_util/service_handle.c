...
static void *CsrPmemRealloc(void *ptr, CsrSize oldSize, CsrSize newSize)
{
    void *new;
...
#define CHECKSUM_MASK ((CsrUint16)~SERVICE_HANDLE_MASK)
#define CALC_HANDLE_CHECKSUM(px) (((*(CsrUint16 *) px) & CHECKSUM_MASK))
...
{
    CsrUint16 listLen;
    CsrUint16 newIndex = 0;
    ServiceHandle newHandle;

    /* Create the required data instance */
    *newInstance = CsrPmemAlloc(newSize);

    /* Check if the index is initialised */
    ...
    if (newIndex >= listLen)
    {
        ...
        if (listLen == 0)
            ...
            src_len = sizeof(service_handle_data_t) + (sizeof(void*) * (listLen -1));

        new_service_handle_data = CsrPmemRealloc(service_handle_data, src_len,
...
void* ServiceHandleGetInstanceData(const ServiceHandle service_handle)
{
    ...
    /* Check if the service_handle is valid */
    if(service_handle == 0)
    {
        return NULL;
    }
...
CsrBool ServiceHandleFreeInstanceData(const ServiceHandle service_handle)
{
    ...
    if(GET_HANDLE_INDEX(service_handle) == service_handle_data->size)
    {
        ...
        while ((service_handle_data->size > 0) && service_handle_data->instances[service_handle_data->size-1] == NULL)
        {
            service_handle_data->size--;
        }
        ...
        {
            CsrPmemFree(service_handle_data);
            service_handle_data = NULL;
        }
        else
        {
            CsrSize src_len = sizeof(service_handle_data_t) + (sizeof(void*) * (service_handle_data->size));

            service_handle_data = (CsrPmemRealloc(service_handle_data, src_len,
