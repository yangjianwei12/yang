...
leAdvDataItem * BapServerGetAdvertisingData( BapServerRole ServerRole)
{
    ...
    if (ServerRole == BAP_SERVER_UNICAST_ROLE)
    {
        data = CsrPmemZalloc(BAP_UNICAST_SERVER_SIZE_ADVERT);
    ...
    {
        data = CsrPmemZalloc(BAP_SCAN_DELEGATOR_SIZE_ADVERT);
...
leAdvDataItem *BapServerGetTargetedAdvertisingData(void)
{
    ...
    data[BAP_UNICAST_SERVER_ANNOUNCEMENT_TYPE_OCTET] = announcement_type_targeted;
...
void BapServerReleaseAdvertisingItems( BapServerRole ServerRole)
{
    if ( ServerRole == BAP_SERVER_UNICAST_ROLE )
    {
        ...
        {
            CsrPmemFree((void *)bapUnicastServerAdvert.data);
    ...
    else if ( ServerRole == BAP_SERVER_BROADCAST_ROLE )
    {
        ...
        {
            CsrPmemFree((void *)bapScanDelegatorAdvert.data);
