                                       ...
                                       GattVcsClientMessageId id)
{
     ...
     * because the structure of all the CCC read confirmations is the same,
     * but we will send the right message using the id parameter */
    MAKE_VCP_MESSAGE_WITH_LEN(VcpReadOffsetStateCccCfm, size_value);
                                      ...
                                      uint8 change_counter)
{
    ...
    message->id = VCP_READ_VOLUME_STATE_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->status = status;
                                     ...
                                     const uint8 *value)
{
    ...
    message->id = VCP_READ_VOLUME_FLAG_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->status = status;
                                             ...
                                             uint8 change_counter)
{
    ...
    message->id = VCP_READ_OFFSET_STATE_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->srvcHndl = srvc_hndl;
    message->status = status;
                                        ...
                                        GattVocsClientAudioLoc value)
{
    ...
    message->id = VCP_READ_AUDIO_LOCATION_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->srvcHndl = srvc_hndl;
    message->status = status;
...
static void vcpSendReadAudioOutputDescCfm(VCP *vcp_inst,
                                          ...
                                          uint8 *value)
{
    MAKE_VCP_MESSAGE_WITH_LEN(VcpReadAudioOutputDescCfm, size_value);

    message->id = VCP_READ_AUDIO_OUTPUT_DESC_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->srvcHndl = srvc_hndl;
    message->status = status;
                               ...
                               ServiceHandle vocsClntHndl)
{
    ...
    if (vcp_inst)
    {
        if (vocsClntHndl)
        {
            ...
            {
                vcpProfileSendReadOffsetStateCfm(vcp_inst, vocsClntHndl, CSR_BT_GATT_RESULT_INTERNAL_ERROR, 0, 0);
            }
        }
        else
        {
                                     ...
                                     uint8 change_counter)
{
    ...
    message->id = VCP_READ_INPUT_STATE_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->srvcHndl = srvc_hndl;
    message->status = status;
                              ...
                              ServiceHandle aicsClntHndl)
{
    ...
    if (vcp_inst)
    {
        if (aicsClntHndl)
        {
            ...
            {
                vcpSendReadInputStateCfm(vcp_inst, aicsClntHndl, CSR_BT_GATT_RESULT_INTERNAL_ERROR, 0, 0, 0, 0);
            }
        }
        else
        {
                                                ...
                                                int8 gain_set_max)
{
    ...
    message->id = VCP_READ_GAIN_SET_PROPERTIES_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->srvcHndl = srvc_hndl;
    message->status = status;
                                    ...
                                    GattAicsClientInputType input_type)
{
    ...
    message->id = VCP_READ_INPUT_TYPE_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->srvcHndl = srvc_hndl;
    message->status = status;
                                     ...
                                     uint8 input_status)
{
    ...
    message->id = VCP_READ_INPUT_STATUS_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->srvcHndl = srvc_hndl;
    message->status = status;
...
static void vcpSendReadAudioInputDescCfm(VCP *vcp_inst,
                                         ...
                                         uint8 *value)
{
    MAKE_VCP_MESSAGE_WITH_LEN(VcpReadAudioInputDescCfm, size_value);

    message->id = VCP_READ_AUDIO_INPUT_DESC_CFM;
    message->prflHndl = vcp_inst->vcp_srvc_hdl;
    message->srvcHndl = srvc_hndl;
    message->status = status;
