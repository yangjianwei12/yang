 ...
 * This function is used to send AVRCP command reject respose to remote.
 * It uses rxData(stored in pending msg list) for Transaction Label and PDU ID.*/
void CsrBtAvrcpTgRejectAvcVendorSend(AvrcpConnInstance_t *connInst, CsrUint8 *rxData, CsrUint8 rType, CsrUint8 status, CsrUint8 tLabel, CsrUint8 pduId)
{
    ...
    CsrBtAvrcpDataInsertAvctpHeader(txData,
                               rxData ? AVRCP_TLABEL_GET(rxData) : tLabel, /* Transaction label */
    ...
    CsrBtAvrcpDataInsertAvcVendorHeader(txData,
                                   rxData ? rxData[AVRCP_DATA_MD_PDU_ID_INDEX] : pduId,
...
void CsrBtAvrcpTgRegisterNotificationRspSend(AvrcpConnInstance_t *connInst, CsrUint8 notiId, CsrUint8 *notiData, CsrUint8 rspType,CsrBool ApChanged)
{
    ...
    else
    {
        ...
        SynMemCpyS(&txData[AVRCP_DATA_PDU_REG_NOTI_RES_VAL_INDEX], rspSize - 1, notiData, rspSize - 1);
    }
        ...
        else
        {
            CsrBtAvrcpTgRejectAvcVendorSend(connInst,
                *rxData,
                ...
                CSR_BT_AVRCP_STATUS_INVALID_PARAMETER,
...
static void csrBtAvrcpTgRegisterNotificationCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    ...
    else
    {
        CsrBtAvrcpTgRejectAvcVendorSend(connInst,
            *rxData,
            CSR_BT_AVRCP_DATA_AVC_RTYPE_REJECTED,
            CSR_BT_AVRCP_STATUS_INVALID_COMMAND,
            CSR_BT_AVRCP_TLABEL_INVALID,
            AVRCP_DATA_PDU_ID_INVALID);
    }
}
...
static void csrBtAvrcpTgSetAbsoluteVolumeCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    ...
    {
        CsrBtAvrcpTgSetVolumeIndSend(connInst, rxDataLen, rxData);
    }
    else
    {
        CsrBtAvrcpTgRejectAvcVendorSend(connInst,
...
static void csrBtAvrcpTgPasGetAttTxtRspSend(AvrcpConnInstance_t *connInst, CsrUint8 tLabel, CsrUint8 cmdAttIdCount, CsrUint8 *cmdAttId)
{
    ...
    for (cmdAttIndex = 0; cmdAttIndex < rspAttCount; cmdAttIndex++)
    {
        ...
        if (tgAttIndex != CSR_BT_AVRCP_TG_LIB_PAS_INVALID_INDEX)
        {
            ...
            SynMemCpyS(&txData[AVRCP_DATA_PDU_PAS_GET_ATT_TXT_RES_TXT_INDEX + rspIndex],
    ...
    if (CsrBtAvrcpTgLibPasAttribGet(pasLen, pas, cmdAttId) != CSR_BT_AVRCP_TG_LIB_PAS_INVALID_INDEX)
    {
        ...
        for (cmdValIndex = 0; cmdValIndex < rspValCount; cmdValIndex++)
        {
            ...
            if (tgValIndex != CSR_BT_AVRCP_TG_LIB_PAS_INVALID_INDEX)
            {
                ...
                SynMemCpyS(&txData[AVRCP_DATA_PDU_PAS_GET_VAL_TXT_RES_TXT_INDEX + rspIndex],
...
static void csrBtAvrcpTgInformBatteryStatusOfCTHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    if ((rxDataLen == (AVRCP_DATA_MD_HEADER_SIZE + AVRCP_DATA_PDU_BAT_STATUS_CMD_HEADER_SIZE)) &&
        ((*rxData)[rxDataLen-1] <= CSR_BT_AVRCP_BATTERY_STATUS_FULL_CHARGE) &&
        ((connInst->instData->tgLocal.configuration & CSR_BT_AVRCP_CONFIG_ROLE_TG_BATT_SUPPORT) != 0) 
    ...
    {/* Inform the corresponding media player and send response to the remote device */
       CsrBtAvrcpTgBatteryStatusOfCtIndSend(connInst->appConnId,connInst->tgLocal->mpAddressed->mpHandle,
                                            ...
                                            (*rxData)[AVRCP_DATA_PDU_BAT_STATUS_INDEX]);
       CsrBtAvrcpDataSimpleVendorFrameSend(connInst,
                   AVRCP_TLABEL_GET((*rxData)),
                   AVRCP_DATA_AVCTP_CR_RES,                 /* Command/response */
                   CSR_BT_AVRCP_DATA_AVC_RTYPE_ACCEPTED,           /* Command/response type */
...
static void csrBtAvrcpTgPasSetValueCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
        ...
        validGetAttTextData((*rxData)[AVRCP_DATA_PDU_PAS_SET_VAL_CMD_ATT_ID_INDEX],(*rxData)[AVRCP_DATA_PDU_PAS_SET_VAL_CMD_VAL_ID_INDEX]))
    {
        ...
        SynMemCpyS(valPtr,
                   valSetCount * sizeof(CsrBtAvrcpPasAttValPair),
                   &(pendingMsgInfo->rxData)[AVRCP_DATA_PDU_PAS_SET_VAL_CMD_ATT_ID_INDEX],
                   valSetCount * sizeof(CsrBtAvrcpPasAttValPair));
...
void CsrBtAvrcpTgSetBrowsedPlayerRspSend(AvrcpConnInstance_t *connInst, CsrUint8 tLabel, CsrBtAvrcpStatus status, CsrUint16 uidCounter, CsrUint32 itemsCount, CsrUint8 folderDepth, CsrUint16 foldersNameLen, CsrUint8 *folderNames)
{
    ...
    SynMemCpyS(&txData[AVRCP_DATA_PDU_SET_BP_RES_FOLDER_PAIRS_INDEX], foldersNameLen, folderNames, foldersNameLen);
...
{
    CsrUint8 *txData;
    CsrUint16 txLength = (CsrUint16)(AVRCP_DATA_BROWSING_HEADER_SIZE + AVRCP_DATA_PDU_GTNI_RES_HEADER_SIZE);
    txData = CsrPmemAlloc(txLength);
    ...
    txData[AVRCP_DATA_PDU_GTNI_RES_STATUS_INDEX] = CSR_BT_AVRCP_STATUS_OPERATION_COMPLETE;
    CSR_COPY_UINT16_TO_BIG_ENDIAN(uidCounter, &txData[AVRCP_DATA_PDU_GTNI_RES_UID_COUNT_INDEX]);
    CSR_COPY_UINT32_TO_BIG_ENDIAN(itemsCount, &txData[AVRCP_DATA_PDU_GTNI_RES_NUM_OF_ITEMS_INDEX]);
...
static void csrBtAvrcpTgSetBrowsedPlayerCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    if (rxDataLen >= (AVRCP_DATA_BROWSING_HEADER_SIZE + AVRCP_DATA_PDU_SET_BP_CMD_HEADER_SIZE))
    {
        ...
        CsrBtAvrcpTgMp *mp = AVRCP_LIST_TG_MP_GET_ID((CsrCmnList_t *)&connInst->instData->tgLocal.mpList, playerId);
    ...
    if ((rxDataLen >= dataLenExpected)  && connInst->tgLocal->mpBrowsed)
    {
        ...
        if (pendingMsgInfo->rxData[AVRCP_DATA_PDU_GFI_CMD_SCOPE_INDEX] == CSR_BT_AVRCP_SCOPE_MP_LIST)
        {
            ...
            {/* Inquire the application about the play status of the media players requested - begin with first player in the request */
                CsrBtAvrcpTgMp  *tmpPlayer = AVRCP_LIST_TG_MP_GET_FIRST((CsrCmnList_t *)&connInst->instData->tgLocal.mpList);
                CsrUint16 i;
                ...
                if ((AVRCP_LIST_TG_MP_GET_COUNT((CsrCmnList_t *)&connInst->instData->tgLocal.mpList) - startIdx) < connInst->tgLocal->itemCnt)
                {
                    connInst->tgLocal->itemCnt = AVRCP_LIST_TG_MP_GET_COUNT((CsrCmnList_t *)&connInst->instData->tgLocal.mpList) - startIdx;
                }
        ...
        {/* Verify parameters in command */
                ...
                (endIdx >= startIdx))
            {
                ...
                if ((attribMask != CSR_BT_AVRCP_ITEM_ATT_MASK_NONE) &&
                    (connInst->tgLocal->obexState != AVRCP_TG_OBEX_SERVER_CONNECTED))
                {
                    CSR_BIT_UNSET(attribMask, CSR_BT_AVRCP_ITEM_ATT_COVER_ART - CSR_BT_AVRCP_ITEM_ATT_OFFSET_FROM_MASK);
                }
#endif
...
static void CsrBtAvrcpTgGetTotalNumberOfItemsCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    if ((rxDataLen >= (AVRCP_DATA_BROWSING_HEADER_SIZE + AVRCP_DATA_PDU_GTNI_CMD_HEADER_SIZE)) && connInst->tgLocal->mpBrowsed)
    {
        CsrUint8 scope = (*rxData)[AVRCP_DATA_PDU_GTNI_CMD_SCOPE_INDEX];
        if (scope == CSR_BT_AVRCP_SCOPE_MP_LIST)
        {
            CsrBtAvrcpTgGetTotalNumberOfItemsRspSend(connInst,
                                             AVRCP_TLABEL_GET(*rxData),
                                             AVRCP_LIST_TG_MP_GET_COUNT((CsrCmnList_t *)&connInst->instData->tgLocal.mpList),
                                             connInst->instData->tgLocal.uidCount);
        }
        else
        {
            if (scope <= CSR_BT_AVRCP_SCOPE_NPL)
            {
                AvrcpTgPendingMsgInfo_t *pendingMsgInfo;
                pendingMsgInfo = CsrBtAvrcpTgUtilMsgQueueAdd(connInst,
                                       CSR_BT_AVCTP_BROWSING_PSM,
                                       CSR_BT_AVRCP_TG_GET_TOTAL_NUMBER_OF_ITEMS_RES,/* Pending message type */
                                       CSR_BT_AVRCP_TIMER_BROWSING,        /* Time */
                                       rxDataLen,
                                       rxData);                     /* Received command for later use */
                CsrBtAvrcpTgGetTotalNumberOfItemsIndSend(pendingMsgInfo, scope);
            }
            else
            {
                CsrBtAvrcpTgNormalRejectBrowsingSend(connInst, *rxData, CSR_BT_AVRCP_STATUS_INVALID_SCOPE);
            }
        }
    }
    else
...
static void csrBtAvrcpTgSearchCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    ...
    if (rxDataLen >= (AVRCP_DATA_BROWSING_HEADER_SIZE + AVRCP_DATA_PDU_SEARCH_CMD_MIN_HEADER_SIZE + rxPayloadLength))
    {
        ...
        SynMemCpyS(text, len, &(*rxData)[AVRCP_DATA_PDU_SEARCH_CMD_SEARCH_TXT_INDEX],len-1);
...
static void csrBtAvrcpTgGetItemAttCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    if ((rxDataLen  >= (AVRCP_DATA_BROWSING_HEADER_SIZE + AVRCP_DATA_PDU_GIA_CMD_MIN_HEADER_SIZE)) && connInst->tgLocal->mpBrowsed)
    {
...
#ifdef CSR_BT_INSTALL_AVRCP_TG_COVER_ART
        if (connInst->tgLocal->obexState != AVRCP_TG_OBEX_SERVER_CONNECTED)
        {
            CSR_BIT_UNSET(attribMask, CSR_BT_AVRCP_ITEM_ATT_COVER_ART - CSR_BT_AVRCP_ITEM_ATT_OFFSET_FROM_MASK);
            isCABitUnset = TRUE;
        }
        ...
        if ((attribMask <= CSR_BT_AVRCP_ITEM_ATT_MASK_ALL) && (attribMask > 0))
        {
            ...
            SynMemCpyS(AvrcpUid, sizeof(CsrBtAvrcpUid), &(pendingMsgInfo->rxData)[AVRCP_DATA_PDU_GIA_CMD_UID_INDEX], sizeof(CsrBtAvrcpUid));
        ...
        else
        {
            if (isCABitUnset)
            {
                CsrUint16 attribDataLen = 0;
                CsrUint8 *pData;
                attribDataLen = AVRCP_DATA_BROWSING_HEADER_SIZE + 2 * sizeof(CsrUint8); /* allocate place for status and number of attributes too */
                pData = CsrPmemAlloc(attribDataLen);
...
void CsrBtAvrcpTgRxBrowsingHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    if (CsrBtAvrcpUtilDataCheckBrowsing(rxDataLen, *rxData))
    {
        switch (AVRCP_DATA_BROWSING_PDU_ID_GET(*rxData))
        {
            ...
            {
                CsrBtAvrcpTgGetTotalNumberOfItemsCmdHandler(connInst, rxDataLen, rxData);
                break;
            }
...
void CsrBtAvrcpTgGetElementAttributesRspSend(AvrcpConnInstance_t *connInst, CsrUint8 tLabel)
{
        ...
        switch (connInst->tgLocal->packetType)
        {
            ...
            {/* Continue or end frame? */
                ...
                else
                {
                    ...
                    txDataLen           = AVRCP_DATA_MD_HEADER_SIZE + resMsg->attribDataLen - connInst->tgLocal->getAttribIndex;
    ...
    if (connInst->tgLocal->packetType != AVRCP_DATA_MD_PACKET_TYPE_SINGLE)
    {
        SynMemCpyS(&txData[txAttribIndex], attribDataToCopy, resMsg->attribData + connInst->tgLocal->getAttribIndex, attribDataToCopy);
        ...
        && (connInst->tgLocal->mpAddressed))
    {
        ...
        if ((attribMask <= CSR_BT_AVRCP_ITEM_ATT_MASK_ALL) && (attribMask > 0))
        {
            ...
            AvrcpTgPendingMsgInfo_t *pendingMsgInfo = CsrBtAvrcpTgUtilMsgQueueAdd(connInst,
                                         ...
                                         rxDataLen,
                                         rxData);
            SynMemCpyS(AvrcpUid, sizeof(CsrBtAvrcpUid), &(pendingMsgInfo->rxData)[AVRCP_DATA_MD_PARAM_INDEX], sizeof(CsrBtAvrcpUid));
            CsrBtAvrcpTgGetAttributesIndSend(pendingMsgInfo,
                                        connInst->tgLocal->mpAddressed,
                                        attribMask,
        ...
        else
        {
            ...
            {
                CsrUint8 *pData;
                CsrBtAvrcpTgGetAttributesRes *resMsg = (CsrBtAvrcpTgGetAttributesRes *) CsrPmemZalloc(sizeof(*resMsg));
                ...
                resMsg->type          = CSR_BT_AVRCP_TG_GET_ATTRIBUTES_RES;
                resMsg->connectionId  = connInst->appConnId;
                resMsg->attribCount   = 0;
                resMsg->status        = CSR_BT_AVRCP_STATUS_OPERATION_COMPLETE;
                resMsg->attribDataLen = headerSize + 2 * sizeof(CsrUint8); /* allocate place for status and number of attributes too */
...
static void csrBtAvrcpTgSetAddressedPlayerCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    if (rxDataLen == (AVRCP_DATA_MD_HEADER_SIZE + AVRCP_DATA_PDU_SET_AP_CMD_HEADER_SIZE))
    {
        /* Find player */
...
void CsrBtAvrcpTgRejectAvcSend(AvrcpConnInstance_t *connInst, CsrUint8 *rxData, CsrUint8 rType)
{
    ...
    SynMemCpyS(&txData[AVRCP_DATA_PT_GN_COMPANY_ID_INDEX], opIdVendorLen, &rxData[AVRCP_DATA_PT_GN_COMPANY_ID_INDEX], opIdVendorLen);
...
static void csrBtAvrcpTgUnitInfoCmdHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 *rxData)
{
    ...
    txData[AVRCP_DATA_AVC_SUBUNIT_TYPE_INDEX]           = 
        ((AVRCP_DATA_AVC_SUBUNIT_TYPE_MASK << AVRCP_DATA_AVC_SUBUNIT_TYPE_SHIFT) | AVRCP_DATA_AVC_SUBUNIT_ID_MASK);
...
void CsrBtAvrcpTgRxControlHandler(AvrcpConnInstance_t *connInst, CsrUint16 rxDataLen, CsrUint8 **rxData)
{
    if (CsrBtAvrcpUtilDataCheckAVC(rxDataLen, *rxData))
    {
        ...
        {
        	if (rxDataLen > AVRCP_DATA_MD_PDU_ID_INDEX)
        	{
	            switch ((*rxData)[AVRCP_DATA_MD_PDU_ID_INDEX])
	            {
#ifdef CSR_BT_INSTALL_AVRCP_TG_13_AND_HIGHER
			...
			{
				CsrBtAvrcpTgRejectAvcSend(connInst, *rxData, CSR_BT_AVRCP_DATA_AVC_RTYPE_REJECTED);
			}	
        }
        else
        {
