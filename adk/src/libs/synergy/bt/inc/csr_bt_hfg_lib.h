 ...
 *        hfgSupportedCodecs             Supported CODECs bitmask.
 *----------------------------------------------------------------------------*/
#define CsrBtHfgActivateExtReqSend(_phandle, _atMode, _numConnections, _serviceName, _supportedFeatures, \
...
do {                                                                                                     \
        ...
        msg->hfgSupportedCodecs = _hfgSupportedCodecs;                                                   \
 ...
 *        hfgSupportedHfIndicators       Pointer to list of local supported HF Indicators, Indicator ID's are 16 bit unsigned
 *                                       integer values defined on the Bluetooth SIG Assigned Number page.
 *        hfgSupportedHfIndicatorsCount  Number of HF Indicators in 'hfSupportedHfIndicators' list.
 *----------------------------------------------------------------------------*/
#define CsrBtHfgActivateReqSend(_phandle, _atMode, _numConnections, _serviceName, _supportedFeatures,    \
                                   _callConfig, _hfgConfig, _hfgSupportedHfIndicators,                   \
                                   _hfgSupportedHfIndicatorsCount)                                       \
        CsrBtHfgActivateExtReqSend(_phandle, _atMode, _numConnections, _serviceName, _supportedFeatures, \
                                   _callConfig, _hfgConfig, _hfgSupportedHfIndicators,                   \
                                   _hfgSupportedHfIndicatorsCount,                                       \
                                   CSR_BT_WBS_CVSD_CODEC_MASK | CSR_BT_WBS_MSBC_CODEC_MASK)
 ...
 *    pcmRealloc      PCM slot reallocation  
 *
 *----------------------------------------------------------------------------*/
#define CsrBtHfgAudioConnectReqSend(_connectionId, _pcmSlot, _pcmRealloc)  \
        CsrBtHfgAudioConnectExtReqSend(_connectionId, _pcmSlot, _pcmRealloc, CSR_BT_HFG_QCE_UNSUPPORTED)

/*----------------------------------------------------------------------------*
 *  NAME
 *      CsrBtHfgAudioConnectExtReqSend
 *
 *  DESCRIPTION
 *        Connect/disconnect the audio path on the established service level connection.
 *        The audio switching is confirmed by a CSR_BT_HFG_AUDIO_CONNECT_CFM.
 ...
 *    pcmSlot:        PCM slot to use
 *    pcmRealloc      PCM slot reallocation  
 *    qceCodecId      QCE codec to use if SWB is enabled else should be CSR_BT_HFG_QCE_UNSUPPORTED
 *
 *----------------------------------------------------------------------------*/
#define CsrBtHfgAudioConnectExtReqSend(_connectionId, _pcmSlot, _pcmRealloc, _qceCodecId) { \
...
/*----------------------------------------------------------------------------*
 *  NAME
 *      CsrBtHfgVoiceRecogReqSend
 *
 *  DESCRIPTION
 *      Used to send the voice recognition state along with the recognized text
 *      from user input. This is also used to disable the voice recognition or
 *      to end the current voice recognition session.
 *
 *  PARAMETERS
 *      connectionId:       HFG connection number to send update on
 *      bvra                Voice recognition on/off
 *      vrecEngineState     State of AG's Voice Recognition Engine
 *      textId              Unique identifier for the recognized text, this is
 *                          valid only in current VR session
 *      textType            Type of the text giving information on how to
 *                          interpret given text by the application
 *      textOperation       Operation that the HF application needs to perform
 *                          on the stored text (REPLACE - 2 or APPEND - 3) or
 *                          store this as new text (NewText - 1)
 *      string              Voice response converted to text by AG, this parameter
 *                          shall be a UTF-8 text string and shall always be contained
 *                          within double-quotes.
 *                          There is no limit in the size of the string.
 *----------------------------------------------------------------------------*/
#define CsrBtHfgVoiceRecogReqSend(_connectionId,              \
                                  _bvra,                      \
                                  _vreState,                  \
                                  _textId,                    \
                                  _textType,                  \
                                  _textOperation,             \
                                  _string) {                  \
        CsrBtHfgVoiceRecogReq *msg = CsrPmemAlloc(sizeof(*msg)); \
        msg->type            = CSR_BT_HFG_VOICE_RECOG_REQ;       \
        msg->connectionId    = _connectionId;                    \
        msg->bvra            = _bvra;                            \
        msg->vreState        = _vreState;                        \
        msg->textId          = _textId;                          \
        msg->textType        = _textType;                        \
        msg->textOperation   = _textOperation;                   \
        msg->string          = _string;                          \
        CsrBtHfgMsgTransport(msg);}
...
#define CsrBtHfgVoiceRecogResSend(_connectionId, _cmeeMode) {           \
        CsrBtHfgVoiceRecogRes *msg = CsrPmemAlloc(sizeof(*msg));        \
 ...
 *
 *  PARAMETERS
 *      connectionId:       The id of the connection to receive the OK response.
 *      length              Length of the AT cmd string
 *      command:            Raw AT command as a null-terminated ASCII string
 *
 *----------------------------------------------------------------------------*/
#define CsrBtHfgSwbRspSend(_connectionId,_len, _cmd) {                 \
        CsrBtHfgSwbRsp *msg = (CsrBtHfgSwbRsp *)CsrPmemAlloc(sizeof(CsrBtHfgSwbRsp)); \
        msg->type = CSR_BT_HFG_SWB_RSP;                              \
        msg->connectionId = _connectionId;                              \
        msg->len = _len;                                                \
        msg->cmd = _cmd;                                                \
        CsrBtHfgMsgTransport(msg);}
...
/*----------------------------------------------------------------------------*
 *  NAME
 *      CsrBtHfgGetConnectedPeerAddress
 *
 *  DESCRIPTION
 *      Returns the address of the connected peer, if hfgConnId is invalid then 
 *      returns the address as 0 
 *
 *  PARAMETERS
 *      hfgConnId:       The id of the connection
 *
 *----------------------------------------------------------------------------*/
CsrBtDeviceAddr CsrBtHfgGetConnectedPeerAddress(CsrBtHfgConnectionId hfgConnId);
#endif
#ifdef __cplusplus
}
#endif
