...
#define UPGRADE_MAX_PARTITION_DATA_BLOCK_SIZE 56
...
typedef enum
{
    UPGRADE_PARTITION_DATA_STATE_ERASE_IMAGE_HEADER,
    UPGRADE_PARTITION_DATA_STATE_ERASE_BANK,
    UPGRADE_PARTITION_DATA_STATE_SIGNATURE_CHECK,
    UPGRADE_PARTITION_DATA_STATE_ERASE,
    ...
    UPGRADE_PARTITION_DATA_STATE_WAIT_FOR_VALIDATION,
    UPGRADE_PARTITION_DATA_STATE_HASH_CHECK,
    UPGRADE_PARTITION_DATA_STATE_HASH_CHECK_HEADER,
    UPGRADE_PARTITION_DATA_STATE_HASH_CHECK_DATA,
    UPGRADE_PARTITION_DATA_STATE_VALIDATION,
    UPGRADE_PARTITION_DATA_STATE_COPY,
    UPGRADE_PARTITION_DATA_STATE_VALIDATED,
    UPGRADE_PARTITION_DATA_STATE_VALIDATION_COMPLETE
...
typedef enum
{
    ...
    */
    ...
    /*! Message sent to application to inform that the upgrade has resumed.
        This message won't be sent if the upgrade resumes during data-xfer. */
    UPGRADE_RESUME_IND,

    /* Message sent to application to inform that the dfu file validation
       is completed and TRANSFER_COMPLETE_RES is received */
    UPGRADE_REBOOT_IND,
...
typedef enum
{
    ...
    upgrade_state_download_completed,

    /*! The upgrade library has completed the validation. */
    upgrade_state_validation_completed,
...
typedef enum
{
    ...
    upgrade_ops_relay_peer_proceed_to_commit,
...
typedef enum {
    ...
    UPGRADE_CONTEXT_GAMING_HEADSET_ADDON
} upgrade_context_t;
...
/*!
    @brief APPUHDR6 file format identifies a partition by two IDs, partition and instance.
*/
typedef struct
{
    uint8 partitionID;
    uint8 instanceID;
} UpgradePartitionInstance;

/*!
    @brief Common type to be used with both APPUHDR5 and APPUHDR6 file formats to identify a partition.
*/
typedef union
{
    uint16 partitionID;
    UpgradePartitionInstance partitionInstance;
} UpgradePartition;
...
typedef void SendCommitStatus_t(uint8 status);
...
typedef struct __upgrade_cmd_response_functions
{
    ...
    /*! Send swap commit status CFM respose */
    SendCommitStatus_t *SendCommitStatusCfm;
...
/*!
    @brief Inform the Upgrade library of a system message sent on completing an asynchronous trap.
...
/*!
    ...
    @param  status swap commit status from peer 
...
/*!
    @brief UpgradeSmHandleProceedToCommit
...
/*!
    @brief UpgradeStartValidation

    Start Upgrade Validation Process which can include hash checking, copying etc.
...
*/
UpgradePartition UpgradeCtxGetFWPartitionNum(void);
...
*/
UpgradePartition UpgradeCtxGetPSKeysLastClosedPartition(void);


/*! @brief Return the first word from pskey

    @return the first word of the current partition from the pskey
*/
...
/*! @brief Wrapper to internal message sent to upgrade lib to delay reboot due to revert by a stipulated time
    ...
    @param delayed the reboot will be delayed or not
    @return None
*/
...
/*! @brief Match the resume point with UPGRADE_RESUME_POINT_POST_REBOOT
    @param None
    @return TRUE if resume point is UPGRADE_RESUME_POINT_POST_REBOOT, else FALSE
*/
bool UpgradeIsResumePointPostReboot(void);

/*! @brief Check if the UpgradeSM state is less as DATA_TRANSFER

    @param None
    @return TRUE if stateis before DATA_TRANSFER, else FALSE
*/

bool UpgradeSmStateIsBeforeDataTransfer(void);

/*! @brief Match the resume point with UPGRADE_RESUME_POINT_PRE_VALIDATE

    @param None
    @return TRUE if resume point is UPGRADE_RESUME_POINT_PRE_VALIDATE, else FALSE
*/
...
/*! @brief Get the resume point stored in PS Keys
...
/*! @brief Handle upgrade validation complete scenario
    @param None
    @return None
*/
...
/*!
    @brief Check if validation is completed
    @return TRUE if the validation is completed, else FALSE
*/
bool UpgradeIsValidationComplete(void);

/*!
    @brief Check if Upgrade lib is waiting for an answer to start validation
    @return TRUE if waiting for validation 
*/
bool UpgradeIsWaitForValidation(void);

/*!
    @brief Returns the total number of partitions available
    @return number of partitions 
*/
uint8 UpgradePartitionGetTotalPartitions(void);

/*!
    @brief Returns the status of Transfer Complete Res
    @return TRUE if UPGRADE_HOST_TRANSFER_COMPLETE_RES is received and handled, else FALSE
*/
