...
void UpgradeHostIFDataSendCommitStatus(uint8 status)
{
    DEBUG_LOG_INFO("UpgradeHostIFDataSendCommitStatus, %d", status);
 
    uint8 *payload = PanicUnlessMalloc(UPGRADE_MSG_HEADER_SIZE + UPGRADE_HOST_COMPLETE_IND_WITH_STATUS);
    uint16 byteIndex = 0;
    byteIndex += ByteUtilsSet1Byte(payload, byteIndex, UPGRADE_HOST_COMPLETE_IND_WITH_STATUS - UPGRADE_HOST_MSG_BASE);
    byteIndex += ByteUtilsSet2Bytes(payload, byteIndex, UPGRADE_HOST_COMPLETE_IND_WITH_STATUS_BYTE_SIZE);
    byteIndex += ByteUtilsSet1Byte(payload, byteIndex, UpgradeCtxGet()->commitStatus);
...
bool UpgradeHostIFDataBuildIncomingMsg(Task clientTask, const uint8 *data, uint16 dataSize, bool* is_data_cfm_needed)
{
    *is_data_cfm_needed = TRUE;
    ...
    switch (msgId)
    {
        ...
        {
            if (msgLen == UPGRADE_HOST_PROCEED_TO_COMMIT_BYTE_SIZE)
            {
                MESSAGE_MAKE(msg, UPGRADE_HOST_PROCEED_TO_COMMIT_T);
                msg->action = ByteUtilsGet1ByteFromStream(msgPayload);
                DEBUG_LOG_INFO("UpgradeHostIFDataBuildIncomingMsg, UPGRADE_HOST_PROCEED_TO_COMMIT, action %u", msg->action);
                MessageSend(clientTask, msgId, msg);
            }
            else
                DEBUG_LOG_ERROR("UpgradeHostIFDataBuildIncomingMsg, UPGRADE_HOST_PROCEED_TO_COMMIT, message size %d incorrect", msgLen);
        }
        break;
