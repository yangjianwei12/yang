    ...
    if (res->response == avctp_response_stable)
    {
        ...
        if (res->caps_id == avrcp_capability_event_supported)
        {
            ...
            if(avrcp_task->bitfields.local_target_features & AVRCP_CATEGORY_2)
            {
                ...
                {
                    mandatory_data[size_mandatory_data++] = avrcp_event_volume_changed;
                }
            }

            if (avrcp_task->bitfields.local_target_features & (AVRCP_CATEGORY_1 | AVRCP_CATEGORY_3 | AVRCP_CATEGORY_4))
            {
                /* categories 1, 3 and 4 */    
                mandatory_data[size_mandatory_data++] = avrcp_event_playback_status_changed;
                mandatory_data[size_mandatory_data++] = avrcp_event_track_changed;
            }
