...
void aghfpHandleSetRemoteMicrophoneGain(AGHFP *aghfp, uint8 gain)
{
	char buf[3];
        ...
        /* Validate "gain" won't use more than 2 chars before using sprintf */
        if (gain > 15)
        {
            AGHFP_DEBUG_PANIC(("AGHFP: gain too long\n"));
            return;
        }
...
void aghfpHandleSetRemoteSpeakerVolume(AGHFP *aghfp, uint8 volume)
{
	char buf[3];
        ...
        /* Validate "volume" won't use more than 2 chars before using sprintf */
        if(volume > 15)
        {
            AGHFP_DEBUG_PANIC(("AGHFP: volume too long\n"));
            return;
        }
...
void aghfpHandleSendNetworkOperator(AGHFP *aghfp, uint8 mode, uint16 size_operator, uint8* operator)
{
    ...
    /* Validate the length of 'mode' before using sprintf */
    if(mode > 15)
    {
        AGHFP_DEBUG_PANIC(("AGHFP: mode too long\n"));
        return;
    }
