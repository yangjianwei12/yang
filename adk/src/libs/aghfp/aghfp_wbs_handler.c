...
static void aghfpHandleWbsCodecConReqProcessing(AGHFP *aghfp)
{
...
#endif /* DEBUG_TEST_AT_BCC_REJECTION */
	{
		...
		if (1)
#else		
        /* start codec negotiation only if both the NB/WB and the QCE codecs
         * haven't already been negotiated */
        if (aghfp->use_codec == 0 && aghfp->qce_codec_mode_id == CODEC_MODE_ID_UNSUPPORTED)
        ...
        else    /* re-use existing codec negotiation */
		{
