...
{
    /* Send a cfm message to the application with bdaddr. */
    MAKE_HFP_MESSAGE(HFP_AUDIO_DISCONNECT_IND);
    hfpGetLinkBdaddr(link, &message->addr);
    message->priority = hfpGetLinkPriority(link);
    message->status = status;
    MessageSend(theHfp->clientTask, HFP_AUDIO_DISCONNECT_IND, message);
}
