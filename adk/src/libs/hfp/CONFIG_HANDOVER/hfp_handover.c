...
const handover_interface hfp_handover_if =
        MAKE_BREDR_HANDOVER_IF(&hfpVeto,
                               &hfpMarshal,
                               &hfpUnmarshal,
                               &hfpHandoverCommit,
                               &hfpHandoverComplete,
...
static uint8 getRemoteServerChannel(hfp_link_data *link)
{
    ...
    {
        /* Connected */
        channel = SinkGetRfcommDlci(link->identifier.sink);
    }
...
static void hfpHandoverCommit(const tp_bdaddr *tp_bd_addr, const bool newRole)
{
    if (newRole)
    {
        ...
        if (link)
        {
            ...
            uint8 dlci = (uint8)(((uintptr)(link->identifier.sink) & 0xFF));
            sink = StreamRfcommSinkFromDlci(tp_bd_addr, dlci);
            if(sink)
            {
