...
/* 0x4/0x400 are signing keys, not used */
/*! Responder distributes the BREDR Link Key to the initiator */
#define KEY_DIST_RESPONDER_LINK_KEY     (0x0800)
/*! Initiator distributes the BREDR Link Key to the responder */
#define KEY_DIST_INITIATOR_LINK_KEY     (0x0008)
/*! \} */
...
/*!
    \brief Supplies information on what APIs are available and size limitations.
           ...
           A message of type CSR_BT_CM_GET_ADV_SCAN_CAPABILITIES_CFM is returned
...
/*!
    \brief Enable or disable Bluetooth low energy (BLE) extended scanning.
           ...
           A message of type CSR_BT_CM_EXT_SCAN_ENABLE_SCANNERS_CFM is returned to
           the task indicated by the theAppTask parameter.

    \param theAppTask Task to send the resulting confirmation message to.

    \param enable Enable extended scanning if TRUE, otherwise extended scanning is disabled.
    ...
    advertising reports sent to the application by Bluestack.
...
/*!
    \brief HCI Status Confirmation for Ext Advertising requests
*/
typedef struct
{
    hci_status  status;         /*!< Status of the operation */
    uint8       adv_handle;     /*!< Advertisng set handle */
} CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T;

/*!
    \brief Connection Lib Status Confirmation for Ext Advertising requests
*/
typedef struct
{
    connection_lib_status   status;         /*! Status of the operation */
    uint8                   adv_handle;     /*!< Advertisng set handle */
} CL_STANDARD_EXT_ADV_COMMAND_CFM_T;
...
/*!
    \brief Sent in response to ConnectionDmBlePeriodicAdvEnableReq().

    This is a BT5.0+ message.
*/
typedef CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T CL_DM_ULP_PERIODIC_ADV_ENABLE_CFM_T;
...
/*!
    \brief  Read the global parameters to be used when scanning.
            ...
            A message of type CSR_BT_CM_EXT_SCAN_GET_GLOBAL_PARAMS_CFM
...
/* Valid values for Own Address type used in Set Extended Scan Parameter command
 */
#define CL_LOCAL_ADDR_PUBLIC            0x00  /* Public Device Address */
#define CL_LOCAL_ADDR_RANDOM            0x01  /* Random Device Address */
#define CL_LOCAL_ADDR_GENERATE_RPA_FBP  0x02  /* Generate Resolvable Private Address (RPA)
    based on local IRK using resolving list else fallback to Public Address*/
#define CL_LOCAL_ADDR_GENERATE_RPA_FBR  0x03  /* Generate Resolvable Private Address (RPA)
    based on local IRK using resolving list else fallback to Random Address */

/*!
    \brief Set global parameters for Bluetooth low energy (BLE) Extended scanning.

    A message of type CSR_BT_CM_EXT_SCAN_SET_GLOBAL_PARAMS_CFM
    is returned to the task that initialised the Connection library
    when operation has completed. The confirm message contains CsrBtResultCode :
    Success for a valid set of parameters or error codes for various failures.
    ...
    \param own_address_type Type of local address to be used in the scan request packets.
                    CL_LOCAL_ADDR_PUBLIC (0)
                    CL_LOCAL_ADDR_RANDOM (1)
                    CL_LOCAL_ADDR_GENERATE_RPA_FBP (2)
                    CL_LOCAL_ADDR_GENERATE_RPA_FBR (3)
    ...
    \param filter_duplicates Filter duplicates in controller using Advertising 
           Data ID (DID)/ Advertising Set ID (SID) cache.

    \param scanning_phys Bitfield denoting PHYs allowed to be used on the
           primary advertising channel:
           CM_EXT_SCAN_LE_1M_PHY_BIT (Bit 0)       - LE 1m
           CM_EXT_SCAN_LE_2M_PHY_BIT (Bit 1)       - Invalid on primary advertising channel (e.g. LE 2M)
           CM_EXT_SCAN_LE_CODED_PHY_BIT (Bit 2)    - LE Coded
...
/*!
    \brief  Sent in response to defining an advertising SID value that can
            be used by multiple advertising sets with the
            ConnectionDmBleExtScanRegisterScannerReq() function.

            This message is sent to the task specified in the above function.

    This is a BT5.0+ message.
*/
typedef CL_STATUS_STANDARD_COMMAND_CFM_T CL_DM_BLE_EXT_ADV_SET_DEFAULT_PARAMS_CFM_T;
...
/* This must always match DM_ULP_EXT_SCAN_MAX_SCANNERS in dm_prim.h */
#define CL_EXT_SCAN_MAX_SCANNERS 5
...
/*!
    ...
    adv_filter - Used to match types of advertising reports like:
        1. Advertising data that belongs to extended advertising
        2. Advertising data that has the limited discovery flag set.
        etc...
    ad_structure_filter -   Used to drill into the ad_structure data to look for
        ...
        1. Only report the AD Structure data for the ad_types registered
           in reg_ad_types.
    ...
    A message of type CSR_BT_CM_EXT_SCAN_REGISTER_SCANNER_CFM is returned to
    the task indicated by the theAppTask parameter.
...
/*!
    \brief Unregister a scanner.
           ...
           A message of type CSR_BT_CM_EXT_SCAN_UNREGISTER_SCANNER_CFM is
...
/*!
    \brief  Get information on how the LE controller's scanner has been configured.
            ...
            A message of type CSR_BT_CM_EXT_SCAN_GET_CTRL_SCAN_INFO_CFM
            is returned to the task indicated by the theAppTask parameter.
...
/*!
    \brief Reports information about all advertising sets (e.g. advertising/registered).
           ...
           A message of type CSR_BT_CM_EXT_ADV_SETS_INFO_CFM is returned to
           the task indicated by the theAppTask parameter.
...
/*!
    \brief Enable or disable Bluetooth low energy (BLE) Extended advertising.
           ...
           A message of type CSR_BT_CM_EXT_ADV_ENABLE_CFM is returned to
           the task indicated by the theAppTask parameter.
...
/*!
    ...
    This is a BT5.0+ message
 */
typedef CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_EXT_ADVERTISE_ENABLE_CFM_T;
...
/*!
    \brief  Enable advertising for X advertising set. This allows multiple
            ...
            A message of type CSR_BT_CM_EXT_ADV_MULTI_ENABLE_CFM is returned to
            the task indicated by the theAppTask parameter.
...
/*!
    \brief  Set parameters for Bluetooth low energy (BLE) Extended Advertising.
            ...
            A message of type CSR_BT_CM_EXT_ADV_SET_PARAMS_CFM is returned to
            the task indicated by the theAppTask parameter.
    ...
    \param  adv_sid     A value in the range 0 to 15 that can be used by a scanner to identify desired extended advertising from a device (On air will be in the ADI field).
                        ...
                        general_sid value can be changed using DM_ULP_EXT_ADV_SET_DEFAULT_PARAMS_REQ.
...
/*!
    \brief  Set the advertising set's random device address to be used when
            ...
            A message of type CSR_BT_CM_EXT_ADV_SET_RANDOM_ADDR_CFM is
            returned to the task indicated by the theAppTask parameter.
...
/*!
    \brief  Set data for Bluetooth low energy (BLE) Extended Advertising.
            ...
            A message of type CSR_BT_CM_EXT_ADV_SET_DATA_CFM is returned to
            the task indicated by the theAppTask parameter.
...
/*!
    \brief  Set scan response data for Bluetooth low energy (BLE) Extended Advertising.
            ...
            A message of type CSR_BT_CM_EXT_ADV_SET_SCAN_RESP_DATA_CFM is returned to
            the task indicated by the theAppTask parameter.
...
/*!
    ...
    This is a BT5.0+ message.
*/
typedef CL_STANDARD_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_SET_EXT_ADV_DATA_CFM_T;

/*!
    \brief  Read the max allowed advertising data length for an advertising set.

            A message of type CSR_BT_CM_EXT_ADV_READ_MAX_ADV_DATA_LEN_CFM is
            returned to the task indicated by the theAppTask parameter.
...
typedef struct
{
    ...
    uint8                   adv_handle;             /*! Advertising set handle. */
} CL_DM_BLE_EXT_ADV_READ_MAX_ADV_DATA_LEN_CFM_T;

/*!
    \brief  Register an application for use of an advertising set.

            A message of type CSR_BT_CM_EXT_ADV_REGISTER_APP_ADV_SET_CFM is returned to
            the task indicated by the theAppTask parameter.
...
/*!
    ...
    This is a BT5.0+ message
 */
typedef CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_EXT_ADV_REGISTER_APP_ADV_SET_CFM_T;

/*!
    \brief  Allows an application to unregister use of an advertising set.

            A message of type CSR_BT_CM_EXT_ADV_UNREGISTER_APP_ADV_SET_CFM is returned to
            the task indicated by the theAppTask parameter.
...
/*!
    ...
    This is a BT5.0+ message
 */
typedef CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_EXT_ADV_UNREGISTER_APP_ADV_SET_CFM_T;
...
/*!
    ...
    This is a BT5.0+ message.
*/
typedef CL_STANDARD_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_EXT_ADV_SET_SCAN_RESPONSE_DATA_CFM_T;

/*
    \brief BLE Extended Advertising Reports received that meet the criteria
    set by the BLE Extended Advertising Filters (ConnectionBleAddExtAdvertisingReportFilter(),
    ConnectionBleClearExtAdvertisingReportFilter()), when extended scanning has been
    enabled using ConnectionBleSetExtScanEnableReq().
...
typedef struct
{
    ...
    typed_bdaddr                    direct_taddr;
    uint8                           data_length;
    uint8                           data[1];
} CL_DM_BLE_EXT_SCAN_ADVERTISING_REPORT_IND_T;
...
/*!
    ...
    This is a BT5.0+ message.
*/
typedef CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_PER_ADV_SET_PARAMS_CFM_T;
...
/*!
    ...
    This is a BT5.0+ message.
*/
typedef CL_STANDARD_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_PER_ADV_SET_DATA_CFM_T;
...
/*!
    ...
    A message of type CSR_BT_CM_PERIODIC_SCAN_SYNC_TO_TRAIN_CFM is returned to
    the task indicated by the theAppTask parameter.
...
/*!
    ...
    A message of type CSR_BT_CM_PERIODIC_SCAN_SYNC_TO_TRAIN_CANCEL_CFM is returned to
    the task indicated by the theAppTask parameter.
...
/*!
    ...
    A message of type CSR_BT_CM_PERIODIC_SCAN_SYNC_TERMINATE_CFM is returned to
    the task indicated by the theAppTask parameter.
...
/*!
    ...
    related to this periodic advertising train to a connected peer.

    A message of type CSR_BT_CM_PERIODIC_SCAN_SYNC_TRANSFER_CFM is returned to
    the task indicated by the theAppTask parameter.
    ...
    \param  sync_handle     A periodic advertising train currently controller is synchronized.
...
/*!
    ...
    A message of type CSR_BT_CM_PERIODIC_SCAN_SYNC_TRANSFER_PARAMS_CFM is returned to
    the task indicated by the theAppTask parameter.
...
/*!
    \brief  Set parameters for Bluetooth low energy (BLE) Periodic Advertising.
            ...
            A message of type CSR_BT_CM_PERIODIC_ADV_SET_PARAMS_CFM is returned to
            the task indicated by the theAppTask parameter.
...
/*!
    \brief  Set data for Bluetooth low energy (BLE) Periodic Advertising.
            ...
            A message of type CSR_BT_CM_PERIODIC_ADV_SET_DATA_CFM is returned to
            the task indicated by the theAppTask parameter.
...
/*!
    \brief  Starts a periodic advertising train.
            ...
            A message of type CSR_BT_CM_PERIODIC_ADV_START_CFM is returned to
            the task indicated by the theAppTask parameter.
...
/*!
    ...
    This is a BT5.0+ message.
*/
typedef CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_PER_ADV_START_CFM_T;

/* Valid values for stop_advertising field */
#define CL_PERIODIC_ADV_STOP_PA         0  /* Stop periodic advertising only */
#define CL_PERIODIC_ADV_STOP_EA_THEN_PA 1  /* Stop periodic advertising and associated
                                              extended advertising */
#define CL_PERIODIC_ADV_STOP_EA         2  /* Stop extended advertising only */

/*!
    \brief  Stops a periodic advertising train or just the associated extended advertising.

            A message of type CSR_BT_CM_PERIODIC_ADV_STOP_CFM is returned to
            the task indicated by the theAppTask parameter.
    ...
    \param  stop_advertising    This field allows the stopping of periodic
                                ...
                                CL_PERIODIC_ADV_STOP_PA - Stop periodic advertising.
                                CL_PERIODIC_ADV_STOP_EA_THEN_PA - Stop extended advertising and 
                                    then stop periodic advertising.
                                CL_PERIODIC_ADV_STOP_EA - Stop extended advertising.
...
/*!
    ...
    This is a BT5.0+ message.
*/
typedef CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_PER_ADV_STOP_CFM_T;
...
/*!
    \brief  Instructs the Controller to communicate sync info for an advertising train
            ...
            A message of type CSR_BT_CM_PERIODIC_ADV_SET_TRANSFER_CFM is returned to
            the task indicated by the theAppTask parameter.
    ...
    \param  adv_handle      Identifies the advertising set to local Controller
...
/*!
    \brief  Read the max allowed periodic advertising data length for an
            ...
            A message of type CSR_BT_CM_PERIODIC_ADV_READ_MAX_ADV_DATA_LEN_CFM is
            returned to the task indicated by the theAppTask parameter.
...
/*!
    \brief  Search for periodic trains that meet a specified ad_structure filter.
            ...
            A message of type CSR_BT_CM_PERIODIC_SCAN_START_FIND_TRAINS_CFM is
            returned to the task indicated by the theAppTask parameter.
...
/*!
    \brief  Stop scanning for periodic trains.
            ...
            A message of type CSR_BT_CM_PERIODIC_SCAN_STOP_FIND_TRAINS_CFM is
            returned to the task indicated by the theAppTask parameter.
...
/*!
    \brief  Sets whether a DM_ULP_PERIODIC_SCAN_SYNC_ADV_REPORT_IND should be
            ...
            A message of type CSR_BT_CM_PERIODIC_SCAN_SYNC_ADV_REPORT_ENABLE_CFM
            is returned to the task indicated by the theAppTask parameter.
...
/*!
    @brief Request to enable Periodic Advertising.

    @param theAppTask  The client task.

    @param adv_handle  a registered advertising set value.

    @param flags    Flags field (bit defines below)
                    ...
                    advertising
    @param  enable  Enable field (bit defines below)
...
{
    hci_status status;       /*!> Status of the operation */
} CL_DM_BLE_SET_EVENT_MASK_CFM_T;

typedef CL_HCI_STATUS_EXT_ADV_COMMAND_CFM_T CL_DM_BLE_SET_DATA_RELATED_ADDRESS_CHANGES_CFM_T; 
