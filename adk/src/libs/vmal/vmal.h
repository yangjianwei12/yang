...
typedef struct
{
    void (*TurnedFrameworkOn)(bool exited_lp_mode);
    void (*TurningFrameworkOff)(bool entering_lp_mode);
} vmal_callbacks_t;

/*!
\brief  Observer registration structure
*/
typedef struct
{
    /*! observers will be informed about events via callbacks */
    const vmal_callbacks_t *callbacks;
} vmal_registry_per_observer_t;
...
/*!
    \brief Enable the main processor
*/
void VmalOperatorFrameworkEnableMainProcessor(void);

/*!
    \brief Enable the main processor asynchronously.
           As VmalOperatorFrameworkEnableMainProcessor(), but returns immediately. Once the
           operator framework state has changed, it returns a message to the specified task.
           This call is provided so that other functionality can be performed while the operator framework is loading
    \return TRUE: Framework is already on, FALSE: Framework is off
*/
bool VmalOperatorFrameworkEnableMainProcessorAsync(void);

/*!
    \brief Disable the main processor
*/
void VmalOperatorFrameworkDisableMainProcessor(void);

/*!
    \brief Enable the second processor
*/
void VmalOperatorFrameworkEnableSecondProcessor(void);

/*!
    \brief Disable the second processor
*/
...
/*! \brief Register vmal observers at initialzation time.
    \param info->observer: observer that register
...
/*! \brief Trigger Notification Start
...
/*! \brief Trigger Notification Stop
 */
void VmalOperatorFrameworkTriggerNotificationStop(void);

/*! \brief Check if the respective processor is enabled
    \param processor_id: id less than #max_number_processors
