...
#define STREAM_CODEC_DAC_ULTRA_QUIET_MODE (0x0320U)
#define ENABLE_DAC_ULTRA_QUIET_MODE       (0x1U)
        Sink dac_sink = StreamAudioSink(AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
        if(dac_sink)
        {
            PanicFalse(SinkConfigure(dac_sink, STREAM_CODEC_DAC_ULTRA_QUIET_MODE, ENABLE_DAC_ULTRA_QUIET_MODE));
            return TRUE;
        }
#endif		
    return FALSE;
...
#ifdef AUDIO_OUTPUT_ULTRA_QUIET_DAC	
#define STREAM_CODEC_DAC_ULTRA_QUIET_MODE (0x0320U)
#define DISABLE_DAC_ULTRA_QUIET_MODE      (0x0U)
    PRINT(("AudioOutputDisableDacUltraQuietMode"));

    Sink dac_sink = StreamAudioSink(AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
    if(dac_sink)
    {
        PanicFalse(SinkConfigure(dac_sink, STREAM_CODEC_DAC_ULTRA_QUIET_MODE, DISABLE_DAC_ULTRA_QUIET_MODE));
...
bool AudioOutputEnableSilenceDetection(void)
{
#ifdef AUDIO_OUTPUT_ULTRA_QUIET_DAC
#define STREAM_CODEC_SILENCE_DETECT_DAC_QUIET_MODE (0x1209U)
#define ENABLE_SILENCE_DETECTION                   (0x0U)

    PRINT("AudioOutputEnableSilenceDetection");
    Sink dac_sink = StreamAudioSink(AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
    if(dac_sink)
    {
        if(SinkConfigure(dac_sink,STREAM_CODEC_SILENCE_DETECT_DAC_QUIET_MODE, ENABLE_SILENCE_DETECTION))
        {
            return TRUE;
        }
    }
...
bool AudioOutputDisableSilenceDetection(void)
{
#ifdef AUDIO_OUTPUT_ULTRA_QUIET_DAC
#define STREAM_CODEC_SILENCE_DETECT_DAC_QUIET_MODE (0x1209U)
#define DISABLE_SILENCE_DETECTION                  (0x1U)

    PRINT("AudioOutputDisableSilenceDetection");
    Sink dac_sink = StreamAudioSink(AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
    if(dac_sink)
    {
        if(SinkConfigure(dac_sink,STREAM_CODEC_SILENCE_DETECT_DAC_QUIET_MODE, DISABLE_SILENCE_DETECTION))
        {
            return TRUE;
        }
    }
