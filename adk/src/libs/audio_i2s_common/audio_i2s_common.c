/****************************************************************************
Copyright (c) 2013 - 2022 Qualcomm Technologies International, Ltd.
...
RETURNS:
    Nothing.
*/
void AudioI2SCommonConfigureSink(Sink sink, uint32 rate)
{
    ...
    AudioI2SMclkConfigureMasterClockIfRequired(sink, rate);

    PRINT(("I2S: AudioI2SCommonConfigureSink A&B, %luHz\n", (unsigned long) rate));
}
...
RETURNS:
    none
*/
void AudioI2SCommonConfigureSource(Source source, uint32 rate, uint16 bit_resolution)
{
   ...
   PanicFalse(SourceConfigure(source, STREAM_AUDIO_SAMPLE_SIZE, I2S_audio_sample_size));
    ...
    /* configure MCLK for the interface */
    AudioI2SMclkSourceConfigureMasterClockIfRequired(source, rate);
}
...
*/
bool AudioI2SCommonMasterIsEnabled(void)
{
...
*/
bool AudioI2SCommonIsMasterClockRequired(void)
{
...
PARAMETERS:
    none
*/
const i2s_config_t* AudioI2SCommonGetConfig(void) {
    return I2S_config;
}
...
RETURNS:
    none
*/
void AudioI2SCommonSetConfig(const i2s_config_t* config) {
    if (config) {
        I2S_config = config;
    }
}
