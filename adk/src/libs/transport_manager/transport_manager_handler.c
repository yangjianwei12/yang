...
#ifdef SYNERGY_FOR_VM
static void transportMgrHandleConnectInd(TRANSPORT_CONNECT_IND_T * msg)
{
    transport_mgr_link_data_t *trans_data = NULL;
    transport_mgr_type_t transport_type = transport_mgr_type_none;

    PRINT(("transportMgrHandleConnectInd\n"));
    ...
    if(transport_type != transport_mgr_type_gatt)
    {
        ...
        if (trans_data->sink)
        {
            /* Sink is not zero, so transport is already connected, reject this connection */
#ifdef SYNERGY_FOR_VM
            TransportConnectResponse(msg->transport, &msg->addr, msg->transport_id, msg->sink, FALSE);
#else
            ConnectionRfcommConnectResponse(task, FALSE,
                                            msg->sink, msg->transport_id,
                                            0);
#endif
        }
        else
        {
            trans_data->sink = msg->sink;
#ifdef SYNERGY_FOR_VM
            trans_data->btConnId = msg->btConnId;
            TransportConnectResponse(msg->transport, &msg->addr, msg->transport_id, msg->sink, TRUE);
#else
...
#ifdef SYNERGY_FOR_VM
                /* Update the btConnId in the data block*/
                transportMgrUpdateBtConnId(msg->transport_id, msg->btConnId, transport_type);
#endif
...
#ifdef SYNERGY_FOR_VM
            else if (msg->btConnId == trans_data->btConnId)
#else 
            else if (msg->sink == trans_data->sink)
#endif
            {
                /* Connection failed, so clear sink to indicate not connected */
                trans_data->sink = 0;
            }
...
static void transportMgrHandleDisconnectCfm(TRANSPORT_DISCONNECT_CFM_T * msg)
{
    ...
    PRINT(("transportMgrHandleDisconnectCfm\n"));
    {
        ...
        {
#ifdef SYNERGY_FOR_VM
            transport_link_id = transportMgrFindTranslinkIdFromBtConnId(msg->btConnId);
#else
            transport_link_id = transportMgrFindTranslinkIdFromSink(msg->sink);
#endif

            if (transport_link_id != INVALID_TRANS_LINK_ID)
            {
                trans_data = PanicNull(transportMgrFindDataWithTransInfo(transport_link_id,transport_type));
#ifdef SYNERGY_FOR_VM
                message->trans_sink = trans_data->sink;
#else
                message->trans_sink = msg->sink;
#endif
                ...
                /* Zero sink indicates transport is not connected */
                trans_data->sink = 0;
            }
            else
                DEBUG_LOG_ERROR("transportMgrHandleDisconnectCfm, transport not found for sink %u", msg->sink);
        }
        else
        {
            UNUSED(trans_data);
...
bool TransportMgrCreateTransLinkData(Task application_task, transport_mgr_link_cfg_t link_cfg, Sink snk)
{
    ...
    if (trans_data)
    {
        ...
        trans_data->sink = snk;
#ifdef SYNERGY_FOR_VM
        trans_data->btConnId = CM_CREATE_RFC_CONN_ID(SinkGetRfcommConnId(snk));
#endif
...
#ifdef SYNERGY_FOR_VM
        trans_data_new->btConnId = CM_CREATE_RFC_CONN_ID(SinkGetRfcommConnId(snk));
#endif
...
static void transportMgrHandleInternalDisConnectReq(TRANSPORT_MGR_INTERNAL_DISCONNECT_REQ_T *msg)
{
    ...
    {
#ifdef SYNERGY_FOR_VM
        /* No need to send disconnect with failure. It can fail if there's a disconnect crossover
         * in which case disconnect processing will be handled in gaiaFrameworkInternal_HandleDisconnectInd */
        PRINT(("transportMgrHandleInternalDisConnectReq - Disconnect Requests Crossover!! \n"));
#else
        MAKE_TRANS_MGR_MESSAGE(TRANSPORT_MGR_LINK_DISCONNECTED_CFM,message);
...
#ifdef SYNERGY_FOR_VM
        case TRANSPORT_CONNECT_IND:
        {
            PRINT(("TRANSPORT_CONNECT_IND\n"));
            transportMgrHandleConnectInd((TRANSPORT_CONNECT_IND_T *) message);
        }
        break;
#endif
