...
/* Enforce only processing a single report at a time */
static bool processing_adv_reports = FALSE;
...
void connectionHandleDmBleExtScanFilteredAdvReportInd(const DM_ULP_EXT_SCAN_FILTERED_ADV_REPORT_IND_T *ind)
{
    ...
    if (processing_adv_reports)
    {
        return;
    }
    processing_adv_reports = TRUE;
    ...
    if (arr_idx)
    {
        ...
        message->num_of_scan_handles = ind->num_of_scan_handles;

        /* Copy the full array, not just the num_of_scan_handles, so output is consistent
         * when comparing P0 and P1 testing.
         */
        memmove(message->scan_handles, ind->scan_handles, CL_EXT_SCAN_MAX_SCANNERS * sizeof(uint8));
