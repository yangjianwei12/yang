...
typedef struct _content_t
{
    ...
    unsigned    privacy_mode:1;     /* default Network (0), or Device (1). */
    unsigned    unused:15;          /* Should be 'spare' */
} content_t;
         ...
         i++)
    {
        ...
        else
        {
            ...
            for (j = 0; j < max_trusted_devices; j++)
            {
                ...
                PsStore(PSKEY_TDL_ATTRIBUTE_BASE + j, NULL, 0);
                /* Delete any associated GATT attribute data */
                PsStore(PSKEY_TDL_GATT_ATTRIBUTE_BASE + j, NULL, 0);
            }
    ...
    )
{
     ...
     *
     * By default: td->content.privacy mode is 0 (network privacy mode).
     */
    if (ble_key_present)
    {
...
void connectionAddDeviceAtTdlPosition(uint8 pos)
{
    ...
    PanicZero((uint32)PsRetrieve(TRUSTED_DEVICE_LIST + pos, td, PS_SIZE_ADJ(SIZE_TD_DATA_T)));
    dm_sm_add_device_req(td);

#ifndef DISABLE_BLE
    /* USPSCRUM-2204: If the device has been set to use Device Privacy Mode
     * previously, set it again in the SM.
     * NOTE:
     * - CFM message will go to task that initialised the CL.
     * - Devices in SM are in Network mode by default.
     * - Only appropriate for LE IRK (ENC_CENTRAL) link key paired devices.
     */
    if (td->content.data_flags & MASK_ENC_CENTRAL &&
        td->content.privacy_mode == privacy_mode_device)
    {
        bdaddr tempAddr;
        ...
        prim->privacy_mode = (uint8) privacy_mode_device;
...
/****************************************************************************
NAME
    ConnectionAuthSetPrivacyMode

FUNCTION
    This function is called to set the privacy mode flag of a device
    stored in the trusted device list.

    Privacy mode is only appropriate for LE bonded devices with the IRK link
    key (ENC_CENTRAL).

RETURNS
    bool - TRUE if the Privacy mode has been set to the required mode.
*/
bool ConnectionAuthSetBlePrivacyMode(
        const typed_bdaddr* taddr,
        const privacy_mode  mode
        )
{
    bool        privacy_mode_stored = FALSE;
    uint16      pos;
    uint16      idx;
    td_data_t   *td = tdl_find_device(taddr->type, &taddr->addr, &pos, &idx);

    /* Device Privacy is only relevant if the IRK (ENC_CENTRAL) key is distributed */
    if ((td != NULL) && (td->content.data_flags & MASK_ENC_CENTRAL))
    {
        /* If the privacy mode in TDL is already set to the mode indicated, then
         * no need to update the TDL.
         */
        if ( !((td->content.privacy_mode == 0 && mode == privacy_mode_network) ||
               (td->content.privacy_mode == 1 && mode == privacy_mode_device)) )
        {
            td->content.privacy_mode = (mode == privacy_mode_device) ? 1 : 0;
            PsStore(TRUSTED_DEVICE_LIST + pos, td, PS_SIZE_ADJ(CALC_TD_SIZE(td)));
        }
        privacy_mode_stored = TRUE;
    }
    free(td);
    return privacy_mode_stored;
}

/****************************************************************************
NAME
    ConnectionAuthGetPrivacyMode

FUNCTION
    Privacy mode is only appropriate for LE bonded devices with the IRK link
    key (ENC_CENTRAL). This function returns the Privacy Mode that has been
    stored for the identified device in the TDL.

RETURNS
    bool - TRUE if the device identified by its taddr was found in the TDL,
    otherwise False
*/
        ...
        )
{
    ...
    if ((td != NULL))
    {
        *mode = (td->content.privacy_mode) ? privacy_mode_device : privacy_mode_network;
        device_found = TRUE;
    }
    free(td);
    return device_found;
