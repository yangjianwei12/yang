        ...
        )
{
    ...
    if (!state->dmExtAdvLock)
    {
        ...
        /* One request at a time, set the ad lock. */
        state->dmExtAdvLock = req->theAppTask;
        state->adv_handle = req->adv_handle;
        ...
        )
{
    if (state->dmExtAdvLock)
    {
        ...
        message->status = connectionConvertHciStatus(cfm->status);
        message->adv_handle = state->adv_handle;
        MessageSend(
                    state->dmExtAdvLock,
    ...
    /* Reset the ad lock. */
    state->dmExtAdvLock = NULL;
    state->adv_handle = ADV_HANDLE_INVALID;
}
...
/****************************************************************************
NAME
    connectionHandleDmBleSetExtAdvertiseEnableReq
...
DESCRIPTION
    Handles ble perodic advertising enable Request

RETURNS
    void
*/
void connectionHandleDmBlePeriodicAdvEnableReq(
          connectionDmPerAdvState *state,
          const CL_INTERNAL_DM_ULP_PERIODIC_ADV_ENABLE_REQ_T *req)
{
    if (!state->dmPerAdvLock)
    {
        ...
        MAKE_PRIM_T(DM_ULP_PERIODIC_ADV_ENABLE_REQ);
    ...
    {
        /* Periodic Adv request already outstanding, queue up this request. */
		MAKE_CL_MESSAGE(CL_INTERNAL_DM_ULP_PERIODIC_ADV_ENABLE_REQ);
        COPY_CL_MESSAGE(req, message);
        MessageSendConditionallyOnTask(
                    connectionGetCmTask(),
...
/****************************************************************************
NAME
    ConnectionDmBlePeriodicAdvEnableReq

DESCRIPTION
     ble perodic advertising enable Request
...
DESCRIPTION
     Confirmation of ble perodic advertising enable Request

RETURNS
    void
*/
void connectionHandleDmBlePeriodicAdvEnableCfm(
        connectionDmPerAdvState *state, 
        const DM_ULP_PERIODIC_ADV_ENABLE_CFM_T *cfm)
{
    if (state->dmPerAdvLock)
    {
        MAKE_CL_MESSAGE(CL_DM_ULP_PERIODIC_ADV_ENABLE_CFM);
...
/****************************************************************************
NAME
    ConnectionDmBleSetDataRelatedAddressChangesReq
    

DESCRIPTION
    Change when/if the RPA changes if Advertsing or Scan Response data changes.
        ...
        )
{
    ...
    MAKE_CL_MESSAGE(CL_INTERNAL_DM_BLE_SET_DATA_RELATED_ADDRESS_CHANGES_REQ);
    message->theAppTask = theAppTask;
    message->adv_handle = adv_handle;
    message->flags = flags;
    message->change_reasons = (uint8)change_reasons;
    MessageSend(
                connectionGetCmTask(),
...
DESCRIPTION
    This function will initiate the Set Data Related Address Change request.

RETURNS
    void
*/
void connectionHandleDmBleSetDataRelatedAddressChangesReq(
        connectionDmExtAdvState *state,
        const CL_INTERNAL_DM_BLE_SET_DATA_RELATED_ADDRESS_CHANGES_REQ_T *req
        )
{
    /* Check the state of the task lock before doing anything. */
    if (!state->dmExtAdvLock)
    {
        MAKE_PRIM_T(DM_ULP_SET_DATA_RELATED_ADDRESS_CHANGES_REQ);
        ...
        prim->adv_handle        = req->adv_handle;
        prim->flags             = req->flags;
        prim->change_reasons    = req->change_reasons;
...
/****************************************************************************
NAME
    connectionHandleDmUlpSetDataRelatedAddressChangesCfm

DESCRIPTION
    Handle the DM_ULP_SET_DATA_RELATED_ADDRESS_CHANGES_CFM message from 
    Bluestack.

RETURNS
   void
*/
void connectionHandleDmUlpSetDataRelatedAddressChangeCfm(
        connectionDmExtAdvState *state,
        const DM_ULP_SET_DATA_RELATED_ADDRESS_CHANGES_CFM_T *cfm
        )
{
    if (state->dmExtAdvLock)
    {
        MAKE_CL_MESSAGE(CL_DM_BLE_SET_DATA_RELATED_ADDRESS_CHANGES_CFM);
        message->status = connectionConvertHciStatus(cfm->status);
        message->adv_handle = state->adv_handle;
        MessageSend(
                    state->dmExtAdvLock,
