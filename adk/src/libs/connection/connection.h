...
/*!
    \brief ble perodic advertising enable Request
    ...
    \param enable
        Enable field (bit defines below)
	...
	\return A message of type #CL_DM_ULP_PERIODIC_ADV_ENABLE_CFM is sent to
    the task indicated by the theAppTask parameter.
*/
...
/*!
    ...
    device. if peer_tpaddr is NULL, read the current random address, else if
    peer_tpaddr is set, read this device's RPA that was used to connect with
    the peer.
...
/*!
    \brief Set privacy mode for a given LE connection. If the remote device
    is paired, the privacy mode will be stored with this device's entry in 
...
/*!
    \brief Get the device privacy mode, stored for the device identified by
    its Bluetooth address, in the Trusted Device List (paired devices).

    \param taddr The remote device address.
    \param mode Will contain the privacy mode value when the function returns.

    \returns TRUE if the device was found in the Trusted Devices list, 
    otherwise FALSE.
 */
bool ConnectionAuthGetBlePrivacyMode(
        const typed_bdaddr* taddr,
        privacy_mode* const mode
        );
...
/* This must walways match DM_ULP_EXT_SCAN_MAX_SCANNERS in dm_prim.h */
...
/*!
    \brief request for read the current and maximum transmit
             power levels of the local Controller

           A message of type CL_ULP_ENHANCED_READ_TRANSMIT_POWER_LEVEL_CFM is returned to
           the task indicated by the theAppTask parameter.

    \param theAppTask Task to send the resulting confirmation message to.
    \param tp_addr  Bluetooth address of the intended device
    \param phy    0x01 = LE 1M PHY
                  0x02 = LE 2M PHY
                  0x03 = LE Coded PHY with S=8 data coding
                  0x04 = LE Coded PHY with S=2 data codin
...
/*!
    \brief  request for read the transmit power level used by the remote Controller

           A message of type CL_ULP_READ_REMOTE_TRANSMIT_POWER_LEVEL_CFM is returned to
           the task indicated by the theAppTask parameter.
...
/*!
    \brief Internal request for set the path loss threshold  reporting parameters.


           A message of type CL_ULP_SET_PATH_LOSS_REPORTING_PARAMETERS_CFM is returned to
           the task indicated by the theAppTask parameter.

    \param theAppTask Task to send the resulting confirmation message to.
    \param tp_addr  Bluetooth address of the intended device
    \param High threshold for path loss
    \param Hysteresis value for the high threshold
    \param Low threshold for path loss
    \param Hysteresis value for the low threshold
    \param Minimum time in number of connection events
...
/*!
    \brief request to enable or disable path loss reporting

           A message of type CL_ULP_SET_PATH_LOSS_REPORTING_ENABLE_CFM is returned to
           the task indicated by the theAppTask parameter.
    ...
    \param enable If the Enable parameter is set to 0x01 and no prior LE Power Control Request
...
/*!
    \brief request to enable transmit power reporting

           A message of type CL_ULP_SET_TRANSMIT_POWER_REPORTING_ENABLE_CFM_T is returned to
           the task indicated by the theAppTask parameter.
    ...
    \param  local enable  0x00 Disable local transmit power reports
                          0x01 Enable local transmit power report

    \param remote_enable  0x00 Disable remote transmit power reports
...
/* Advertsing Set ID Values */
#define CL_EXT_ADV_SID_INVALID              0x100
#define CL_EXT_ADV_SID_SHARE                0x200
#define CL_EXT_ADV_SID_ASSIGNED_BY_STACK    0x400
...
/*!
    ...
    \param  adv_sid             Advertsing set ID, can be set to:
                                CL_EXT_ADV_SID_INVALID            - For legacy advertising
                                CL_EXT_ADV_SID_ASSIGNED_BY_STACK  - Stack will assign unique value
                                0 to 15                           - Application decides unique value
                                CL_EXT_ADV_SID_SHARE + (0 to 15)  - More than 1 advertising set can have this value
...
typedef struct
{
    uint8           num_of_scan_handles;
    uint8           scan_handles[CL_EXT_SCAN_MAX_SCANNERS];
    uint16          event_type;
    uint16          primary_phy;
...
/*!
    \brief  Sent in response to a reading the current and maximum transmit
     power levels of the local Controller using the func()
            function.
...
typedef struct
{
    ...
    hci_status   status;
    uint8 phy;
    int8 current_tx_pwr_level;
    int8 max_tx_pwr_level;
} CL_ULP_ENHANCED_READ_TRANSMIT_POWER_LEVEL_CFM_T;


/*!
    \brief  Sent in response to read the transmit power level used by
     the remote Controller using the func() function.
...
typedef struct
{
    ...
    uint8 phy;
    int8 tx_pwr_level;
    uint8 tx_pwr_level_flag;
    int8 delta;
    uint8 reason;
}CL_ULP_READ_REMOTE_TRANSMIT_POWER_LEVEL_CFM_T;


/*!
    \brief  Sent in response to set the path loss threshold reporting parameters using
     the func() function.
...
typedef struct
{
    ...
    typed_bdaddr            taddr;
    hci_status   status;
}CL_ULP_SET_PATH_LOSS_REPORTING_PARAMETERS_CFM_T;


/*!
    \brief  Sent in response to enabling or disabling path loss reporting using
            the func()function.
...
typedef struct
{
    ...
    typed_bdaddr            taddr;
    hci_status   status;
}CL_ULP_SET_PATH_LOSS_REPORTING_ENABLE_CFM_T;


/*!
    \brief  Sent in response to enabling or disabling the reporting to the local Host
     of transmit power level changes in the local and remote Controllers using
            the function.
...
typedef struct
{
    ...
    typed_bdaddr            taddr;
    hci_status   status;
}CL_ULP_SET_TRANSMIT_POWER_REPORTING_ENABLE_CFM_T;

/*!
    \brief  This prim will be send once an periodic HCI_LE_Path_Loss_Threshold
     event is generated.
...
typedef struct
{
    ...
    typed_bdaddr  taddr;
    uint8         curr_path_loss; /* Indicates the current path loss value as
                                    calculated by the Controller */
    uint8         zone_entered; /* Parameter indicates which zone was entered */
}CL_ULP_PATH_LOSS_THRESHOLD_IND_T;


/*!
    \brief  This prim will be send once an periodic
            HCI_LE_Transmit_Power_Reporting event is generated.
...
{
    typed_bdaddr  taddr;
    uint8         reason; /* Reason for power change */
    uint8         phy; /* type of phy, will indicate whether the
                          link is on 1M, 2M or Coded PHY type*/
    int8          tx_pwr_level;   /* Transmit power level */
    uint8         tx_pwr_level_flag;
    int8 delta;   /* Transmit power level at Min/Max */
                  /* Change in transmit power level (zero means unchanged)*/
}CL_ULP_TRANSMIT_POWER_REPORTING_IND_T;


/*! 
    \brief Enum for 'change reasons' bitfield, used by the Data Related Address
    change function / confirmation.

    bits 0-1:
       Says if the advertiser's address will change when using controller based 
       RPA generation and data changes:
       0 = No address change
...
typedef enum 
{
    /*! 0 = No address change reason. */
    cl_ble_rpa_no_change                           = 0x00,
    /*! 1 = Address change if advertising data changes. */
    cl_ble_rpa_advertising                         = 0x01,
    /*! 2 = Address change if scan response data changes. */
    cl_ble_rpa_scan_response                       = 0x02,
    /*! 3 = Address change if advertising or scan response data changes. */
    cl_ble_rpa_advertising_or_scan_response        = 0x03
} cl_ble_rpa_change_reasons;

/*!
    \brief Used to control the Resolvable Private Address (RPA) changes when
    advertising and / or scan response data is changed. 
    
    This is configurable per advertising set (adv_handle).

    \param  theAppTask Task to send the resulting confirmation message to.

    \param  adv_handle A registered adv_handle.

    \param  flags Reserved - always set to 0.

    \param  change_reasons Reason to trigger changing the RPA.

    \return A message of type #CL_DM_BLE_SET_DATA_RELATED_ADDRESS_CHANGES_CFM
    is sent to theAppTask in response.
*/
void ConnectionDmBleSetDataRelatedAddressChangesReq(
        Task                        theAppTask,
        uint8                       adv_handle,
        uint8                       flags,
        cl_ble_rpa_change_reasons   change_reasons
        );

/*!
    \Brief Sent in response to ConnectionDmBleSetDataRelatedAddressChangesReq()
    to confirm the status of that request.
*/
