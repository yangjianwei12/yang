...
#ifdef SYNERGY_FOR_VM
/*!
@brief The Transport Connect indication message.
*/
typedef struct
{
    /*! Transport type chosen. */
...
#ifdef SYNERGY_FOR_VM
/*! @brief Associate task with connection id inside synergy connection manager.
...
/*! @brief Sends connect response to the remote device with
           the specified bluetooth address and transport id.

    @param transport Type of underlying transport
    @param tpaddr Typed Bluetooth address with the Transport type
    @param transport_id channel id of the transport
    @param sink Sink allocated for the transport
    @param response TRUE if connection request accepted otherwise FALSE
