...
{
    uint16 id;
    uint16 capture_interval_ms;
} eft_set_capture_param_t;
...
typedef struct
{
    ...
    uint16 sample_rate;
    uint16 channels;
} aptxad_r3_encoder_config_params_msg_t;

typedef aptxad_96K_encoder_rf_signal_params_msg_t aptxad_r3_encoder_rf_signal_params_msg_t;
...
{
    uint16 id;
    int16 gains[ahm_static_gain_max];
...
{
    uint16 id;
    uint16 current_noise_id;
} noise_id_set_current_noise_id_msg_t;
...
{
    uint16 id;
    uint16 makeup_gain_msw;
    uint16 makeup_gain_lsw;
} compander_set_makeup_gain_msg_t;
...
typedef struct
{
    uint16 msg_id;
    uint16 endpoint;
}lc3_dec_sco_iso_id_connect_sco_endpoint_t;
...
typedef struct
{
    uint16 msg_id;
    uint16 num_of_channels;
}lc3_enc_sco_iso_set_num_of_channels_t;
...
typedef struct
{
    uint16 msg_id;
    uint16 endpoint;
}lc3_enc_sco_iso_id_connect_sco_endpoint_t;

typedef struct
{
    uint16 msg_id;
    uint16 frame_duration;
}aptx_lite_dec_sco_iso_frame_duration_t;
...
typedef struct
{
    ...
    uint16 channel_mode;
    uint16 frame_duration;
} aptx_lite_enc_sco_iso_encoding_param_t;

typedef struct
{
    uint16 msg_id;
    uint16 qhs_level;
} aptx_adaptive_enc_sco_iso_set_qhs_level_t;
...
typedef struct
{
    ...
    uint16 profile;
    uint16 packet_size;
} aptx_adaptive_test_sco_iso_encoding_param_t;

typedef struct
{
    uint16 msg_id;
    uint16 frame_duration;
}aptx_adaptive_dec_sco_iso_frame_duration_t;

typedef struct
{
    uint16 msg_id;
    uint16 predecode_duration;
} aptx_adaptive_dec_sco_iso_predecode_duration_t;
...
{
    /* Sample rates are in 25 hz unit */
...
{
    aptxad_r3_encoder_config_params_msg_t msg;

    PanicNull(params);

    msg.id          = APTXAD_R3_ENCODER_CONFIG_PARAMS;
    msg.sample_rate = getSampleRateUnitsForAudioSubsystem(params->sample_rate);
    msg.channels    = (uint16)params->channels;
...
void OperatorsAptxAdR3EncoderRfSignalParams(Operator op, aptxad_r3_encoder_rf_signal_params_t *params)
{
    ...
    msg.id      = APTXAD_R3_ENCODER_RF_SIGNAL_PARAMS;
    msg.rssi    = (uint16)params->rssi;
    msg.cqddr   = (uint16)params->cqddr;
    msg.quality = (uint16)params->quality;
...
{
    OperatorsStandardSetControl(cvc_snd_op, cvc_send_omni_mode_control, 1);
}

void OperatorsCvcSendDisableOmniMode(Operator cvc_snd_op)
{
    OperatorsStandardSetControl(cvc_snd_op, cvc_send_omni_mode_control, 0);
}
...
{
    lc3_dec_sco_iso_id_connect_sco_endpoint_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_DEC_SCO_ISO_ID_CONNECT_SCO_ENDPOINT;
    msg.endpoint = endpoint; /* 1= SCO, 0 = ISO */
    PanicFalse(VmalOperatorMessage(op, &msg, size_msg, NULL, 0));
}
...
{
    lc3_enc_sco_iso_set_num_of_channels_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_ENC_SCO_ISO_SET_NUM_OF_CHANNELS;
    msg.num_of_channels = num_of_channels;
...
{
    lc3_enc_sco_iso_id_connect_sco_endpoint_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_ENC_SCO_ISO_ID_CONNECT_SCO_ENDPOINT;
    msg.endpoint = endpoint; /* 1= SCO, 0 = ISO */
    PanicFalse(VmalOperatorMessage(op, &msg, size_msg, NULL, 0));
}
...
{
    aptx_lite_dec_sco_iso_frame_duration_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = APTX_LITE_DEC_SCO_ISO_FRAME_DURATION;
    msg.frame_duration = frame_duration;
...
{
    aptx_lite_enc_sco_iso_encoding_param_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = APTX_LITE_ENC_SCO_ISO_ENCODING_PARAM;
    msg.sample_rate = getSampleRateUnitsForAudioSubsystem(sample_rate);
...
{
    aptx_adaptive_test_sco_iso_encoding_param_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);
...
{
    aptx_adaptive_enc_sco_iso_set_qhs_level_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = APTX_ADAPTIVE_ENC_SCO_ISO_SET_QHS_LEVEL;
    msg.qhs_level = qhs_level;
...
{
    aptx_adaptive_dec_sco_iso_frame_duration_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = APTX_ADAPTIVE_DEC_SCO_ISO_FRAME_DURATION;
    msg.frame_duration = frame_duration;
...
{
    aptx_adaptive_dec_sco_iso_predecode_duration_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = APTX_ADAPTIVE_DEC_SCO_ISO_PREDECODE_DURATION;
    msg.predecode_duration = predecode_duration;
...
{
    PanicFalse(equ_op_id != 0);
    eft_equ_op_id_t set_equ_op_id;
    set_equ_op_id.id = EARBUD_FIT_TEST_SET_EQU_OP_ID;
    set_equ_op_id.equ_op_id = equ_op_id;
    PanicFalse(VmalOperatorMessage(op, &set_equ_op_id, SIZEOF_OPERATOR_MESSAGE(set_equ_op_id), NULL, 0));
}

void OperatorsEarbudFitTestStartSingleCapture(Operator op, uint16 capture_interval_ms)
{
    eft_set_capture_param_t eft_set_param;
    int16 response[1];
    eft_set_param.id = EARBUD_FIT_TEST_SINGLE_CAPTURE_PARAM;
    eft_set_param.capture_interval_ms = capture_interval_ms;
    PanicFalse(VmalOperatorMessage(op, &eft_set_param, SIZEOF_OPERATOR_MESSAGE(eft_set_param), response, SIZEOF_OPERATOR_MESSAGE(response)));
}
...
void OperatorsEarbudFitTestSetPowerSmoothTimer(Operator op, standard_param_value_t timer)
{
    set_params_data_t* set_params_data = OperatorsCreateSetParamsData(1);
    set_params_data->number_of_params = 1;
    set_params_data->standard_params[0].id = EARBUD_FIT_TEST_POWER_SMOOTH_FACTOR;
    set_params_data->standard_params[0].value = timer;
    OperatorsStandardSetParameters(op, set_params_data);
    free(set_params_data);
}
...
{
    eft_get_capture_bin_param_t eft_get_capture;
    eft_get_capture_bin_result_t response;
    eft_get_capture.id = EARBUD_FIT_TEST_GET_CAPTURED_BINS;
    eft_get_capture.signal_id = signal_id;
    eft_get_capture.bin_nr = part_id;

    PanicFalse(VmalOperatorMessage(op, &eft_get_capture, SIZEOF_OPERATOR_MESSAGE(eft_get_capture), &response, SIZEOF_OPERATOR_MESSAGE(response)));
    memcpy(bin_power, &response, sizeof(response));
}

void OperatorsEarbudFitTestSetSysmodeCtrl(Operator op, eft_sysmode_t mode)
{
    OperatorsStandardSetControl(op, EARBUD_FIT_TEST_SYSMODE_CTRL, (unsigned) mode);
}
...
void OperatorsAhmSetStaticGain(Operator op, int16 *gains, int16 *gains_response)
{
    ...
    ahm_get_static_gain_msg_t gain_response_msg;
    gain_msg.id = AHM_SET_STATIC_GAIN;

    PanicNull(gains);
    PanicNull(gains_response);

    memcpy(&gain_msg.gains[0], gains, sizeof(uint16) * ahm_static_gain_max);

    PanicFalse(VmalOperatorMessage(op, &gain_msg, SIZEOF_OPERATOR_MESSAGE(gain_msg), &gain_response_msg, SIZEOF_OPERATOR_MESSAGE(gain_response_msg)));

    memcpy(gains_response, &gain_response_msg.gains[0], sizeof(uint16) * ahm_static_gain_max);
}
...
uint16 OperatorsAhmGetFineGainWithInstance(Operator op, audio_anc_instance inst, audio_anc_path_id audio_anc_path)
{
    ...
    switch(audio_anc_path)
    {
        case AUDIO_ANC_PATH_ID_FFA:
            static_gain_index = ahm_static_gain_fb_fine;
            break;
        case AUDIO_ANC_PATH_ID_FFB:
            static_gain_index = ahm_static_gain_ff_fine;
            break;
        case AUDIO_ANC_PATH_ID_FB:
            static_gain_index = ahm_static_gain_ec_fine;
            break;
        default:
            break;
    }
    ...
    return (uint16) get_gains.gains[static_gain_index];
}

int16 OperatorsAhmGetCoarseGain(Operator op, audio_anc_path_id audio_anc_path)
{
    return OperatorsAhmGetCoarseGainWithInstance(op, AUDIO_ANC_INSTANCE_NONE, audio_anc_path);
}
...
int16 OperatorsAhmGetCoarseGainWithInstance(Operator op, audio_anc_instance inst, audio_anc_path_id audio_anc_path)
{
    ...
    {
        case AUDIO_ANC_PATH_ID_FFA:
            static_gain_index = ahm_static_gain_fb_coarse;
        break;
        case AUDIO_ANC_PATH_ID_FFB:
            static_gain_index = ahm_static_gain_ff_coarse;
        break;
        case AUDIO_ANC_PATH_ID_FB:
            static_gain_index = ahm_static_gain_ec_coarse;
        break;
        default:
            break;
    }
    ...
    gain_msg.inst = inst;
    PanicFalse(VmalOperatorMessage(op,&gain_msg,SIZEOF_OPERATOR_MESSAGE(gain_msg), &get_gains, SIZEOF_OPERATOR_MESSAGE(get_gains)));
    return get_gains.gains[static_gain_index];
}
...
void OperatorsAhmSetWindyModeRampDurationParameters(Operator op, uint32 ff_ramp_duration, uint32 fb_ramp_duration)
{
    ...
    set_params_data_t* set_params_data = OperatorsCreateSetParamsData(num_params);
    set_params_data->number_of_params = num_params;

    set_params_data->standard_params[0].id = AHM_WIND_FF_RAMP_DURATION_INDEX;
    set_params_data->standard_params[0].value = ff_ramp_duration;

    set_params_data->standard_params[1].id = AHM_WIND_FB_RAMP_DURATION_INDEX;
    set_params_data->standard_params[1].value = fb_ramp_duration;
...
void OperatorsAhmSetWindyModeGainParameters(Operator op, uint32 ff_fine_gain, uint32 fb_fine_gain)
{
    ...
    set_params_data->standard_params[0].id = AHM_WIND_FF_FINE_GAIN_INDEX;
    set_params_data->standard_params[0].value = ff_fine_gain;

    set_params_data->standard_params[1].id = AHM_WIND_FB_FINE_GAIN_INDEX;
    set_params_data->standard_params[1].value = fb_fine_gain;
...
{
    compander_set_makeup_gain_msg_t makeup_gain_msg;

    makeup_gain_msg.id = COMPANDER_SET_MAKEUP_GAIN;
    makeup_gain_msg.makeup_gain_lsw = (uint16)( (uint32)gain & 0xffff);
    makeup_gain_msg.makeup_gain_msw = (uint16)( (uint32)gain >> 16);

    PanicFalse(VmalOperatorMessage(op,&makeup_gain_msg,SIZEOF_OPERATOR_MESSAGE(makeup_gain_msg), NULL, 0));
...
void OperatorsAncLinkHwManager(Operator op, unsigned link, uint32 ahm_op_id)
{
    ...
    lnk_msg.op_id = (uint16)(ahm_op_id & 0xffff);
...
{
    OperatorsStandardSetControl(op, WIND_DETECT_INTENSITY_UPDATE_CTRL, (unsigned) update);
}
...
{
    uint16 ff_ramp_duration_index = 0, fb_ramp_duration_index = 0, ff_fine_gain_index = 0, fb_fine_gain_index = 0;
    get_params_data_t* get_params_data = OperatorsCreateGetParamsData(WIND_DETECT_GET_MITIGATION_PARAMTERS_COUNT);
    bool ret_val = FALSE;

    switch (intensity)
    {
    case wind_detect_intensity_low:
        ff_ramp_duration_index = WIND_DETECT_LOW_INTENSITY_FF_RAMP_DURATION_INDEX;
        fb_ramp_duration_index = WIND_DETECT_LOW_INTENSITY_FB_RAMP_DURATION_INDEX;
        ff_fine_gain_index = WIND_DETECT_LOW_INTENSITY_FF_FINE_GAIN_INDEX;
        fb_fine_gain_index = WIND_DETECT_LOW_INTENSITY_FB_FINE_GAIN_INDEX;
        break;

    case wind_detect_intensity_medium:
        ff_ramp_duration_index = WIND_DETECT_MEDIUM_INTENSITY_FF_RAMP_DURATION_INDEX;
        fb_ramp_duration_index = WIND_DETECT_MEDIUM_INTENSITY_FB_RAMP_DURATION_INDEX;
        ff_fine_gain_index = WIND_DETECT_MEDIUM_INTENSITY_FF_FINE_GAIN_INDEX;
        fb_fine_gain_index = WIND_DETECT_MEDIUM_INTENSITY_FB_FINE_GAIN_INDEX;
        break;

    case wind_detect_intensity_high:
        ff_ramp_duration_index = WIND_DETECT_HIGH_INTENSITY_FF_RAMP_DURATION_INDEX;
        fb_ramp_duration_index = WIND_DETECT_HIGH_INTENSITY_FB_RAMP_DURATION_INDEX;
        ff_fine_gain_index = WIND_DETECT_HIGH_INTENSITY_FF_FINE_GAIN_INDEX;
        fb_fine_gain_index = WIND_DETECT_HIGH_INTENSITY_FB_FINE_GAIN_INDEX;
        break;
    ...
    if ((intensity != wind_detect_intensity_none) && (wind_params))
    {
        get_params_data->standard_params[0].id = ff_ramp_duration_index;
        get_params_data->standard_params[1].id = fb_ramp_duration_index;
        get_params_data->standard_params[2].id = ff_fine_gain_index;
        get_params_data->standard_params[3].id = fb_fine_gain_index;

        OperatorsStandardGetParameters(op, get_params_data);

        wind_params->ff_ramp_duration = get_params_data->standard_params[0].value;
        wind_params->fb_ramp_duration = get_params_data->standard_params[1].value;
        wind_params->ff_fine_gain = get_params_data->standard_params[2].value;
        wind_params->fb_fine_gain = get_params_data->standard_params[3].value;
...
{
    OperatorsStandardSetControl(op, FBC_SYSMODE_CTRL, (unsigned) mode);
}

int32 OperatorsAahGetFbLimitThreshold(Operator op)
{
    int32 limit_level_fb_db = 0;
    get_params_data_t* get_params_data = OperatorsCreateGetParamsData(1);
    get_params_data->standard_params->id = AAH_LIMIT_LEVEL_FB_DB;
    OperatorsStandardGetParameters(op, get_params_data);
    limit_level_fb_db = (int32)get_params_data->standard_params->value;
    free(get_params_data);
    return limit_level_fb_db;
}

void OperatorsAahSetFbLimitThreshold(Operator op, int32 fb_limit)
{
    standard_param_t parameter = {AAH_LIMIT_LEVEL_FB_DB, (standard_param_value_t)fb_limit};
    operatorsSetSingleParameter(op, &parameter);
}

int32 OperatorsAahGetCombinedLimitThreshold(Operator op)
{
    int32 limit_level_comb_db = 0;
    get_params_data_t* get_params_data = OperatorsCreateGetParamsData(1);
    get_params_data->standard_params->id = AAH_LIMIT_LEVEL_COMB_DB;
    OperatorsStandardGetParameters(op, get_params_data);
    limit_level_comb_db = (int32)get_params_data->standard_params->value;
    free(get_params_data);
    return limit_level_comb_db;
}

void OperatorsAahSetCombinedLimitThreshold(Operator op, int32 combined_limit)
{
    standard_param_t parameter = {AAH_LIMIT_LEVEL_COMB_DB, (standard_param_value_t)combined_limit};
    operatorsSetSingleParameter(op, &parameter);
}

void OperatorsGeqSetSysmodeCtrl(Operator op, geq_sysmode_t mode)
{
    OperatorsStandardSetControl(op, GEQ_SYSMODE_CTRL, (unsigned) mode);
}
...
{
    OperatorsStandardSetControl(op, NOISE_ID_SYSMODE_CTRL, (unsigned) mode);
}

void OperatorsNoiseIDSetCategory (Operator op, noise_id_category_t current_category)
{
    noise_id_set_current_noise_id_msg_t set_msg;
    set_msg.id = NOISE_ID_SET_CURRENT_CATEGORY;
    set_msg.current_noise_id = current_category;

    PanicFalse(VmalOperatorMessage(op, &set_msg, SIZEOF_OPERATOR_MESSAGE(set_msg), NULL, 0));
}
...
uint32 OperatorsAncCompanderGetConfig(Operator op)
{
    ...
    get_params_data_t* get_params_data = OperatorsCreateGetParamsData(1);
    get_params_data->standard_params->id = ADRC_COMPANDER_CONFIG_PARAM_INDEX;
    OperatorsStandardGetParameters(op, get_params_data);
    compander_config = (uint32)(get_params_data->standard_params->value);
    free(get_params_data);
...
{
    standard_param_t parameter = {ADRC_COMPANDER_CONFIG_PARAM_INDEX, (standard_param_value_t)compander_config};
    operatorsSetSingleParameter(op, &parameter);
}
