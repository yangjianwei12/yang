...
{
    unsigned memory_size = (unsigned) sizeof(iir_coefficients_t) + ((unsigned) sizeof(standard_param_value_t) * num_coefficients);
    iir_coefficients_t* set_params_data = PanicUnlessMalloc(memory_size);
    memset(set_params_data, 0, memory_size);
    set_params_data->num_coefficients = num_coefficients;
    return set_params_data;
}
