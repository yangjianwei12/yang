...
/*!
 * @brief Adds a file specific information entry to the library (but not if that file index already exists for that role).
 *
 * @param role The file information can be accessed based on a role, which is simply an unsigned integer.
 * @param index The file's index used by the file system.
 * @param related_data Should contain the address to a user defined structure to be passed in the library,
 ...
 *         The function will not cause a panic.
 */
bool FileListAddFileOnce(unsigned role, FILE_INDEX index, const file_related_data_t *related_data);

/*!
 * @brief Get the handles to the file entries of a specific role.
 *
 ...
 *        It is zero based and allows the user to loop over all the respective entries.
 *
 * @return A handle to access the information stored for that entry.
 *         If such an entry doesn't exist the handle will be a NULL pointer.
 *         The function will not cause a panic.
 */
file_list_handle FileListGetFileHandle(unsigned role, unsigned number);

/*!
 * @brief Get the handles to the file entries of a specific role and file index.
 *
 * @param role The role that was used when adding the file entry to the library.
 * @param index The index that was used when adding the file entry to the library.
 * @param number Since multiple file entries can have the same role and index, we need an additional identifier.
 *        It is zero based and allows the user to loop over all the respective entries.
 *
 * @return A handle to access the information stored for that entry.
 *         If such an entry doesn't exist the handle will be a NULL pointer.
 *         The function will not cause a panic.
 */
file_list_handle FileListGetFileHandleWithIndex(unsigned role, FILE_INDEX index, unsigned number);
