...
{
    if (FileListGetFileHandleWithIndex(role, index, 0))
        return TRUE;

    return FileListAddFile(role, index, related_data);
}
...
{
    unsigned current_number = 0, handle_index = 0;
    file_list_handle handle;

    for(handle_index = 0; current_number <= number; handle_index++)
    {
        handle = FileListGetFileHandle(role, handle_index);
        if (!handle)
            return NULL;

        if (FileListGetFileIndex(handle) == index)
        {
