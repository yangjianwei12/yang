...
/*! \brief Action to perform during a key value list merge conflict */
typedef enum
{
    /*! Target list remains as it is */
    KV_LIST_MERGE_CONFLICT_ACTION_ACCEPT_TARGET_LIST,

    /*! Resolve conflict by copying the source list value to target list  */
    KV_LIST_MERGE_CONFLICT_ACTION_ACCEPT_SOURCE_LIST,

    /*! Merge conflict is unresolvable */
    KV_LIST_MERGE_CONFLICT_ACTION_UNRESOLVABLE
} key_value_list_merge_action_t;

/*! \brief Opaque type for a key-value list object. */
typedef struct key_value_list_tag * key_value_list_t;

/*! \brief The key type is an unsigned integer. */
typedef uint32 key_value_key_t;

/*! \brief Callback function type for key value list merge resolving */
typedef key_value_list_merge_action_t (*key_value_list_merge_resolve_callback_t)(key_value_list_t source,
                                                                                 key_value_list_t target,
                                                                                 key_value_key_t key);

/*! \brief Opaque type for a key-value list iterator object. */
typedef struct key_value_list_iterator_tag * key_value_list_iterator_t;
...
/*! \brief Merge source key-value list with the target key-value list
    ...
    The callback function can return either KV_LIST_MERGE_CONFLICT_ACTION_ACCEPT_TARGET_LIST or
    KV_LIST_MERGE_CONFLICT_ACTION_ACCEPT_SOURCE_LIST to resolve the conflict.ie, for accepting
    target list value or source list value as merged value respectively.
    If it returns with KV_LIST_MERGE_CONFLICT_ACTION_UNRESOLVABLE, then we will panic.
...
/*! \brief Get an iterator for the specified list.
    \warning Adding/Removing list elements after iterator creation can lead to undefined behaviour.
             DO NOT use non-iterator APIs during the iterators lifecycle.

    \param[in] list The list the iterator will correspond to.

    \return A handle to the iterator.
*/
key_value_list_iterator_t KeyValueList_CreateIterator(key_value_list_t list);

/*! \brief Destroy the list iterator

    \param[in/out] Iterator to be destroyed (the handle value will be set to NULL)
*/
void KeyValueList_DestroyIterator(key_value_list_iterator_t *iterator);

/*! \brief Iterate to the next item in the list

    \param[in/out] Iterator handle for that list

    \return The current key value element indicated by the iterator.
            If all elements have been iterated through, the elements of the struct returned will all be zero
            and the iterator will be automatically destroyed (the handle value will be set to NULL).
*/
key_value_element_t KeyValueList_Next(key_value_list_iterator_t *iterator);

/*! \brief Get the length of the key-value list.
    \param[in] list Key-value list handle.
    \return The length
*/
