...
/*! \brief Different actions that are supported to perform during a device merge conflict  */
typedef enum
{
    /*! Target list remains as it is */
    DEVICE_MERGE_ACTION_ACCEPT_TARGET_LIST = KV_LIST_MERGE_CONFLICT_ACTION_ACCEPT_TARGET_LIST,

    /*! Resolve conflict by copying the source list value to target list  */
    DEVICE_MERGE_ACTION_ACCEPT_SOURCE_LIST = KV_LIST_MERGE_CONFLICT_ACTION_ACCEPT_SOURCE_LIST,

    /*! Merge conflict is unresolvable */
    DEVICE_MERGE_ACTION_UNRESOLVABLE = KV_LIST_MERGE_CONFLICT_ACTION_UNRESOLVABLE
} device_merge_action_t;
...
typedef device_merge_action_t (*device_merge_resolve_callback_t)(device_t source_device,
                                                                 device_t target_device,
                                                                 device_property_t property_id);

/*! \brief Handler to be executed on property changes.
...
/*! \brief Register a function to run on properties being set.
