...
{
    /*! Do not proceed if:
     * 1. Delta fine gain is zero or
     * 2. ANC respective path is muted.
     */

    if((delta_fine_gain_q_format == 0) || (gain_config->gain == 0))
    {
        return;
    }

    int16 golden_fine_gain_q_format = convertGainTo16BitQFormat(gain_config->gain);
    uint16 gain_shift = gain_config->gain_shift;

    golden_fine_gain_q_format += delta_fine_gain_q_format;

    while((golden_fine_gain_q_format > UPPER_LIMIT_Q_FORMAT) || (golden_fine_gain_q_format < LOWER_LIMIT_Q_FORMAT))
    {
        /*! Stop if coarse value reachs at max / min value */
        if(((gain_shift == (uint16)GAIN_SHIFT_UPPER_LIMIT) && (golden_fine_gain_q_format > UPPER_LIMIT_Q_FORMAT)) ||
                ((gain_shift == (uint16)GAIN_SHIFT_LOWER_LIMIT) && golden_fine_gain_q_format < LOWER_LIMIT_Q_FORMAT))
            break;
        ...
        else if(golden_fine_gain_q_format < LOWER_LIMIT_Q_FORMAT)
        {
    ...
    /*! Do not change golden fine and coarse gains if evaluated fine gain is not in specified limit. */
    if(golden_fine_gain_q_format > UPPER_LIMIT_Q_FORMAT || golden_fine_gain_q_format < LOWER_LIMIT_Q_FORMAT)
    {
        return;
    }

    gain_config->gain = convert16BitQFormatToGain(golden_fine_gain_q_format);
    gain_config->gain_shift = gain_shift;
}

static void populateDeviceSpecificAncPathGains(anc_config_t * config_data)
{
    /* This call is to retrieve the size of the existing USR13 PSKey. (Size 0 and buffer as NULL in parameters) */
    uint16 tune_data_actual_size = PsRetrieve(device_ps_key_config.device_ps_key, NULL, 0);

    if(tune_data_actual_size == FINE_GAIN_TUNE_DATA_SIZE)
    {
        int16 delta_fine_gain_tune[FINE_GAIN_TUNE_DATA_ENTRIES];
        uint16 ret = PsRetrieve(device_ps_key_config.device_ps_key, delta_fine_gain_tune, FINE_GAIN_TUNE_DATA_SIZE);
...
#ifdef ANC_UPGRADE_FILTER
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_a.rxmix_gain_config, delta_fine_gain_tune[FFA_PATH_INDEX]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_a.rxmix_gain_config, delta_fine_gain_tune[FFA_PATH_INDEX]);

            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_b.rxmix_gain_config, delta_fine_gain_tune[FFB_PATH_INDEX]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_b.rxmix_gain_config, delta_fine_gain_tune[FFB_PATH_INDEX]);
#endif
        }
    }

    else if(tune_data_actual_size == FINE_GAIN_TUNE_DATA_SIZE_HS && ancDataGetTopology() == anc_single_filter_topology)
    {
        int16 delta_fine_gain_tune[FINE_GAIN_TUNE_DATA_ENTRIES_HS];
        uint16 ret = PsRetrieve(device_ps_key_config.device_ps_key, delta_fine_gain_tune, FINE_GAIN_TUNE_DATA_SIZE_HS);

        if(ret)
        {
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_a.gain_config, delta_fine_gain_tune[FFA_PATH_INDEX]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_a.gain_config, delta_fine_gain_tune[FFA_PATH_INDEX + ANC_INSTANCE_1_INDEX_OFFSET]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_b.gain_config, delta_fine_gain_tune[FFB_PATH_INDEX]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_b.gain_config, delta_fine_gain_tune[FFB_PATH_INDEX + ANC_INSTANCE_1_INDEX_OFFSET]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_back.gain_config, delta_fine_gain_tune[FB_PATH_INDEX]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_back.gain_config, delta_fine_gain_tune[FB_PATH_INDEX + ANC_INSTANCE_1_INDEX_OFFSET]);

#ifdef ANC_UPGRADE_FILTER
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_a.rxmix_gain_config, delta_fine_gain_tune[FFA_PATH_INDEX]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_a.rxmix_gain_config, delta_fine_gain_tune[FFA_PATH_INDEX + ANC_INSTANCE_1_INDEX_OFFSET]);

            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_b.rxmix_gain_config, delta_fine_gain_tune[FFB_PATH_INDEX]);
            populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_b.rxmix_gain_config, delta_fine_gain_tune[FFB_PATH_INDEX + ANC_INSTANCE_1_INDEX_OFFSET]);
#endif
        }
    }
    ...
    {
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_a.gain_config, 0);
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_a.gain_config, 0);
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_b.gain_config, 0);
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_b.gain_config, 0);
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_back.gain_config, 0);
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_back.gain_config, 0);

#ifdef ANC_UPGRADE_FILTER
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_a.rxmix_gain_config, 0);
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_a.rxmix_gain_config, 0);

        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_0_INDEX].feed_forward_b.rxmix_gain_config, 0);
        populateAncSpecificPathGain(&config_data->mode.instance[ANC_INSTANCE_1_INDEX].feed_forward_b.rxmix_gain_config, 0);
#endif
     }
}
...
bool ancReadLpfCoefficients(audio_anc_instance inst, audio_anc_path_id path, uint8* lpf_shift_1, uint8* lpf_shift_2)
{
    ...
    if(instance)
    {
        ...
        case AUDIO_ANC_PATH_ID_FFA:
        {
            lpf_config = &instance->feed_forward_a.lpf_config;
        }
        break;

        case AUDIO_ANC_PATH_ID_FFB:
        {
            lpf_config = &instance->feed_forward_b.lpf_config;
        }
        break;

        case AUDIO_ANC_PATH_ID_FB:
        {
            lpf_config = &instance->feed_back.lpf_config;
        }
        break;
    ...
    if (lpf_config)
    {
        *lpf_shift_1 = (lpf_config->lpf_shift1 & 0xF);
        *lpf_shift_2 = (lpf_config->lpf_shift2 & 0xF);
        status = TRUE;
    }
...
{
    anc_instance_config_t *instance = getInstanceConfig(inst);
    dc_filter_config_t* dc_filter_config = NULL;
    bool status = FALSE;
    ...
    if(instance)
    {
        ...
        case AUDIO_ANC_PATH_ID_FFA:
        {
            dc_filter_config = &instance->feed_forward_a.dc_filter_config;
        }
        break;

        case AUDIO_ANC_PATH_ID_FFB:
        {
            dc_filter_config = &instance->feed_forward_b.dc_filter_config;
        }
        break;
    ...
    if (dc_filter_config)
    {
        *filter_shift = (dc_filter_config->filter_shift & 0xF);
        *filter_enable = (dc_filter_config->filter_enable & 0x1);
        status = TRUE;
    }
...
{
    anc_instance_config_t *instance = getInstanceConfig(inst);
    small_lpf_path_config_t* small_lpf_config = instance != NULL ? &instance->small_lpf : NULL;
    bool status = FALSE;

    if (small_lpf_config)
    {
        *filter_shift = (small_lpf_config->small_lpf_config.filter_shift & 0xF);
        *filter_enable = (small_lpf_config->small_lpf_config.filter_enable & 0x1);
        status = TRUE;
    }
