...
Provides an interface between Heracles and building a DFU file
    ...
    -p --projects_selected    Specifies the projects to select.
                              For example:
    ...
    -v --version_file         Specifies the file containing version information.
    -e --encrypted            Specifies whether DFU file is to be encrypted or not.
                              For example:
                              -e true
                              -e encrypted
    -d --delay_encrypted      Specifies whether DFU is to be delay_encrypted.
                              For example:
                              -d true
                              -d delay_encrypted
    -a --aes_128_file         Specifies the file containing the AES-128 software encryption key.
    -f --folder_for_rsa_files Specifies the folder containing the RSA-2048 security files.
    -s --signing_mode         Specifies whether "all" projects are signed or just the "header".
                              If 'all' then sign all projects.
                              If 'header' then only sign the DFU file's image header.
    
    The optional parameters are:
    -o --build_output_folder  Specify location for object files and other build artefacts.
    --build_system            Specify which build system to use.
                              --build_system make
                              --build_system scons
                              Default: make
"""
...
def add_layout(lines, file_type, xuv_filename, params, encrypted):
    ...
    for item, value in params.iteritems():
        if item == "id" or item == "attribs":
            # For these fields, value is string so put it in quotes
            line = '            "{}": "{}",\n'.format(item, value)
            lines.append(line)
        else:
...
def prepare_dfu_file_generator_config(devkit_root, projects, dfg_filepath,
    upd_filepath, output_path, encrypted, hardware_encrypted, delay_encrypted,
    software_encryption_file, folder_for_rsa_files, signing_mode, chip_type, flash_config):
    """
    ...
    if encrypted == True:
        ...
        lines.append(line)
        if hardware_encrypted == False and delay_encrypted == False:
            line = '    "encryption_file": r"{}",\n'.format(software_encryption_file)
            lines.append(line)
        if delay_encrypted == True:
            line = '    "delay_encrypt": {},\n'.format(delay_encrypted)
            lines.append(line)
            line = '    "short_hashtable": {},\n'.format(delay_encrypted)
            lines.append(line)
...
def prepare_and_run_dfu_file_generator(devkit_root, projects, projects_selected,
    outpath, nowstring, version_info, encrypted, hardware_encrypted, delay_encrypted,
    software_encryption_file, folder_for_rsa_files, signing_mode, chip_type, flash_config):
    """
...
class show_encryption_selection_ui(object):
    def __init__(self, root):
        ...
        self.delay_encrypted_button = Tkinter.Radiobutton(self.button_frame,
            text = "Delay encrypted", variable = self.var, value = 3)
        self.delay_encrypted_button.pack( anchor = Tkinter.W )
        select_button = Tkinter.Button(self.button_frame, text="Proceed",
            command=self._proceed).pack(side=Tkinter.LEFT)
        cancel_button = Tkinter.Button(self.button_frame, text="Cancel",
            command=self._cancel).pack(side=Tkinter.RIGHT)
...
def parse_args(args):
    ...
    parser.add_argument('-d', '--delay_encrypted',
                        default=None,
                        help='specifies whether DFU is to be delay_encrypted')
    # E.g. -d "delay_encrypted"
    # Strings (case insensitive) of "delay_encrypted" or "true" indicate delay encrypted
    # Any other strings indicate unencrypted
    # The -d option prevents the encryption selection UI being shown                    
...
def main(args):
    ...
    if parsed_args.delay_encrypted != None:
        print("delay_encrypted: %s" % (parsed_args.delay_encrypted))
    ...
    if projects != None:
        ...
        # dfu_file_generator.py script
            ...
            parsed_args.delay_encrypted == None or \
    ...
    while uistate != UISTATE_EXIT and uistate != UISTATE_PROCEED:
        ...
        if uistate == UISTATE_LOCATION_SELECTION:
            ...
            if parsed_args.encrypted == None or parsed_args.delay_encrypted == None:
                if parsed_args.aes_128_file == None:
                    x = select_encrypted(top)
                    if x == 1: # Unencrypted
                        encrypted = False
                    elif x == 2: # Encrypted
                        encrypted = True
                        delay_encrypted = False
                    elif x == 3: # Delay encrypted
                        encrypted = True
                        hardware_encrypted = False
                        delay_encrypted = True
                    else:
                        encrypted = None
                else:
                    encrypted = True
                ...
                lower_denc = parsed_args.delay_encrypted.lower()
                ...
                if lower_denc == 'delay_encrypted' or lower_denc == 'true':
                    if encrypted == False:
                        raise ValueError("Cannot have delay_encrypted True and encrypted False")
                    delay_encrypted = True
                    hardware_encrypted = False
                else:
                    delay_encrypted = False
            ...
            if encrypted != None:
                if encrypted == True and delay_encrypted == False:
    ...
    if uistate == UISTATE_PROCEED:
        ...
        if encrypted == True:
            ...
            elif delay_encrypted == True:
                print("Delay encrypted\n")
            else:
        ...
        if prepare_and_run_dfu_file_generator(parsed_args.devkit_root, projects,
                ...
                hardware_encrypted, delay_encrypted, software_encryption_file,
