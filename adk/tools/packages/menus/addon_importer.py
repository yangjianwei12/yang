...
default_ro_fs_templates_project_file = 'addon_ext_ro_fs_x2p.xml'
...
class Importer(object):
    def __init__(self, args):
        ...
        self.ro_fs_templates_project_files = [default_ro_fs_templates_project_file]
        
        if "INCLUDE_AAC_PROMPTS" in self.addon_utils.readAppProjectProperty("DEFS", self.args.project):
        	self.ro_fs_templates_project_files.append(aac_ro_fs_templates_project_file)
        else:
        	self.ro_fs_templates_project_files.append(sbc_ro_fs_templates_project_file)
    ...
    def _createImportToolFilePaths(self):
        ...
        self.ro_fs_templates = self._getTemplates(self.ro_fs_templates_project_files)
        self.addon_templates = self._getTemplates([self.addon_templates_project_file])

    def _getTemplates(self, template_types):
        """Gets the all the addon templates of a set of specific types

        E.g.:
            self.getTemplates([self.app_templates_project_file])

            will return all templates for the application project file.
            The main addon templates are always returned first, any templates from
            additional options are returned after.
