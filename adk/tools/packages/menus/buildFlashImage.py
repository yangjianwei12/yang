...
def prompt_user_for_encryption_selection(parsed_args):
    ...
    if parsed_args.delay_encrypt is None:
        delay_encrypt = False
    else:
        if parsed_args.delay_encrypt == 'True' or parsed_args.delay_encrypt == 'true':
            delay_encrypt = True
        elif parsed_args.delay_encrypt == 'False' or parsed_args.delay_encrypt == 'false':
            delay_encrypt = False
        else:
            print("Invalid delay_encypt value")
        ...
        if uistate == UISTATE_ENCRYPTION_SELECTION:
            ...
            if encrypted is True:
                ...
                if hardware_encrypted is not None:
                    if hardware_encrypted is True or delay_encrypt is True:
                        uistate = UISTATE_PROCEED
                    else:
        ...
        if encrypted is True:
            if hardware_encrypted is True:
                ...
                print("Software encryption file is {}\n"
                    .format(software_encryption_file))
                if delay_encrypt is True:
                    print("Delayed encryption\n")
        else:
            print("Unencrypted\n")
        sys.stdout.flush()
    ...
    return (uistate == UISTATE_PROCEED, encrypted,
            hardware_encrypted, delay_encrypt, software_encryption_file)
...
def parse_args(args):
    ...
    parser.add_argument('--delay_encrypt',
                        default=None,
                        help='Delay the encryption of the flash image \
                              to make use of the OEM unique key')
...
def main(args):
    ...
    success, encrypted, hardware_encrypted, delay_encrypt, software_encryption_file = \
