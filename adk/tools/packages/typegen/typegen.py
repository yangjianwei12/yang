...
try:
    from vm_build.codegen import CommentBlock, CommentBlockDoxygen, HeaderGuards, ConditionalBlock, Enumeration, Array, StructTypedef, UnionTypedef, Include, Indented, EnumerationTypedef
    
except ImportError:
    file_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(file_dir)
    sys.path.insert(0, os.path.join(parent_dir, "codegen"))
    from codegen import CommentBlock, CommentBlockDoxygen, HeaderGuards, ConditionalBlock, Enumeration, Array, StructTypedef, UnionTypedef, Include, Indented, EnumerationTypedef
...
class TypesGenerator(object):
    ...
    def __init__(self, element_tree_root):
        ...
        self._extract_source_conditional()
    ...
    def _extract_source_conditional(self):
        ''' First validate there is only one source_conditional tag '''
        source_conditional_count = len(self._root.findall('source_conditional'))  
        if source_conditional_count > 1:
            raise Exception("Only one source_conditional tag is allowed")    
                     
        ''' Now extract conditional flag from source_conditional tag. '''
        for t in self._root:
            if t.tag in ["source_conditional"]:
                self._conditional = t.get('name')
                ''' Now validate that only one conditional is included in the name '''
                if ' ' in self._conditional:
                    raise Exception("Only one macro (without spaces) is supported in source_conditional tag")
    ...
    def _generate_type_definitions(self):
        ...
        for t in self._root:
            ...
            elif t.tag == "source" or t.tag == "include_header" or t.tag == "inherit" or t.tag == 'typedef_header_source' or t.tag == "source_conditional":
    ...
    def _generate_marshal_source_body(self):
        self._print_headers(["app/marshal/marshal_if.h",
    ...
    def generate_typedef_header(self):
        ...
        if self._conditional == "":
            self._generate_typedef_header_body() 
        else:
            with ConditionalBlock(self._conditional):
                self._generate_typedef_header_body() 
    ...
    def generate_marshal_header(self):
        ...
        if self._conditional == "":
            self._generate_marshal_header_body()
        else:                    
            with ConditionalBlock(self._conditional):
                self._generate_marshal_header_body()
    ...
    def generate_marshal_source(self):
        ...
        if self._conditional == "":
            self._generate_marshal_source_body()
        else: 
            with ConditionalBlock(self._conditional):
                self._generate_marshal_source_body()
