...
def pack_dir(dir_to_pack, out, packfile=DEFAULT_PACKFILE, offset=DEFAULT_OFFSET, appsfs=False):
    ...
    try:
        # Redirect the output from packfile.exe through Python so we can
        # convert it to XML automatically if required. See ubuildXML class
        # in ubuild.py.
        print(subprocess.check_output([packfile, dir_to_pack, tmp_packed_img],
