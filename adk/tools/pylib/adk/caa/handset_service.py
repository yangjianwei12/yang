...
class HandsetService(FirmwareComponent):
    ...
    def _active_connections_generator(self):
        ...
        for conn in connections:
            ...
            le_bdaddr = IAdkStructHandler.handler_factory("bdaddr")(self._core, conn.ble_sm.le_addr.taddr.addr)
    ...
    def _generate_report_body_elements(self):
        ...
        tbl = interface.Table(["BdAddr", "State", "LE Address", "LE State", "ACL attempts", "Disc Reason", "Profiles Req"])
        for (addr, conn, le_addr) in self._active_connections_generator():
            ...
            le_state = conn.ble_sm.le_state.symbolic_value
            tbl.add_row([addr, state, le_addr, le_state, conn.acl_attempts , disconnect_reason ,self._decode_profiles(conn.profiles_requested)])
        grp.append(tbl)
        return [grp]
