...
class Device(FirmwareComponent, UserDict):
    ...
    def _decode_key_values_multi_array_key_value_list(self, key_value_list):
        ...
        if key_value_list.keys.value != 0:
            keys = self.env.cast(key_value_list.keys, key_value_list.keys.deref.typename, array_len=keys_len)
            ...
            if key_value_list.values8.value != 0:
                values8 = self.env.cast(key_value_list.values8, key_value_list.values8.deref.typename, array_len=len8)
            if key_value_list.values16.value != 0:
                values16 = self.env.cast(key_value_list.values16, key_value_list.values16.deref.typename, array_len=len16)
            if key_value_list.values32.value != 0:
                values32 = self.env.cast(key_value_list.values32, key_value_list.values32.deref.typename, array_len=len32)
...
class DeviceList(FirmwareComponent):
    ...
    def _read_device_list(self):
        ...
        if device_list_pointer.value != 0:
            # The device list is an array of pointers to device_t objects. Some
            # of the pointers may be NULL.
            raw_device_list = self.env.cast(device_list_pointer, device_list_pointer.deref.typename, array_len = maxlen)
