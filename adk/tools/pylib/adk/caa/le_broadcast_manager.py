...
class LeBroadcastManager(FirmwareComponent):
    ...
    def _generate_report_body_elements(self):
        ...
        source_items = ["ID", "Sync Handle", "Address",
                        "Target PA", "Target BIS", 
                        "BIS L", "BIS R",
        ...
        with self._le_broadcast_manager.footprint_prefetched():
            ...
            overview.add_row(['Target BIS Source ID', lebm.target_bis_source_id])
            ...
            for source in lebm.broadcast_source_receive_state:
                if source.source_id.value != 0:
                    ...
                    target_pa = "{name} ({value})".format(name = target_pa.symbolic_value[len("le_bm_pa_sync_"):],
                    ...
                    source_addr = source.current_taddr

                    sources.append([source.source_id,
                                    source.sync_handle,
                                    source.current_taddr,
                                    target_pa,
                                    target_bis,
                                    self.bis_index_present(source.bis_info[0].base_bis_index), # broadcast_manager_bis_location_left_or_stereo
                                    self.bis_index_present(source.bis_info[1].base_bis_index), # broadcast_manager_bis_location_right
                                    source.bis_info[0].bis_handle,
                                    source.big_handle,
                                    sample_rate,
                                    frame_ms,
                                    octets_per_frame,
