...
class CcProtocol(FirmwareComponent):
    ...
    def _generate_report_body_elements(self):
        with self._cc_protocol.footprint_prefetched():
            ...
            tbl.add_row(['Dormant vetos', self.dormant_vetos])
            grp.append(tbl)
    ...
    @property
    def dormant_vetos(self):
        try:
            dv = []
            veto = self._cc_protocol.cc_allow_dormant.value
            for i in self.env.enums["cc_dormant_inhibit_t"].keys():
                if (i is not 'cc_dormant_mask'):
                    v = int(self.env.enums["cc_dormant_inhibit_t"][i])
                    if (veto & v):
                        dv.append(i)
            
        except:
            dv = "Not supported in this firmware"

        return dv
    
    # For firmwares that support cc_protocol preventing dormant, we provide a developer command to clear
    # the ChargerCommd dbg "dormant veto" flag. This functionality is not provided in the application itself.
    def allow_ccdbg_dormant(self):
        try:
            self._core.fw.call.ccProtocol_ClearDormantFlag(self.env.enums["cc_dormant_inhibit_t"]["cc_transport_dorm"])
        except:
            raise self.NotDetected
        return
