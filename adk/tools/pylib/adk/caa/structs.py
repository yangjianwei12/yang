...
class DeviceFlags(BitfieldDecoder):
    ...
    DEVICE_FLAGS_QHS_CONNECTED =                   (1 << 12)
    DEVICE_FLAGS_FIRST_CONNECT_AFTER_DFU =         (1 << 13)
    DEVICE_FLAGS_SWB_NOT_SUPPORTED =               (1 << 14)
    DEVICE_FLAGS_DELETE_ON_BOOT =                  (1 << 15)

    def __init__(self, core, value):
        BitfieldDecoder.__init__(self, core, value, "DEVICE_FLAGS_")

class LeAudioContext(BitfieldDecoder):
    ''' Decoder for LE Audio Context. Flags are #defined in the code,
        so are redefined and manually decoded in this class
    '''

    AUDIO_CONTEXT_TYPE_UNSPECIFIED     = (1 << 0)
    AUDIO_CONTEXT_TYPE_CONVERSATIONAL  = (1 << 1)
    AUDIO_CONTEXT_TYPE_MEDIA           = (1 << 2)
    AUDIO_CONTEXT_TYPE_GAME            = (1 << 3)
    AUDIO_CONTEXT_TYPE_INSTRUCTIONAL   = (1 << 4)
    AUDIO_CONTEXT_TYPE_VOICE_ASSISTANT = (1 << 5)
    AUDIO_CONTEXT_TYPE_LIVE            = (1 << 6)
    AUDIO_CONTEXT_TYPE_SOUND_EFFECTS   = (1 << 7)
    AUDIO_CONTEXT_TYPE_NOTIFICATIONS   = (1 << 8)
    AUDIO_CONTEXT_TYPE_RINGTONE        = (1 << 9)
    AUDIO_CONTEXT_TYPE_ALERTS          = (1 << 10)
    AUDIO_CONTEXT_TYPE_EMERGENCY_ALARM = (1 << 11)

    def __init__(self, core, value):
        BitfieldDecoder.__init__(self, core, value, "AUDIO_CONTEXT_TYPE_")

class LeAudioCisDirection(BitfieldDecoder):
    ''' Decoder for LE Audio CIS Direction. Flags are #defined in the code,
        so are redefined and manually decoded in this class
    '''

    LE_AUDIO_ISO_DIRECTION_DL         = (1 << 0)
    LE_AUDIO_ISO_DIRECTION_UL         = (1 << 1)

    def __init__(self, core, value):
        BitfieldDecoder.__init__(self, core, value, "LE_AUDIO_ISO_DIRECTION_")
