...
enum sm_internal_message_ids
{
    ...
    SM_INTERNAL_TIMEOUT_OUT_OF_EAR_MEDIA,           /*!< Timeout to pause Media when earbud removed from ear while streaming. */
    SM_INTERNAL_TIMEOUT_OUT_OF_EAR_SCO,            /*!< Timeout to transfer SCO to AG when earbud removed from ear while call active. */
    SM_INTERNAL_TIMEOUT_IN_EAR_MEDIA_START,        /*!< Timeout within which restart Media if earbud put back in ear. */
    ...
    SM_INTERNAL_TIMEOUT_BLOCK_RESET_HANDSET_DELETE,/*!< Timeout during which we will not reset following handset deletion */
    SM_INTERNAL_APP_POWER_ON,                      /*!< Message to call appPowerOn() */
