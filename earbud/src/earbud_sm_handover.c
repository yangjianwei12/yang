...
static bool appSm_HandoverMarshalLe(const typed_bdaddr *tbdaddr, 
                                    marshal_type_t type,
                                    void **marshal_obj);
static app_unmarshal_status_t appSm_HandoverUnmarshalLe(const typed_bdaddr *tbdaddr, 
                                                  marshal_type_t type,
                                                  void *unmarshal_obj);
...
const marshal_type_info_t earbud_sm_marshal_types[] = {
    ...
    MARSHAL_TYPE_INFO(earbud_sm_handover_state_t, MARSHAL_TYPE_CATEGORY_GENERIC),
};
...
/*! Register with application handover the types and function pointers to use during marshalling for LE Connection. */
REGISTER_HANDOVER_INTERFACE_LE(EARBUD_SM, &earbud_sm_marshal_types_list, appSm_HandoverVeto, appSm_HandoverMarshalLe, appSm_HandoverUnmarshalLe, appSm_HandoverCommit);
                              ...
                              void **marshal_obj)
{
    ...
    switch (type)
    {
        ...
        {
            int32 first_due = 0;
            if (MessagePendingFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_OUT_OF_EAR_MEDIA, &first_due))
            {
                SmGetTaskData()->timers.out_of_ear_media_pause = appSm_HandoverTimerAbsoluteTime(first_due);
                SmGetTaskData()->timers.out_of_ear_media_pause_valid = 1;
                DEBUG_LOG_VERBOSE("appSm_HandoverMarshal SM_INTERNAL_TIMEOUT_OUT_OF_EAR_MEDIA due ms %d", first_due);
                status = TRUE;
            }
            ...
            if (MessagePendingFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_IN_EAR_MEDIA_START, &first_due))
            {
                SmGetTaskData()->timers.in_ear_media_play = appSm_HandoverTimerAbsoluteTime(first_due);
                SmGetTaskData()->timers.in_ear_media_play_valid = 1;
                DEBUG_LOG_VERBOSE("appSm_HandoverMarshal SM_INTERNAL_TIMEOUT_IN_EAR_MEDIA_START due %d", first_due);
                status = TRUE;
            }
        ...
        case MARSHAL_TYPE(earbud_sm_handover_state_t):
            *marshal_obj = &SmGetTaskData()->handover_state;
            appSmSetMarshalledState(TRUE);
            status = TRUE;
        break;
                                                  ...
                                                  void *unmarshal_obj)
{
    ...
    switch (type)
    {
        ...
        case MARSHAL_TYPE(earbud_sm_handover_state_t):
            SmGetTaskData()->handover_state = *(earbud_sm_handover_state_t*)unmarshal_obj;
            result = UNMARSHAL_SUCCESS_FREE_OBJECT;
            break;
...
/*!
    ...
    \param[in] bd_addr      Bluetooth address of the BLE link to be marshalled
                            \ref typed_bdaddr
    \param[in] type         Type of the data to be marshalled \ref marshal_type_t
    \param[out] marshal_obj Holds address of data to be marshalled.
    \return TRUE: Required data has been copied to the marshal_obj.
...
static bool appSm_HandoverMarshalLe(const typed_bdaddr *tbdaddr, 
                                    ...
                                    void **marshal_obj)
{
    return appSm_HandoverMarshal(&tbdaddr->addr, type, marshal_obj);
}
...
/*! 
    ...
    \param[in] bd_addr      Bluetooth address of the BLE link to be unmarshalled
                            \ref typed_bdaddr
    \param[in] type         Type of the unmarshalled data \ref marshal_type_t
    \param[in] unmarshal_obj Address of the unmarshalled object.
    \return unmarshalling result. Based on this, caller decides whether to free
...
static app_unmarshal_status_t appSm_HandoverUnmarshalLe(const typed_bdaddr *tbdaddr, 
                                                  ...
                                                  void *unmarshal_obj)
{
    return appSm_HandoverUnmarshal(&tbdaddr->addr, type, unmarshal_obj);
}
...
static void appSm_HandoverCommit(bool is_primary)
{
    ...
    */
    if (!is_primary)
    {
        MessageCancelFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_OUT_OF_EAR_MEDIA);
        MessageCancelFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_OUT_OF_EAR_SCO);
        MessageCancelFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_IN_EAR_MEDIA_START);
    }
    else
    {
        if (SmGetTaskData()->timers.out_of_ear_media_pause_valid)
        {
            uint32 offset = appSm_HandoverTimerOffset(SmGetTaskData()->timers.out_of_ear_media_pause);
            DEBUG_LOG_VERBOSE("appSm_HandoverCommit SM_INTERNAL_TIMEOUT_OUT_OF_EAR_MEDIA %d", offset);
            MessageSendLater(SmGetTask(), SM_INTERNAL_TIMEOUT_OUT_OF_EAR_MEDIA, NULL, offset);
        }
        ...
        if (SmGetTaskData()->timers.in_ear_media_play_valid)
        {
            uint32 offset = appSm_HandoverTimerOffset(SmGetTaskData()->timers.in_ear_media_play);
            DEBUG_LOG_VERBOSE("appSm_HandoverCommit SM_INTERNAL_TIMEOUT_IN_EAR_MEDIA_START %d", offset);
            MessageSendLater(SmGetTask(), SM_INTERNAL_TIMEOUT_IN_EAR_MEDIA_START, NULL, offset);
        }
    }
