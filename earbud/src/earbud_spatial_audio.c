...
static const uint8 spatial_audio_hidd_service_record[] =
{
   ...
   0x09,0x01,0x00,       /* ServiceName(0x0100) = "Earbuds" */
...
/*! Spatial audio internal messages handled along with spatial data messages. */
typedef enum
{
    /*! Spatial audio reset of spatial data */
    SPATIAL_AUDIO_RESET_ATTITUDE_FILTER_TIMEOUT
} spatial_audio_internal_messages_t;
...
/*! Local functions */
static bool earbudSpatialAudio_ContextFrameworkProvider(unsigned * context_data, uint8 context_data_size);
static void earbudSpatialAudio_Register(void);
static void earbudSpatialAudio_TaskHandler(Task task, MessageId id, Message message);
static TaskData earbudSpatialAudioTask = {earbudSpatialAudio_TaskHandler};
...
bool EarbudSpatialAudio_Init(Task task)
{
    ...
    earbudSpatialAudio_Register();

    ContextFramework_RegisterContextProvider(context_spatial_audio_info, earbudSpatialAudio_ContextFrameworkProvider);
    return TRUE;
}

bool EarbudSpatialAudio_Enable(uint16 sensor_sample_rate_hz, spatial_data_report_id_t report_id)
{
    DEBUG_LOG_INFO("%s:Sensor sample rate:%d Data report:%d", __func__, sensor_sample_rate_hz, report_id);

    /* Enabling spatial audio should be invoked only from primary.*/
    if (!appSmIsPrimary())
    {
        DEBUG_LOG_WARN("%s:Invoked in secondary - NOT ALLOWED",__func__);
        return FALSE;
    }
    else
    {
...
{
    DEBUG_LOG_INFO("%s:Input command:%d data_len:%d", __func__, command, data_len);

    bool ret = FALSE;

    /* The command is always inserted at the first byte of the sync data send. */
    uint16 data_send_len = 1 + data_len;

    uint8 *data_send = (uint8 *)(PanicUnlessMalloc(data_send_len));

    /* First byte is the sync command. */
    data_send[0] = (uint8) command;

    /* Copy the input data to the memory starting from data_send[1]  */
    if (data_len)
    {
        memcpy(&data_send[1], data, data_len);
    }

    /* Invoke spatial data function to send synchronised data. */
    ret = SpatialData_SendSynchronisedData(data_send_len, data_send);
    free(data_send);
...
static bool earbudSpatialAudio_ContextFrameworkProvider(unsigned * context_data, uint8 context_data_size)
{
    DEBUG_LOG_VERBOSE("earbudSpatialAudio_ContextFrameworkProvider");
    ...
    {
        DEBUG_LOG_VERBOSE("earbudSpatialAudio_ContextFrameworkProvider - report disabled so enabling");
        /* We do not attempt to change the report type if already enabled. */
        EarbudSpatialAudio_Enable(SPATIAL_AUDIO_REPORT_RATE, spatial_data_report_1);
        return FALSE;
    }
...
static void earbudSpatialAudio_Register(void)
{
    ...
    {
        data_source = spatial_data_left;
    }
    else
    {
        data_source = spatial_data_right;
    }

    /* Get the SPI/I2C and Sensor IDs for the motion sensor chip. */
    bus_id = AppSensorBus_Bus();

    /* We're using LSM6DSTQ motion sensor chip. */
    ...
    ConnectionRegisterServiceRecord(&earbudSpatialAudioTask, sizeof(spatial_audio_pnp_record), spatial_audio_pnp_record);
#endif
    ...
    PanicFalse(SpatialData_Register(&earbudSpatialAudioTask, data_source));
}

static void earbudSpatialAudio_HandleCmPrim(Message message)
{
    CsrBtCmPrim *prim = (CsrBtCmPrim *) message;

    if (*prim == CSR_BT_CM_SDS_REGISTER_CFM)
    {
        DEBUG_LOG("earbudSpatialAudio_HandleCmPrim: SDS_REGISTER_CFM %d",
                  (((const CsrBtCmSdsRegisterCfm *) message)->resultCode));
    }
    else
    {
        DEBUG_LOG("earbudSpatialAudio_HandleCmPrim Unhandled CM Prim 0x%04x", *prim);
    }
...
static void earbudSpatialAudio_TaskHandler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case SPATIAL_DATA_SYNC_DATA_IND:
        {
            SPATIAL_DATA_SYNC_DATA_IND_T *spatial_data = (SPATIAL_DATA_SYNC_DATA_IND_T *)message;
            rtime_t current_time = 0;
            rtime_t delta_time = 0;

            PanicNull(spatial_data);

            /* Calculate delta from the current timestamp, *
             * and then start a timer to act on the input values based on the *
             * command - the first byte of the data.
             * NOTE: this is only for demo purposes. */
            current_time = SystemClockGetTimerTime();
            if (rtime_gt(current_time, spatial_data->timestamp))
            {
                delta_time = rtime_sub(current_time, spatial_data->timestamp);
            }
            else
            {
                delta_time = rtime_sub(spatial_data->timestamp, current_time);
            }

            DEBUG_LOG_INFO("SPATIAL_DATA_SYNC_DATA_IND:Time:%dus sync data len:%d Data[0]:%d Data[1]:%d Delta:%dus",
                            spatial_data->timestamp, spatial_data->data_len, 
                            spatial_data->data[0], spatial_data->data[1], delta_time);

            DEBUG_LOG_DATA_INFO(spatial_data->data, spatial_data->data_len);

#ifdef INCLUDE_ATTITUDE_FILTER
            /* Schedule to trigger spatial data reset at the delta time.. */
            if (spatial_data->data[0] == spatial_audio_sync_reset_attitude_filter_command)
            {
                MessageSendLater(&earbudSpatialAudioTask,
                                 SPATIAL_AUDIO_RESET_ATTITUDE_FILTER_TIMEOUT,
                                 NULL,
        ...
        case SPATIAL_AUDIO_RESET_ATTITUDE_FILTER_TIMEOUT:
        {
            DEBUG_LOG_WARN("%s:SPATIAL_AUDIO_RESET_ATTITUDE_FILTER_TIMEOUT",__func__);
            if (!SpatialData_AttitudeFilterReset())
            {
                DEBUG_LOG_INFO("%s:SpatialData_Reset failed", __func__);
            }
        }
        break;
#endif /* INCLUDE_ATTITUDE_FILTER */
...
#ifdef USE_SYNERGY
        case CM_PRIM:
            earbudSpatialAudio_HandleCmPrim(message);
            break;
#endif
