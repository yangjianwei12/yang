...
typedef enum sm_application_states
{
    ...
    APP_STATE_IN_CASE                           = APP_END_OF_SUBSTATES<<1,
        ...
        APP_STATE_IN_CASE_SOPORIFIC_PAIRING     = APP_STATE_IN_CASE + APP_SUBSTATE_SOPORIFIC + APP_SUBSTATE_HANDSET_PAIRING,
        APP_STATE_IN_CASE_IDLE_PAIRING          = APP_STATE_IN_CASE + APP_SUBSTATE_IDLE + APP_SUBSTATE_HANDSET_PAIRING,
    ...
    APP_STATE_OUT_OF_CASE                                   = APP_STATE_IN_CASE<<1,
        ...
        APP_STATE_OUT_OF_CASE_SOPORIFIC_PAIRING             = APP_STATE_OUT_OF_CASE + APP_SUBSTATE_SOPORIFIC + APP_SUBSTATE_HANDSET_PAIRING,
        APP_STATE_OUT_OF_CASE_IDLE_PAIRING                  = APP_STATE_OUT_OF_CASE + APP_SUBSTATE_IDLE + APP_SUBSTATE_HANDSET_PAIRING,
        APP_STATE_OUT_OF_CASE_BUSY_PAIRING                  = APP_STATE_OUT_OF_CASE + APP_SUBSTATE_BUSY + APP_SUBSTATE_HANDSET_PAIRING,
    ...
    APP_STATE_IN_EAR                            = APP_STATE_OUT_OF_CASE<<1,
        ...
        APP_STATE_IN_EAR_IDLE_PAIRING           = APP_STATE_IN_EAR + APP_SUBSTATE_IDLE + APP_SUBSTATE_HANDSET_PAIRING,
        APP_STATE_IN_EAR_BUSY_PAIRING           = APP_STATE_IN_EAR + APP_SUBSTATE_BUSY + APP_SUBSTATE_HANDSET_PAIRING,
...
/*! \brief Return TRUE if the state is idle */
#define appSmSubStateIsPairing(state) (0 != ((state) & (APP_SUBSTATE_HANDSET_PAIRING)))
...
/*! \brief Definitions of which role the Earbud is currently in. */
typedef enum
{
    EARBUD_ROLE_PRIMARY = EARBUD_ROLE_MESSAGE_BASE,   /*!< Primary Role */
    EARBUD_ROLE_SECONDARY,                            /*!< Secondary Role */
    EARBUD_ROLE_NO_ROLE,                              /*!< No Role Yet */
...
typedef struct
{
    ...
    bool restart_pairing_after_role_selection:1;
                                            ...
                                            /*!  Flag used to restart handset reconnection after peer role 
                                             confirmed as primary */
    bool restart_reconnection_after_role_selection:1;
    ...
    bool ANC_state_when_out_of_ear:1;   /*!< Saves the ANC state when the earbud is taken out of the ear */
    bool touchpad_deliberately_disabled:1; /*!< For example in demos; flag is reset when put into the case  */

    uint16 pfr_start_lock;              /*!< Peer find role start lock */
    ...
    earbud_sm_handover_state_t handover_state; /*!< Misc state that is handed-over. */

    /*! The clients of the application include the UI Indicators, which are sent messages on
     *  application state changes. */
    task_list_t client_tasks;

    /*! Flag to indicate earbud sm data has been marshalled */
    bool apps_sm_marshalled;
} smTaskData;
...
/*! \brief Set the reconnect_post_handover flag */
#define appSmSetReconnectPostHandover(value) do { SmGetTaskData()->handover_state.reconnect_post_handover = (value); } while(0)
/*! \brief Get the reconnect_post_handover flag */
#define appSmGetReconnectPostHandover() SmGetTaskData()->handover_state.reconnect_post_handover

/*! \brief Set the marshalled flag to TRUE/FALSE */
#define appSmSetMarshalledState(value) do { SmGetTaskData()->apps_sm_marshalled = (value); } while(0)
/*! \brief Get the marshalled flag */
#define appSmGetMarshalledState() SmGetTaskData()->apps_sm_marshalled

/*! \brief Set the continue link loss reconnection post handover flag */
#define appSmSetContinueLinkLossReconnectPostHandover(value) do { SmGetTaskData()->handover_state.continue_link_loss_reconnection_post_handover = (value); } while(0)
/*! \brief Get the continue link loss reconnection post handover flag */
#define appSmGetContinueLinkLossReconnectPostHandover() SmGetTaskData()->handover_state.continue_link_loss_reconnection_post_handover
...
/*! \brief Get peer find role start lock. */
#define appSmPfrStartLockGet() (SmGetTaskData()->pfr_start_lock)
/*! \brief Set peer find role start lock. TRUE to lock till the peer find role starts,
 *         FALSE otherwise.
 */
#define appSmPfrStartLockSet(lock_val) appSmPfrStartLockGet() = lock_val
/*! \brief Clear the peer find role start lock */
#define appSmPfrStartLockClear() appSmPfrStartLockGet() = FALSE
...
/*! Check if reconnection should be restarted after role selection (if we are primary) */
#define appSmRestartReconnectionAfterRoleSelection() (SmGetTaskData()->restart_reconnection_after_role_selection)

/*! Set whether reconnection should be restarted after role selection */
#define appSmSetRestartReconnectionAfterRoleSelection(_restart) (SmGetTaskData()->restart_reconnection_after_role_selection = (_restart))

/*! @brief Query if ANC was enabled when the earbud was taken out of the ear.
    @return TRUE if ANC was enabled when the earbud was taken out of the ear, otherwise FALSE. */
bool appSmGetANCStateWhenOutOfEar(void);

/*! \brief Record the state of ANC when the earbud was taken out of the ear.
    \param ANC_state  state when the earbud was taken out of the ear.
*/
...
/*! \brief Determine if an Media restart timer is pending. */
...
/*! \brief Enable multipoint. */
void appSmEnableMultipoint(void);
/*! \brief Disable multipoint. */
...
/*! \brief Enable/disable entry to upgrade mode when entering the case

    \param enableDfuMode Enable or disable this mode
...
/*! \brief Determine if Topology should be kept alive for DFU
