...
/*! \brief Enum of the different Earbud primary messages we may have. */
enum earbud_primary_rules_messages
{
    ...
    CONN_RULES_HANDSET_CONNECT,

    /*! Start all handset disconnection */
    CONN_RULES_ALL_HANDSETS_DISCONNECT,

    /*! Start least recently used handset disconnection. */
    CONN_RULES_LRU_HANDSET_DISCONNECT,

    /*! Start timer to pause MEDIA streaming. */
    CONN_RULES_MEDIA_PAUSE,
    ...
    /*! Cancel running timers for pausing media */
    CONN_RULES_MEDIA_TIMEOUT_CANCEL,
    ...
    /*! Control the playback enabled state for audio tone and prompt indications. */
    CONN_RULES_AUDIO_UI_IND_CONTROL,


#ifdef INCLUDE_SENSOR_PROFILE
    /*! Disable spatial audio if put in case. */
    CONN_RULES_SPATIAL_AUDIO_DISABLE,
#endif
...
typedef enum
{
    RULE_CONNECT_REASON_USER,       /*!< User initiated connection */
    RULE_CONNECT_REASON_LINK_LOSS,  /*!< Link loss recovery initiated connection */
    RULE_CONNECT_REASON_ROLE_SWITCH,/*!< Switched role to primary */
} ruleConnectReason;
...
typedef struct
{
    /*! Reason that caused the handset connect rule to run. */
    ruleConnectReason reason;
} CONN_RULES_HANDSET_CONNECT_T;
...
/*! \brief Definition of #CONN_RULES_AUDIO_UI_IND_CONTROL message. */
typedef struct
{
    /*! TRUE enable UI Tones and prompts playback, FALSE disable UI Tones and prompts playback. */
    bool enable_playback;
} CONN_RULES_AUDIO_UI_IND_CONTROL_T;
...
#define RULE_EVENT_HANDSET_LINK_LOSS             (1ULL << 3)     /*!< Handset link loss */
#define RULE_EVENT_PEER_CONNECTED                (1ULL << 4)     /*!< Peer connected */
#define RULE_EVENT_PEER_DISCONNECTED             (1ULL << 5)     /*!< Peer disconnected */
#define RULE_EVENT_PEER_UPDATE_LINKKEYS          (1ULL << 6)    /*!< Linkey for handset updated, potentially need to forward to peer */

#define RULE_EVENT_IN_EAR                        (1ULL << 7)    /*!< Earbud put in ear. */
#define RULE_EVENT_OUT_EAR                       (1ULL << 8)    /*!< Earbud taken out of ear. */
#define RULE_EVENT_IN_CASE                       (1ULL << 9)    /*!< Earbud put in the case. */
#define RULE_EVENT_OUT_CASE                      (1ULL << 10)    /*!< Earbud taken out of case. */

#define RULE_EVENT_PEER_IN_CASE                  (1ULL << 11)    /*!< Peer put in the case */
#define RULE_EVENT_PEER_OUT_CASE                 (1ULL << 12)    /*!< Peer taken out of the case */
#define RULE_EVENT_PEER_IN_EAR                   (1ULL << 13)    /*!< Peer put in the ear */
#define RULE_EVENT_PEER_OUT_EAR                  (1ULL << 14)    /*!< Peer taken out of ear */

#define RULE_EVENT_SCO_ACTIVE                    (1ULL << 15)    /*!< SCO call active */

#define RULE_EVENT_CHECK_DFU                     (1ULL << 16)    /*!< Check whether upgrades should be allowed */

#define RULE_EVENT_ROLE_SWITCH                   (1ULL << 17)    /*!< Role switch occured */

#define RULE_EVENT_A2DP_ACTIVE                   (1ULL << 18)    /*!< A2DP active */
#define RULE_EVENT_A2DP_INACTIVE                 (1ULL << 19)    /*!< A2DP inactive */

#define RULE_EVENT_CONNECT_HANDSET_USER          (1ULL << 20)    /*!< User requested handset connection */
#define RULE_EVENT_DISCONNECT_LRU_HANDSET_USER   (1Ull << 21)    /*!< User requested LRU handset disconnection */
#define RULE_EVENT_DISCONNECT_ALL_HANDSETS_USER  (1ULL << 22)    /*!< User requested all handset disconnection */

#define RULE_EVENT_CASE_LID_OPEN                 (1ULL << 23)    /*!< Case lid opened */
...
/*! \brief Initialise the connection rules module. 
	\param init_task The task that is to be initialised
