...
static void appHandleCommonInitCfm(void)
{
    ...
    AppInit_CompleteInitialisation();
}
...
static void appHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case SYSTEM_STATE_STATE_CHANGE:
            ...
            {
                DEBUG_LOG_INFO("appSmHandleMessage MESSAGE_FROM_HOST");
                appSmHandleHostMessagesForProductionTest(message);
            }
            break;
#endif
...
#ifdef PRODUCTION_TEST_MODE
	MessageHostCommsTask(appGetAppTask());
#endif
