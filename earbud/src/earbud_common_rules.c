...
DEFINE_RULE(ruleInEarAudioUiIndEnable);
DEFINE_RULE(ruleInCaseAudioUiIndDisable);
...
const rule_entry_t Common_rules_set[] =
{
    ...
    RULE(RULE_EVENT_IN_EAR,                     ruleInEarAudioUiIndEnable,          CONN_RULES_AUDIO_UI_IND_CONTROL),
    RULE(RULE_EVENT_IN_CASE,                    ruleInCaseAudioUiIndDisable,        CONN_RULES_AUDIO_UI_IND_CONTROL),
    /*! \} */
};
...
static rule_action_t ruleInEarAudioUiIndEnable(void)
{
    bool isEitherEarbudInEar = TRUE;
    COMMON_RULE_LOG("ruleInEarAudioUiIndEnable");
    return RULE_ACTION_RUN_PARAM(isEitherEarbudInEar);
}

static rule_action_t ruleInCaseAudioUiIndDisable(void)
{
    bool isEarbudInEar = FALSE;
    COMMON_RULE_LOG("ruleInCaseAudioUiIndDisable");
    return RULE_ACTION_RUN_PARAM(isEarbudInEar);
}
