...
#include "license_override.h"
    ...
    static const pcm_callbacks_t Peripherals_Lis25baCallbacks =
    {
        /*! \brief Returns PCM interface settings for this device */
        .AudioPcmCommonGetPcmInterfaceSetting = Peripherals_Lis25baGetPcmInterfaceSettings,
        /*! \brief Initialize I2C communication interface for this device */
        .AudioPcmCommonInitializeI2cInterface = Peripherals_Lis25baInitializeI2cInterface,
        /*! \brief Enable the device by writing I2C commands */
        .AudioPcmCommonEnableDevice = Peripherals_Lis25baEnableDevice,
        /*! \brief Disable the device by writing I2C commands */
        .AudioPcmCommonDisableDevice = Peripherals_Lis25baDisableDevice,
    };

    static const pcm_registry_per_user_t Peripherals_Lis25baRegistry =
    {
        .user = pcm_user,
        .callbacks = &Peripherals_Lis25baCallbacks,
    };
#endif
...
static bool appInit_LicenseCheck(Task init_task)
{
    ...
    if (FeatureVerifyLicense(APTX_CLASSIC_MONO))
        DEBUG_LOG_VERBOSE("appInit_LicenseCheck: aptX Classic Mono is licensed, aptX TWS+ A2DP CODEC is enabled");
    else
        DEBUG_LOG_WARN("appInit_LicenseCheck: aptX Classic Mono not licensed, aptX TWS+ A2DP CODEC is disabled");
    ...
    if (FeatureVerifyLicense(APTX_ADAPTIVE_DECODE) || FeatureVerifyLicense(APTX_ADAPTIVE_MONO_DECODE))
        DEBUG_LOG_VERBOSE("appInit_LicenseCheck: aptx Adaptive Decode is licensed");
    else
        DEBUG_LOG_VERBOSE("appInit_LicenseCheck: aptx Adaptive Decode is not licensed");
#endif
...
#endif
#if defined(INCLUDE_LE_AUDIO_UNICAST) || defined(INCLUDE_LE_AUDIO_BROADCAST)
    if (FeatureVerifyLicense(LC3_EPC_EARBUD))
    ...
    if (FeatureVerifyLicense(CVC_EARBUD_3MIC_IE))
        DEBUG_LOG_VERBOSE("appInit_LicenseCheck: cVc 3Mic is licensed");
    else
        DEBUG_LOG_WARN("appInit_LicenseCheck: cVc 3Mic is not licensed");
#endif
...
static bool appInit_DeviceDbSerialiser(Task init_task)
{
    ...
    DeviceList_Init(appConfigMaxDevicesSupported() + appConfigMaxDeviceInstancesUsedForPairing());

    /* Set maximum supported BT devices */
    BtDevice_SetMaxTrustedDevices(appConfigMaxDevicesSupported() + appConfigMaxDeviceInstancesUsedForPairing());
...
static const InputActionMessage_t* appInit_GetInputActions(uint16* input_actions_dim)
{
    ...
    if (appConfigIsRight())
    {
        DEBUG_LOG_VERBOSE("appInit_GetInputActions voice_assistant_message_group");
...
static bool earbud_CaseDfuAppRegister(Task init_task)
{
    UNUSED(init_task);
    DEBUG_LOG_VERBOSE("earbud_CaseDfuAppRegister");
    DfuCase_ClientRegister(SmGetTask());
...
static bool appInit_SoCInit(Task init_task)
{
    ...
    Soc_Init();
    Soc_SetReportingLimits(appSocBatteryLow(), appSocBatteryFull());
    return TRUE;
}
...
static const system_state_step_t appInitTable[] =
{
    ...
    {AppInitBt_ConfigInit,         INIT_READ_LOCAL_NAME_CFM, AppInitBt_HandleReadLocalBdAddrCfm},
#endif
...
#endif
#ifdef INCLUDE_SENSOR_PROFILE
    {SensorProfile_Init,    SENSOR_PROFILE_INIT_CFM, NULL},
#endif
#if defined INCLUDE_SPATIAL_DATA || defined INCLUDE_ACCESSORY
...
#ifdef INCLUDE_SPATIAL_AUDIO
    {EarbudSpatialAudio_Init,  0, NULL},
#endif
    ...
    {TwsTopology_Init,      TWS_TOPOLOGY_INIT_COMPLETED, NULL},
    ...
    {FitTest_ContinuousInit,0,NULL},
#endif

#ifdef ENABLE_WIND_DETECT
    {WindDetect_Init,0,NULL},
#endif

#ifdef ENABLE_SELF_SPEECH
    {SelfSpeech_Init,0,NULL},
#endif
...
#endif
#if defined(INCLUDE_UI_USER_CONFIG) && defined(INCLUDE_GAIA)
    {UiUserConfigGaiaPlugin_Init,0,NULL},
#endif
...
#endif
#ifdef INCLUDE_DFU_CASE
    {DfuCase_EarlyInit, 0, NULL},
    {earbud_CaseDfuAppRegister, 0, NULL},
    {DfuCase_Init,  0, NULL},
...
#endif
#ifdef INCLUDE_GBSS
    {GenericBroadcastScanServer_Init, 0, NULL},
#endif

#ifdef INCLUDE_QSS
    {QssManager_Init, 0, NULL},
#endif

    {AppInitBt_RegisterForBtMessages, 0, NULL},
};
...
static void appInit_CompleteUiInitialisation(void)
{
    ...
    config_table = AppUi_GetConfigTable(&size);
    Ui_SetConfigurationTable(config_table, size);

#ifdef INCLUDE_UI_USER_CONFIG
    const ui_user_config_gesture_id_map_t * gesture_map = AppUi_GetGestureMap(&size);
    UiUserConfig_RegisterGestureIdMap(gesture_map, (uint8)size);

#ifdef INCLUDE_VOICE_UI
    const ui_user_config_composite_gesture_t * composite_va_gesture = AppUi_GetCompositeGestureMap(&size);
    UiUserConfig_RegisterCompositeGestureMap(composite_va_gesture, (uint8)size);
#endif

    UiUserConfigGaiaPlugin_SetNumberOfTouchpads(2);
#endif
    ...
    touch_events = AppUi_GetCapsenseEventTable(&size);
    TouchSensorClientRegister(LogicalInputSwitch_GetTask(), size, touch_events);
#endif
...
#ifdef INCLUDE_LIS25BA_ACCELEROMETER
    AudioPcmCommonRegisterUser(&Peripherals_Lis25baRegistry);
#endif
}
...
#ifdef HAVE_RDP_UI
    UiTones_SetTonePlaybackEnabled(appPhyStateGetState() != PHY_STATE_IN_CASE);
    UiPrompts_SetPromptPlaybackEnabled(appPhyStateGetState() != PHY_STATE_IN_CASE);
#endif
...
#ifdef INCLUDE_LE_AUDIO_UNICAST
    /* Set the GAP appearance of the device to "Earbud" */
    GattServerGap_SetAppearanceValue(GATT_SERVER_GAP_BT_APPEARANCE_EARBUD);
#endif
