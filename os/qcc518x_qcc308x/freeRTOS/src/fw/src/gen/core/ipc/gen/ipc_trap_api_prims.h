...
typedef struct IPC_CHARGER_COMMS_UART_DIAGNOSTICS {
    IPC_HEADER header;
    charger_comms_diagnostics * diagnostics;
} IPC_CHARGER_COMMS_UART_DIAGNOSTICS;
...
typedef struct IPC_DEBUG_PARTITION_SAVE_MINI_CORE_DUMP {
    IPC_HEADER header;
} IPC_DEBUG_PARTITION_SAVE_MINI_CORE_DUMP;
...
typedef struct IPC_VM_GET_RESET_DATA {
    IPC_HEADER header;
    vm_reset_data_t * data;
} IPC_VM_GET_RESET_DATA;

typedef struct IPC_VM_SET_LICENSE_KEY {
    IPC_HEADER header;
    const uint8 * key;
    uint16 size;
} IPC_VM_SET_LICENSE_KEY;

typedef struct IPC_UTIL_RAW_ENTROPY {
    IPC_HEADER header;
    uint8 * data;
    uint16 length;
} IPC_UTIL_RAW_ENTROPY;

typedef struct IPC_VM_GET_HW_VERSION {
    IPC_HEADER header;
} IPC_VM_GET_HW_VERSION;

typedef struct IPC_PSSTORES_RSP {
    IPC_HEADER header;
    PsStores ret;
} IPC_PSSTORES_RSP;
typedef struct IPC_UTIL_RAW_ENTROPY_RESULT_RSP {
    IPC_HEADER header;
    util_raw_entropy_result ret;
} IPC_UTIL_RAW_ENTROPY_RESULT_RSP;
...
typedef struct IPC_STREAM_HCI_TAP_SOURCE {
    IPC_HEADER header;
} IPC_STREAM_HCI_TAP_SOURCE;
...
#if TRAPSET_INFO
typedef struct IPC_VM_INFO_GET_AUDIO_MEM_USAGE {
    IPC_HEADER header;
    uint16 proc_id;
    audio_get_mem_usage * info;
} IPC_VM_INFO_GET_AUDIO_MEM_USAGE;

typedef struct IPC_VM_INFO_GET_FIRMWARE_MEM_USAGE {
    IPC_HEADER header;
    uint16 * percentage;
} IPC_VM_INFO_GET_FIRMWARE_MEM_USAGE;
...
typedef struct IPC_STREAM_RA_PARTITION_SOURCE {
    IPC_HEADER header;
    raPartition_handle * handle;
    uint32 offset;
    uint32 length;
} IPC_STREAM_RA_PARTITION_SOURCE;
...
typedef struct IPC_CRYPTO_AES128_DERIVED {
    IPC_HEADER header;
    bool encrypt;
    const uint8 * seed;
    const uint8 * source_data;
...
typedef union IPC_RSP {
    ...
    IPC_UTIL_RAW_ENTROPY_RESULT_RSP ipc_util_raw_entropy_result_rsp;
