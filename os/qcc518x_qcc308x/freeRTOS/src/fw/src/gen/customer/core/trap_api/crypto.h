...
/**
 *  \brief Encrypt and decrypt using AES128 with a derived key.
 *           The key is generated internally from the efuse key and the provided
 *  seed.
 *             This trap is only supported on encrypted devices and will return
 *  FALSE otherwise.
 *           
 *  \param encrypt TRUE for an encryption or FALSE for decryption.
 *  \param seed Pointer to the 128-bit (16 bytes) seed use to create the derived key.
 *  \param source_data Pointer to the data to feed into the algorithm.
 *                   Should be multiple of 128 bit (16 bytes).
 *  \param dest_data Pointer to the memory where the output data from the algorithm is to be written.
 *  \param length Length of the data in bytes.
 *  \return TRUE - Operation completed successfully, FALSE - Some input parameters are
 *  invalid.
 * 
 * \ingroup trapset_crypto
 */
bool CryptoAes128Derived(bool encrypt, const uint8 * seed, const uint8 * source_data, uint8 * dest_data, uint16 length);
