...
/**
 *  \brief Get the memory usage of the specified Audio processor. 
 *         
 *  \param proc_id Identifier of processor to query, 0 or 1 
 *  \param info Pointer to structure of memory usage parameters
 *             to write the information if successful.
 *             If NULL the function return FALSE.
 *             
 *  \return TRUE on success, else FALSE. 
 * 
 * \ingroup trapset_info
 */
bool VmInfoGetAudioMemUsage(uint16 proc_id, audio_get_mem_usage * info);

/**
 *  \brief Get the memory usage as a percentage. 
 *         
 *  \param percentage The percentage of memory already used. 
 *  \return TRUE on success, else FALSE. 
 * 
 * \ingroup trapset_info
 */
bool VmInfoGetFirmwareMemUsage(uint16 * percentage);
#endif /* TRAPSET_INFO */
#if TRAPSET_CORE
...
/**
 *  \brief This trap returns information on the cause of the last system reset.
 *            The format of the data returned is chip specific.
 *      
 *  \return TRUE if the data is valid. Should be ignored if FALSE
 * 
 * \ingroup trapset_core
 */
bool VmGetResetData(vm_reset_data_t * data);

/**
 *  \brief Override the FeatureLicenceKey MIB with the key provided.
 *             The execution is restricted to a single attempt per boot cycle.
 *  \param key The licence key to override FeatureLicenseKey MIB with.
 *  \param size Size of the key.
 *  \return TRUE on success, else FALSE.
 * 
 * \ingroup trapset_core
 */
bool VmSetLicenseKey(const uint8 * key, uint16 size);

/**
 *  \brief Read the hardware version.
 *  \return           Return the chip product code as a binary coded decimal number 
 *           using four nibbles or zero if not available.
 * 
 * \ingroup trapset_core
 */
uint32 VmGetHwVersion(void );
#endif /* TRAPSET_CORE */
...
/**
 *  \brief Retrieves public device address or static random address for a given resolvable
 *  random address.
 *                      Note: To be used only on an established link.
 *  \param random_addr Random address. 
 *  \param public_addr If the public device address or static random address is found it will be
 *  returned to the location pointed at by this value. 
 *  \return TRUE if public device address or static random address was found for a given
 *  resolvable random address, FALSE otherwise.
 * 
 * \ingroup trapset_bluestack
 */
 ...
 *  unmarshalling
 * and not because of a DM Sync connection initiation from the application. In such
 ...
 * This function allows the application to override local Bluetooth device address.
