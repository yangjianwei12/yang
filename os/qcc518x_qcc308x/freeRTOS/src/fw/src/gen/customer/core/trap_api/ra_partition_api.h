...
/**
 *  \brief Return a source with the contents of the specified data.
 *         If the length is greater than (Partition size - offset)
 *         then, the length is truncated at the end of the partition.
 *           
 *  \param handle Pointer to the handle obtained by RaPartition[Named]Open() that contains the
 *  stream data
 *  \param offset Offset into the partition of the stream data, in bytes
 *  \param length Length of the stream data, in bytes
 *  \return 0 if the parameters are invalid.
 * 
 * \ingroup trapset_ra_partition
 */
Source StreamRaPartitionSource(raPartition_handle * handle, uint32 offset, uint32 length);
