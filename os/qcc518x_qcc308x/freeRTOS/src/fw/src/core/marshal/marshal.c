...
static bool write_pointer_index(marshal_base_t *base, mob_t *referred)
{
    ...
    {
        assert_fn(mobs_has_object(&base->shared_member_set, referred, &referred_index));
        referred_index += mobs_size(&base->object_set);
    }
#else
    {
        assert_fn(mobs_has_object(&base->object_set, referred, &referred_index));
    }
#endif
...
static bool count_remaining_values(mobs_t *set, mob_index_t index, mob_t *object)
{
    ...
    assert_fn(object_tree_traverse(base, object, &callbacks));
    ...
    switch (m->state)
    {
        case REGISTER_POINTERS:
        {
            ...
            if (mobs_push(set, &object))
            {
                ...
                assert_fn(mobs_iterate(set, m->next_values_index, register_pointer_members));
                ...
                if (m->base.has_shared_objects)
                {
                    ...
                    member set. */
                    assert_fn(mobs_iterate(set, m->next_values_index, register_shared_members));
...
size_t marshal_remaining(marshaller_t m)
{
    ...
    switch (m->state)
    {
        ...
        case MARSHAL_VALUES:
            assert_fn(mobs_iterate(set, m->next_values_index, count_remaining_values));
