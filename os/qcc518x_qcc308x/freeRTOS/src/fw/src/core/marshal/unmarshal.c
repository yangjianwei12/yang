...
static bool unmarshal_pointer_indexes(mobs_t *set, mob_index_t index, mob_t *object)
{
    ...
    if (read_uint8(base, &referred_index))
    {
        ...
        if (referred_index >= size)
        {
            ...
            assert_fn(mobs_get_object(&base->shared_member_set, new_index, &referred));
        }
        else
        {
            assert_fn(mobs_get_object(&base->object_set, referred_index, &referred));
        }
#else
...
static bool unmarshal_values(unmarshal_context_t *u)
{
    ...
    {
        assert_fn(mobs_pop(pointer_member_mobs, NULL));
    }
    ...
    switch (u->state)
    {
        case UNMARSHAL_VALUES:
        {
            ...
            if (u->base.has_shared_objects)
            {
                ...
                assert_fn(mobs_iterate(&u->base.object_set, u->root_index, register_shared_members));
            }
#endif
        ...
        case UNMARSHAL_POINTER_INDEXES:
        {
            ...
            {
                mob_t object;
                assert_fn(mobs_get_object(&u->base.object_set, u->root_index, &object));
                *addr = object.address;
                *type = object.type;
