...
/**
 * Return an 8-bit pointer to read data from a buffer at the TAIL !!!!
 *
 * The buffer is mapped for read access at the output location
 * (\c tail) and a pointer is returned.  The view of the data
 * is 8-bit-wide.
 *
 * WARNING: you are most likely doing something wrong if you are
 * using this function.  Please check buf_raw_read_map_8bit
 *
 * The only use case for this function is the HCI TAP Stream that
 * copies the data after it has been DMA to BT subsystem
 * and just before freeing it (i.e. advancing the tail to outdex)
 */
extern const uint8 *buf_raw_read_map_8bit_tail(const BUFFER *buf);
