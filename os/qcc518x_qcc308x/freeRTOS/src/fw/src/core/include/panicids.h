...
#include "nfc/nfc/nfc_panicids.h"
    ...
    */
    ...
    /**
     * When handling the mini coredump data in the Debug Partition module, the
     * context pointer is invalid.
     */
    PANIC_DEBUG_PARTITION_MINICOREDUMP_BAD_CONTEXT = 0x10a,

    /**
     * Bad CCP received in the Debug Partition module
     */
    PANIC_DEBUG_PARTITION_BAD_CCP = 0x10b,

    /**
     * Charger termination on IMON has had an ADC reading not complete in time.
     */
    PANIC_CHARGER_TERM_IMON_ADC_READING_WATCHDOG = 0x10c,

    /**
     * Testtunnel packet too big to be sent over wireless isp
     */
    PANIC_TTPKT_TOO_BIG = 0x10d,

    /**
     * Inbound data clear event not received
     */
    PANIC_INBOUND_CLEAR_EVENT_TIMEOUT = 0x10e,

    /**
     * Device being marshaled is not found
     */
    PANIC_MARSHAL_DEVICE_NOT_FOUND = 0x10f,

    /**
     * Device being unmarshaled is not found
     */
    PANIC_UNMARSHAL_DEVICE_NOT_FOUND = 0x110,

    /**
     * Write verification failed on flash
     */
    PANIC_SIFLASH_WRITE_VERIFY_FAIL = 0x111,

    /**
     * Siflash erase or write API called with an address in the boot sector. 
     */
    PANIC_SIFLASH_CMD_ADDR_SECTOR_ZERO = 0x112,

    /**
     * Siflash command context checksum failed. 
     */
    PANIC_SIFLASH_CONTEXT_CORRUPTED = 0x113,
