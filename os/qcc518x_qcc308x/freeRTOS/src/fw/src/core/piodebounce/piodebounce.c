...
#if PIODEBOUNCE_NUMBER_OF_GROUPS > 2
static void piodebounce_kick_2(void);
#endif
static void debounce_callback(uint16 group, uint16 bank);
                        ...
                        piodebounce_cb callback)
{
    ...
    pio_debounce[group].priority = priority;
...
#if PIODEBOUNCE_NUMBER_OF_GROUPS == 3
static void piodebounce_kick_2(void)
{
    piodebounce_kick(2);
}
...
#elif PIODEBOUNCE_NUMBER_OF_GROUPS == 2
                            group ? piodebounce_kick_1 : piodebounce_kick_0
#else
#error Unsupported number of Pio debounce groups
#endif
                        );
    }
}
...
void piodebounce_kick(uint16 group)
{
    ...
    switch (pio_debounce[group].nreads)
    {
        case 0:
            ...
            for (bank = 0; bank < NUMBER_OF_PIO_BANKS; bank++)
            {
                if (pio_debounce[group].priority)
                {
                    debounce_callback(group, bank);
                }
                else
                {
...
/* Function to process debounce output of a pio group and 
 * inform the clients about the new output. 
 * NOTE: This must be called in interrupt context.
 */
static void debounce_callback(uint16 group, uint16 bank)
{
    ...
    {
        pio_debounce[group].callback(group,
                                     bank,
                                     output,
                                     get_time());
    }

    L4_DBG_MSG3("Piodebounce: PIOs in group %d bank %d changed, finished call back at %u usec",
                group, bank, get_time());
}
