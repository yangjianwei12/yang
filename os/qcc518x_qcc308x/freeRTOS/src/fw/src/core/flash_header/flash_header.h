 ...
 * @param reread_nonce clear cache but reread the nonce
 */
void flash_header_clear_cached_data_for_other_image(uint8 qspi_device, bool reread_nonce);
...
/**
 * Reads the given key as applied to the given section_id
 *
 * @param qspi_device QSPI device from where the data is to be read.
 * @param boot_image TRUE to read from the boot image, FALSE for
 * the other image
 * @param section_id ID of the section from the flash_image_defs.h file
 * @param section_key FLASH_ELEMENT_ID_ value to read for that section
 * @param value Returns the value of the key
 * @return TRUE if the section information was found and returned
 */
bool flash_header_read_section_key(uint8 qspi_device,
                                   bool boot_image,
                                   uint32 section_id,
                                   uint32 section_key,
                                   uint32 *value);
...
/**
 * Self-authenticase the given section with the given hash by writing into
 * the image header
 *
 * @param [in]  section_id Input image header section for hashing
 * @param [in]  hash Pointer to the hash to write
 * @return TRUE if hash calculated and written, FALSE otherwise.
 */
extern bool flash_header_self_authenticate_section(uint32 section_id, uint32 *hash);
...
/**
 * Returns the pointer to image nonce value for the other image bank.
 *
 * \param qspi_device The SQIF device to return
 * \return Pointer to image nonce value
 */
uint32 *flash_header_get_other_image_nonce(uint8 qspi_device);
