...
typedef struct
{
     ...
     * this structure. */
    tCasualTimerStruct event; /*lint -esym(754, event)*/
...
/*
 * Function implementation since it uses the first parameter twice, which
 * if implemented as a macro would cause C side effects to occur twice.
 */
tTimerId timer_schedule_bg_event_at_alt(
        TIME time_when,
        tTimerEventFunctionAlt TimerEventFunction,
        uint16 iarg,
        void *data_pointer)
{
    return timer_schedule_bg_event_at_between_alt(
                time_when, time_when, TimerEventFunction,
                iarg, data_pointer);
}
 ...
 * if implemented as a macro would cause C side effects to occur twice.
 */
tTimerId timer_schedule_bg_event_in_alt(
        INTERVAL time_when,
        tTimerEventFunctionAlt TimerEventFunction,
        uint16 iarg,
        void *data_pointer)
{
    return timer_schedule_bg_event_in_between_alt(
                time_when, time_when, TimerEventFunction,
                iarg, data_pointer);
}
