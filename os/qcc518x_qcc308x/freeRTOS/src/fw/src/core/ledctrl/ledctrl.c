...
/* Array mapping logical LED id onto allowed PIOs. Pad with FFs */
#define NUM_POSSIBLE_PIOS 5
static const uint8 led_ctrl_pios[NUM_OF_LED_CTRLS][NUM_POSSIBLE_PIOS] =
 { {     6,     18,     36, 58, 0xff},
   { 1,  7,     19,     37, 59  },
   { 2,  8,     20,     38, 60  },
   { 3,     15, 21,     39, 61  },
...
bool ledctrl_led_has_pio(led_id led)
{
    ...
    /* Look up whether a pin is assigned to this controller */
    for (pin_offset = 0; pin_offset < NUM_POSSIBLE_PIOS; pin_offset++)
    {
        pin = led_ctrl_pios[led][pin_offset];
        if (pin == 0xff)
        {
