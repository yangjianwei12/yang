...
.MODULE $interrupt;
   ...
   // counter to support nested blocking/unblocking of interrupts and exceptions
   .VAR block_ie_count = 0;
...
// *****************************************************************************
// MODULE:
//    $block_interrupts_and_exceptions
//
// DESCRIPTION:
//    Block interrupts and exceptions subroutine.
//
// INPUTS:
//    - none
//
...
//
// *****************************************************************************
.MODULE $M.interrupt_and_exceptions.block;
   .CODESEGMENT INTERRUPT_BLOCK_PM;
   .MINIM;

   $_interrupt_and_exceptions_block:
   $_block_interrupts_and_exceptions:
   $interrupt_and_exceptions.block:
   .BlockIntsExcStart:
   ...
   Null = M[$INT_UNBLOCK];
   if NZ jump $_block_interrupts_and_exceptions;

   //Now IRQs are disabled, update the block_ie_count
   r0 = M[$interrupt.block_ie_count];
   r0 = r0 + 1;
   M[$interrupt.block_ie_count] = r0;
...
// *****************************************************************************
// MODULE:
//    $unblock_interrupts_and_exceptions
//
// DESCRIPTION:
//    Unblock interrupts and exceptions subroutine.
//
// INPUTS:
//    - none
//
...
//
// *****************************************************************************
.MODULE $M.interrupt_and_exceptions.unblock;
   .CODESEGMENT INTERRUPT_BLOCK_PM;
   .MINIM;

   $_interrupt_and_exceptions_unblock:
   $_unblock_interrupts_and_exceptions:
   $interrupt_and_exceptions.unblock:
   .UnblockIntsExcStart:
...
/* Debug frame information for both Block and UnblockInterrupt_and_exceptions
 * functions. This is just a blank one because there are no
 * push/pop instructions.
 */
    .section ".debug_frame"
.IEBLK_CIE_Pointer:
    .4byte  .IEBLK_CIE_End-.IEBLK_CIE_Start   /* CIE length */
.IEBLK_CIE_Start:
    .4byte  0xFFFFFFFF  /* CIE_id */
    .byte   0x01        /* CIE version */
    .string ""          /* CIE augmentation */
...
.IEBLK_CIE_End:
    ...
    .4byte  .IEBLK_FDE_End-.IEBLK_FDE_Start       /* FDE length */
.IEBLK_FDE_Start:
    .4byte  .IEBLK_CIE_Pointer                   /* CIE_pointer */
    .4byte  $_block_interrupts_and_exceptions   /* initial_location */
    .4byte  .BlockIntsExcEnd-.BlockIntsExcStart /* address_range */
    .byte   DW_CFA_advance_loc2
    .2byte  .BlockIntsExcEnd-.BlockIntsExcStart
    .byte   DW_CFA_nop
.IEBLK_FDE_End:

    .4byte  .IEUBLK_FDE_End-.IEUBLK_FDE_Start       /* FDE length */
.IEUBLK_FDE_Start:
    .4byte  .IEBLK_CIE_Pointer                   /* CIE_pointer */
    .4byte  $_unblock_interrupts_and_exceptions     /* initial_location */
    .4byte  .UnblockIntsExcEnd-.UnblockIntsExcStart /* address_range */
    .byte   DW_CFA_advance_loc2
    .2byte  .UnblockIntsExcEnd-.UnblockIntsExcStart
    .byte   DW_CFA_nop
.IEUBLK_FDE_End:
    .previous
...
no_shallow_sleep:
    ...
    M[$ALLOW_GOTO_SHALLOW_SLEEP] = Null;
