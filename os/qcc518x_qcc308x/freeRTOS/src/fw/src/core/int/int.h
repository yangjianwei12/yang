...
/**
 * Unblock interrupts and exceptions
 *
 * Unblock all interrupts and exceptions. The caller is expected to call
 * this (very!) shortly after a call to block_interrupts_and_exceptions().
 */
extern void unblock_interrupts_and_exceptions(void);

/**
 * Block interrupts and exceptions
 *
 * Block all interrupts and exceptions. The caller is expected to call
 * unblock_interrupts_and_exceptions() (very!) shortly after calling this function.
 *
 * These two function co-operate to ensure that nested pairs of calls
 * behave as expected; only the final call to unblock_interrupts_and_exceptions()
 * actually unblocks interrupts.
 *
 * These functions are intended for ensuring atomic access to data that
 * is shared between code running at multiple interrupt priority levels.
 */
extern void block_interrupts_and_exceptions(void);
