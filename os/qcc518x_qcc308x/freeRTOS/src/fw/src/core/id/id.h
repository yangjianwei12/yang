...
/* SLT header starts at 0x80 in the image. */
#define SLT_LOCATION_HEAD_PTR (uint32 *)((uint8 *)MAP_SQIF1_CACHED_LOWER + 0x80)
...
/**
 * Get the p1 build identifier as a uint32
 *
 * Each application build has a unique build ID. This function returns
 * that number.
 */
extern uint32 get_p1_build_id_number(void);
