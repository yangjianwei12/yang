...
/**
 * The minimum time for which it's worth P1 doing deep sleep signalling.
 *
 * IPC_SIGNAL_ID_P1_DEEP_SLEEP_MSG takes a few hundred microseconds at most to
 * be sent to P0 and processed. P0 needs to know P1 is willing to deep sleep
 * before P0 will consider whether there is enough time to deep sleep or whether
 * it should shallow sleep with scaled clock and reduced Apps core clock to save
 * power. So we need the minimum time in P1 to be quite short.
 */
#define DEEP_SLEEP_MIN_TIME     (1 * MILLISECOND)
