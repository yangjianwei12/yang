...
uint32 pmalloc_current_bytes_out;
uint32 pmalloc_highest_bytes_out;
#ifdef PMALLOC_RECORD_SNAPSHOT
uint8 *pmalloc_snapshot;
#endif
#endif
...
void pmalloc_configure(pmalloc_pool_config *pool_config, unsigned pool_config_len)
{
    ...
    /* If we're doing pool snapshots, reserve space for the snapshot */
#if defined(PMALLOC_RECORD_USAGE_LEVEL) && defined(PMALLOC_RECORD_SNAPSHOT)
    pmalloc_snapshot = (uint8 *)pools_end;
    pools_end = (pmalloc_pool *)((uint8 *)pools_end + pmalloc_num_pools);
#endif
