                              ...
                              uint16 count, uint16 period, pio_group_priority priority)
{
    ...
    piodebounce_config(group, bank, mask, count, period, priority, pio_handler);
    return 0;
}

Task MessagePioGroupTask(Task task, uint16 group)
{
    L4_DBG_MSG2("PIO trap: MessagePioGroupTask(0x%x, %d)", task, group);
    return trap_api_register_message_group_task(task, group);
}
