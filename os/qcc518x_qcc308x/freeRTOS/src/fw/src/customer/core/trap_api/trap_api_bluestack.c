...
{
    IPC_VM_CIS_CONN_GET_RSSI send_prim;
    IPC_VM_CIS_CONN_GET_RSSI_RSP rsp_prim;
    TP_BD_ADDR_T tpaddr_fw;

    /* Copy the supplied App-space structure into a temporary fw-space
     * structure that's the right shape to go to firmware */
    tpaddr_fw.tp_type = (PHYSICAL_TRANSPORT_T)tpbdaddr->transport;
    tpaddr_fw.addrt.type = tpbdaddr->taddr.type;
    tpaddr_fw.addrt.addr.lap = tpbdaddr->taddr.addr.lap;
    tpaddr_fw.addrt.addr.uap = tpbdaddr->taddr.addr.uap;
    tpaddr_fw.addrt.addr.nap = tpbdaddr->taddr.addr.nap;

    send_prim.tpbdaddr = &tpaddr_fw;
    send_prim.handle = handle;

    ipc_transaction(IPC_SIGNAL_ID_VM_CIS_CONN_GET_RSSI, &send_prim, sizeof(send_prim),
                    IPC_SIGNAL_ID_VM_CIS_CONN_GET_RSSI_RSP, &rsp_prim);
    if (rssi != NULL)
    {
      *rssi = rsp_prim.rssi;
    }
...
{
    IPC_VM_READ_NEXT_SNIFF_CLOCK send_prim;
    IPC_BOOL_RSP rsp_prim;
    TP_BD_ADDR_T tpaddr_fw;
    ...
    tpaddr_fw.addrt.addr.nap = tpaddr->taddr.addr.nap;
    send_prim.tpaddr = &tpaddr_fw;
    send_prim.next_sniff = next_sniff;

    ipc_transaction(IPC_SIGNAL_ID_VM_READ_NEXT_SNIFF_CLOCK, &send_prim, sizeof(send_prim),
                    IPC_SIGNAL_ID_VM_READ_NEXT_SNIFF_CLOCK_RSP, &rsp_prim);
