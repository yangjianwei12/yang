...
typedef enum mdm_prim_tag
{
    ...
    ENUM_MDM_LINK_CREATE_EXT_REQ,
    ENUM_MDM_LE_SET_RELAY_ENABLE_REQ,
    ...
    ENUM_MDM_LE_SET_RELAY_ENABLE_CFM,
    ENUM_MDM_LE_RELAY_ENABLE_IND,
...
/**
 * Type definition used to specify mirror properties type.
 */
typedef uint16_t mirror_properties_t;

#define MIRROR_PROPERTIES_ENABLE_NONE   ((mirror_properties_t)0x0000)
#define MIRROR_PROPERTIES_ENABLE_TXNR   ((mirror_properties_t)0x0001)


/**
 * LE Relay enable values
 */
#define LE_RELAY_DISABLE                ((uint8_t) 0x00)
#define LE_RELAY_ENABLE                 ((uint8_t) 0x01)

/**
 * LE Relay direction values
 */
#define LE_RELAY_DIR_OWNER_TO_MIRROR    ((uint8_t) 0x00)
#define LE_RELAY_DIR_MIRROR_TO_OWNER    ((uint8_t) 0x01)
...
#define MDM_LINK_CREATE_EXT_REQ   ((mdm_prim_t)ENUM_MDM_LINK_CREATE_EXT_REQ)
#define MDM_LE_SET_RELAY_ENABLE_REQ     ((mdm_prim_t)ENUM_MDM_LE_SET_RELAY_ENABLE_REQ)
...
#define MDM_LE_SET_RELAY_ENABLE_CFM ((mdm_prim_t)ENUM_MDM_LE_SET_RELAY_ENABLE_CFM)
#define MDM_LE_RELAY_ENABLE_IND  ((mdm_prim_t)ENUM_MDM_LE_RELAY_ENABLE_IND)
...
/*! \brief Confirmation of mirror eSCO link creation
    ...
    (MDM_LINK_CREATE_REQ / MDM_LINK_CREATE_EXT_REQ) on a primary device.
...
/*! \brief Setup a mirror ACL or eSCO link on a primary device
    ...
    This prim also allows the application to set the mirroring properties 
    if any.
...
typedef struct
{
    mdm_prim_t          type;              /*!< Always MDM_LINK_CREATE_EXT_REQ */
    phandle_t           phandle;           /*!< Destination phandle */
    TP_BD_ADDR_T        mirror_bd_addr;    /*!< Bluetooth address of remote device which is being mirrored */
    TP_BD_ADDR_T        secondary_bd_addr; /*!< Bluetooth address of secondary device */
    link_type_t         link_type;         /*!< Type of mirror link, either ACL or eSCO */
    mirror_properties_t mirror_properties; /*!< Mirroring properties */
} MDM_LINK_CREATE_EXT_REQ_T;
...
/*! \brief Enable relay for CIS or BIS

    This primitive is used by the primary earbud to enable or disable the Connection 
    Relay or Broadcast Relay feature on a given CIS or BIS respectively. Using 
    this command, the local device either requests the remote device to take 
    the Mirror role on a CIS or BIS that the local device owns, or requests the 
    remote device that the local device takes the Mirror role on a CIS or BIS 
    that the remote device owns.

    Primary device shall receive MDM_LE_SET_RELAY_ENABLE_CFM once the CIS relay
    is done.

    Secondary device shall receive MDM_LE_RELAY_ENABLE_IND on successful enabling 
    of relay CIS.
*/
typedef struct
{
    mdm_prim_t              type;           /*!< Always MDM_LE_SET_RELAY_ENABLE_REQ */
    phandle_t               phandle;        /*!< Destination phandle */
    uint8_t                 enable;         /*!< Enable or disable relay, choose 
                                                 values from LE_RELAY_* */
    TP_BD_ADDR_T            buddy_bd_addr;  /*!< Secondary device's address */
    hci_connection_handle_t cis_bis_handle; /*!< CIS or BIS handle to be relayed */
    uint8_t                 relay_nse;      /*!< Number of relay Sub-events, this is 
                                                 reserved for future use, Set the 
                                                 value to 0xFF.*/
    uint8_t                 relay_dir;      /*!< Relay direction, choose values from 
                                                 LE_RELAY_DIR_* */
} MDM_LE_SET_RELAY_ENABLE_REQ_T;

/*! \brief Event received on LE relay enable completion

    This prim provides the status of LE relay enable request.
*/
typedef struct
{
    mdm_prim_t              type;           /*!< Always MDM_LE_SET_RELAY_ENABLE_CFM */
    phandle_t               phandle;        /*!< Destination phandle */ 
    hci_connection_handle_t cis_bis_handle; /*!< CIS or BIS handle to be relayed */
    uint8_t                 status;         /*!< Status */
    uint8_t                 enable;         /*!< Relay enable/disable */
} MDM_LE_SET_RELAY_ENABLE_CFM_T;

typedef MDM_LE_SET_RELAY_ENABLE_CFM_T MDM_LE_RELAY_ENABLE_IND_T;
...
typedef union
{
    ...
    MDM_LINK_CREATE_EXT_REQ_T         mdm_link_create_ext_req;
    MDM_LE_SET_RELAY_ENABLE_REQ_T     mdm_le_set_relay_enable_req;
    ...
    MDM_LE_SET_RELAY_ENABLE_CFM_T     mdm_le_set_relay_enable_cfm;
    MDM_LE_RELAY_ENABLE_IND_T         mdm_le_relay_enable_ind;
