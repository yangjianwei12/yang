...
typedef enum
{
    ...
    STREAM_PCM_24BITS_IN_32BIT_SLOT = 0x011a,              /*!< The Key when set, overrides both STREAM_PCM_SAMPLE_FORMAT and STREAM_AUDIO_SAMPLE_SIZE and enables 24 bit in 32 bit mode.
                                                                0: Default behavior dictated by STREAM_AUDIO_SAMPLE_SIZE or STREAM_PCM_SAMPLE_FORMAT
                                                                1: 24 Bits in 32-bit slot. */
    ...
    STREAM_HCI_TAP_FILTER_CMD_EVT = 0x1500,                /*! This key enables or disables the logging of HCI Commands and HCI
                                                                Events. */
    STREAM_HCI_TAP_FILTER_L2CAP_LENGTH = 0x1501,           /*! This key controls the maximum length of L2CAP packets that will 
                                                                be loggged. Packets larger than that will be discarded. */
    STREAM_HCI_TAP_FILTER_RESET = 0x1502                   /*! Reset to the filter log everything */    
} stream_config_key;
...
/*!
  @brief Various values for the STREAM_HCI_TAP_FILTER_CMD_EVT key to control 
  which HCI commands and events are logged.
*/
typedef enum
{
    STREAM_HCI_TAP_FILTER_CMD_EVT_NONE  = 0,
    STREAM_HCI_TAP_FILTER_CMD_EVT_ALL = 1,
    STREAM_HCI_TAP_FILTER_CMD_EVT_LOW_VOLUME = 2
} stream_hci_tap_filter_cmd_evt_t;

/*!
  @brief Various values for the STREAM_HCI_TAP_FILTER_ACL_LENGTH key to 
  control the ACL packet logged.
*/

/* All ACL packets are recorded (this should be a #define rather than an enum). */
#define STREAM_HCI_TAP_FILTER_ACL_LENGTH_ALL ((uint16) 0)

/*!
  @brief Various values for the STREAM_HCI_TAP_FILTER_RESET key to 
  control HCI TAP Stream
*/
/* Reset to the default filter*/
#define STREAM_HCI_TAP_FILTER_RESET_DEFAULT ((uint16) 0)
