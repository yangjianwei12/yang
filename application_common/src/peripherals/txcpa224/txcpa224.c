...
/*! \brief Set up I2C PIO function */
static void txcpa224_SetUpI2C(const proximityConfig *config, bool set_io, bool set_fn)
{
    i2c_pio_func i2c_pios[] = {{config->pios.i2c_scl, BITSERIAL_1_CLOCK_OUT},
                    {config->pios.i2c_scl, BITSERIAL_1_CLOCK_IN},
                    {config->pios.i2c_sda, BITSERIAL_1_DATA_OUT},
                    {config->pios.i2c_sda, BITSERIAL_1_DATA_IN}};

    PioCommonSetUpI2C(i2c_pios, ARRAY_DIM(i2c_pios), set_io, set_fn);
...
static bool txcpa224_ReadRegister(uint8 reg,  uint8 *value)
{
    ...
    txcpa224_SetUpI2C(config, FALSE, TRUE);
...
static void txcpa224_Enable(const proximityConfig *config)
{
    ...
    txcpa224_SetUpI2C(config, TRUE, FALSE);
...
static void txcpa224_sensorInitialization(proximityTaskData *prox)
{
    ...
    DEBUG_LOG_VERBOSE("txcpa224_sensorInitialization");
...
bool appProximityClientRegister(Task task)
{
    ...
    if (NULL == prox->clients)
    {
        ...
        /* Don't enable the KEEP_ALIVE_TIMER if Spatial Audio is enabled */
#ifndef INCLUDE_SPATIAL_AUDIO
